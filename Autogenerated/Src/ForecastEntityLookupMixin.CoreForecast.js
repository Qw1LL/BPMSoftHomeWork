define("ForecastEntityLookupMixin", ["ForecastEntityLookupMixinResources", "SectionDesignerEnums",
	...
	function(resources, SectionDesignerEnums) {
		/**
		 * @class BPMSoft.configuration.mixins.ForecastEntityLookupMixin
		 * Provides methods for search forecast objects.
		 */
		Ext.define("BPMSoft.configuration.mixins.ForecastEntityLookupMixin", {
			alternateClassName: "BPMSoft.ForecastEntityLookupMixin",
			...
			init: function(callback, scope) {
				BPMSoft.chain(
					...
					function(next) {
						BPMSoft.EntitySchemaManager.initialize(next);
					},
					function() {
			...
			_getSectionEntitiesList: function() {
				...
				sections.each(function(item) {
					const entitySchema = this.BPMSoft.EntitySchemaManager.findItemByName(item.code);
			...
			_getOnlySections: function() {
				return this.BPMSoft.SectionManager.filterByFn(function(item) {
			...
			_addSeparatorItem: function(lookupList, caption) {
				lookupList[this.BPMSoft.generateGUID()] = this._getSeparator(caption);
			},
			...
			_getSortedListByDisplayValue: function(list) {
				const sortedList = this.Ext.create("BPMSoft.Collection", {
			...
			loadNextForecastEntities: function(listParams) {
				...
				sortedList.add(this.BPMSoft.generateGUID(),
					this._getSeparator(resources.localizableStrings.OtherObjectsSeparatorCaption));
				const filteredItems = this.BPMSoft.EntitySchemaManager.getItems()
			...
			_isExtendBaseObjects: function(parentUId) {
				...
				return this.BPMSoft.contains(baseObjectsList, parentUId);
			}

		});

		return BPMSoft.ForecastEntityLookupMixin;
