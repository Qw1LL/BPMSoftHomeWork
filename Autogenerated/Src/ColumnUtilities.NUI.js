define("ColumnUtilities", ["BPMSoft", "ColumnUtilitiesResources"],
		function(BPMSoft, resources) {
			Ext.define("BPMSoft.configuration.ColumnUtilities", {
				extend: "BPMSoft.BaseObject",
				alternateClassName: "BPMSoft.ColumnUtilities",
				statics: {
					...
					updateProfileColumnCaptions: function(config) {
						...
						if (!BPMSoft.isEmptyObject(profile.DataGrid)) {
							profile.DataGrid = this.updateProfileColumnConfig(profileConfig);
						}
						if (!BPMSoft.isEmptyObject(profile.DataGridVerticalProfile)) {
					...
					isProfileCultureEqualsUserCulture: function(profile) {
						...
						var userCultureId = BPMSoft.SysValue.CURRENT_USER_CULTURE.value;
					...
					findNotFoundColumns: function(rowConfig) {
						var notFoundColumns = [];
						if (BPMSoft.isEmptyObject(rowConfig)) {
							return notFoundColumns;
						}
						var keys = BPMSoft.keys(rowConfig);
