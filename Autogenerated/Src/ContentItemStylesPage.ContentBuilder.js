			...
			methods: {
				...
				_onCanChangeBordersChanged: function(model, value) {
					...
					if (!value) {
						...
						BPMSoft.each(borderProperties, function(property, name) {
				...
				_buildBorderStyleJson: function() {
					const result = {};
					const borderProperties = this._getBorderProperties();
					BPMSoft.each(borderProperties, function(sideConf, side) {
				...
				_onBorderConfigChanged: function(config) {
					const borderConfig = BPMSoft.calculateBorderStyles(config);
					const borderProperties = this._getBorderProperties();
					const borderWidth = [];
					const borderStyles = [];
					const borderColor = [];
					...
					if (borderColor.length === 1 && borderColor[0].toLowerCase() !== "initial") {
						const rgbColor = BPMSoft.rgbString2Array(borderColor[0]);
						this.$BorderColor = BPMSoft.rgb2hex(rgbColor);
					}
				...
				_onBackgroundImageChanged: function(model, image) {
					image = BPMSoft.utils.html.sanitizeHTML(image);
				...
				_canChangeMobileBackground: function() {
					return BPMSoft.Features.getIsEnabled("EmailMobileBackgroundFeature")
				...
				_clearMobileBackground: function() {
					...
					this.$BackgroundMobileImage = BPMSoft.emptyString;
					this.$BackgroundMobileDisplayValue = BPMSoft.emptyString;
				},
				...
				_clearBackground: function() {
					this.$BackgroundImage = BPMSoft.emptyString;
					this.$BackgroundImageDisplayValue = BPMSoft.emptyString;
				},
				...
				_canUploadBackground: function() {
					return BPMSoft.Features.getIsEnabled("ContentItemUploadBackgroundFeature") &&
				...
				_setConfigProperties: function(config) {
					...
					BPMSoft.each(this._workProperties, function(parameterName) {
				...
				isContentItemCustomStylesFeatureEnable: function() {
					return BPMSoft.Features.getIsEnabled("ContentItemCustomStylesFeature");
				},
