define("ColumnEditMixin", ["ext-base", "ColumnEditMixinResources", "EntityStructureHelperMixin"],
	function(Ext) {
		...
		Ext.define("BPMSoft.configuration.mixins.ColumnEditMixin", {
			alternateClassName: "BPMSoft.ColumnEditMixin",

			mixins: {
				EntityStructureHelper: "BPMSoft.EntityStructureHelperMixin"
			},

			getColumnsFiltrationMethod: BPMSoft.emptyFn,
			...
			loadColumnsData: function(filterValue, list, columnName) {
				...
				this.getColumns(entityStructureConfig, function(columns) {
					list.clear();
					var columnsCollection = this.Ext.create("BPMSoft.Collection");
					var objects = {};
					...
					columnsCollection = columnsCollection.filter(
						"displayValue",
						filterValue,
						BPMSoft.StringFilterType.START_WITH
					);
			...
			getEntityStructureConfig: function(columnName) {
				...
				entityStructureConfig = BPMSoft.deepClone(entityStructureConfig);
				...
				BPMSoft.each(sourceToValueDecoupling, function(decoupling) {
			...
			initColumnsLists: function() {
				BPMSoft.each(this.columns, function(columnConfig, columnName) {
					...
					if (entityStructureConfig && entityStructureConfig.entitySchemaName) {
						this.set(columnName + "List", this.Ext.create("BPMSoft.Collection"));
					}
				}, this);
			},
