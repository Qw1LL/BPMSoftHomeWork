Ext.define("BPMSoft.configuration.ActivityGridCachedOperation", {
	alternateClassName: "BPMSoft.configuration.ActivityGridPage.CachedOperation",
	...
	constructor: function(config) {
		...
		this.adjustedStartDate = BPMSoft.Date.add(startDate, Ext.Date.DAY, cacheSizeInOneDirection);
		this.adjustedDueDate = BPMSoft.Date.add(dueDate, Ext.Date.DAY, -cacheSizeInOneDirection);
	},
...
Ext.define("BPMSoft.configuration.ActivityGridPageProxy", {
	alternateClassName: "BPMSoft.configuration.ActivityGridPage.Proxy",
	extend: "BPMSoft.Proxy.BaseProxy",
...
Ext.define("BPMSoft.configuration.store.ActivityGridPage", {
	...
	config: {
		...
		controller: "BPMSoft.configuration.controller.ActivityGridPage",
	...
	beforeLoadStore: function(store, operation) {
		...
		if (operation.forceRefresh) {
			BPMSoft.configuration.ActivityGridCachedOperation.clearCache();
		}
		if (!operation.useCache) {
			this.setProxyByOnlineMode(queryConfig);
			return;
		}
		var cachedOperation = Ext.create("BPMSoft.configuration.ActivityGridCachedOperation", {
			operation: operation
		});
	...
	onBeforeAddRecords: function(store, records) {
		BPMSoft.configuration.ActivityGridCachedOperation.add(records);
	},
	...
	onBeforeRemoveRecords: function(store, records) {
		BPMSoft.configuration.ActivityGridCachedOperation.remove(records);
	},
