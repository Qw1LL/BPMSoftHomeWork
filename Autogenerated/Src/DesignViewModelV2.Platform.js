define("DesignViewModelV2", ["ext-base", "BPMSoft", "DesignViewModelV2Resources", "PageDesignerUtilities",
		"SectionDesignDataModule", "SectionDesignerEnums", "SectionDesignerUtils"],
	function(Ext, BPMSoft, resources, pageDesignerUtilities, designData, sectionDesignerEnums, sectionDesignerUtils) {
		var globalNamespace = "BPMSoft";
		var modelNamespace = "BPMSoft.model";
		...
		function defineDesignViewModel(name, viewModelClass) {
			...
			var detailsConfig = {};
			BPMSoft.each(viewModelClass.prototype.details, function(detail, detailName) {
				detailsConfig[detailName] = BPMSoft.deepClone(detail);
			}, this);
			BPMSoft.each(viewModelClass.prototype.columns, function(column, columnName) {
				if (resourceColumnRegex.test(columnName)) {
					columnsConfig[columnName] = BPMSoft.deepClone(column);
				}
			}, this);
			...
			var config = {
				...
				showDetailWindowCallback: function(detailConf) {
					var schemaClone = BPMSoft.deepClone(this.get("schema"));
				...
				generateUniqName: function(currentName) {
					...
					var newGuid = BPMSoft.generateGUID();
				...
				removeItem: function() {
					var schema = this.get("schema");
					var clonedSchema = BPMSoft.deepClone(schema);
				...
				incrementWidth: function() {
					var schema = this.get("schema");
					var newSchema = BPMSoft.deepClone(schema);
				...
				createModuleCaptionColumns: function() {
					...
					BPMSoft.iterateChildItems(viewConfig, function(iterationConfig) {
				...
				createDetailsCaptionColumns: function() {
					...
					pageDesignerUtilities.getDetailsInfo(function(detailsInfo) {
						BPMSoft.iterateChildItems(viewConfig, function(iterationConfig) {
							...
							if (item.itemType === BPMSoft.ViewItemType.DETAIL) {
				...
				configureDetail: function() {
					...
					if (!detail) {
						BPMSoft.showInformation(localizableStrings.CanNotEditParentSchemaDetails, null, null,
							{buttons: ["ok"]});
						return;
					}
					if (!detail.filter) {
				...
				removeDetail: function() {
					...
					if (item && item.item && (item.item.itemType === BPMSoft.ViewItemType.DETAIL)) {
						var parent = item.parent;
						delete schemaClone.details[tag];
						BPMSoft.each(parent.items, function(it, key) {
							if (it.name === tag) {
				...
				sortCurrentTab: function() {
					...
					BPMSoft.each(items, function(item, key) {
						...
						if (item.itemType === BPMSoft.ViewItemType.DETAIL) {
							...
							var cachedDetails = BPMSoft.DomainCache.getItem("SectionDesigner_DetailsInfo");
							BPMSoft.each(cachedDetails, function(detail) {
				...
				showTabItemsWindowCallback: function(tabItems) {
					...
					var viewConfig = schemaClone.viewConfig[0];
					var newItems = [];
					BPMSoft.each(tabItems, function(tab, key) {
				...
				customizeTabs: function() {
					...
					var tabsConf = [];
					BPMSoft.each(tabItems, function(item, key) {
						var index = parseInt(key, 10);
						var tab = {
				...
				showTabsWindowCallback: function(tabs) {
					...
					var viewConfig = schemaClone.viewConfig[0];
					var newTabs = [];
					BPMSoft.each(tabs, function(tab, key) {
						var pos = parseInt(key, 10);
				...
				cleanRegisteredDetails: function(schemaClone) {
					var viewConfig = schemaClone.viewConfig[0];
					var details = schemaClone.details;
					BPMSoft.each(details, function(det, key) {
				...
				init: function(initValues) {
					BPMSoft.each(initValues, function(value, itemName) {
				...
				initTabs: function() {
					...
					BPMSoft.each(tabsCaptions, function(item) {
						this.initTab(item);
					}, this);
				},
				...
				initTab: function(tabsName) {
					...
					BPMSoft.each(tabs.item.tabs, function(item) {
						var itemName = item.name;
					...
					var tabsCollection = new BPMSoft.BaseViewModelCollection({
				...
				getSchemaItemInfoByName: function(name, parent) {
					...
					BPMSoft.iterateChildItems(parent, function(iterationConfig) {
						var item = iterationConfig.item;
						if (item.name === name) {
							result = {
								item: item,
