		...
		function createConstructor(context) {
			...
			var getViewModel = function(scope) {
				var viewModel = Ext.create('BPMSoft.BaseViewModel', {
					...
					values: {
						allCommentsGridData: new BPMSoft.BaseViewModelCollection(),
						commentsGridData: new BPMSoft.BaseViewModelCollection(),
						currentLength: 0,
						commentsAllCount: 0,
						visibleCommentsCount: BPMSoft.SysSettings.cachedSettings.CommentsN || 3,
						commentN: BPMSoft.SysSettings.cachedSettings.CommentsN || 3,
						...
						loadCommentsCollection: function() {
							...
							select.getEntityCollection(function(response) {
								...
								if (response && response.success) {
									...
									var newCollection = new BPMSoft.BaseViewModelCollection();
									...
									BPMSoft.each(slicedItems.reverse(), function(item) {
			...
			function checkCanEdit(context, canEditOrDeleteComment) {
				...
				if (canEditOrDeleteComment || contact.value === BPMSoft.SysValue.CURRENT_USER_CONTACT.value) {
			...
			function getContactPhotoConfig() {
				...
				var imageUrl = BPMSoft.ImageUrlBuilder.getUrl(contactImageConfig);
				var imageConfig = {
