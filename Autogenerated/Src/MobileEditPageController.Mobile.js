/**
 * @class BPMSoft.configuration.controller.EditPage
 * Provides custom edit page with specified columns and business rules.
 */
Ext.define("BPMSoft.configuration.controller.EditPage", {
	...
	getColumnSetsConfigs: function(columnConfigs) {
		...
		var columnSet = columnSetsConfig.add({
			isPrimary: true,
			columns:  Ext.create("BPMSoft.OrderedCollection")
		});
		for (var i = 0, ln = columnConfigs.length; i < ln; i++) {
			var columnConfig = BPMSoft.sdk.RecordPage.resolveColumnConfig(this.self.Model.getName(),
				columnConfigs[i]);
	 ...
	 * @param {BPMSoft.QueryConfig} queryConfig Query config.
	 * @protected
	 * @virtual
	 */
