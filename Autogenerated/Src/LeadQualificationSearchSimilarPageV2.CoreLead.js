define("LeadQualificationSearchSimilarPageV2", ["BusinessRuleModule", "BPMSoft",
	...
	function(BusinessRuleModule, BPMSoft, resources, LookupUtilities, ConfigurationConstants, enums) {
		/** @enum
		 * ##### # ############. */
		BPMSoft.AccountRelationship = {
			/** ####### #####. */
			NEW: "NEW",
			/** ####### # #########. */
			EXIST: "EXIST",
		...
		return {
			...
			methods: {
				...
				getSimilarRecordsDetailFilter: function(config) {
					...
					if (config.name) {
						...
						var nameFilter = BPMSoft.createColumnFilterWithParameter(
					...
					if (config.phone) {
						...
						if (fiterConfig.searchValue) {
							var existsSubPhoneFilters = BPMSoft.createFilterGroup();
							existsSubPhoneFilters.addItem(BPMSoft.createColumnFilterWithParameter(
								fiterConfig.comparisonType, "SearchNumber", fiterConfig.searchValue));
							var existsPhoneFilters = BPMSoft.createExistsFilter(config.existsFilterPath,
					...
					if (config.email) {
						...
						var existsSubEmailFilters = BPMSoft.createFilterGroup();
						existsSubEmailFilters.addItem(BPMSoft.createColumnFilterWithParameter(
						...
						var existsEmailFilters = BPMSoft.createExistsFilter(config.existsFilterPath,
							existsSubEmailFilters);
						filterGroup.addItem(existsEmailFilters);
					}
					if (config.web) {
						fiterConfig = this.getFilterComparisonType(config.web);
						var existsSubWebFilters = BPMSoft.createFilterGroup();
						existsSubWebFilters.addItem(BPMSoft.createColumnFilterWithParameter(
							fiterConfig.comparisonType, "Number", fiterConfig.searchValue));
						existsSubWebFilters.addItem(BPMSoft.createColumnFilterWithParameter(
							BPMSoft.ComparisonType.EQUAL, "CommunicationType",
							ConfigurationConstants.CommunicationTypes.Web));
						var existsWebFilters = BPMSoft.createExistsFilter(config.existsFilterPath,
							existsSubWebFilters);
				...
				getFilterComparisonType: function(searchValue) {
					...
					if (searchValue.length === 1 && startsWithAny) {
						config.comparisonType = this.BPMSoft.ComparisonType.CONTAIN;
						config.searchValue = "";
						return config;
					}
					if (!startsWithAny) {
						config.comparisonType = this.BPMSoft.ComparisonType.START_WITH;
				...
				onDetailActiveRowChanged: function(config) {
					...
					if (schemaName === "Contact") {
						...
						if (!this.Ext.isEmpty(account) &&
							(accountRelationship === BPMSoft.AccountRelationship.EXIST)) {
				...
				setInitialValues: function() {
					...
					this.loadInitialData(parameters.LeadId, null, null, function(entity) {
						...
						if (!this.Ext.isEmpty(qualifiedAccount)) {
							...
							this.set("AccountRelationship", BPMSoft.AccountRelationship.EXIST);
				...
				initDisqualifyReasonsMenu: function() {
					...
					var disqualifyReasonsMenu = Ext.create("BPMSoft.BaseViewModelCollection");
					...
					esq.getEntityCollection(function(response) {
						...
						collection.each(function(item) {
							...
							disqualifyReasonsMenu.add(itemId, Ext.create("BPMSoft.BaseViewModel", {
								values: {
									Id: itemId,
				...
				onAccountRelationshipChanged: function(checked, value) {
					if (checked) {
						...
						if (value === BPMSoft.AccountRelationship.EXIST) {
							this.onAccountSearchOptionsChanged();
						} else {
							config.value = false;
				...
				updateAccountAttributes: function(value) {
					var isNew = (value === BPMSoft.AccountRelationship.NEW);
					var isExist = (value === BPMSoft.AccountRelationship.EXIST);
					var isBreak = (value === BPMSoft.AccountRelationship.BREAK);
				...
				clearFields: function(fields) {
					BPMSoft.each(fields, function(item) {
						this.set(item, null);
					}, this);
				},
			...
			diff: /**SCHEMA_DIFF*/[
							...
							"enabled": {
								...
								"bindConfig": {
									converter: function(value) {
										return (value && value === BPMSoft.AccountRelationship.EXIST);
									}
								}
							}
						}
				...
				},
				{
					...
					"values": {
						"labelConfig": {"visible": false},
						"layout": {"column": 13, "row": 0, "colSpan": 1},
						"classes": {"wrapClass": ["search-option"]}
					},
					"parentName": "ContactSearchSetting",
					"propertyName": "items"
				...
				},
				{
					...
					"values": {
						"labelConfig": {"visible": false},
						"layout": {"column": 13, "row": 1, "colSpan": 1},
						"classes": {"wrapClass": ["search-option"]}
					},
					"parentName": "ContactSearchSetting",
					"propertyName": "items"
						...
						"items": [
							{
								...
								"value": BPMSoft.AccountRelationship.NEW,
								"controlConfig": {
							...
							},
							{
								...
								"value": BPMSoft.AccountRelationship.EXIST,
								"controlConfig": {
									"checkedchanged": {"bindTo": "onAccountRelationshipChanged"}
								}
							},
							{
								...
								"value": BPMSoft.AccountRelationship.BREAK,
								"controlConfig": {
									"checkedchanged": {"bindTo": "onAccountRelationshipChanged"}
								}
							}
