define("MultiLookupModule", ["LookupPage", "LookupPageViewGenerator", "LookupUtilities", "css!LookupPageCSS"],
	function(LookupPage, LookupPageViewGenerator) {
		...
		/**
		 * @class BPMSoft.configuration.MultiLookupModule
		 * ##### ###### ######## ###### ######## ## ########## ############.
		 */
		return Ext.define("BPMSoft.configuration.MultiLookupModule", {
			alternateClassName: "BPMSoft.MultiLookupModule",
			...
			onTabChanged: function(activeTab) {
				...
				BPMSoft.each(lookupsInfo, function(lookupInfo) {
			...
			_getCaptionFromManager: function(schemaName) {
				if(schemaName) {
					var schema = this.BPMSoft.EntitySchemaManager.findItemByName(schemaName);
					if(schema) {
			...
			_getCaptionFromStructure: function(schemaName) {
				if(schemaName) {
					var schemaConfig = this.BPMSoft.configuration.ModuleStructure[schemaName];
					if(schemaConfig) {
			 ...
			 * @param {BPMSoft.Collection} callback.collection ######### ############.
			 */
			loadLookups: function(callback) {
				...
				var tabCollection = Ext.create("BPMSoft.Collection");
				BPMSoft.each(lookupsInfo, function(lookupInfo, index) {
			...
			createLookupsTabs: function(collection) {
				var lookups = Ext.create("BPMSoft.BaseViewModelCollection");
				var activeLookupTabName;
				collection.each(function(item) {
					var tabId = item.Id;
					var tabName = item.Name;
					lookups.add(tabId, Ext.create("BPMSoft.BaseViewModel", {
						values: {
							Id: tabId,
			...
			/**
			 * @inheritdoc BPMSoft.LookupPage#renderLookupView
			 * @overridden
			 */
