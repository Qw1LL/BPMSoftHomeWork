define('EditPageDesignerHelper', ['ext-base', 'BPMSoft', 'ConfigurationEnumsResources', 'StorageUtilities'],
	function(Ext, BPMSoft, resources, storageUtilities) {
		var mainContainersIds = ['customPanel', 'leftPanel', 'rightPanel'];
		function updateSchema(schema, modificationList) {
			BPMSoft.each(modificationList.getItems ?
		...
		function applyModification(schema, modification) {
			...
			switch (modification.type) {
				case 'add':
					addElement(schema, BPMSoft.deepClone(modification.value), modification.position, modification.containerName);
					break;
				case 'move':
		...
		function getModifications(editPageName, callback, scope) {
			if (EditPageModifications[editPageName]) {
				var cacheList = new BPMSoft.Collection();
				var items = BPMSoft.deepClone(EditPageModifications[editPageName]);
			...
			select.filters.add('filterModifications', BPMSoft.createColumnFilterWithParameter(
				BPMSoft.ComparisonType.EQUAL, 'EditPageName', editPageName));
			var innerCallback = function(response) {
				...
				if (response.success && response.collection.getCount()) {
					...
					if (!Ext.isEmpty(modifications)) {
						var items = BPMSoft.deepClone(modifications);
