define("LeadSimilarEntitiesUtilities",["LeadSimilarEntitiesUtilitiesResources", "ServiceHelper"],
	function(resources, ServiceHelper) {
		Ext.define("BPMSoft.configuration.mixins.LeadSimilarEntitiesUtilities", {
			alternateClassName: "BPMSoft.LeadSimilarEntitiesUtilities",
			...
			openSimilarRecordsLookup: function(schemaName, masterColumnName) {
				...
				var similarRecordsFilter = BPMSoft.createColumnInFilterWithParameters("Id",
					this.getSimilarCollection());
				similarRecordsFilter.comparisonType = BPMSoft.ComparisonType.EQUAL;
			...
			_processSimilarRecordsCollection: function(response) {
				if (response) {
					var collectionJson = BPMSoft.decode(response);
		...
		return BPMSoft.LeadSimilarEntitiesUtilities;
	});
