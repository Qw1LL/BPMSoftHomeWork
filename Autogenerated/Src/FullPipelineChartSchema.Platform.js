define("FullPipelineChartSchema", [
	...
	"FullPipelineModuleResources",
	"DashboardEnums",
	"FullPipelineServiceRequest"
], function(GoogleTagManagerUtilities,resources) {
	return {
		...
		properties: {
			 ...
			 * @type {BPMSoft.DashboardEnums.PipelineSliceType}
			 */
			defaultPipelineSliceType: BPMSoft.DashboardEnums.PipelineSliceType.BY_COUNT
		},
		attributes: {
			/**
		...
		methods: {
			...
			/**
			 * @inheritDoc BPMSoft.ChartViewModel#init
			 * @override
			 */
			init: function() {
			...
			_fillChartLegendItemsCollection: function(callback, scope) {
				...
				this.BPMSoft.each(this.$entities, function(entity, index) {
					const itemColor = BPMSoft.DashboardEnums.WidgetColorSet[index];
			...
			_loadReferencedSchemas: function(callback, scope) {
				this.BPMSoft.eachAsync(
					this.$entities,
					function(entityConfig, next) {
						var schemaName = entityConfig.schemaName;
						var entity = this.BPMSoft[schemaName];
						if (schemaName && this.Ext.isEmpty(entity)) {
							this.BPMSoft.require([entityConfig.schemaName], next, this);
						} else {
			...
			_createChartLegendItem: function(caption, itemColor) {
				return this.Ext.create("BPMSoft.BaseSchemaViewModel", {
			...
			_getSliceProperties: function(predicate) {
				return this.BPMSoft.findWhere(this.get("SlicesValuesProperties"), predicate);
			},
			...
			getViewConfigClassName: function() {
				return "BPMSoft.FullPipelineViewConfig";
			},
			...
			_sanitizeFilter: function(config) {
				const filter = BPMSoft.deserialize(config.filters);
				if (!filter) {
					return;
				}
			...
			getRequestConfig: function() {
				const entitiesConfig = BPMSoft.deepClone(this.$entities);
				this.BPMSoft.each(entitiesConfig, this._sanitizeFilter);
				return {
					pipelineConfig: {
			...
			_getEncodedValues: function(stages) {
				stages.forEach(function(stage) {
					stage.displayValue = this.BPMSoft.encodeHtml(stage.displayValue);
				});
			},
			...
			_getCollapsedGroups: function(stageGroups, expandedRootStagesIdList) {
				...
				return this.BPMSoft.filter(stageGroups, function(group) {
			...
			_getChartDataFieldSumByGroup: function(stageGroup, rawData, fieldName) {
				...
				this.BPMSoft.each(
					...
					function(stageId) {
						...
						nextElement = BPMSoft.findWhere(row.elements, {
							name: fieldName
						});
			...
			_getAggregatedDataByStageGroups: function(stageGroupList, rawData, fieldName) {
				let data = this.BPMSoft.deepClone(rawData);
				this.BPMSoft.each(
					stageGroupList,
					function(stageGroup) {
						...
						const rawDataFieldIndex = this.BPMSoft.map(rootStageDataElement.elements, function(element) {
							return element.name;
						}).indexOf(fieldName);
			...
			_collapseGroups: function(stageGroupListToCollapse) {
				let result = this.BPMSoft.deepClone(this.$Stages);
				this.BPMSoft.each(
			...
			_excludeStagesFromStageInfo: function(stageIdList, stagesInfo) {
				return this.BPMSoft.filter(stagesInfo, function(info) {
			...
			_findRawDataRow: function(stageId, rawData) {
				return this.BPMSoft.where(rawData.data, { stageId: stageId })[0];
			},
			...
			_toggleStageState: function(stageId) {
				...
				if (!expandedStages.includes(stageId)) {
					...
					expandedStages = this.BPMSoft.without(expandedStages, stageId);
				}
			...
			processData: function(rawData) {
				...
				this.BPMSoft.each(
					...
					function(stageInfo, index) {
						...
						const stage = this.BPMSoft.findWhere(aggregatedRawData.data, {
							stageId: stageInfo.id
						});
						const displayData = stage
							? this.BPMSoft.findWhere(stage.elements, {
			...
			_isExpandableStage: function(stageId) {
				...
				BPMSoft.each(this.$entities, function(entity) {
					BPMSoft.each(entity.groups, function(group) {
			...
			_checkPipelineHasData: function(data, displayValueAttributeName) {
				const notEmptyStagesData = this.BPMSoft.filter(
					data,
					function(stageItem) {
						var element = this.BPMSoft.where(stageItem.elements, {
							name: displayValueAttributeName,
							value: 0
						});
			...
			_getSchemaCaption: function(schemaName) {
				const entity = this.BPMSoft[schemaName];
			...
			formStageElementDisplayValue: function(elementData, defaultsConfig) {
				...
				const value =
					(elementData && String(this.BPMSoft.roundValue(elementData.value, 2))) ||
			 ...
			 * @param {BPMSoft.DashboardEnums.PipelineSliceType} slice Slice type.
			 * @private
			 */
			_setActiveSlice: function(slice) {
				this.set("CurrentSliceType", slice);
				let sliceData = this.BPMSoft.findWhere(this.$Slices, {
					type: slice
				});
			...
			getFullPipelineServiceRequestClassName: function() {
				return "BPMSoft.FullPipelineServiceRequest";
			},
			...
			onStartDateChange: function() {
				if (this.$StartDate) {
					this.set("StartDate", this.BPMSoft.startOfDay(this.$StartDate), {silent: true});
				}
			...
			onDueDateChange: function() {
				if (this.$DueDate) {
					this.set("DueDate", this.BPMSoft.endOfDay(this.$DueDate), {silent: true});
				}
			...
			/**
			 * @inheritdoc BPMSoft.ChartViewModel#getProfileKey
			 * @override
			 */
			...
			/**
			 * @inheritdoc BPMSoft.ChartViewModel#getProfileKeyByEntitySchemaName
			 * @override
			 */
			getProfileKeyByEntitySchemaName: function() {
				return this.Ext.String.format(
					"{0}_{1}",
					this.getProfileKey(),
					this.BPMSoft.map(this.$entities, function(e) {
						return e.schemaName;
					}).join("_")
				);
			},
			...
			_getPeriodByDay: function(periodName) {
				...
				switch (periodName) {
					case "Yesterday":
						startDate = this.BPMSoft.startOfDay(this.Ext.Date.add(startDate, "d", -1));
			...
			_getPeriodByWeek: function(periodName) {
				...
				switch (periodName) {
					case "PastWeek":
						startDate = this.BPMSoft.startOfWeek(this.Ext.Date.add(startDate, "d", -7));
			...
			_getPeriodByMonth: function(periodName) {
				...
				switch (periodName) {
					case "PastMonth":
						startDate = this.BPMSoft.startOfMonth(this.Ext.Date.add(startDate, "mo", -1));
						dueDate = this.BPMSoft.endOfMonth(startDate);
						break;
					case "NextMonth":
						startDate = this.BPMSoft.startOfMonth(this.Ext.Date.add(startDate, "mo", 1));
			...
			_initPrimaryCurrencySymbol: function(callback, scope) {
				...
				this.BPMSoft.SysSettings.querySysSettingsItem("PrimaryCurrency", initSymbolCallback, this);
			},
					...
					function(response) {
						if (response && response.success) {
							...
							if (this.Ext.isEmpty(symbol)) {
								...
								this.log(warnMessage, this.BPMSoft.LogMessageType.WARNING);
							} else {
			...
			getCurrencyEntitySchemaQuery: function() {
				...
				var entitySchemaQuery = this.Ext.create("BPMSoft.EntitySchemaQuery", {
					...
					serverESQCacheParameters: {
						cacheLevel: this.BPMSoft.ESQServerCacheLevels.SESSION,
						cacheGroup: "Currency",
			...
			/**
			 * @inheritdoc BPMSoft.GridUtilities#reloadGridData
			 * @protected
			 */
			reloadGridData: BPMSoft.emptyFn
		}
	};
});
