define("DynamicContentReplicaBuilder", ["ext-base", "ReplicaCaptionBuilder"], function(Ext) {
	/**
	 * @class BPMSoft.DynamicContentReplicaBuilder
	 */
	Ext.define("BPMSoft.configuration.DynamicContentReplicaBuilder", {
		alternateClassName: "BPMSoft.DynamicContentReplicaBuilder",
		...
		_checkStaticReplica: function(templateJson) {
			var result = true;
			BPMSoft.each(templateJson.Items, function(item) {
				if (item.ItemType === "blockgroup" && !BPMSoft.isEmpty(item.Items)) {
					result = false;
				}
			}, this);
			return result;
		...
		_getStaticReplicaConfig: function(templateJson) {
			var replicaConfig = BPMSoft.deepClone(templateJson);
		...
		_getFirstBlockByPriority: function(blocks, excludedIndexes) {
			...
			var block;
			BPMSoft.each(sortedBlocks, function(el) {
		...
		_getMaskByIndexes: function(indexes) {
			return BPMSoft.reduce(indexes, function(summ, current) {
				return summ + current;
			}, 0);
		},
		...
		_addGroupBlockByMask: function(replicaConfig, blocks, replicaMask) {
			BPMSoft.each(blocks, function(block) {
				var blockIndex = replicaMask.find(function(index) {
					return index === block.Index;
				});
				if (BPMSoft.isEmpty(blockIndex)) {
					return;
				}
		...
		_getAttibutesInUse: function(templateJson) {
			var attributesInUse = [];
			BPMSoft.each(templateJson.Attributes, function(attr) {
		 ...
		 * @private
		 * @param {Object} templateJson Source template config.
		 * @returns {BPMSoft.Collection} Collection of unique masks with block indexes.
		 */
		_getUniqueMasksForAllCombinations: function(templateJson) {
			...
			var masks = new BPMSoft.Collection();
			BPMSoft.each(combinationsToExclude, function(attributesToExclude) {
		 ...
		 * @param {BPMSoft.ReplicaCaptionBuilder} captionBuilder Instance of caption builder for current template.
		...
		_internalGenerateReplicas: function(templateJson, captionBuilder) {
			...
			BPMSoft.each(masks, function(mask) {
		...
		getReplicaConfig: function(templateJson, replicaMask, captionBuilder){
			...
			replicaConfig.Items = [];
			replicaConfig.BlockIndexes = [];
			BPMSoft.each(templateJson.Items, function(group) {
		 ...
		 * @protected
		 * @param {object} template Dynamic template config (JSON).
		 * @return {BPMSoft.ReplicaCaptionBuilder} Instance of replica caption builder.
		 */
		createReplicaCaptionBuilder: function(templateJson) {
			return new BPMSoft.ReplicaCaptionBuilder(templateJson.Attributes);
		},
	...
	return Ext.create(BPMSoft.DynamicContentReplicaBuilder);
});
