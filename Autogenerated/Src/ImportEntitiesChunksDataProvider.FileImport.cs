namespace BPMSoft.Configuration.FileImport
{
	...
	public class ImportEntitiesChunksDataProvider : ImportChunkDataProvider<EntityChunkData>, IImportEntitiesChunksDataProvider
	{
		...
		private const int MaxImportExcelRecordCountDefaultValue = 2000;
		private const string MaxImportExcelRecordCountSysSetting = "MaxImportExcelRecordCount";
		...
		private void CreateChunkFromFileProcessor(ImportParameters importParameters) {
			...
			var maxImportExcelRecordCount = GetMaxImportExcelRecordCount();
			var importEntities = fileProcessor.ReadEntities(importSessionId).Take(maxImportExcelRecordCount);
		...
		private void CreateChunksFromParameters(ImportParameters importParameters) {
			var maxImportExcelRecordCount = GetMaxImportExcelRecordCount();
			var entitiesChunks = importParameters.Entities.Take(maxImportExcelRecordCount).SplitOnChunks(importParameters.ChunkSize);
		...
		private int GetMaxImportExcelRecordCount() {
			var maxImportExcelRecordCount = SystemSettings.GetValue(UserConnection, MaxImportExcelRecordCountSysSetting, 0);
			return maxImportExcelRecordCount < 1 ? MaxImportExcelRecordCountDefaultValue : maxImportExcelRecordCount;
		}
