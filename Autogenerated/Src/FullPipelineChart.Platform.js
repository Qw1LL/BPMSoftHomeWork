define("FullPipelineChart", ["DashboardEnums", "FullPipelineChartResources",
	...
	function() {
		Ext.define("BPMSoft.controls.FullPipelineChart", {
			extend: "BPMSoft.Chart",
			alternateClassName: "BPMSoft.FullPipelineChart",

			/**
			 * @inheritDoc
			 * @override
			 */
			chartProviderFactoryClassName: "BPMSoft.FullPipelineChartProviderFactory",
			...
			/**
			 * @inheritdoc BPMSoft.Chart#setDrillDownMode
			 * @override
			 */
			setDrillDownMode: BPMSoft.emptyFn,
			...
			/**
			 * @inheritdoc BPMSoft.HighchartsWrapper#initChart
			 * @override
			 */
			initChart: function() {
			...
			/**
			 * @inheritdoc BPMSoft.HighchartsWrapper#updateSize
			 * @override
			 */
			updateSize: function() {
			...
			_getSeriesAggregateValues: function(seriesData) {
				let colorMap = {};
				let heightSum = 0;
				BPMSoft.each(seriesData, function(seriesDataElement) {
					heightSum += seriesDataElement.value;
					if (!colorMap[seriesDataElement.schemaName]) {
						colorMap[seriesDataElement.schemaName] =
							BPMSoft.DashboardEnums.WidgetColorSet[Object.keys(colorMap).length];
					}
				});
				return {
			...
			_prepareSeriesElements: function(series) {
				...
				BPMSoft.each(seriesData, function(element) {
					const elementSchema = element.schemaName;
					if (previousSchemaName !== elementSchema) {
						const dataCount = BPMSoft.where(seriesData, { schemaName: elementSchema }).length;
						multiplier = Math.min(
			...
			/**
			 * @inheritdoc BPMSoft.Chart#setSeries
			 * @override
			 */
			setSeries: function(series) {
				if (BPMSoft.isEqual(this.series, series)) {
					return;
				}
			...
			_setDataLabelAlignCenter: function() {
				...
				BPMSoft.each(chartFirstSerieData, function(point, index) {
					...
					point.dataLabel.attr({
						x: BPMSoft.getIsRtlMode() ? dX + pointWidth - labelWrapperWidth : dX,
						y: dataLabelHeight
					});
				}, this);
			},
			...
			_addStageExpandButtonsListener: function(context) {
				...
				BPMSoft.each(stageExpandButtons, function(item) {
			...
			_getFunnelConfig: function() {
				...
				const isRtl = BPMSoft.getIsRtlMode();
				return {
					chart: {
						marginRight: 0,
						marginBottom: 0,
