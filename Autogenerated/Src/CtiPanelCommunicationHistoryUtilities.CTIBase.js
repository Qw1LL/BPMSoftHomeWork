define("CtiPanelCommunicationHistoryUtilities", ["CtiPanelCommunicationHistoryUtilitiesResources", "CtiConstants",
		...
		function(resources, CtiConstants, ConfigurationConstants) {
			...
			/**
			 * @class BPMSoft.configuration.mixins.CtiPanelCommunicationHistoryUtilities
			 * Call history mixin.
			 * @type {BPMSoft.BaseObject}
			 */
			Ext.define("BPMSoft.configuration.mixins.CtiPanelCommunicationHistoryUtilities", {
				extend: "BPMSoft.core.BaseObject",
				alternateClassName: "BPMSoft.CtiPanelCommunicationHistoryUtilities",
				...
				getCallDateString: function(callDate) {
					...
					var dateType = (isTodayDate) ? BPMSoft.DataValueType.TIME : BPMSoft.DataValueType.DATE_TIME;
					return BPMSoft.getTypedStringValue(callDate, dateType);
				},
				...
				getHistoryCallType: function(call) {
					if (call.direction === BPMSoft.CallDirection.OUT) {
						return CtiConstants.CallType.OUTGOING;
					} else {
						var inactiveStates = [BPMSoft.GeneralizedCallState.NONE, BPMSoft.GeneralizedCallState.ALERTING,
							BPMSoft.GeneralizedCallState.UNKNOWN, BPMSoft.GeneralizedCallState.BUSY];
				 ...
				 * @param {BPMSoft.Collection} identificationFieldsData Identification Fields.
				 ...
				 * @protected
				 * @param {BPMSoft.EntitySchemaQuery} esq Communication history query.
				 */
				addCommunicationHistoryRelationColumns: function(esq) {
					...
					this.BPMSoft.each(entityConnectionColumns, function(columnName) {
				...
				addCommunicationHistoryQueryColumns: function(esq) {
					...
					dateColumn.orderDirection = BPMSoft.OrderDirection.DESC;
				 ...
				 * @protected
				 * @return {BPMSoft.EntitySchemaQuery} Call history query.
				 */
				getCommunicationHistoryQuery: function() {
					var esq = Ext.create("BPMSoft.EntitySchemaQuery", {rootSchemaName: "VwRecentCall"});
					...
					esq.filters.add("CurrentUserFilter", esq.createColumnFilterWithParameter(
						BPMSoft.ComparisonType.EQUAL, "CreatedById", BPMSoft.SysValue.CURRENT_USER_CONTACT.value));
					return esq;
				},
				...
				getHistoryPanelConfig: function(historySubscriber) {
					...
					var isOutgoingCall = (callDirection === BPMSoft.CallDirection.OUT);
				...
				applyCallRelationHistoryPanelConfig: function(panelConfig, historySubscriber) {
					var relationColumns = this.BPMSoft.deepClone(this.get("EntityConnectionColumnList"));
					var relationColumnsConfig = {EntityConnectionColumnList: this.BPMSoft.deepClone(relationColumns)};
					if (!Ext.isEmpty(historySubscriber)) {
						this.BPMSoft.each(relationColumns, function(columnName) {
				 ...
				 * @protected
				 * @param {BPMSoft.integration.telephony.Call} call The call.
				 */
