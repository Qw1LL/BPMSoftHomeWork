define("EmailsSearchMixin", ["EmailHelper"],
		function(EmailHelper) {
			/**
			 * @class BPMSoft.configuration.mixins.EmailsSearchMixin
			 * Mixin using to Elasticsearch for seacrh emails.
			 */
			Ext.define("BPMSoft.configuration.mixins.EmailsSearchMixin", {
				alternateClassName: "BPMSoft.EmailsSearchMixin",
				...
				_callService: function(searchConfig, callback, scope) {
					BPMSoft.ConfigurationServiceProvider.callService(searchConfig, callback, scope);
				},
				 ...
				 * @param {BPMSoft.Collection} oldValues Old values collection.
				 * @private
				 */
				...
				getCurrentLookupConfig: function(lookupName) {
					...
					if (!lookupConfig) {
						lookupConfig = {
							oldValues: this.Ext.create("BPMSoft.Collection"),
				...
				saveLookupValues: function(item, lookupName) {
					...
					this.BPMSoft.each(itemValues, function(itemValue) {
