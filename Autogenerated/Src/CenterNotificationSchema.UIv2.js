define("CenterNotificationSchema", ["CenterNotificationSchemaResources", "RemindingsUtilities",
		...
		function(resources, RemindingsUtilities, DesktopPopupNotification, NetworkUtilities,
			...
			return {
				mixins: {
					TitleNotificationUtilities: "BPMSoft.TitleNotificationUtilities"
				},
				messages: {
					/**
				...
				attributes: {
					 ...
					 */
					ActiveTabName: {
						ataValueType: BPMSoft.DataValueType.TEXT,
					...
					DefaultPopupImage: {
						...
						type: BPMSoft.ViewModelColumnType.VIRTUAL_COLUMN,
						value: BPMSoft.ImageUrlBuilder.getUrl(resources.localizableImages.DefaultPopupIcon)
					},
					...
					PopupImageCounter: {
						dataValueType: BPMSoft.DataValueType.IMAGE,
						type: BPMSoft.ViewModelColumnType.VIRTUAL_COLUMN,
						value: BPMSoft.ImageUrlBuilder.getUrl(resources.localizableImages.Reminding)
					},
				...
				methods: {
					...
					init: function(callback, scope) {
						...
						this.BPMSoft.ServerChannel.on(this.BPMSoft.EventName.ON_CONNECTION_INITIALIZED,
					 ...
					 * @protected
					 * @virtual
					 * @returns {BPMSoft.NotificationCountersServiceRequest} Request instance.
					 */
					getNotificationCountersServiceRequest: function() {
						return this.Ext.create("BPMSoft.NotificationCountersServiceRequest");
					},
					...
					checkCountersValue: function() {
						var countValueSpan = document.querySelectorAll('.ts-imagetabpanel-required > span');
						countValueSpan.forEach(element => {
						  if (element.innerHTML == "&nbsp;") {
							element.style.background = 'transparent';
						   } else {
						  element.style.background = '#67748E';
					...
					initLocalStore: function() {
						var localStore = BPMSoft.StoreManager.registerStore({
					...
					generateView: function(scope) {
						var config = this.BPMSoft.findItem(scope.viewConfig, {tag: "ModuleContainer"});
						...
						this.BPMSoft.each(modulesConfig, function(moduleConfig) {
					...
					prepareTabsCollection: function() {
						...
						this.BPMSoft.each(tabsConfig, function(tab) {
							...
							tabsCollection.addItem(this.Ext.create("BPMSoft.BaseViewModel", item));
						}, this);
					...
					getGeneratedModulesConfig: function(tabs) {
						var modulesConfig = [];
						this.BPMSoft.each(tabs, function(tabConfig) {
					 ...
					 * @protected
					 * @param {BPMSoft.BaseViewModelCollection} items Collection of view model.
					 * @param {Object} properties Key-value pairs to filter.
					 * @return {BPMSoft.BaseViewModel} Found item.
					 */
					findWhereInViewModelCollection: function(items, properties) {
						var foundItem;
						this.BPMSoft.each(items, function(viewModel) {
							var values = viewModel.values || viewModel.changedValues;
							this.BPMSoft.each(values, function(value, name) {
								var isFoundItem = this.BPMSoft.isEqual(value, properties[name]);
								if (isFoundItem) {
									foundItem = viewModel;
									return false;
								}
					...
					*/
					...
					getImagesUrlConfig: function(notActiveName, activeName) {
						var imageUrlBuilder = this.BPMSoft.ImageUrlBuilder;
					...
					/**
					 * Changes active tab.
					 * @param {BPMSoft.BaseViewModel} tab Tab configuration object.
					 * @param {Number} tabIndex Index of selected tab.
					 * @param {BPMSoft.BaseViewModelCollection} tabs Collection of tabs.
					 */
					...
					onNotificationReceived: function(response) {
						...
						var isResponseNotChanged =
							this.BPMSoft.isEqual(lastNotificationsResponse, response);
						...
						var notificationSenders = this.get("NotificationSenders");
						if (!this.BPMSoft.contains(notificationSenders, sender)) {
							return;
						}
					...
					getCountersValue: function(config) {
						...
						this.BPMSoft.each(countersMap, function(item) {
					...
					getCombinedCounters: function() {
						...
						this.BPMSoft.each(counterValue, function(counters) {
					...
					setNotificationInfoConfigCounters: function(config, counters) {
						...
						this.BPMSoft.each(counters, function(value, key) {
							key = key.toLowerCase();
					...
					setNotificationInfoGroupPopupsConfig: function(popupsConfig, notifications) {
						if (notifications) {
							this.BPMSoft.each(notifications, function(value) {
					...
					_getChannelByGroupName: function(groupName) {
						var notificationChannelConfigs = this.get("NotificationChannelConfigs");
						var channelConfig = this.BPMSoft.findWhere(notificationChannelConfigs,
							{ groupName: groupName });
					...
					_subscribeOnGetLastRemindTime: function() {
						...
						this.BPMSoft.each(notificationChannelConfigs, function(channelConfig) {
					...
					mergeCounters: function(countersData) {
						...
						if (counters && !countersData.resetCounters) {
							var countersMap = this.get("NotificationCountersMap");
							this.BPMSoft.each(countersMap, function(counterName) {
					...
					updateTabItemsAndCounterValues: function(config) {
						...
						if (this.Ext.isNumber(counterValue) && isTabActive) {
							...
							if (!actualCounterValue && isPanelActive && (counterValue > 0)) {
								if(BPMSoft.Features.getIsEnabled("UseQueryForMarkNewNotificationAsRead")) {
					...
					_getNewPopups: function(reminders) {
						...
						return BPMSoft.filter(reminders, function(notify, key) {
					...
					_setRemindersNotificationId: function(reminders) {
						BPMSoft.each(reminders, function(item, key) {
					...
					_storeAllPopups: function(reminders) {
						var storedPopups = BPMSoft.map(reminders, function(notify, key) {
							return key;
						});
						var localStore = this.get("LocalStore");
					...
					_showAllPopups: function(notifications) {
						BPMSoft.each(notifications, function(notify) {
					...
					getNotifyIcon: function(notify) {
						...
						if (!this.BPMSoft.isGUID(imageId) || this.BPMSoft.isEmptyGUID(imageId)) {
							icon = this.get("DefaultPopupImage");
						} else {
							icon = this.BPMSoft.ImageUrlBuilder.getUrl({
					...
					showPopupWithDecorationIcon: function(notify, config) {
						...
						this.BPMSoft.ResourceUtility.getDataUrl(resourceConfig);
					},
					...
					openEntityPage: function(openPageConfig) {
						...
						const moduleStructure = BPMSoft.ModuleUtils.getModuleStructureByName(entitySchemaName);
				...
				diff: [
					{
						...
						"name": "ImageTabPanel",
						"values": {
							"itemType": BPMSoft.ViewItemType.IMAGE_TAB_PANEL,
