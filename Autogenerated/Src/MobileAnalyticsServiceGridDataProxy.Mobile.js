/**
 * @class BPMSoft.configuration.AnalyticsServiceGridDataProxy
 */
Ext.define("BPMSoft.configuration.AnalyticsServiceGridDataProxy", {
	...
	convertDataToRecords: function(data) {
		data = BPMSoft.AnalyticsServiceResponseParser.parseData(data, this.getColumns());
		var records = [];
		for (var i = 0, ln = data.length; i < ln; i++) {
			records.push(BPMSoft.model.SimpleModel.create({
				Data: data[i]
			}));
		}
		return records;
	...
	doRequest: function() {
		...
		BPMSoft.AnalyticsService[methodName]({
			...
			success: function(data) {
				...
				var total = BPMSoft.Proxy.ProxyUtils.calculateTotalRecords(this.operation, records.length);
