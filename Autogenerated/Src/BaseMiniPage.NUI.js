...
define("BaseMiniPage", ["BaseMiniPageResources", "sandbox", "NetworkUtilities", "RightUtilities",
	...
	return {
		mixins: {
			SecurityUtilitiesMixin: "BPMSoft.configuration.mixins.SecurityUtilitiesMixin",
			CallExtendedMenu: "BPMSoft.configuration.mixins.CallExtendedMenu",
			EmailExtendedMenu: "BPMSoft.configuration.mixins.EmailExtendedMenu",
			LinkedEntitiesExtendedMenu: "BPMSoft.configuration.mixins.LinkedEntitiesExtendedMenu",
			EntityResponseValidationMixin: "BPMSoft.configuration.mixins.EntityResponseValidationMixin",
			EntityRelatedColumnsMixin: "BPMSoft.configuration.mixins.EntityRelatedColumnsMixin",
		...
		methods: {
			...
			/**
			 * @inheritdoc BPMSoft.BaseViewModel#getLookupComparisonType
			 * @override
			 */
			...
			onRender: function() {
				...
				this.initEntity(function() {
					...
					if (this.isAddMode()) {
						...
						BPMSoft.focusFirstInput(selector);
					}
			...
			getDefaultValues: function() {
				...
				this.set("DefaultValues", BPMSoft.deepClone(defaultValues));
			...
			/**
			 * @inheritdoc BPMSoft.BaseEntityPage#getCardModuleResponseConfig
			 * @override
			 */
			...
			adjustMiniPagePosition: function(callback, scope) {
				...
				var isEditMode = this.get("Mode") === BPMSoft.ConfigurationEnums.CardOperation.EDIT;
			...
			updateMaxHeight: function(container) {
				...
				if (this.get("Mode") === BPMSoft.ConfigurationEnums.CardOperation.VIEW) {
					var alignType = BPMSoft.get(container, "alignConfig.alignConfig.alignType");
					if (!container.alignConfig || alignType === BPMSoft.AlignType.BOTTOM) {
						maxHeight = documentHeight - (wrapEl ? wrapEl.getTop(true) : 0);
					} else if (alignType === BPMSoft.AlignType.TOP) {
			...
			openCurrentEntityPage: function() {
				BPMSoft.chain(
					...
					function goToFullCard() {
						...
						if (elementUId) {
							BPMSoft.ProcessModuleUtilities.tryShowProcessCard.call(this, {
			 ...
			 */
			isEditable: function() {
				var editMode = BPMSoft.ConfigurationEnums.CardOperation.EDIT;
			...
			switchMiniPageMode: function() {
				var cardOperationEnum = BPMSoft.ConfigurationEnums.CardOperation;
			...
			isEditMode: function() {
				var currentMode = this.get("Mode");
				return (currentMode === BPMSoft.ConfigurationEnums.CardOperation.EDIT);
			},
			...
			isAddMode: function() {
				var currentMode = this.get("Mode");
				return (currentMode === BPMSoft.ConfigurationEnums.CardOperation.ADD);
			},
			...
			isViewMode: function(columnNames) {
				var currentMode = this.get("Mode");
				var visible = (currentMode === BPMSoft.ConfigurationEnums.CardOperation.VIEW);
			...
			getAdditionalColumnVisibility: function(columnName) {
				var currentMode = this.get("Mode");
				if (currentMode === BPMSoft.ConfigurationEnums.CardOperation.VIEW) {
					return false;
				}
			...
			getModelColumnsList: function() {
				var columnsKeys = BPMSoft.keys(this.columns);
				var columnsList = BPMSoft.getPropertyValuesArray(columnsKeys, this.columns);
				var filteredColumnsList = columnsList.filter(function(item) {
					return this.Ext.isObject(item) &&
						item.type !== BPMSoft.ViewModelColumnType.RESOURCE_COLUMN;
				}, this);
			 ...
			 * @protected
			 * @param {BPMSoft.Collection} bindMap Binding map.
			 */
			 ...
			 * @return {BPMSoft.MiniPageViewGenerator}
			 */
			getViewGenerator: function() {
				return this.Ext.create("BPMSoft.MiniPageViewGenerator");
			},
			 ...
			 * @param {BPMSoft.BaseViewModel} model View model instance.
			 */
			getRequiredColumnsViewConfig: function(itemView, model) {
				var requiredColumns = model.get("RequiredColumns");
				var items = [];
				BPMSoft.each(requiredColumns, function(column) {
			 ...
			 * @protected
			 * @param {BPMSoft.Collection} bindMap List of the model bindings.
			 */
			...
			/**
			 * @inheritdoc BPMSoft.BaseSchemaViewModel#invertColumnValue
			 * @override
			 */
