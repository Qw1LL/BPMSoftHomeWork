 ...
 */
define("BaseProcessSchemaElementPropertiesPage", ["BPMSoft", "BaseProcessSchemaElementPropertiesPageResources",
	"ProcessSchemaParameterViewConfig", "ConfigurationItemGenerator", "LookupUtilitiesV2", "MappingEditMixin", "css!ProcessSchemaElementPropertiesEditCSS"],
		...
		methods: {
			...
			setValidationConfig: function() {
				...
				BPMSoft.each(this.columns, function(column) {
					if (column && column.dataValueType === BPMSoft.DataValueType.MAPPING) {
						this.addColumnValidator(column.name,
							BPMSoft.ProcessSchemaDesignerUtilities.mappingValidator);
					}
				}, this);
			},
			 ...
			 * @protected
			 * @virtual
			 * @return {BPMSoft.BaseViewModel} Item drop-down list the function button.
			 */
			...
			onElementDataLoad: function(element, callback, scope) {
				...
				this.setBgColorForAsidePanel(element)
			...
			setBgColorForAsidePanel: function(element) {
				var defaultBackgroundColor = "#FFFFFF";
				if (element?.tag === "Campaign" || element?.parentSchema?.tag === "Campaign") {
					this.set("ColorPanel", element.color);
				}
				else {
					this.set("ColorPanel", defaultBackgroundColor);
				}
			},
			...
			getIsEditPageDefault: function(editPageSchemaName) {
				return editPageSchemaName === BPMSoft.process.constants.DEFAULT_EDIT_PAGE_SCHEMA_NAME;
			},
			 ...
			 * @private
			 * @param {BPMSoft.BaseViewModelCollection} sortableCollection Collection for sorting.
			...
			getContainerConfig: function(id, wrapClassName, items, visible) {
				return BPMSoft.ProcessSchemaParameterViewConfig.getContainerConfig(id, wrapClassName, items, visible);
			},

			/**
			 * @inheritdoc BPMSoft.ProcessSchemaElementEditable#onSaveElementProperties
			 ...
			 * @param {BPMSoft.ValidationType} [validationResult.validationType] Validation type.
			 * @private
			 */
			 ...
			 * @protected
			 * @param {BPMSoft.BaseModel} validationInfo Validation result model.
			 */
			...
			applyCustomValidation: function(validationInfo) {
				...
				BPMSoft.each(customValidationResults, this.saveValidatioResult, this);
			},
			...
			setValidationState: function(isElementValid) {
				...
				processElement.setPropertyValue("isValidateExecuted", true, {
					silent: (processElement instanceof BPMSoft.BaseProcessSchema)
				});
			 ...
			 * @param {BPMSoft.ValidationType} [validationInfo.validationType] Validation type.
			 * @protected
			 */
			 ...
			 * @param {BPMSoft.Collection} args.selectedRows A collection of selected records.
			...
			getImageConfig: function() {
				...
				var imageUrl = BPMSoft.ImageUrlBuilder.getUrl(titleImage);
				return {
			...
			isNoCompilationFeatureEnableConverter: function(value, defaultValue) {
				if (BPMSoft.Features.getIsEnabled("NoCompilationFeature")) {
					return value;
				}
