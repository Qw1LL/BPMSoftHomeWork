define("MobileDesignerApplicationManifest", ["MobileDesignerManifestColumnResolver"],
	function() {
		...
		Ext.define("BPMSoft.MobileDesignerApplicationManifest", {
			...
			getModelResolver: function(modelName) {
				...
				if (!resolver) {
					resolver = this.resolvers[modelName] = Ext.create("BPMSoft.MobileDesignerManifestColumnResolver", {
						modelName: modelName
					});
				}
				return resolver;
			...
			applyFeatures: function(manifest) {
				...
				if (features) {
					BPMSoft.each(features, function(feature, featureCode) {
			...
			updateModelDataImportConfigSection: function(resolverModelConfigs) {
				...
				BPMSoft.each(resolverModelConfigs, function(resolverModelConfig, modelName) {
			...
			resolveManifest: function(config) {
				...
				BPMSoft.each(resolvers, function(resolver) {
					resolver.resolve({
		...
		return BPMSoft.MobileDesignerApplicationManifest;
