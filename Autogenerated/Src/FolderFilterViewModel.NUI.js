define('FolderFilterViewModel', ['ext-base', 'sandbox', 'BPMSoft', 'FolderFilterViewModelResources'],
	function(Ext, sandbox, BPMSoft, resources) {
		...
		function generate(schema) {
			var viewModelConfig = {
				...
				values: {
					...
					folderFilterColumnList: new BPMSoft.Collection(),
			...
			if (schema.inFolderEntitySchema) {
				...
				BPMSoft.each(schema.inFolderEntitySchema.columns, function(schemaColumn) {
				...
				BPMSoft.each(schema.entitySchema.columns, function(schemaColumn) {
					if (!folderColumnName && schemaColumn.referenceSchemaName ===
						schema.folderEntitySchema.name) {
						folderColumnName = schemaColumn.name;
					}
		...
		function getFiltersInfo() {
			var filters = this.get('filters');
			var filterValues = new BPMSoft.Collection();
			filters.each(function(filterInfo) {
...
//			}
		...
		function initialize() {
			...
			var filterCollection = new BPMSoft.Collection();
			if (filterState) {
				BPMSoft.each(filterState, function(val) {
			...
			if (selectedFiltersInfo.selectedFolderFilters) {
				...
				BPMSoft.each(selectedFiltersInfo.selectedFolderFilters, function(val) {
