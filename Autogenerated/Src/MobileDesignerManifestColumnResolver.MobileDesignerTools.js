define("MobileDesignerManifestColumnResolver", ["sandbox", "EntityStructureHelperMixin"],
	function(sandbox) {
		...
		/**
		 * @class BPMSoft.MobileDesignerManifestColumnResolver
		 * Class that computes the configuration of the manifest model.
		 */
		Ext.define("BPMSoft.MobileDesignerManifestColumnResolver", {
			...
			isLookupColumn: function(columnConfig) {
				...
				return BPMSoft.isLookupDataValueType(columnConfig.dataValueType);
			},
			...
			loadEntitySchemas: function(callback) {
				...
				if (!BPMSoft.utils.array.contains(modelNames, primaryModelName)) {
			...
			processRecordPageColumns: function(modelName, usedColumnNames) {
				...
				for (var i = 0, ln = allColumnNames.length; i < ln; i++) {
					...
					if (this.isLookupSubColumn(columnName)) {
						this.addLookupSubcolumn(modelName, columnName);
						continue;
					} else if (BPMSoft.utils.array.contains(usedColumnNames, columnName)) {
						if (islookupColumn) {
							this.addRequiredModelFromColumn(column);
						}
						this.addColumnToRequiredModel(modelName, columnName);
					...
					if (islookupColumn && columnDefValueSource === BPMSoft.core.enums.EntitySchemaColumnDefSource.SETTINGS) {
						this.requiredSysSettings[columnDefValueSettingsName] = true;
						this.addRequiredModelFromColumn(column);
					} else if (islookupColumn && columnDefValueSource === BPMSoft.core.enums.EntitySchemaColumnDefSource.CONST) {
						this.addRequiredModelFromColumn(column);
					}
				}
			},
			...
			resolve: function(config) {
				...
				this.loadEntitySchemas(function() {
					...
					BPMSoft.each(this.standartDetails, function(detailColumnNames, detailModelName) {
						this.processRecordPageColumns(detailModelName, detailColumnNames);
					}, this);
					BPMSoft.each(this.embeddedDetails, function(config, detailModelName) {
		...
		return BPMSoft.MobileDesignerManifestColumnResolver;
