define("ProcessModule", ["ext-base", "BPMSoft", "sandbox", "ProcessModuleResources", "ProcessModuleUtilities",
	"MaskHelper"
], function(Ext, BPMSoft, sandbox, resources, ProcessModuleUtilities, MaskHelper) {
	...
	function init() {
		...
		BPMSoft.ServerChannel.on(BPMSoft.EventName.ON_MESSAGE, onNextProcessElementReady, this);
	...
	function changeCurrentProcExecItemInHistoryState(processExecDataItem) {
		...
		const currentState = sandbox.publish("GetHistoryState");
		const newState = BPMSoft.deepClone(currentState.state || {});
	...
	function changeNextProcExecDataHistoryState(nextProcessExecData) {
		...
		BPMSoft.each(nextProcessExecData, function(procExecDataItem) {
			if (currentProcElUId) {
				return;
			}
			if (procExecDataItem &&
	...
	function changeHistoryState(processExecDataItem, currentState, newState) {
		...
		if (processExecDataItem.urlToken) {
			...
			const config = BPMSoft.configuration.ModuleStructure[processExecDataItem.entitySchemaName];
	...
	function saveExecutionContext(executionContext, cardModuleSandbox) {
		...
		BPMSoft.each(details, function(detail) {
			if (!detail.collapsed) {
