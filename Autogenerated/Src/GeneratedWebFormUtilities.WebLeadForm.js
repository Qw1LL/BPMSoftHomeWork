define("GeneratedWebFormUtilities", ["ext-base", "BPMSoft", "GeneratedWebFormUtilitiesResources", "RightUtilities"],
	function(Ext, BPMSoft, resources, RightUtilities) {
		...
		function saveFormCodeToFile(scope, recordId) {
			...
			select.filters.add("Id",
				select.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL, "Id", recordId));
					...
					generateFormCode(scope, recordId, function(formCode) {
						...
						ajaxProvider.request({
							...
							callback: function(request, success, response) {
								if (success) {
									var dataToSaveKey = BPMSoft.decode(response.responseText);
									var txtFile = document.createElement("a");
									txtFile.href = "../rest/GeneratedWebFormService/SaveFormDataToFile/" +
										"BPMCRM_" + formName + "/" + dataToSaveKey;
									txtFile.download = "BPMCRM_" + formName + ".txt";
		...
		function generateScript(entityItem) {
			...
			var requestUrl = BPMSoft.loaderBaseUrl + "0/ServiceModel/GeneratedWebFormService.svc/SaveWebFormLeadData";
		...
		function generateFormCode(scope, recordId, callback) {
			...
			select.getEntityCollection(function(response) {
				...
				var entityDefaultValuesCollection = new BPMSoft.Collection();
