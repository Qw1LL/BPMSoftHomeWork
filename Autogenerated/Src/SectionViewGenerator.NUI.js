define('SectionViewGenerator', ['ext-base', 'BPMSoft', 'sandbox', 'SectionViewGeneratorResources',
	'ActionUtilitiesModule', 'GridUtilities'],
	function(Ext, BPMSoft, sandbox, resources, actionUtilities, gridUtilities) {
		var entitySchema;
...
//				columnsConfig: {bindTo: 'columnsConfig'},
		...
		function getControlConfig(schemaItem, bindings) {
			...
			var controlConfig = BPMSoft.getControlConfigByDataValueType(dataValueType);
		...
		function generateView(container, columnsObject, bindings) {
			...
			BPMSoft.each(columnsObject, function(schemaItem) {
		...
		function isLookupSchemaItem(schemaItem) {
			...
			switch (schemaItemType) {
				case BPMSoft.ViewModelSchemaItem.METHOD:
					isLookup = (schemaItem.dataValueType === BPMSoft.DataValueType.LOOKUP);
					break;
				case BPMSoft.ViewModelSchemaItem.ATTRIBUTE:
		...
		function getButtonAddConfig(entitySchema, getEditPages, viewName) {
			...
			if (getEditPages) {
				...
				var structureConfig = BPMSoft.configuration.ModuleStructure[entitySchema.name];
				editPages = structureConfig.pages;
			}
			if (!Ext.isEmpty(editPages)) {
				var menuItemConfig = [];
				BPMSoft.each(editPages, function(item) {
					var pageConfig = {
		...
		function generateTabItems(views) {
			...
			if (views.length > 1) {
				var menuItems = [];
				BPMSoft.each(views, function(viewItem) {
					menuItems.push({
		...
		return {
			...
			generate: function(viewModelSchema, views, columnsSettingsProfile, reports, linksConfig, sectionInfo) {
				...
				BPMSoft.each(views, function(view) {
					var viewConfig;
