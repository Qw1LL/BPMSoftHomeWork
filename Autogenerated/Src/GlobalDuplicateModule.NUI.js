define('GlobalDuplicateModule', ['ext-base', 'BPMSoft',
	'GlobalDuplicateModuleResources', 'GlobalDuplicateModuleView'],
	function(Ext, BPMSoft, resources, GlobalDuplicateModuleView) {
	function upgrade(viewModel, entitySchemaName) {
		//viewModel.set('duplicateGrid', new BPMSoft.Collection());
		var baseGetViews =viewModel.methods.getViews;
		viewModel.methods.getViews = function() {
			this.set('duplicateGridData', new BPMSoft.Collection());
		...
		viewModel.methods.loadDuplicatesChilds = function(responce) {
			...
			esq.filters.addItem(BPMSoft.createColumnInFilterWithParameters('Entity1',responce));
