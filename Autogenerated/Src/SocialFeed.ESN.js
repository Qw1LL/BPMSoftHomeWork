define("SocialFeed", ["SocialFeedResources", "ESNConstants", "RightUtilities", "ESNFeedConfig",
	...
	function(resources, ESNConstants, RightUtilities, ESNFeedConfig, ConfigurationEnums,
		...
		return {
			mixins: {
				SocialFeedUtilities: "BPMSoft.SocialFeedUtilities",
			...
			methods: {
				...
				clearUserProfile: function() {
					...
					this.BPMSoft.saveUserProfile(profileKey, undefined, false);
				},
				...
				updatePosts: function(channels, channel) {
					...
					if (channel) {
						filterPosts.each(function(post) {
							...
							var entity = this.BPMSoft.deepClone(post.get("Entity"));
				 ...
				 * @private
				 * @param {Array} channels ###### #######.
				 * @param {BPMSoft.SocialMessageViewModel} post #### # #####.
				 * @return {Boolean} True, #### #### ### ########### # ##### ##
				 * #######, ##### False.
				 */
				...
				setDefaultValues: function(recordInfo) {
					...
					var channelFilter =
						this.BPMSoft.createColumnFilterWithParameter(this.BPMSoft.ComparisonType.EQUAL, "EntityId",
							channelId);
				...
				mergeObjectProperties: function() {
					...
					this.BPMSoft.each(objects, function(obj) {
						for (var prop in obj) {
				...
				onInsertSocialMessageReceived: function(config) {
					...
					if (this.BPMSoft.SysValue.CURRENT_USER.value === receivedMessage.sysAdminUnitId) {
				 ...
				 * @private
				 * @param {BPMSoft.BaseViewModel} socialMessage Social message.
				 */
				...
				getChannelImage: function() {
					...
					return this.Ext.isEmpty(channel)
						? this.BPMSoft.ImageUrlBuilder.getUrl(this.get("Resources.Strings.NoChannel"))
								...
								if (this.Ext.isEmpty(result)) {
									var filter =
										this.BPMSoft.createColumnFilterWithParameter(
											BPMSoft.ComparisonType.EQUAL, "PublisherRightKind", 1);
				...
				getSocialChannelSelectQuery: function() {
					...
					esq.addMacrosColumn(this.BPMSoft.QueryMacrosType.PRIMARY_IMAGE_COLUMN, "primaryImageValue");
					var displayColumn = esq.addMacrosColumn(this.BPMSoft.QueryMacrosType.PRIMARY_DISPLAY_COLUMN,
						"displayValue");
					displayColumn.orderPosition = 1;
					displayColumn.orderDirection = this.BPMSoft.OrderDirection.DESC;
					return esq;
				},
				 ...
				 * @param {BPMSoft.BaseFilter} filter Messages filter.
				 */
				...
				getRoleSubscriptionFilter: function(userId) {
					...
					return this.BPMSoft.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL, path, userId);
				},
				...
				getUserUnsubscriptionFilter: function(userId) {
					...
					var filter = this.BPMSoft.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL, path,
						userId);
					filterGroup.addItem(filter);
					return this.BPMSoft.createNotExistsFilter(path, filterGroup);
				},
				...
				getCurrentUserPostsFilter: function() {
					...
					var subFilterGroup = this.Ext.create("BPMSoft.FilterGroup");
					subFilterGroup.logicalOperation = this.BPMSoft.LogicalOperatorType.OR;
				 ...
				 * ######### ### ######### ##### # ###### ####### # #####.
				 * @protected
				 * @param {BPMSoft.BaseViewModel} result #####, ####### ##### ######## ### ########.
				 */
				...
				initMessagesNumberEverySearchIteration: function(callback, scope) {
					this.BPMSoft.SysSettings.querySysSettingsItem("MessagesNumberEverySearchIteration",
						function(value) {
				 ...
				 * @param {BPMSoft.BaseViewModel} channel
				 */
				...
				/**
				 * @inheritdoc BPMSoft.BasePageV2#subscribeViewModelEvents
				 * @overridden
				 */
				...
				setSortCaption: function() {
					const socialMessageSortColumns = this.get("SocialMessageSortColumns");
					const currentSortColumnName = this.get("sortColumnName");
					const createdOn = "Resources.Strings.SortCreatedOn";
					const lastActionOn = "Resources.Strings.SortLastActionOn";

					return currentSortColumnName === socialMessageSortColumns[0]
						? this.get(createdOn)
						: this.get(lastActionOn);
				}
			...
			diff: [
				...
				//SocialMessageActionsContainer
				{
					...
					"values": {
						...
						"itemType": BPMSoft.ViewItemType.CONTAINER,
						"classes": {
							wrapClassName: ["postPublishActions", "feedMaxWidth", "postMessage"]
						},
						"items": []
					}
				},
				...
				//PublishButton
				{
					...
					"values": {
						...
						"classes": {
							textClass: ["btn-public", "btn-public-long"]
						},
