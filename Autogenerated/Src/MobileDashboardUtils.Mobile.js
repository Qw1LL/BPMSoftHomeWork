/**
 * @class BPMSoft.configuration.DashboardUtils
 * Dashboard utilities.
 */
Ext.define("BPMSoft.configuration.DashboardUtils", {
	alternateClassName: "BPMSoft.DashboardUtils",
	singleton: true,

	/**
	 * @property {String} Dashboard view generator class name.
	 */
	DashboardViewGeneratorClassName: "BPMSoft.configuration.DashboardViewGenerator",
	...
	convertGridLookupColumn: function(value, modelName) {
		...
		return BPMSoft.Grid.generateRecordLink(modelName, value.value, value.displayValue);
	},

	/**
	 * Gets class name of widget.
	 * @param {BPMSoft.DashboardItemWidgetType} type Widget type.
	 * @returns {String} Class name of widget.
	 */
	getCustomWidgetClassName: function(type) {
		var customClassName = BPMSoft.DashboardItemClassName[type];
	...
	getIsSupported: function(type) {
		return BPMSoft.DashboardItemWidgetType[type] || this.getCustomWidgetClassName(type) !== null;
	},
	...
	getGridColumnConfig: function(config) {
		...
		if (model) {
			...
			primaryColumnName = BPMSoft.BaseModel.getIdProperty();
		...
		if (config.columnName === displayColumnName) {
			gridColumnConfig.convert = function(value, values) {
				if (values[primaryColumnName]) {
					return BPMSoft.Grid.generateRecordLink(config.modelName, values[primaryColumnName], value);
				} else {
					return value;
				}
			};
	 ...
	 * @param {BPMSoft.DashboardStyle} style Dashboard style.
