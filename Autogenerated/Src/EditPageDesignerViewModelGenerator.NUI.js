define('EditPageDesignerViewModelGenerator', ['ext-base', 'BPMSoft', 'EditPageModification',
	'EditPageDesignerViewModelGeneratorResources', 'EditPageDesignerViewGenerator', 'ConfigurationEnums', 'ColumnHelper'],
	function(Ext, BPMSoft, EditPageModification,
		...
		function generate(viewModelSchema, info) {
			...
			fullViewModelSchema.methods.insertModifications = function() {
				var query = Ext.create('BPMSoft.InsertQuery', {
				...
				columnValues.setParameterValue('EditPageName', this.name, BPMSoft.DataValueType.TEXT);
				columnValues.setParameterValue('SerializedData', modificationList, BPMSoft.DataValueType.TEXT);
			...
			fullViewModelSchema.methods.save = function(isValid) {
				...
				var saveFunction = function(responce) {
					...
					select.filters.add('filterModifications', BPMSoft.createColumnFilterWithParameter(
						BPMSoft.ComparisonType.EQUAL, 'EditPageName', this.name));
					select.getEntityCollection(function(result) {
						if (result.success && result.collection.getCount()) {
							var item = result.collection.getByIndex(0);
							var modificationList = Ext.JSON.encode(this.get('ModificationsList').getItems());
							item.columns.SerializedData.type = BPMSoft.ViewModelColumnType.ENTITY_COLUMN;
							item.set('SerializedData', modificationList, BPMSoft.DataValueType.TEXT);
			...
			fullViewModelSchema.methods.skipAction = function(action, element) {
				return element.type === BPMSoft.ViewModelSchemaItem.DETAIL && action.tag === 'edit';
			};
			...
			fullViewModelSchema.methods.hideColumn = function() {
				...
				BPMSoft.each(elements.getItems(), function(el) {
			...
			fullViewModelSchema.methods.isContainer = function(element) {
				return element.type === BPMSoft.ViewModelSchemaItem.GROUP;
			};
			fullViewModelSchema.methods.isDetail = function(element) {
				return element.type === BPMSoft.ViewModelSchemaItem.DETAIL;
			};
			...
			fullViewModelSchema.methods.GetConfigFromExt = function(extElement) {
				...
				if (extElement.hasOwnProperty('items')) {
					BPMSoft.each(extElement.items.items, function(item) {
			...
			fullViewModelSchema.methods.addGroup = function() {
				var callback = function(returnCode, controlData) {
					if (returnCode === 'ok' && controlData.name.value) {
						...
						var groupName = BPMSoft.generateGUID();
						var columnConfig = {
						...
						this.insertModification({
							type: 'add',
							value: BPMSoft.deepClone(columnConfig),
			...
			fullViewModelSchema.methods.addNewColumn = function(tag) {
				...
				var handler = function(args) {
					...
					var value = BPMSoft.deepClone(args.itemConfig);
			...
			fullViewModelSchema.methods.editColumn = function() {
				var item = BPMSoft.deepClone(this.getCurrentColumnInfo());
				if (this.isContainer(item)) {
					var callback = function(returnCode, controlData) {
						if (returnCode === 'ok' && controlData.name.value) {
							var columnConfig = BPMSoft.deepClone(item);
				...
				if (this.isColumn(item)) {
					...
					var handler = function(args) {
						...
						this.insertModification({
							type: 'modify',
							value: BPMSoft.deepClone(args.itemConfig),
							name: prevName
						}, {EditStructureContainerId: Id});
						this.setCurrentElement({
							itemName: args.itemConfig.name,
			...
			fullViewModelSchema.methods.addColumn = function() {
				...
				var handler = function(args) {
					...
					var value = BPMSoft.deepClone(columnConfig);
					value.isDesigner = true;
