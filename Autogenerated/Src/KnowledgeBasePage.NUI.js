define('KnowledgeBasePage', ['ext-base', 'BPMSoft', 'sandbox', 'KnowledgeBase', 'KnowledgeBasePageStructure',
	...
	function(Ext, BPMSoft, sandbox, KnowledgeBase, structure, resources, GeneralDetails, ConfigurationEnums,
		...
		structure.userCode = function() {
			...
			this.methods.runQueriesInBatch = function(queriesArray) {
				var initBq = Ext.create('BPMSoft.BatchQuery');
				BPMSoft.each(queriesArray, function(item) {
			...
			this.methods.modificateBeforeBind = function() {
				if (!this.get('knowBaseImagesCollection')) {
					this.set('knowBaseImagesCollection', new BPMSoft.BaseViewModelCollection());
					this.set('knowBaseImagesCollection', new BPMSoft.BaseViewModelCollection());
				}
			...
			this.methods.setFavorites = function() {
				...
				if (!setFavorite) {
					...
					insert.setParameterValue('KnowledgeBase', this.get('Id'), BPMSoft.DataValueType.GUID);
					...
					deleteQuery.filters.addItem(deleteQuery.createColumnFilterWithParameter(
						BPMSoft.ComparisonType.EQUAL, 'KnowledgeBase', this.get('Id')));
			...
			this.methods.setLikeIt = function() {
				...
				if (!likeSet) {
					...
					deleteQuery.execute(function() {
						...
						insert.setParameterValue('Contact', BPMSoft.SysValue.CURRENT_USER_CONTACT.value,
							BPMSoft.DataValueType.GUID);
						insert.setParameterValue('LikeIt', 1, BPMSoft.DataValueType.INTEGER);
						insert.execute(this.getLikeItCount, this);
					}, this);

				} else {
					this.set('likeSet', false);
					var deleteLikeQuery = Ext.create('BPMSoft.DeleteQuery', {
						rootSchemaName: 'Like'
					});
		...
		function getFavorites() {
			...
			select.addColumn('Contact');
			select.addColumn('KnowledgeBase');
			select.filters.addItem(select.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL, 'Contact',
		...
		function getLikeItCount() {
			...
			select.addAggregationSchemaColumn('LikeIt', BPMSoft.AggregationType.SUM, 'LikeItSUM');
			select.filters.addItem(select.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL,
				'KnowledgeBase', this.get('Id')));
			var callback = function(response) {
				if (response.success && response.collection.getCount() > 0) {
