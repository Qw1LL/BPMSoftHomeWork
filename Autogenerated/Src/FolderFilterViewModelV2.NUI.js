define("FolderFilterViewModelV2", ["FolderFilterViewModelV2Resources",
	...
	function(resources, quickFilterViewResources, ConfigurationConstants) {
		...
		Ext.define("BPMSoft.configuration.FilterViewModel", {
			extend: "BPMSoft.BaseViewModel",
			alternateClassName: "BPMSoft.FilterViewModel",
			mixins: [
		...
		function generate(config) {
			...
			var values = {
				...
				filtersViews: Ext.create("BPMSoft.Collection"),
				currentFilterName: null,
				folderFilterColumnList: Ext.create("BPMSoft.Collection"),
			...
			if (config.inFolderEntitySchema) {
				...
				BPMSoft.each(config.inFolderEntitySchema.columns, function(schemaColumn) {
				...
				BPMSoft.each(config.entitySchema.columns, function(schemaColumn) {
					if (!folderColumnName && schemaColumn.referenceSchemaName ===
						config.folderEntitySchema.name) {
						folderColumnName = schemaColumn.name;
					}
		...
		function getFilters() {
			...
			filterCollection.logicalOperation = BPMSoft.LogicalOperatorType.OR;
			var filterValues = [];
			filters.each(function(filterInfo) {
				var filter = filterInfo.get("filter");
				if (filter) {
					filterCollection.add(filterInfo.get("filterName"), BPMSoft.deserialize(filter));
				} else {
					filterValues.push(filterInfo.get("value"));
				}
			});
			if (filterValues.length > 0) {
				filterCollection.add(this.get("staticFoldersFilter"), BPMSoft.createColumnInFilterWithParameters(
		...
		function getFiltersInfo() {
			var filters = this.get("filters");
			var Ext = this.getExt();
			var filterValues = Ext.create("BPMSoft.Collection");
			filters.each(function(filterInfo) {
				var value = filterInfo.get("value");
				if (!filterInfo.get("folderType") && BPMSoft.isGUID(value)) {
		...
		function addFilterValue(filterName, filterValues) {
			if (Ext.isArray(filterValues)) {
				BPMSoft.each(filterValues, function(currentValue) {
		...
		function initialize() {
			...
			var Ext = this.getExt();
			var filterCollection = Ext.create("BPMSoft.Collection");
			if (filterState) {
				var entitySchema = this.get("folderEntitySchema");
				BPMSoft.each(filterState, function(filter) {
					var folder = Ext.create(BPMSoft.FolderFilterItemViewModel, {
						entitySchema: entitySchema,
						columns: BPMSoft.deepClone(entitySchema.columns),
						isNew: false,
						values: {
							Id: filter.value,
		...
		function getFilterViewModel(filterName, config, folder) {
			...
			var filterViewModel = Ext.create("BPMSoft.FilterViewModel", filterViewModelConfig);
