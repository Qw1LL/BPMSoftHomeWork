define("BaseMultilingualEntityLookupPage", [
	...
	return {
		mixins: {
			LanguageTabMixin: "BPMSoft.LanguageTabMixin",
			TagUtilitiesV2Mixin: "BPMSoft.TagUtilities"
		},
		attributes: {
			/**
		...
		methods: {
			...
			/**
			 * Set primary language from BPMSoft global object.
			 * @private
			 */
			_initSysLanguage: function() {
				this.set("SysLanguage", {
					"Id": BPMSoft.SysValue.PRIMARY_LANGUAGE.value,
					"Name": BPMSoft.SysValue.PRIMARY_LANGUAGE.displayValue,
					"Code": BPMSoft.SysValue.PRIMARY_LANGUAGE.code,
					"id": BPMSoft.SysValue.PRIMARY_LANGUAGE.value,
					"name": BPMSoft.SysValue.PRIMARY_LANGUAGE.displayValue,
					"code": BPMSoft.SysValue.PRIMARY_LANGUAGE.code
				});
			...
			_loadDataFromCard: function(languageId) {
				...
				var multiLanguageContent = language.get("MultiLanguageContent");
				BPMSoft.each(multiLanguageContent, function(item) {
			...
			getContentsUpdateEsq: function(language) {
				...
				updateQuery.filters.add("recordIdFilter", updateQuery.createColumnFilterWithParameter(
					BPMSoft.ComparisonType.EQUAL, this.get("MultilingualConnectionColumnName"),
					this.get("Id")));
				return updateQuery;
			},
			...
			addLanguageItemToCollection: function(item, isCopyMode) {
				...
				var languageViewModel = Ext.create("BPMSoft.BaseViewModel", {
					"values": {
						...
						"RecordId": isCopyMode ? BPMSoft.generateGUID() : item.get("Id"),
			 ...
			 * @param {BPMSoft.Collection} languages Languages to be saved.
			 * @protected
			 * @obsolete
			 * @return {BPMSoft.BatchQuery} Returns batch query for inserting multilingual data.
			 */
			getContentsInsertBatchEsq: function(languages) {
				var bq = Ext.create("BPMSoft.BatchQuery");
				BPMSoft.each(languages, function(language) {
					var insert = this.getContentsInsertEsq(language);
					bq.add(insert);
				}, this);
				return bq;
			 ...
			 * @protected
			 * @return {BPMSoft.FilterGroup} Returns lookup filter.
			 */
			...
			addSetupLanguageMenuItem: function() {
				...
				menuItems.addItem(this.Ext.create("BPMSoft.BaseViewModel", {
					values: {
			...
			addLanguageFromQuickMenu: function(tag) {
				this.showCopyCurrentLangConfirmation(function(resultCode) {
					var copyLanguageId = resultCode === BPMSoft.MessageBoxButtons.YES.returnCode
			 ...
			 * @return {BPMSoft.InsertQuery} Insert Query for record.
			 * @protected
			 */
			getContentsInsertEsq: function(language) {
				...
				insert.setParameterValue("Id",
					language.get("RecordId"), BPMSoft.DataValueType.GUID);
				return insert;
			},
				...
				BPMSoft.chain(
					...
					function(next, languages) {
						...
						this.showCopyCurrentLangConfirmation(function(resultCode) {
							var copyLanguageId = resultCode === BPMSoft.MessageBoxButtons.YES.returnCode
								? this.get("ActiveLanguageId") : false;
							BPMSoft.each(selectedLanguages, function(language) {
