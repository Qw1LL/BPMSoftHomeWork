define('FileDetail', ['ext-base', 'BPMSoft', 'sandbox', 'FileDetailStructure', 'FileDetailResources',
	'ConfigurationConstants', 'ConfigurationEnums', 'ConfigurationFileApi', 'ViewUtilities'],
	function(Ext, BPMSoft, sandbox, structure, resources, ConfigurationConstants, ConfigurationEnums,
		...
		structure.userCode = function() {
			...
			this.methods.onFileSelect = function(files) {
				...
				this.add(null, function() {
					...
					loadedFileId = BPMSoft.ConfigurationFileApi.upload(config, callback);
			...
			this.methods.reloadKeys = function() {
				...
				BPMSoft.each(items, function(gridDataItem, index) {
			...
			this.methods.changeLock = function(isLocked) {
				...
				BPMSoft.each(collection, function(item) {
					...
					update.setParameterValue("LockedOn", currentDateTime, BPMSoft.DataValueType.DATE_TIME);
					if (isLocked) {
						update.setParameterValue("LockedBy", BPMSoft.SysValue.CURRENT_USER_CONTACT.value,
							BPMSoft.DataValueType.GUID);
					} else {
						update.setParameterValue("LockedBy", null, BPMSoft.DataValueType.GUID);
					}
					update.execute(function(response) {
						var gridData = this.get('gridData');
						var items = gridData.getItems();
						gridData.clear();
						BPMSoft.each(items, function(gridDataItem) {
