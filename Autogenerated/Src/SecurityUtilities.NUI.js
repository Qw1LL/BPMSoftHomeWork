define("SecurityUtilities", ["ext-base", "BPMSoft", "SecurityUtilitiesResources", "RightUtilities"],
		function(Ext, BPMSoft, resources, RightUtilities) {
			var definedClass = Ext.ClassManager.get("BPMSoft.SecurityUtilities");
			if (definedClass) {
				return Ext.create(definedClass);
			}

			Ext.define("BPMSoft.configuration.mixins.SecurityUtilitiesMixin", {
				alternateClassName: "BPMSoft.SecurityUtilitiesMixin",
				 ...
				 * @virtual
				 */
				hideBodyMask: this.BPMSoft.emptyFn,
				...
				checkAvailability: function(callback, scope) {
					...
					BPMSoft.chain.apply(this, checkChain);
				},
				...
				handleResponseCanEditColumns: function(config, response) {
					...
					if (response) {
						var columnRights = {};
						this.BPMSoft.each(response, function(column) {
