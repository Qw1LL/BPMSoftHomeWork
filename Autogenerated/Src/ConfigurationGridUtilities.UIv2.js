define("ConfigurationGridUtilities", ["BusinessRulesApplierV2", "ConfigurationEnums", "BusinessRuleModule"
	 ...
	 */
	Ext.define("BPMSoft.configuration.mixins.ConfigurationGridUtilities", {
		alternateClassName: "BPMSoft.ConfigurationGridUtilities",
		 ...
		 * @type {String}
		 */
		currentActiveColumnName: BPMSoft.emptyString,
		 ...
		 * #########, ########## ## ######## ####### ######.
		 * @private
		 * @param {BPMSoft.BaseViewModel} activeRow ###### #######.
		 * @return {Boolean} ######### ########.
		 */
		getIsRowChanged: function(activeRow) {
			var changed = activeRow.isNew;
			BPMSoft.each(activeRow.changedValues, function(changedValue, columnName) {
				var column = this.columns[columnName];
				var columnType = column && column.type;
				changed = changed || columnType === BPMSoft.ViewModelColumnType.ENTITY_COLUMN;
				return !changed;
			}, activeRow);
			return changed;
		},
		...
		activeRowSaved: function(row, callback, scope) {
			scope = scope || this;
			callback = callback || BPMSoft.emptyFn;
		...
		setDefaultFocus: function(row) {
			...
			BPMSoft.each(profileColumns, function(column, name) {
				var methodName = BusinessRulesApplier.getMethodName(BusinessRuleModule.enums.Property.ENABLED,
					name);
		...
		unfocusRowControls: function(activeRow) {
			BPMSoft.each(activeRow.columns, function(changedValue, columnName) {
				var column = this.columns[columnName];
		...
		getRowViewGeneratorClassName: function() {
			...
			return isEnabled === false ? "BPMSoft.DisableControlsGenerator" : "BPMSoft.ViewGenerator";
		},
		 ...
		 * @protected
		 * @param {BPMSoft.EntitySchemaColumn} entitySchemaColumn Registry cells column.
		...
		getRowDefaultValues: function(typeColumnValue) {
			...
			return BPMSoft.deepClone(defaultValues);
		},
		 ...
		 * @protected
		 * @virtual
		 * @param {BPMSoft.BaseViewModelCollection} dataCollection ######### ##### #####.
		 ...
		 * @private
		 * @param {BPMSoft.Collection} gridData ######### ###### #######.
		 * @param {BPMSoft.Collection} dataCollection ######### ##### ########### #####.
		 * @param {Number} index ####### ########### ###### # #########.
		 */
		addItemToGridDataCollection: function(gridData, dataCollection, index) {
			var tempGridDataCollection = new BPMSoft.Collection();
		...
		initEditableGridRowViewModel: function(callback, scope) {
			...
			const schemaBuilder = Ext.create("BPMSoft.SchemaBuilder");
			let config = {
	...
	return Ext.create("BPMSoft.ConfigurationGridUtilities");
});
