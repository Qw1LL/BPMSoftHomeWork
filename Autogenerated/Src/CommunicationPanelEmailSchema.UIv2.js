		...
		function(resources, ContainerListGenerator, ConfigurationConstants, ConfigurationEnums, ModalBox, NetworkUtilities,
			...
			return {
				...
				mixins: {
					...
					/**
					 * @class BPMSoft.CheckModuleDestroyMixin Implements publish and show CanBeDestroy message.
					 */
					CheckModuleDestroyMixin: "BPMSoft.CheckModuleDestroyMixin",
					/**
					 * @class BPMSoft.RelatedEmailsMixin Provides methods for related eamils search.
					 */
				...
				attributes: {
					...
					"IsEmailBoxSettingsButtonVisible": {
						dataVlueType: this.BPMSoft.DataValueType.BOOLEAN
					},
					...
					/**
					 * Current panel display state. State options taken from {@link BPMSoft.configuration.EmailConstants}.
					 */
					"DisplayState": {
						dataValueType: BPMSoft.DataValueType.INTEGER,
						value: 0
				...
				methods: {
					 ...
					 * @private
					 * @param {BPMSoft.EntitySchemaQuery} select Emails entity schemq query.
					...
					initializePageableOptions: function(select, config) {
						...
						if (BPMSoft.useOffsetFetchPaging) {
							...
							if (isNextPageLoading) {
								...
								conditionalValues = this.Ext.create("BPMSoft.ColumnValues");
					...
					getEntityConnectionTypeColumnList: function(entityConnectionColumnNames) {
						...
						const moduleStructure = BPMSoft.configuration.ModuleStructure;
						this.BPMSoft.each(entityConnectionColumnNames, function(columnName) {
					 ...
					 * @private
					 * @return {BPMSoft.EntitySchemaQuery} Email accounts select.
					 */
					getMailboxSettingsSelect: function() {
						...
						userEmailColumn.orderDirection = this.BPMSoft.OrderDirection.ASC;
						var filters = this.BPMSoft.createFilterGroup();
						var currentUserId = this.BPMSoft.SysValue.CURRENT_USER.value;
						filters.logicalOperation = this.BPMSoft.LogicalOperatorType.OR;
						var sharedMailBoxFilters = this.BPMSoft.createFilterGroup();
						...
						sharedMailBoxFilters.add("UserCanSeeSharedEmailsFilter",
							settingsEsq.createColumnFilterWithParameter(this.BPMSoft.ComparisonType.EQUAL,
						...
						filters.add("filterSysAdminUnit", settingsEsq.createColumnFilterWithParameter(
							this.BPMSoft.ComparisonType.EQUAL, "SysAdminUnit", currentUserId));
					 ...
					 * @private
					 * @param {BPMSoft.BaseViewModel} model Email model instance.
					 */
					...
					_internalBuildSchema: function(buildConfig, callback, scope) {
						var schemaBuilderName = this.get("SchemaBuilderName") || "BPMSoft.SchemaBuilder";
					 ...
					 * @private
					 * @param {BPMSoft.FilterGroup} filters Emails query filters.
					...
					_addOutgoingFilter: function(filters, roleColumn, rolesIds) {
						var createColumnInFilter = BPMSoft.createColumnInFilterWithParameters;
						var createColumnFilter = BPMSoft.createColumnFilterWithParameter;
						var equal = BPMSoft.ComparisonType.EQUAL;
						...
						filters.add("SendDateFilter", this.BPMSoft.createColumnIsNotNullFilter("SendDate"));
					},
					...
					_addDraftFilter: function(filters) {
						filters.add("SendStatusFilter", this.BPMSoft.createColumnFilterWithParameter(
							BPMSoft.ComparisonType.NOT_EQUAL,
							"Activity.EmailSendStatus",
							ConfigurationConstants.Activity.EmailSendStatus.Sended));
						filters.add("EmptyMailboxSyncSettingsFilter", this.BPMSoft.createColumnIsNullFilter(
							"MailboxSyncSettings"));
					},
					 ...
					 * @private
					 * @param {BPMSoft.FilterGroup} filters Emails query filters.
					 * @param {BPMSoft.EntitySchemaQuery} esq Emails query.
					 */
					_addFilterByEmailType: function(filters, esq) {
						var roleColumn = "Role";
						var emailType = this.get("EmailType");
						if (emailType === EmailConstants.emailType.DRAFT) {
							var currentUserContact = BPMSoft.SysValue.CURRENT_USER_CONTACT.value;
							filters.add("currentContactFilter", this.BPMSoft.createColumnFilterWithParameter(
									BPMSoft.ComparisonType.EQUAL, "Owner", currentUserContact));
						}
					...
					_anyMailboxSelected: function() {
						return !this.BPMSoft.isEmpty(this._getSelectedMailboxesIds());
					},
					...
					_getCurrentTabSettings: function() {
						...
						this.BPMSoft.each(attributesNames, function(attributeName) {
							tabSettings[attributeName] = this.BPMSoft.deepClone(this.get(attributeName));
						}, this);
						return tabSettings;
					},
					 ...
					 * @private
					 * @param {BPMSoft.BaseViewModelCollection} emailTypeMenuItems Menu items collection to fill.
					 */
					_addMailboxesFiltersMenuItems: function(emailTypeMenuItems) {
						if (this.isNotEmpty(this.$EmailAccounts)) {
							...
							var allMailboxesItem = this.Ext.create("BPMSoft.BaseViewModel", {
								values: {
					 ...
					 * @private
					 * @param {BPMSoft.BaseViewModelCollection} menuItemsCollection Menu items collection.
					...
					_addMailboxesMenuItems: function(menuItemsCollection, configFn) {
						this.BPMSoft.each(this.$EmailAccounts, function(emailAccount) {
							let valuesConfig = this.Ext.apply({
								"Id": "emailAccount_" + this.BPMSoft.generateGUID(),
								"Caption": emailAccount.Caption,
								"Tag": emailAccount.Id,
								"MarkerValue": emailAccount.Caption
							}, configFn.call(this, emailAccount));
							var emailAccountItem = this.Ext.create("BPMSoft.BaseViewModel", {
								values: valuesConfig
							});
					...
					_loadActivitiesByIds: function(clearCollection, ids, callback, scope) {
						...
						let primaryColumnFilter = this.BPMSoft.createColumnInFilterWithParameters("Id", ids);
						let filters = this.BPMSoft.createFilterGroup();
					 ...
					 * @private
					 * @param {BPMSoft.BaseViewModel} email Email instance.
					 */
					...
					_loadAttachmentsInfo: function(ids) {
						...
						esq.addColumn("Activity");
						let primaryRecordFilter = this.BPMSoft.createColumnInFilterWithParameters("Activity", ids);
						let filters = this.BPMSoft.createFilterGroup();
						filters.add("ActivityFilter", primaryRecordFilter);
						let notInlineFilter = this.BPMSoft.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL, "Inline", false);
					 ...
					 * @protected
					 * @param {BPMSoft.EntitySchemaQuery} esq The request, which will be added to the column.
					 */
					addEsqColumns: function(esq) {
						var columns = this.getEmailSelectColumns();
						this.BPMSoft.each(columns, function(columnName) {
					 ...
					 * @protected
					 * @param {BPMSoft.EntitySchemaQuery} esq The request, which filters will be added.
					 */
					addFilters: function(esq) {
						var isProcessed = this.get("IsProcessed");
						var filters = this.BPMSoft.createFilterGroup();
						filters.add("isProcessed", this.BPMSoft.createColumnFilterWithParameter(
								BPMSoft.ComparisonType.EQUAL, "IsNeedProcess", isProcessed));
					 ...
					 * @protected
					 * @param {BPMSoft.EntitySchemaQuery} esq Query instance.
					 */
					addSorting: function(esq) {
						...
						sortByColumn.orderDirection = BPMSoft.OrderDirection.DESC;
					},

					/**
					 * Adds columns to email senders query.
					 * @protected
					 * @param {BPMSoft.EntitySchemaQuery} sendersESQ Email senders query.
					 */
					...
					initParameters: function() {
						this.set("EmailTypeMenuItems", this.Ext.create("BPMSoft.BaseViewModelCollection"));
						this.set("EmailCollection", this.Ext.create("BPMSoft.BaseViewModelCollection"));
						this.set("EmdCollection", this.Ext.create("BPMSoft.BaseViewModelCollection"));
					 ...
					 * @protected
					 * @param {BPMSoft.Collection} collection Email messages collection.
					 */
					 ...
					 * @protected
					 * @return {BPMSoft.BaseViewModelCollection} Email types menu items collection.
					 */
					getEmailTypeMenuItems: function() {
						...
						this.BPMSoft.each(emailTypesItemsConfigs, function(config) {
					 ...
					 * @protected
					 * @return {BPMSoft.BaseViewModelCollection} Email processed or not processed menu items collection.
					 */
					getEmailProcessedMenuItems: function() {
						...
						this.BPMSoft.each(configs, function(config) {
							var emailTypeMenuItem = this._getTypeEmailMenuItem(config);
					 ...
					 * @return {BPMSoft.BaseViewModel} Menu item model.
					 */
					...
					subscribeSandboxEvents: function() {
						...
						this.BPMSoft.ServerChannel.on(BPMSoft.EventName.ON_MESSAGE, this.onMailboxesListChanged,
							this);
					},
					 ...
					 * @protected
					 * @param {BPMSoft.BaseViewModelCollection} collection Email collection.
					 * @param {BPMSoft.BaseViewModel} item Email item.
					 */
					 ...
					 * @return {BPMSoft.BaseViewModelCollection} modified @param collection.
					 */
					...
					onGetItemConfig: function(itemConfig) {
						var viewConfig = this.BPMSoft.deepClone(this.get("EmailViewConfig"));
					...
					onEmailsLoaded: function(result, clearCollection) {
						if (result.success) {
							...
							var data = this.Ext.create("BPMSoft.BaseViewModelCollection");
							var collection = this.get("EmailCollection");
							dataCollection.each(function(item) {
					...
					updateEntityConnectionTypeColumnList: function(item, model) {
						...
						BPMSoft.each(entityConnectionTypeColumnList, function(entityConnectionTypeColumn) {
					 ...
					 * @return {BPMSoft.BaseViewModel} Email viewmodel instance.
					 */
					getEmailViemModelInstance: function() {
						...
						var viewModel = this.Ext.create(viewModelClass, {
							...
							values: {
								"ProcessParametersArray": this.BPMSoft.deepClone(processList),
								"EntityConnectionColumnList": this.BPMSoft.deepClone(relationColumns),
								"DefaultEntityConnectionColumns": this.BPMSoft.deepClone(defaultRelationColumns)
							}
						});
					 ...
					 * @param {BPMSoft.BaseViewModel} entity Email message content.
					 * @param {BPMSoft.BaseViewModel} [viewModel] Email viewmodel instance.
					 * @return {BPMSoft.BaseViewModel} Email viewmodel instance.
					 */
					 ...
					 * @param {BPMSoft.BaseViewModelCollection} settingsList Email accounts collection.
					 */
					...
					initBuildType: function(callback) {
						...
						BPMSoft.SysSettings.querySysSettings(sysSettings, function(response) {
					...
					initEmailProcessTag: function(callback) {
						this.BPMSoft.SysSettings.querySysSettingsItem("EmailProcessTag", function(value) {
					...
					createEmail: function() {
						...
						var config = {
							sandbox: this.sandbox,
							entitySchemaName: EmailConstants.entitySchemaName,
							primaryColumnValue: this.BPMSoft.GUID_EMPTY,
					...
					loadProcessList: function(callback) {
						...
						var select;
						if (this.BPMSoft.ProcessEntryPointUtilities.getCanRunProcessFromSection()) {
					...
					getProcessFilters: function() {
						...
						filters.push(BPMSoft.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL, "TagProperty",
								emailProcessTag));
						return filters;
					},
					 ...
					 * @param {BPMSoft.Collection} processList Avaliable rocesses list.
					 */
					 ...
					 * @param {BPMSoft.BaseViewModel} model Email message model instance.
					 */
					 ...
					 * @param {BPMSoft.DataValueType.GUID} value Email type filter value.
					 */
					...
					getEmptyMessageConfig: function(config) {
						...
						var preparedConfig = this.BPMSoft.deepClone(this.get("EmptyMessageConfig"));
					...
					initEmptyMessageConfig: function(callback, scope) {
						var emptyMessageGeneratorConfig = this.BPMSoft.deepClone(this.get("EmptyMessageGeneratorConfig"));
					...
					onItemClick: function(e) {
						const clickedEl = document.querySelector(`[id*='${e}']`)
				...
				diff: /**SCHEMA_DIFF*/[
					...
					{
						"operation": "insert",
						"name": "EmailActionsContainer",
						"propertyName": "items",
						"parentName": "EmailTabHeader",
						"values": {
							"itemType": BPMSoft.ViewItemType.CONTAINER,
							"classes": {"wrapClassName": ["email-actions-container"]},
							"items": []
						}
					},
					{
						...
						"name": "AddEmail",
						"propertyName": "items",
						"parentName": "EmailActionsContainer",
						"values": {
					...
					{
						"operation": "insert",
						"name": "TypeButtonsContainer",
						"propertyName": "items",
						"parentName": "EmailTabHeader",
						"values": {
							"itemType": BPMSoft.ViewItemType.CONTAINER,
							"classes": {"wrapClassName": ["type-buttons-container"]},
							"items": []
						}
					},
					{
						"operation": "insert",
						"parentName": "TypeButtonsContainer",
						"propertyName": "items",
						"name": "MailTypeButton",
						"values": {
							"itemType": BPMSoft.ViewItemType.BUTTON,
