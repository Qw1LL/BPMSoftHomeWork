		...
		function createConstructor(context) {
			...
			function loadCardView(renderTo, moduleInstance) {
				...
				if (processData && !Ext.isEmpty(processData.parameters)) {
					BPMSoft.each(processData.parameters, function(param, name) {
						if (ProcessHelper.getIsDateTimeDataValueType(param.dataValueType)) {
							processData.parameters[name] = BPMSoft.parseDate(param.value);
						}
					}, this);
				}
				...
				if (processData) {
					BPMSoft.each(processData.parameters, function(value, name) {
						valuePairs.push({
							name: name,
							value: value
						});
					...
					require([schemaName, detailsCollapsedProfileKey], function(schema, detailsCollapsedProfile) {
						...
						createSchemaParents(schema, function(schemaConfig, parentSchemaConfig) {
							...
							EditPageDesignerHelper.getModifications(schemaName, function(editPageModifications) {
								...
								BPMSoft.each(viewModelSchema.schema.leftPanel, function(schemaItem) {
									setDetailCollapsed(schemaItem, detailsCollapsedProfile);
								});
								BPMSoft.each(viewModelSchema.schema.rightPanel, function(schemaItem) {
									setDetailCollapsed(schemaItem, detailsCollapsedProfile);
								});
								...
								var reportsSelectedCallback = function(reports) {
									...
									sandbox.subscribe("DetailInfo", function(detailId) {
										BPMSoft.each(detailsInfo, function(item) {
									...
									viewModel.changeDetailSubscribeByName = function(name, handler) {
										...
										BPMSoft.each(detailsInfo, function(detailItem) {
									...
									viewModel.ajaxProvider = BPMSoft.AjaxProvider;

									var detailsInfo = info.details;

									BPMSoft.each(viewModel.columns, function(column) {
										if (column.isCollection === true) {
											viewModel.set(column.name, new BPMSoft.Collection());
										}
									});
									...
									viewModel.onDetailCollapsedChanged = function(collapsed, tag) {
										...
										BPMSoft.utils.saveUserProfile(detailProfileKey, detailProfile, false);
										if (collapsed) {
											return;
										}
										BPMSoft.each(this.entitySchemaInfo.details, function(detailInfo) {
									...
									viewConfig = BPMSoft.deepClone(generatedViewConfig);

									view = Ext.create(generatedViewConfig.className || "BPMSoft.Container",
										generatedViewConfig);
									...
									BPMSoft.each(info.sysSettings, function(item) {
										SysSettingsArray.push(item.sysSettingName);
									}, this);
									if (SysSettingsArray.length > 0) {
										BPMSoft.SysSettings.querySysSettings(SysSettingsArray, function() {
											if (instance.isDestroyed) {
												return;
											}
			...
			function createSchemaParents(schemaConfig, callback) {
				...
				if (parentSchemaName === "BPMSoft.model.BaseViewModel") {
			...
			function extendStructure(parent, child) {
				var structure = BPMSoft.deepClone(parent);
			...
			function setDetailCollapsed(schemaItem, profile) {
				if (schemaItem.type === BPMSoft.ViewModelSchemaItem.DETAIL) {
			...
			function createSchema(schemaConfig, parentSchemaConfig) {
				...
				var parentSchema = BPMSoft.deepClone(parentSchemaConfig || schemaConfig);
			...
			function prepareAdditionalValues() {
				...
				BPMSoft.each(info.sysSettings, function(item) {
					if (item.isLookup) {
						...
						prepareValues.push({
							...
							value: BPMSoft.SysSettings.cachedSettings[item.sysSettingName]
						});
					} else {
				...
				BPMSoft.each(valuePairs, function(item) {
					...
					} else if (!Ext.isEmpty(column) && column.dataValueType === BPMSoft.DataValueType.BOOLEAN) {
						additionalValues.push({
							columnName: columnName,
				...
				if (prepareValues.length > 0) {
					...
					BPMSoft.each(prepareValues, function(item) {
			...
			function getPrepareValuesQuery(prepareValue) {
				...
				BPMSoft.each(info.columns[column.name], function(additionalColumnName) {
			...
			function prepareEntity() {
				if (action === "edit" || action === "view") {
					viewModel.loadEntity(recordId, function() {
						...
						BPMSoft.each(additionalValues, function(item) {
			...
			function prepareProcessDataParameters() {
				if (processData && processData.parameters && viewModel && viewModel.columns) {
					BPMSoft.each(processData.parameters, function(value, name) {
						var parameterValue = BPMSoft.deepClone(value);
			...
			function requiredLabelReset(viewModel) {
				BPMSoft.each(viewModel.columns, function(column, name) {
			...
			function bindDependencies(viewModel, infoDependencies) {
				BPMSoft.each(infoDependencies, function(dependency) {
					var changelist = "";
					BPMSoft.each(dependency.dependencies, function(item) {
			...
			function loadLookup(name, value) {
				...
				esq.addMacrosColumn(BPMSoft.QueryMacrosType.PRIMARY_COLUMN, "value");
				esq.addMacrosColumn(BPMSoft.QueryMacrosType.PRIMARY_DISPLAY_COLUMN, "displayValue");
				BPMSoft.each(this.entitySchemaInfo.columns[name], function(additionalColumnName) {
					if (!esq.columns.contains(additionalColumnName)) {
						esq.addColumn(additionalColumnName);
					}
				}, this);
