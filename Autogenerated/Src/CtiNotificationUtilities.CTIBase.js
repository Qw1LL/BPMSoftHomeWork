define("CtiNotificationUtilities", ["ext-base", "BPMSoft", "sandbox", "CtiNotificationUtilitiesResources",
		"DesktopPopupNotification", "CtiConstants"],
	function(Ext, BPMSoft, sandbox, resources, DesktopPopupNotification, CtiConstants) {
		/**
		 * Object of notification configurations cache.
		 * @type {Object}
		 */
		var callNotificationCache = BPMSoft.configuration.Storage.CallNotifications || (function() {
			var cache = {};

			BPMSoft.each(CtiConstants.CallType, function(item) {
				cache[item] = new BPMSoft.Collection();
			});
		...
		var showCallNotification = function(callId, message, callType, iconId) {
			...
			imageConfig = BPMSoft.isGUID(iconId) ? imageConfig : callImage;
			if (Boolean(imageConfig)) {
				iconUrl = BPMSoft.ImageUrlBuilder.getUrl(imageConfig);
			}
