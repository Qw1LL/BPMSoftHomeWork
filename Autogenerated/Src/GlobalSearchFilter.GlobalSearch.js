	...
	return {
		...
		methods: {
			...
			sectionsGrouping: function() {
				...
				BPMSoft.AjaxProvider.abortRequestByInstanceId(this.get("SearchRequestId"));
				const searchRequestId = BPMSoft.generateGUID();
			 ...
			 * @private
			 * @param {Object} item Section item.
			 * @return {BPMSoft.BaseViewModel} view model of the section item.
			 */
			getSectionItemViewModel: function(item) {
				var viewModel = Ext.create("BPMSoft.BaseViewModel", {values: item});
				return viewModel;
			},
			...
			/**
			 * Sets active section.
			 * @param {BPMSoft.BaseViewModel} viewModel Section view model.
			...
			getSectionIcon: function(item) {
				...
				return this.BPMSoft.ImageUrlBuilder.getUrl(icon);
			},
