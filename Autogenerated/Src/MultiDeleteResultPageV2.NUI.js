		...
		function(Resources, SchemaBuilder, ViewGenerator, ConfigurationConstants, NetworkUtilities, ModuleUtils,
			...
			return {
				...
				methods: {
					...
					/**
					 * @inheritDoc BPMSoft.BaseViewModel#initRunProcessButtonMenu
					 * @overridden
					 */
					...
					initInChain: function(callback, scope) {
						...
						this.set("ByRightsData", this.Ext.create("BPMSoft.BaseViewModelCollection"));
						this.set("ByConstraintsData", this.Ext.create("BPMSoft.Collection"));
						this.set("RecordsData", this.Ext.create("BPMSoft.Collection"));
					 ...
					 * @return {BPMSoft.Container} Container for the constraint results.
					 * @private
					 */
					 ...
					 * @param {BPMSoft.Container} resultsContainer Results container.
					...
					getKeyForSelect: function() {
						...
						return historyState.primaryColumnValue || this.BPMSoft.GUID_EMPTY;
					},
					...
					addDenyReasoneFilterForConstraints: function(select) {
						...
						select.filters.addItem(this.BPMSoft.createColumnInFilterWithParameters("DenyReason",
							exceptionIds));
					},
					...
					getEsqOfMultiDeleteErrorResult: function(multiDeleteDeniedTypeId) {
						...
						createdOnColumn.orderDirection = this.BPMSoft.OrderDirection.ASC;
						esq.addColumn("Id");
					 ...
					 * @param {BPMSoft.Collection} collection The results of errors.
					 * @private
					 */
					 ...
					 * @return {BPMSoft.FilterGroup} Filter for select.
					 * @protected
					 */
					getFilterToSelectMultiDeleteErrorResult: function(denyReasoneId) {
						...
						if (denyReasoneId) {
							filter.addItem(this.BPMSoft.createColumnFilterWithParameter(
									this.BPMSoft.ComparisonType.EQUAL, "DenyReason", denyReasoneId));
						}
						if (isCascade) {
							filter.addItem(this.BPMSoft.createColumnFilterWithParameter(
									this.BPMSoft.ComparisonType.EQUAL, "EntitySchemaUId",	deleteSchemaUId));
						}
						return filter;
					},
					...
					getFilters: function() {
						...
						var filter = this.BPMSoft.createColumnInFilterWithParameters("Id", resData.getKeys());
						return filter;
					},
					...
					/**
					 * @inheritDoc BPMSoft.BaseSectionV2#initGridData
					 * @overridden
					 */
					...
					initializeSchema: function(callback, scope) {
						...
						this.BPMSoft.require([schemaName], function(response) {
					...
					initPageProfile: function(callback, scope) {
						...
						SchemaBuilder.getProfile(profileKey, function(response) {
							var profile = this.BPMSoft.deepClone(response);
					...
					getFormattedProfile: function(profile) {
						...
						profile.DataGrid = this.BPMSoft.deepClone(profile);
						return profile;
					},
					...
					initDeletedSchemaName: function(callback, scope) {
						...
						esq.getEntityCollection(function(response) {
							...
							if (collection) {
								...
								var schemaUId = record ? record.get("EntitySchemaUId") : this.BPMSoft.GUID_EMPTY;
					...
					decorateItem: function(item) {
						...
						item.getConstraintDetailsButton = this.getConstraintDetailsButton;
					...
					showItemConstraints: function() {
						...
						var btn = this.getConstraintDetailsButton(id);
						btn.removeCls(constraintDetailsHideClass);
						if (!isDetailsLoaded) {
					...
					hideItemConstraints: function() {
						...
						var btn = this.getConstraintDetailsButton(id);
						btn.addCls(constraintDetailsHideClass);
					},
					 ...
					 * @return {BPMSoft.Container} Container for the details with constraints.
					 * @protected
					 */
					...
					getConstraintDetailsButton: function(itemId) {
						return this.Ext.get("Constraint-" +
								itemId + "-CardModuleV2_MultiDeleteResultPageV2");
					},
					...
					getConstraintsItemConfig: function(itemConfig) {
						...
						var config = {
							...
							classes: {
								wrapClassName: ["constraints-item-container", constraintDetailsHideClass]
							}
						};
					...
					callDeleteService: function(operation) {
						var operationKey = this.BPMSoft.generateGUID();
					...
					ignoreRecord: function() {
						...
						var idFilter = this.BPMSoft.createColumnFilterWithParameter(
								this.BPMSoft.ComparisonType.EQUAL, "Id", this.get("Id"));
				...
				diff: [
					...
					{
						"operation": "insert",
						"name": "ByConstraintsContainerHeaderLabel",
						"parentName": "ByConstraintsContainerHeader",
						"propertyName": "items",
						"values": {
