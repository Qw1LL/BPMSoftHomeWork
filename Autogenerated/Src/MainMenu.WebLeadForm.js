define("MainMenu", ["ext-base", "BPMSoft", "sandbox", "MainMenuResources", "SysModule", "SysModuleFolder",
	...
	function(Ext, BPMSoft, sandbox, resources, SysModule, SysModuleFolder, LocalizationUtilities, ModuleUtils,
		...
		var getView = function(menuItems) {
			...
			BPMSoft.each(menuItems, function(item) {
			...
			return Ext.create("BPMSoft.BaseViewModel", {
				methods: {
					...
					openDetailWizard: function() {
						var detailWizardUrl = BPMSoft.workspaceBaseUrl + "/NUI/ViewModule.aspx?vm=DetailWizard#New";
					...
					exitClick: function() {
						ServiceHelper.callService("MainMenuService", "Logout", function() {
							window.logout = true;
							window.location.replace(BPMSoft.loaderBaseUrl);
						}, {}, this);
					},
		...
		var generate = function(scope) {
			...
			var select = Ext.create("BPMSoft.EntitySchemaQuery", {rootSchema: SysModule});
			...
			select.filters.add("locationFilter", BPMSoft.createColumnInFilterWithParameters(folderLocation.columnPath,
				["left", "right"]));
			select.filters.add("positionFilter", BPMSoft.createColumnFilterWithParameter(BPMSoft.ComparisonType.LESS,
				modulePosition.columnPath, "100"));
			folderPosition.orderDirection = BPMSoft.OrderDirection.ASC;
			folderPosition.orderPosition = 1;
			modulePosition.orderDirection = BPMSoft.OrderDirection.ASC;
			modulePosition.orderPosition = 2;
			select.getEntityCollection(function(result) {
				var menuItems = [];
				BPMSoft.each(result.collection.collection.items, function(entity) {
		...
		function updateMenuItemFolderCaptions(menuItems, callback, scope) {
			var select = Ext.create("BPMSoft.EntitySchemaQuery", {rootSchema: SysModuleFolder});
			LocalizationUtilities.addLocalizableColumn(select, "Caption");
			select.getEntityCollection(function(result) {
				var folders = {};
				if (result.success) {
