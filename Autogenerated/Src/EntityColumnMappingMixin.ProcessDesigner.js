define("EntityColumnMappingMixin", ["BPMSoft", "ModalBox", "EntityColumnMappingMixinResources",
	"EntityColumnMappingViewModel", "ProcessLookupPageMixin"],
	function(BPMSoft, ModalBox, resources) {

		/**
		 * Mixin for the Column-Value block.
		 */
		Ext.define("BPMSoft.configuration.mixins.EntityColumnMappingMixin", {
			alternateClassName: "BPMSoft.EntityColumnMappingMixin",
			...
			initEntityColumnMappings: function(element, entityColumns, callback, scope) {
				var controlList = Ext.create("BPMSoft.ObjectCollection");
				if (entityColumns && !entityColumns.isEmpty()) {
					...
					var metaDataCollectionValue = BPMSoft.decode(parameter.sourceValue.value);
			...
			getEmptyColumnMappingParameterValue: function(column) {
				...
				var dataValueTypeUId = BPMSoft.convertToServerDataValueType(dataValueType);
				var value = {
					value: "",
					displayValue: "",
			...
			getColumnMappingParameterValue: function(columnMapping) {
				var parameterValue = BPMSoft.decode(columnMapping.parameterValue.value);
				var referenceSchemaUId = parameterValue.ReferenceSchemaUId;
				if (BPMSoft.isEmptyGUID(referenceSchemaUId)) {
				...
				if (dataValueTypeUId && !BPMSoft.isEmptyGUID(dataValueTypeUId)) {
					dataValueType = BPMSoft.convertToClientDataValueType(dataValueTypeUId);
				}
				return {
					value: value,
			 ...
			 * @return {BPMSoft.EntityColumnMappingViewModel}
			 */
			getEntityColumnMappingViewModel: function(config) {
				...
				var toolsButtonMenuViewModelCollection = Ext.create("BPMSoft.BaseViewModelCollection", {
				...
				var localizableParameterValueId = mapping.ItemUId || BPMSoft.generateGUID();
				var sourceSchemaColumns = this.get("SourceSchemaColumns");
				var viewModel = Ext.create("BPMSoft.EntityColumnMappingViewModel", {
			 ...
			 * @param {BPMSoft.process.enums.ProcessSchemaParameterValueSource} source The source of the value.
			 * @return {Boolean} True, if value is localizable, otherwise - false.
			 */
			getIsValueLocalizable: function(source, dataValueTypeUId) {
				var sources = BPMSoft.process.enums.ProcessSchemaParameterValueSource;
				if (source === sources.Script || source === sources.Mapping) {
					return false;
				}
				var dataValueType = BPMSoft.ServerDataValueType[dataValueTypeUId];
				var isTextDataValueType = BPMSoft.isTextDataValueType(dataValueType);
			 ...
			 * @return {String|BPMSoft.LocalizableString} return.value Item value.
			...
			getEntityColumnMappingsParameterValue: function(mappingValue, dataValueTypeUId) {
				var parameterValue = {
					"$type": "BPMSoft.Core.Process.ProcessSchemaParameterValue, BPMSoft.Core",
					"Source": mappingValue.source,
					"DisplayValue": "",
					"Value": "",
					"MetaPath": mappingValue.metaPath || null,
					"MetaDataValue": null,
					"ModifiedInSchemaUId": BPMSoft.GUID_EMPTY,
					"SchemaUId": BPMSoft.GUID_EMPTY,
					"SchemaManagerName": "",
					"DataValueTypeUId": dataValueTypeUId,
					"ReferenceSchemaUId": mappingValue.referenceSchemaUId || BPMSoft.GUID_EMPTY,
					"ResourceItemName": null
				};
				return BPMSoft.encode(parameterValue);
			},
			 ...
			 * @param {BPMSoft.Collection} callback.entityColumns The collection of the entity schema columns.
			...
			queryMappingEntitySchemaColumns: function(schemaUId, callback, scope) {
				var entityColumns = Ext.create("BPMSoft.Collection");
				var packageUId = this.get("packageUId");
				var config = {
					schemaUId: schemaUId,
					packageUId: packageUId
				};
				const prohibitedResources = [ 'LinkedIn' ];
				BPMSoft.EntitySchemaManager.findBundleSchemaInstance(config, function(schema) {
					if (schema) {
						schema.columns.each(function(column) {
							...
							if(prohibitedResources.some(substring => column.name.toLowerCase().includes(substring.toLowerCase())))
								return;
