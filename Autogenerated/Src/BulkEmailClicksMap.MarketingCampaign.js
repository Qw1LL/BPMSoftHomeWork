define("BulkEmailClicksMap", ["BaseNestedModule"], function() {
	Ext.define("BPMSoft.configuration.BulkEmailClicksMapModule", {
		extend: "BPMSoft.BaseNestedModule",
		alternateClassName: "BPMSoft.BulkEmailClicksMapModule",
		Ext: null,
		sandbox: null,
		BPMSoft: null,
		...
		_extractWebLinksFromTemplate: function() {
			...
			this.BPMSoft.each(allLinks, function(linkEl) {
		...
		_getNormalizedLinksData: function() {
			...
			this.BPMSoft.each(currentLinks, function(linkData) {
		...
		_getBpmTrackIdFromLink: function (url) {
			var paramValue = BPMSoft.getQueryParameter(url, this.bpmTrackId);
		...
		_processForDCTemplate: function(linksFromHtml) {
			this.BPMSoft.each(linksFromHtml, function(templateLink) {
		...
		initMaxClicksCount: function() {
			...
			this.BPMSoft.each(this.moduleConfig.linksData, function(linkData) {
		...
		getLinkCountInfo: function(linkUrl, linkClicksData) {
			...
			return {
				clicksCount: BPMSoft.getFormattedNumberValue(clicksCount, {type: BPMSoft.DataValueType.INTEGER}),
		...
		getUrlWithoutExtraParameters: function(link) {
			...
			return this.BPMSoft.removeQueryParameters(link, extraParameters);
		},
		...
		/**
		 * @inheritdoc BPMSoft.BaseNestedModule#render
		 * @override
		 */
		render: function(renderTo) {
			var content = this.moduleConfig.htmlText || "";
			this.view = this.Ext.create("BPMSoft.HtmlControl", {
				renderTo: renderTo,
	...
	return BPMSoft.BulkEmailClicksMapModule;
});
