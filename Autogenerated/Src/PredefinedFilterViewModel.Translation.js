define("PredefinedFilterViewModel", ["ext-base", "BPMSoft", "BaseSchemaViewModel", "css!PredefinedFilterViewModel",
		"PredefinedFilterItemViewModel"], function(Ext, BPMSoft) {
	Ext.define("BPMSoft.model.PredefinedFilter", {
		extend: "BPMSoft.BaseSchemaViewModel",
		alternateClassName: "BPMSoft.PredefinedFilter",
		...
		columns: {
			"FilterItemsList": {
				dataValueType: BPMSoft.DataValueType.ENTITY_COLLECTION
			}
		},
		 ...
		 * @private
		 * @param {BPMSoft.PredefinedFilterItemViewModel} item Filter.
		 */
		 ...
		 * @param {String} config.columnPath Filter column path.
		 * @param {String} config.caption Filter caption.
		 * @return {BPMSoft.PredefinedFilterItemViewModel}
		 */
		createFilterItem: function(config) {
			...
			var filterItem = this.Ext.create("BPMSoft.PredefinedFilterItemViewModel", {
				values: values,
		...
		getFilters: function() {
			...
			var instance = this.instance = this.BPMSoft.createFilterGroup();
			instance.logicalOperation = this.BPMSoft.LogicalOperatorType.OR;
		 ...
		 * @private
		 * @param {BPMSoft.PredefinedFilterItemViewModel} item Filter item.
		 */
		...
		initColumnsList: function() {
			var columnsList = this.Ext.create(BPMSoft.BaseViewModelCollection);
		...
		initFilterItemsCollection: function() {
			var filterItems = this.Ext.create(this.BPMSoft.BaseViewModelCollection);
		...
		createFilter: function(columnPath) {
			switch (this.comparisonType) {
				case this.BPMSoft.ComparisonType.IS_NULL:
					return this.BPMSoft.createColumnIsNullFilter(columnPath);
				case this.BPMSoft.ComparisonType.IS_NOT_NULL:
					return this.BPMSoft.createColumnIsNotNullFilter(columnPath);
				default:
		...
		setValue: function(value) {
			this.BPMSoft.each(value, this.prepareValue, this);
		},
		...
		prepareValue: function(item) {
			...
			var filter = this.BPMSoft.deserialize(item.filter);
