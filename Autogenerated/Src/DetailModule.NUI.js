...
//			var inputModels = new BPMSoft.Collection();
			var inputViews = new BPMSoft.Collection();
			var view;
			var viewConfig;
			var viewModel;
			var loadedSchemas = [];
			...
			function saveDetails() {
				...
				if (viewModel.schemaType === BPMSoft.SchemaType.GRID_DETAIL_VIEW_MODEL_SCHEMA) {
					return true;
				}
				...
				if (isDetailValid) {
					...
					BPMSoft.each(detailItems, function(model) {
			...
			function createItem(item, renderTo, viewModelConfigName, action) {
				...
				BPMSoft.each(itemViewModel.columns, function(column, columnName) {
				...
				BPMSoft.each(itemViewModel.columns, function(column) {
					if (column.isCollection === true) {
						itemViewModel.set(column.name, new BPMSoft.Collection());
					}
				});
				...
				var itemView = Ext.create(container.className || 'BPMSoft.Container', container);
			...
			function loadView(renderTo, sandbox, Ext, instance) {
				if (viewModel && !viewModel.isObsolete) {
					...
					view = Ext.create(config.className || 'BPMSoft.Container',
						config);
					view.bind(viewModel);
					viewModel.reload();
					...
					if (moduleId === args.Id) {
						...
						require([detailSchema, 'profile!' + columnsSettingsProfileKey],
							function(schemaConfig, columnsSettingsProfile) {
								...
								var schemaConfigCopy = BPMSoft.deepClone(schemaConfig);
								getSchemaUserCodes(schemaConfigCopy, [], function(schema, userCodes) {
									if (instance.isDestroyed) {
										return;
									}
									schema.extend = 'BPMSoft.model.BaseViewModel';
									var type = schema.type;
									schema.filterPath = args.filterPath;
									schema.parentModuleSandboxId = args.cardModuleSandboxId;
									if (args && !Ext.isEmpty(BPMSoft[args.entitySchemaName])) {
										schema.entitySchema = BPMSoft[args.entitySchemaName];
									}
									var schemaCopy = BPMSoft.deepClone(schema);
									...
									schemaCopy = BPMSoft.deepClone(schema);
									...
									BPMSoft.each(viewModel.columns, function(column) {
										if (column.isCollection === true) {
											viewModel.set(column.name, new BPMSoft.BaseViewModelCollection());
										}
									});
									...
									if (type === BPMSoft.SchemaType.GRID_DETAIL_VIEW_MODEL_SCHEMA) {
										...
										viewModel.add = function(tag) {
											...
											if (this.defValues) {
												BPMSoft.each(this.defValues, function(column) {
										...
										viewModel.onLoadData = function(result) {
											...
											if (result && result.collection) {
												...
												BPMSoft.each(args.predefinedRecords, function(item) {
													gridData.add(item.get(item.primaryColumnName),
														createItem(item, renderTo, itemViewModelConfig.name,
															args.operationType));
												});
										...
										viewModel.reload = function() {
											...
											collection.each(function(itemViewModel) {
												...
												var itemView = Ext.create(container.className || 'BPMSoft.Container',
													container);
										...
										EditDetailViewGenerator.generate(schema, userCodes,
											...
											function(config, typeItems) {
												...
												viewConfig = BPMSoft.deepClone(config);
												view = Ext.create(config.className || 'BPMSoft.Container',
													config);
												view.bind(viewModel);
												view.render(renderTo);
			...
			function getSchemaUserCodes(schema, userCodes, callback) {
				...
				require([parentSchemaName], function(parentSchema) {
					var parentSchemaCopy = BPMSoft.deepClone(parentSchema);
