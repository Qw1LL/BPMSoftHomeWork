		...
		methods: {
			...
			_applyTagListFilters: function (esq, filter) {
				...
				const lookupFilter = esq.createPrimaryDisplayColumnFilterWithParameter(
					lookupComparisonType, filter, BPMSoft.DataValueType.TEXT);
				filterGroup.addItem(lookupFilter);
				const tagTypesFilterGroup = this.getTagTypesFilter();
				if (BPMSoft.CurrentUser.userType !== BPMSoft.UserType.SSP) {
				...
				switch (typeId) {
					...
					default:
						...
						return {
							...
							TagTypeId: typeId,
							customHtml: "<div data-value=\"" +
							this.BPMSoft.GUID_EMPTY + "\" class=\"listview-new-item\">" + newValText + "</div>"
						};
				}
			},
			...
			checkTagExists: function(name, type, callback, scope) {
				...
				filterGroup.add("NameFilter", this.BPMSoft.createColumnFilterWithParameter(
						BPMSoft.ComparisonType.EQUAL, "Name", name));
				filterGroup.add("TypeFilter", this.BPMSoft.createColumnFilterWithParameter(
						BPMSoft.ComparisonType.EQUAL, "Type", type));
			 ...
			 */
			addNewTagInEntity: BPMSoft.emptyFn,
			...
			tryCreateEntityOrOpenCard: function(searchValue, columnName, additionalColumns) {
				...
				if (!canAdd.success) {
					...
					var createEntityConfig = {
						...
						valuePairsFromFilters: new this.BPMSoft.Collection(),
			...
			createEntitySilently: function(config) {
				this.checkTagExists(config.displayColumnValue, config.additionalColumns.TagTypeId, function() {
					...
					insert.setParameterValue("Type", config.additionalColumns.TagTypeId, BPMSoft.DataValueType.GUID);
