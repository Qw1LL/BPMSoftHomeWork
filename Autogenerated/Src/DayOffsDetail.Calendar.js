		...
		function(resources, BusinessRulesApplier, TimezoneUtils) {
			return {
				...
				methods: {
					...
					/**
					 * @inheritdoc BPMSoft.BaseGridDetailV2#deleteRecords
					 * @overridden
					 * */
					...
					getCellControlsConfig: function(entitySchemaColumn) {
						...
						if (activeRowEnabledConfig && activeRowEnabledConfig.rowId) {
							...
							enabled = (entitySchemaColumn.usageType !== this.BPMSoft.EntitySchemaColumnUsageType.None) &&
					...
					/**
					 * @inheritdoc BPMSoft.ConfigurationGridUtiliteso#generateActiveRowControlsConfig
					 * @overridden
					 */
					...
					/**
					 * @inheritdoc BPMSoft.ConfigurationGridUtilites#onCurrentRowChange
					 * @overridden
					 */
					...
					informationDialogButtonHandler: function(returnCode) {
						if (returnCode === this.BPMSoft.MessageBoxButtons.OK.returnCode) {
					...
					/**
					 * @inheritdoc BPMSoft.ConfigurationGridUtiliteso#loadGridDataRecord
					 * @overridden
					 */
					...
					getWorkingIntervalsQuery: function() {
						...
						sortByDay.orderDirection = this.BPMSoft.OrderDirection.ASC;
						var sortByInterval = esq.addColumn("From");
						sortByInterval.orderPosition = 0;
						sortByInterval.orderDirection = this.BPMSoft.OrderDirection.ASC;
						esq.filters.add("calendarFilter", this.BPMSoft.createColumnFilterWithParameter(
								this.BPMSoft.ComparisonType.EQUAL, "DayOff.Calendar", calendarId));
						return esq;
					},
					...
					saveRowChanges: function(row) {
						...
						if (!Ext.isObject(row)) {
							...
							if(this.isPreviousDate(row)){
								BPMSoft.utils.showInformation(this.get("Resources.Strings.ErrorOnSave"));
								return;
							}
							if(!this.validateEmptyDateType(row) || !this.validateEmptyDate(row)){
								BPMSoft.utils.showInformation(this.get("Resources.Strings.EmptyDateOrDateTypeErrorMessage"));
								return;
							}
					...
					getDuplicateFilter: function(id, date, calendar) {
						var duplicatesFilter = BPMSoft.createColumnFilterWithParameter(
								BPMSoft.ComparisonType.EQUAL, "Date", date);
						var notSelfFilter = BPMSoft.createColumnFilterWithParameter(
						...
						if (calendar) {
							var duplicatesCalendarFilter = BPMSoft.createColumnFilterWithParameter(
									BPMSoft.ComparisonType.EQUAL, "Calendar", calendar);
					...
					duplicateDateCallback: function(args, resultQuery) {
						...
						if (resultQuery.success && resultQuery.collection.getItems()[0].get("Count") > 0) {
							this.BPMSoft.utils.showInformation(this.get("Resources.Strings.DuplicateDateErrorMessage"));
						} else {
					...
					insertDefaultWorkingInterval: function(row) {
						...
						var newIntervalId = this.BPMSoft.generateGUID();
						insert.setParameterValue("Id", newIntervalId, this.BPMSoft.DataValueType.GUID);
						insert.setParameterValue("DayOff", row.get("Id"), this.BPMSoft.DataValueType.GUID);
						insert.setParameterValue("From", defaultInterval.from, this.BPMSoft.DataValueType.DATE_TIME);
						insert.setParameterValue("To", defaultInterval.to, this.BPMSoft.DataValueType.DATE_TIME);
					...
					/**
					 * @inheritdoc BPMSoft.GridUtilities#sortGrid
					 * @overridden
					 */
					...
					sortGridData: function() {
						var tempGridDataCollection = new this.BPMSoft.Collection();
						var gridData = this.getGridData();
					...
					/**
					 * @inheritdoc BPMSoft.GridUtilitiesV2#getCopyRecordMenuItem
					 * @overridden
					 */
					getCopyRecordMenuItem: this.BPMSoft.emptyFn,
					/**
					 * @inheritdoc BPMSoft.GridUtilitiesV2#getEditRecordMenuItem
					 * @overridden
					 */
					getEditRecordMenuItem: this.BPMSoft.emptyFn,
					/**
					 * @inheritdoc BPMSoft.GridUtilitiesV2#addDetailWizardMenuItems
					 * @overridden
					 */
					addDetailWizardMenuItems: this.BPMSoft.emptyFn,
					/**
					 * @inheritdoc BPMSoft.GridUtilitiesV2#getGridSortMenuItem
					 * @overridden
					 */
					...
					/**
					 * @inheritdoc BPMSoft.BaseGridDetails#getExportToExcelFileMenuItem
					 * @overridden
					 */
					...
					initQuerySorting: function(esq) {
						...
						sortedColumn.orderDirection = BPMSoft.OrderDirection.DESC;
					}
				}
			};
		});
