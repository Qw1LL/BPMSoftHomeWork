define("CommunicationHistoryItem", ["BPMSoft", "CtiConstants", "NetworkUtilities", "ConfigurationEnums",
			"ConfigurationConstants", "CtiBaseHelper", "LookupQuickAddMixin",
			"EntityConnectionLinksPanelItemUtilities"],
		function(BPMSoft, CtiConstants, NetworkUtilities, ConfigurationEnums, ConfigurationConstants) {
			return {
				messages: {
				...
				methods: {
					...
					constructor: function() {
						...
						this.set("CurrentRelationItemsList", this.Ext.create("BPMSoft.Collection"));
						BPMSoft.CtiModel.on("connected", function() {
							this.set("IsTelephonyAvailable", true);
						}, this);
						BPMSoft.CtiModel.on("disconnected", function() {
					...
					getPhoto: function() {
						...
						if (Ext.isEmpty(photoId) || BPMSoft.isEmptyGUID(photoId)) {
					...
					updateCall: function(value, columnName) {
						...
						update.setParameterValue(columnName, value || null, BPMSoft.DataValueType.GUID);
					...
					processColumnValueChange: function(value, columnName) {
						...
						if (value != null && value.value === BPMSoft.GUID_EMPTY) {
					...
					openNewRecordCard: function(entitySchemaName, pageSchemaName, valuePairs) {
						var moduleId = this.BPMSoft.generateGUID();
					 ...
					 * @param {BPMSoft.BaseViewModel} callback.connectedRecordData View model of the connected record.
					 */
					getConnectedRecordData: function(schemaName, recordId, callback) {
						var esq = this.Ext.create("BPMSoft.EntitySchemaQuery", {rootSchemaName: schemaName});
						esq.addColumn("Id");
					 ...
					 * @protected
					 * @param {BPMSoft.EntitySchemaQuery} esq Connected record entity schema query.
					 * @param {String} schemaName Entity schema name.
					 */
					...
					getSubscriberCommunicationInfo: function(schemaName, callback) {
						BPMSoft.CtiBaseHelper.queryCtiSettings(function(ctiSettings) {
							var config;
							switch (schemaName) {
								case "Contact":
									config = {
					...
					updateSubscriberCommunicationNumber: function(communicationsSchemaName, number, parentItemName,
						...
						esq.addColumn("Id");
						esq.filters.addItem(BPMSoft.createColumnFilterWithParameter(
							BPMSoft.ComparisonType.EQUAL, "CommunicationType", communicationTypeId,
							BPMSoft.DataValueType.GUID));
						esq.filters.addItem(BPMSoft.createColumnFilterWithParameter(
							BPMSoft.ComparisonType.EQUAL, parentItemName, subscriberId,
							BPMSoft.DataValueType.GUID));
						esq.filters.addItem(BPMSoft.createColumnFilterWithParameter(
							BPMSoft.ComparisonType.EQUAL, "Number", number, BPMSoft.DataValueType.TEXT));
						esq.execute(function(response) {
							...
							insert.setParameterValue("CommunicationType", communicationTypeId,
								BPMSoft.DataValueType.GUID);
							insert.setParameterValue(parentItemName, subscriberId, BPMSoft.DataValueType.GUID);
					 ...
					 * @param {BPMSoft.BaseViewModel} record View model of the connected record.
					 */
					 ...
					 * @protected
					 * @param {String} schemaName Entity schema name of the connected record.
					 * @param {BPMSoft.BaseViewModel} connectedRecordData View model of the connected record.
					 */
					updateCallWithConnectedRecordData: function(schemaName, connectedRecordData) {
						...
						updateQuery.filters.addItem(BPMSoft.createColumnFilterWithParameter(
							BPMSoft.ComparisonType.EQUAL, "Id", callDatabaseUId));
					 ...
					 * @protected
					 * @param {BPMSoft.UpdateQuery} updateQuery "Call" schema update query.
					 * @param {String} schemaName Entity schema name of the connected record.
					 * @param {BPMSoft.BaseViewModel} connectedRecordData View model of the connected record.
					 */
					setUpdateCallQueryParameters: function(updateQuery, schemaName, connectedRecordData) {
						...
						switch (schemaName) {
							case "Contact":
								updateQuery.setParameterValue("Contact", recordId, BPMSoft.DataValueType.GUID);
								updateQuery.setParameterValue("Account", connectedRecordData.get("Account").value,
									BPMSoft.DataValueType.GUID);
								break;
							case "Account":
								updateQuery.setParameterValue("Account", recordId, BPMSoft.DataValueType.GUID);
								break;
							default:
								break;
						}
					 ...
					 * @protected
					 * @param {BPMSoft.BaseViewModel} record View model of the connected contact record.
					 */
					 ...
					 * @protected
					 * @param {BPMSoft.BaseViewModel} record View model of the connected account record.
					 */
					 ...
					 * Updates history panel attributes according to the connected record.
					 * @protected
					 * @param {BPMSoft.BaseViewModel} columnName Call column name.
					 * @param {BPMSoft.BaseViewModel} record View model of the connected account record.
					 */
					...
					subscribeOnConnectedRecordAttributesChanges: function() {
						...
						if (!Ext.isEmpty(callRelationColumns)) {
							this.BPMSoft.each(callRelationColumns, function(columnName) {
					...
					openCallPage: function() {
						...
						var config = {
							...
							moduleId: this.BPMSoft.generateGUID()
						};
					...
					onLinkContactClick: function() {
						...
						var config = {
							lookupModuleId: this.BPMSoft.generateGUID(),
					...
					/**
					 * @inheritdoc BPMSoft.BaseViewModel#onLookupDataLoaded
					 * @overridden
					 */
