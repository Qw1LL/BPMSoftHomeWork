define("MiniPageEntityConnectionsUtils", ["NetworkUtilities", "EntityConnectionLinksResourceUtilities",
	"MiniPageResourceUtilities", "MiniPageViewGenerator", "CallExtendedMenu", "EmailExtendedMenu"],
		function(NetworkUtilities, entityConnectionResources, miniPageResources) {
			Ext.define("BPMSoft.configuration.mixins.MiniPageEntityConnectionsUtils", {
				alternateClassName: "BPMSoft.MiniPageEntityConnectionsUtils",
				...
				entityColumnChanged: function(columnName, columnValue) {
					if (this.Ext.isFunction(this.beforeEntityColumnChanged)) {
						this.beforeEntityColumnChanged(columnName, columnValue);
					}
				 ...
				 * @private
				 * @param  {BPMSoft.EntityConnectionViewModel} item ContainerList item.
				 * @param  {String} connectionName Connection name.
				 * @param  {String} columnName Column name.
				 */
				 ...
				 * @param {BPMSoft.EntityConnectionViewModel} item Entity connection ViewModel item.
				 */
				getEntityConnectionViewConfig: function(itemConfig, item) {
					...
					if (columnName) {
						...
						var viewGenerator = this.Ext.create("BPMSoft.MiniPageViewGenerator");
						...
						var entityConnectionConfig = viewGenerator.generatePartial(
							...
							},
							{
								...
								"viewModelClass": this.Ext.ClassManager.get("BPMSoft.ActivityMiniPageActivityViewModel"),
								"schema": {}
							}
						);
				...
				getEntityConnectionHref: function(columnName) {
					...
					return this.BPMSoft.combinePath(
						this.BPMSoft.workspaceBaseUrl, "Nui", "ViewModule.aspx#" + relativeUrl);
				},
				 ...
				 * @private
				 * @param {BPMSoft.BaseViewModelCollection} inputCollection Input collection.
				 */
				...
				forEachEntityConnection: function(handler, scope) {
					...
					if (entityConnectionsList && entityConnectionsList.getItems) {
						this.BPMSoft.each(entityConnectionsList.getItems(), function(entityConnection) {
				 ...
				 * @private
				 * @param {String} inputString Input string.
				 * @return {BPMSoft.EntitySchemaQuery} Query for fetching entities.
				 */
				getItemsQuery: function(inputString) {
					...
					if (inputString.length !== 0) {
						esq.filters.add("startFilter", this.BPMSoft.createColumnFilterWithParameter(
							this.BPMSoft.ComparisonType.CONTAIN, referenceSchema.primaryDisplayColumnName,
							inputString));
					}
