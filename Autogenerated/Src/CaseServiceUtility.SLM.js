define("CaseServiceUtility", ["ServiceDeskConstants"],
		function(ServiceDeskConstants) {
			...
			/* jshint ignore:end */
			/**
			 * @class This.bpmsoftconfiguration.mixins.CaseServiceUtility
			 * Mixin, that implements work with service item on page.
			 */
			this.Ext.define("BPMSoft.configuration.mixins.CaseServiceUtility", {
				alternateClassName: "BPMSoft.CaseServiceUtility",
				/**
				 ...
				 * @protected
				 * @return {BPMSoft.EntitySchemaQuery} Query that select case category of the service item.
				 */
				...
				onRecalculateCaseTerms: function(response) {
					...
					var result = this.BPMSoft.decode(response.CalculateTermsResult);
					var reactionTime = result.ReactionTime ? this.BPMSoft.parseDate(result.ReactionTime) : null;
					var solutionTime = result.SolutionTime ? this.BPMSoft.parseDate(result.SolutionTime) : null;
				...
				encodeDateToJsonFormat: function(dateTime) {
					...
					try {
						encodedDate = JSON.parse(this.BPMSoft.encodeDate(dateTime));
					} catch (exception) {
				 ...
				 * @protected
				 * @return {BPMSoft.BaseFilter} Service item column filter.
				 */
				getServiceItemFilter: function() {
					var serviceItemFilter = this.BPMSoft.createColumnFilterWithParameter(
							this.BPMSoft.ComparisonType.EQUAL, "Status.Active", true);
				...
				onRegisteredOnChanged: function() {
					if (this.getIsFeatureEnabled("ServiceTerms") && this.isNew) {
						...
						var strategiesIds = this.BPMSoft.getPropertyValuesArray(
								this.BPMSoft.keys(strategies), strategies);
						if (this.BPMSoft.contains(strategiesIds, strategyId)) {
							this.recalculateServiceTerms();
						}
					}
				},
				 ...
				 * @private
				 * @param {BPMSoft.Collection} collection Strategy collection.
				 * @returns {BPMSoft.Entity} Default strategy, if found.
				 */
				 ...
				 * @private
				 * @param {BPMSoft.Entity} defaultStrategy Default strategy.
				 * @param {BPMSoft.Collection} collection Strategy collection.
				 * @returns {BPMSoft.Collection} Strategy sequence.
				 */
				buildStrategySequence: function(defaultStrategy, collection) {
					var sequence = this.Ext.create("BPMSoft.Collection");
