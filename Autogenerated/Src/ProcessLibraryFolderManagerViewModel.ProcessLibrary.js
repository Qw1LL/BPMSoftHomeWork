...
function(resources) {
	return Ext.define("BPMSoft.ProcessLibraryFolderManagerViewModel", {
		...
		_getProcessStartEventFilter: function(config) {
			...
			if (Ext.isEmpty(schemaUId)) {
				filterGroup.addItem(
					BPMSoft.createExistsFilter("[SysEntityPrcStartEvent:ProcessSchema:SysSchemaId].Id"));
			} else {
				filterGroup.addItem(BPMSoft.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL,
		...
		_getProcessStartTimerFilter: function(config) {
			...
			filterGroup.logicalOperation = BPMSoft.LogicalOperatorType.AND;
			filterGroup.addItem(
				BPMSoft.createExistsFilter("[SysStartTimerInProcess:ProcessUId:UId].Id"));
		...
		_getLoadProcessStartEventGroupsQuery: function() {
			...
			var filterGroup = BPMSoft.createFilterGroup();
			filterGroup.logicalOperation = BPMSoft.LogicalOperatorType.AND;
			filterGroup.add(BPMSoft.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL,
		...
		_handleProcessStartEventGroupsResult: function(queryResults) {
			...
			result = BPMSoft.mergeSame(result,
				function(firstItem, secondItem) {
					return firstItem.SchemaUId === secondItem.SchemaUId;
				},
				function (firstItem, secondItem) {
					var group = BPMSoft.first(secondItem.ModificationTypes);
		...
		_getFilterConfig: function(folderConfig) {
			...
			switch (folderConfig.filter) {
				...
				case "ProcessStartTimerFilter":
					...
					break;
				default:
					result = BPMSoft.createFilterGroup();
					break;
			}
			return result;
		},
		 ...
		 * @override
		 * @protected
		 * @returns {BPMSoft.BatchQuery} Folder manager view model load batch query.
		 */
