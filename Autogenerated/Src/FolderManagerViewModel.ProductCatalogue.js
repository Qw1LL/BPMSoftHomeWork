define("FolderManagerViewModel", ["FolderManagerViewModelResources",
	...
	function(resources, ConfigurationConstants, LookupUtilities, ResponseExceptionHelper, MaskHelper, ServiceHelper,
		...
		function generate(parentSandbox, config) {
			...
			var viewModelConfig = {
				...
				methods: {
					...
					/**
					 * ########## ####### ######## ########, ####### ############ ### ########.
					 * @param {BPMSoft.Collection} catalogueFolders ###### ####### ########.
					 * @returns {Number} ####### ######## ######## ############### ### ######## # ########## ########
					 * ### null, #### ####### ## ### ######.
					 */
					...
					initAllFoldersSelect: function(scope) {
						...
						if (!scope.get("UseStaticFolders")) {
							select.filters.addItem(select.createColumnFilterWithParameter(
								BPMSoft.ComparisonType.NOT_EQUAL, "FolderType",
					 ...
					 * ######### ###### ## ####### ######### ########
					 * @protected
					 * @param {BPMSoft.BatchQuery} batch
					 * @param {object} scope
					 */
					...
					getParentRowsFilters: function(row, rowItem) {
						...
						filtersGroup.addItem(BPMSoft.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL,
						...
						if (parentData) {
							for (var i = position - 1; i >= 0; i--) {
								...
								if (catalogueFolder) {
									...
									filtersGroup.addItem(BPMSoft.createColumnFilterWithParameter(
					...
					getCatalogueLevelItemsSelect: function(rowItem, subRow) {
						...
						column.orderPosition = 0;
						var columnPath = rowItem.get("ColumnPath");
						select.addParameterColumn(columnPath, BPMSoft.DataValueType.TEXT, "ColumnPath");
						select.addParameterColumn(rowItem.get("Id"), BPMSoft.DataValueType.GUID, "ParentId");
						select.addParameterColumn(rowItem.get("Name"), BPMSoft.DataValueType.TEXT, "ParentName");
						select.addParameterColumn(rowItem.get("Position"), BPMSoft.DataValueType.INTEGER, "Position");
						select.addParameterColumn(rowItem.get("ReferenceSchemaName"), BPMSoft.DataValueType.TEXT,
							"ReferenceSchemaName");
						select.rowCount = -1;
						select.filters.addItem(
							BPMSoft.createColumnIsNotNullFilter(
						...
						if (this.catalogAdditionalFiltersValues) {
							...
							BPMSoft.each(this.catalogAdditionalFiltersValues, function(filterValue) {
					 ...
					 * ############## ####### ##### ########
					 * @protected
					 * @param {BPMSoft.Collection} catalogueFolderValues
					 * @param {object} rowConfig
					 * @param {object} scope
					 * @param {BPMSoft.Collection} resultItems
					 */
					prepareCatalogueFoldersAndGetSelected: function(catalogueFolderValues, rowConfig, scope, resultItems) {
						...
						if (catalogueFolderValues.length) {
							BPMSoft.each(catalogueFolderValues[0].rows, function(item) {
					...
					load: function(setExpandedLevels, callback, scope) {
						...
						batch.execute(function(response) {
							if (response && response.success) {
								...
								BPMSoft.each(response.queryResults, function(item) {
					...
					editFolderFilters: function() {
						...
						if (Ext.isEmpty(searchData)) {
							deserializeData = BPMSoft.createFilterGroup();
						} else {
							deserializeData = BPMSoft.deserialize(searchData);
						}
					...
					changeFolderParent: function(folders, parentId, callback) {
						...
						var folder = Ext.create("BPMSoft.FolderFilterItemViewModel", getFolderViewModelConfig(this.entitySchema));
					...
					_setChildrenFoldersIds: function(foldersIds, parentId) {
						...
						BPMSoft.each(children, function(item) {
					 ...
					 * @protected
					 * @param {object} rowConfig
					 * @param {BPMSoft.Collection} itemValues
					 * @returns {BaseViewModel|*}
					 */
					...
					applyCatalogueFilters: function(rowId, addToQuickFilter) {
						...
						var filtersGroupResult = BPMSoft.createFilterGroup();
						...
						if (rowId && rowId !== this.catalogueRootRecordItem.value) {
							...
							for (var i = position - 1; i >= 0; i--) {
								...
								if (rowItem) {
									var currentParentID = parentData.get("ActualCatalogueFolderId");
									filtersGroup.addItem(BPMSoft.createColumnFilterWithParameter(
										BPMSoft.ComparisonType.EQUAL, rowItem.get("ColumnPath"), currentParentID));
									parentData = this.getGridColumn(parentData.get("Parent"));
								}
							}
					...
					applyFolderFilters: function(rowId, addToQuickFilter) {
						...
						if (!isRootFolder) {
							...
							if (isGeneralFolder || isMailBoxFolder || isSubEmailFolder) {
								...
								this.log(resources.localizableStrings.EmptyFiltersGroupData, BPMSoft.LogMessageType.WARNING);
							}
							var serializationInfo = filtersGroup.getDefSerializationInfo();
							serializationInfo.serializeFilterManagerInfo = true;
							resultFiltersObject = {
						...
						if (currentItemType && currentItemType.value === ConfigurationConstants.Folder.Type.Search) {
							folderMenuItemConfig = {
								...
								filter: !Ext.isEmpty(searchData)
									? BPMSoft.deserialize(searchData)
									: null,
		...
		function getFolderViewModelConfig(entitySchema, mainViewModel) {
					...
					saveLookupFolder: function(serializedFilters, callback, scope) {
						if (this.isNew) {
							...
							if (parent && parent.value) {
								...
								if (!BPMSoft.contains(expandedLevels, parent.value)) {
									expandedLevels.push(parent.value);
								}
			...
			viewModelConfig.columns = BPMSoft.deepClone(entitySchema.columns);
