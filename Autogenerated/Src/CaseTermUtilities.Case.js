define("CaseTermUtilities", ["CasesEstimateLabel", "FormatUtils"],
		function(CasesEstimateLabel, FormatUtils) {
			/**
			 * @class BPMSoft.configuration.mixins.CaseTermUtilities
			 * Mixin, that implements work with terms on page.
			 */
			Ext.define("BPMSoft.configuration.mixins.CaseTermUtilities", {
				alternateClassName: "BPMSoft.CaseTermUtilities",
					...
					this.getContactTimeAsync(this.BPMSoft.SysValue.CURRENT_USER_CONTACT.value,
							function(response) {
								if (response && response.GetContactTimezoneResult) {
									this.set("CurrentUserDate", this.BPMSoft.utils.date.parseDate(response.GetContactTimezoneResult));
								}
