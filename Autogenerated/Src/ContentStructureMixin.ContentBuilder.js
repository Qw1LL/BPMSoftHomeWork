define("ContentStructureMixin", ["ContentStructureMixinResources", "BaseContentStructureItemViewModel"],
	function(resources) {
		Ext.define("BPMSoft.configuration.mixins.ContentStructureMixin", {
			alternateClassName: "BPMSoft.ContentStructureMixin",
			...
			_fillAllItemTypes: function(parent, itemTypes) {
				BPMSoft.each(parent.$StructureItems, function(structureItem) {
					var itemType = structureItem.$Config.ItemType;
					if (!itemTypes[itemType]) {
						itemTypes[itemType] = 1;
					}
					if (!BPMSoft.isEmpty(structureItem.$StructureItems)) {
			...
			initStructureItems: function(isReverse) {
				if (!this.get("StructureItems")) {
					this.set("StructureItems", new BPMSoft.BaseViewModelCollection());
				}
				var collection = new BPMSoft.BaseViewModelCollection();
				BPMSoft.each(this.$Config.Items, function(item) {
			 ...
			 * @returns {BPMSoft.BaseContentStructureItemViewModel} Structure item view model.
			 */
			 ...
			 * @protected
			 * @param {BPMSoft.BaseViewModel} parent Item view model with structure items.
