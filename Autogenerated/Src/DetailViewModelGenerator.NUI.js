define('DetailViewModelGenerator', ['ext-base', 'BPMSoft', 'sandbox', 'CardViewModelGenerator',
	'DetailViewModelGeneratorResources', 'GridUtilities', 'GridProfileHelper', 'MaskHelper'],
	function(Ext, BPMSoft, sandbox, CardViewModelGenerator, resources, GridUtilities, GridProfileHelper, MaskHelper) {
		var cardEntityInfo;
		var entitySchema;
		var relativePath;
		...
		function getFullViewModelSchema(sourceViewModelSchema, userCodes) {
			...
			viewModelSchema.methods.applyFilter = function(select, args) {
				...
				if (Ext.isArray(filterPath)) {
					var schemaFiltersGroup = BPMSoft.createFilterGroup();
			...
			viewModelSchema.methods.setColumnsProfile = function() {
				...
				BPMSoft.utils.saveUserProfile(columnsSettingsProfileKey, columnsSettingsProfile, false);
			...
			viewModelSchema.methods.loadAllRowsCount = function() {
				...
				select.addAggregationSchemaColumn(this.entitySchema.primaryColumnName,
					BPMSoft.AggregationType.COUNT, 'CountRows');
			...
			viewModelSchema.methods.addSelectColumns =  function(select) {
				...
				var config = BPMSoft.configuration.EntityStructure[this.entitySchema.name];
			...
			viewModelSchema.methods.reload = function() {
				var gridData = this.get('gridData');
				var tempCollection = new BPMSoft.BaseViewModelCollection();
			...
			function addDetail(tag, customAction, scope) {
				...
				if (!Ext.isEmpty(customAction)) {
					...
					var config = BPMSoft.configuration.EntityStructure[scope.entitySchema.name];
		...
		function addFilterColumn(target, name, value) {
			...
			target.add(name + '-filter-' + value, BPMSoft.createColumnFilterWithParameter(
				BPMSoft.ComparisonType.EQUAL, name, value));
		}
		...
		return {
			...
			generateItem: function(viewModel, userCodes, args) {
				var fullViewModelSchema = BPMSoft.deepClone(viewModel);
				...
				var itemViewModelConfig = {
					...
					values: {
						...
						gridLoading: false,
						gridSorting: false,
						gridSortDirection: BPMSoft.OrderDirection.ASC,
