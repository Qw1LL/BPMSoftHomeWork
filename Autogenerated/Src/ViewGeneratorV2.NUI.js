define("ViewGeneratorV2", ["BusinessRulesApplierV2", "ext-base", "BPMSoft", "ViewGeneratorV2Resources",
	"LinkColumnHelper", "GridLayoutUtils", "ModuleUtils", "ColumnUtilities", "BaseGeneratorV2"
], function(BusinessRulesApplier, Ext, BPMSoft, resources, LinkColumnHelper, GridLayoutUtils) {

	/**
	 * #####-######### #############
	 */
	Ext.define("BPMSoft.configuration.ViewGenerator", {
		extend: "BPMSoft.configuration.BaseGenerator",
		alternateClassName: "BPMSoft.ViewGenerator",
		 ...
		 * @type {Number}
		 */
		defaultScheduleEditBottomPadding: BPMSoft.convertEmToPx("2em"),
		...
		defaultControlSuffix: {
			"BPMSoft.GridLayout": "GridLayout",
			"BPMSoft.Grid": "Grid",
			"BPMSoft.TabPanel": "TabPanel",
			"BPMSoft.ImageTabPanel": "ImageTabPanel",
			"BPMSoft.Label": "Label",
			"BPMSoft.TipLabel": "TipLabel",
			"BPMSoft.Button": "Button",
			"BPMSoft.MemoEdit": "MemoEdit",
			"BPMSoft.TextEdit": "TextEdit",
			"BPMSoft.HtmlEdit": "HtmlEdit",
			"BPMSoft.FloatEdit": "FloatEdit",
			"BPMSoft.IntegerEdit": "IntegerEdit",
			"BPMSoft.DateEdit": "DateEdit",
			"BPMSoft.TimeEdit": "TimeEdit",
			"BPMSoft.ComboBoxEdit": "ComboBoxEdit",
			"BPMSoft.LookupEdit": "LookupEdit",
			"BPMSoft.MappingEdit": "MappingEdit",
			"BPMSoft.CheckBoxEdit": "CheckBoxEdit",
			"BPMSoft.ControlGroup": "ControlGroup",
			"BPMSoft.Container": "Container",
			"BPMSoft.RadioButton": "RadioButton",
			"BPMSoft.ScheduleEdit": "ScheduleEdit",
			"BPMSoft.BaseProgressBar": "BaseProgressBar",
			"BPMSoft.InformationButton": "InformationButton",
			"BPMSoft.Hyperlink": "Hyperlink",
			"BPMSoft.DraggableContainer": "DraggableContainer",
			"BPMSoft.Component": "Component",
			"BPMSoft.BaseSearchEdit": "SearchEdit",
			"BPMSoft.IframeControl": "Iframe",
			"BPMSoft.ExternalWidget": "ExternalWidget"
		},
		...
		defaultGridCellCaptionConfig: {
			visible: true,
			position: BPMSoft.CaptionPositionType.ABOVE,
			align: BPMSoft.CaptionAlignType.LEFT
		},
		...
		setGeneratorsByModule: function(moduleName, module) {
			...
			BPMSoft.each(generators, function(generator, itemName) {
		...
		fillCustomGenerators: function(config, generators) {
			BPMSoft.iterateChildItems(config, function(iterationConfig) {
		...
		requireCustomGenerators: function(viewConfig, callback, scope) {
			...
			require(modulesToRequire, function() {
				...
				BPMSoft.each(modulesToRequire, function(moduleName, index) {
		...
		/**
		 * Appends to service properties className property if it's not BPMSoft.Label child class name.
		 * @private
		...
		processLabelCustomClassName: function(config, serviceProperties) {
			...
			if (className) {
				...
				var labelClass = Ext.ClassManager.get("BPMSoft.Label");
		...
		processFeaturedView: function(viewConfig) {
			...
			BPMSoft.each(featureConfigList, function(featureConfig) {
				var featureName = featureConfig.featureName;
				if (BPMSoft.Features.getIsEnabled(featureName)) {
		 ...
		 * ########## ############ ############# ######## #####
		 * @protected
		 * @throws {BPMSoft.InvalidObjectState} ####### ########## #### ####### ## ### ############
		 * @param {Object} config ############ ######## ############# #####
		 * @return {Object} ########## ############### ############# ########
		 */
		...
		generateStandardItem: function(config) {
			...
			var result = null;
			switch (itemType) {
				case BPMSoft.ViewItemType.GRID_LAYOUT:
					result = this.generateGridLayout(config);
					break;
				case BPMSoft.ViewItemType.TAB_PANEL:
					result = this.generateTabPanel(config);
					break;
				case BPMSoft.ViewItemType.IMAGE_TAB_PANEL:
					result = this.generateImageTabPanel(config);
					break;
				case BPMSoft.ViewItemType.DETAIL:
					result = this.generateDetail(config);
					break;
				case BPMSoft.ViewItemType.MODULE:
					result = this.generateModule(config);
					break;
				case BPMSoft.ViewItemType.BUTTON:
					result = this.generateButton(config);
					break;
				case BPMSoft.ViewItemType.LABEL:
					result = this.generateLabel(config);
					break;
				case BPMSoft.ViewItemType.CONTAINER:
					result = this.generateContainer(config);
					break;
				case BPMSoft.ViewItemType.TIP:
					result = this.generateTip(config);
					break;
				case BPMSoft.ViewItemType.TIP_LABEL:
					result = this.generateControlLabel(config);
					break;
				case BPMSoft.ViewItemType.MENU:
					result = this.generateMenu(config);
					break;
				case BPMSoft.ViewItemType.MENU_ITEM:
					result = this.generateMenuItem(config);
					break;
				case BPMSoft.ViewItemType.MENU_SEPARATOR:
					result = this.generateSeparatorMenuItem(config);
					break;
				case BPMSoft.ViewItemType.MODEL_ITEM:
					result = this.generateModelItem(config);
					break;
				case BPMSoft.ViewItemType.SECTION_VIEWS:
					result = this.generateSectionViews(config);
					break;
				case BPMSoft.ViewItemType.SECTION_VIEW:
					result = this.generateSectionView(config);
					break;
				case BPMSoft.ViewItemType.GRID:
					result = this.generateGrid(config);
					break;
				case BPMSoft.ViewItemType.CONTROL_GROUP:
					result = this.generateControlGroup(config);
					break;
				case BPMSoft.ViewItemType.RADIO_GROUP:
					result = this.generateRadioGroup(config);
					break;
				case BPMSoft.ViewItemType.DESIGN_VIEW:
					result = this.generateDesignedView(config);
					break;
				case BPMSoft.ViewItemType.SCHEDULE_EDIT:
					result = this.generateScheduleEdit(config);
					break;
				case BPMSoft.ViewItemType.COLOR_BUTTON:
					result = this.generateColorButton(config);
					break;
				case BPMSoft.ViewItemType.HYPERLINK:
					result = this.generateHyperlink(config);
					break;
				case BPMSoft.ViewItemType.INFORMATION_BUTTON:
					result = this.generateInformationButton(config);
					break;
				case BPMSoft.ViewItemType.COMPONENT:
					result = this.generateComponent(config);
					break;
				case BPMSoft.ViewItemType.GRID_LAYOUT_EDIT:
					result = this.generateGridLayoutEdit(config);
					break;
				case BPMSoft.ViewItemType.IFRAMECONTROL:
					result = this.generateIframeControl(config);
					break;
				case BPMSoft.ViewItemType.EXTERNAL_WIDGET:
		...
		getConfigWithoutProperties: function(config, properties) {
			var result = BPMSoft.deepClone(config);
			BPMSoft.each(properties, function(propertyName) {
				delete result[propertyName];
			}, this);
			BPMSoft.each(result, function(obj) {
		...
		generateGridLayout: function(config) {
			var controlId = this.getControlId(config, "BPMSoft.GridLayout");
			var result = {
		...
		/**
		 * Generates options for creating controls {BPMSoft.ViewItemType.GRID_LAYOUT_EDIT}.
		 * @protected
		...
		/**
		 * ########## ############ ############# ### ####### {BPMSoft.ViewItemType.TAB_PANEL}
		 * @protected
		 * @virtual
		 * @param {Object} config ############ #######
		 * @return {Object} ########## ############### ############# #######
		...
		generateTabPanel: function(config) {
			...
			var result = [tabPanel];
			BPMSoft.each(config.tabs, function(tab) {
		...
		/**
		 * ########## ############ ############# ### ####### {BPMSoft.ViewItemType.IMAGE_TAB_PANEL}
		 * @protected
		 * @virtual
		 * @param {Object} config ############ #######
		 * @return {Object} ########## ############### ############# #######
		...
		generateImageTabPanel: function(config) {
			config.className = "BPMSoft.ImageTabPanel";
		...
		generateTabPanelControl: function(config) {
			var className = config.className || "BPMSoft.TabPanel";
		...
		/**
		 * Generates view of one tab for {BPMSoft.ViewItemType.TAB_PANEL}
		 * @protected
		 * @virtual
		...
		generateDetail: function(config) {
			var detailContainerId = this.getControlId(config, "BPMSoft.Container");
		...
		/**
		 * Generates view config for module (@link BPMSoft.ViewItemType.MODULE).
		 * @protected
		...
		generateModule: function(config) {
			...
			if (config.makeUniqueId === true) {
				containerId = this.getControlId(config, "BPMSoft.Container");
			}
		...
		/**
		 * ########## ############ ############# ### ########## {BPMSoft.ViewItemType.CONTAINER}
		 * @protected
		 * @virtual
		 * @param {Object} config ############ ##########
		 * @return {Object} ########## ############### ############# ##########
		...
		generateContainer: function(config) {
			...
			BPMSoft.each(config.items, function(childItem) {
				if (BPMSoft.contains(config.excludeItemTypes, childItem.itemType)) {
					return;
				}
		...
		/**
		 * Generate label configuration {BPMSoft.ViewItemType.LABEL}.
		 * @protected
		 * @virtual
		...
		generateLabel: function(config) {
			...
			if (Ext.isEmpty(caption)) {
				...
				this.log(errorMessage, BPMSoft.LogMessageType.ERROR);
			}
			var id = this.getControlId(config, "BPMSoft.Label");
			var label = {
		...
		/**
		 * ########## ############ ############# ### ########### {BPMSoft.ViewItemType.HYPERLINK}
		 * @protected
		 * @virtual
		 * @param {Object} config ############ ###########.
		 * @return {Object} ########## ############### ############# ###########.
		...
		generateHyperlink: function(config) {
			var id = this.getControlId(config, "BPMSoft.Hyperlink");
			var hyperlink = {
		...
		addLabelSizeClass: function(label, config) {
			var textSizeClasses = (config.textSize === BPMSoft.TextSize.LARGE)
		...
		/**
		 * Generate view configuration for button {BPMSoft.ViewItemType.BUTTON}
		 * @protected
		 * @virtual
		 * @param {Object} config Configuration information.
		...
		generateButton: function(config) {
			var id = this.getControlId(config, "BPMSoft.Button");
			var result = {
			...
			if (Ext.isArray(config.menu)) {
				var menuConfig = {
					itemType: BPMSoft.ViewItemType.MENU,
					items: config.menu
				};
		...
		/**
		 * Generate view configuration for IframeControl {BPMSoft.ViewItemType.IFRAMECONTROL}
		 * @protected
		 * @virtual
		 * @param {Object} config Configuration information.
		 * @return {Object} Returns generated view.
		...
		generateIframeControl: function(config) {
			var id = this.getControlId(config, "BPMSoft.IframeControl");
			var result = {
		...
		/**
		 * Generate view configuration for component {BPMSoft.ViewItemType.COMPONENT}
		 * @protected
		 * @virtual
		 * @param {Object} config Configuration information.
		...
		generateComponent: function(config) {
			var id = this.getControlId(config, "BPMSoft.Component");
		...
		generateControlGroup: function(config) {
			var controlId = this.getControlId(config, "BPMSoft.ControlGroup");
			var result = {
			...
			BPMSoft.each(config.tools, function(childItem) {
		...
		/**
		 * ########## ############ ############# ### ###### ############## {BPMSoft.ViewItemType.RADIO_GROUP}
		 * @protected
		 * @virtual
		 * @param {Object} config ############ ###### ##############
		 * @return {Object} ########## ############### ############# ###### ##############
		...
		generateRadioGroup: function(config) {
			var containerId = this.getControlId(config, "BPMSoft.Container");
		...
		generateRadioButton: function(config) {
			var containerId = this.getControlId(config, "BPMSoft.Container");
			var radioId = this.getControlId(config, "BPMSoft.RadioButton");
			var controlConfig = {
		...
		generateMenu: function(config) {
			...
			BPMSoft.each(config.items, function(menuItemConfig) {
				menuItemConfig.itemType = menuItemConfig.itemType || BPMSoft.ViewItemType.MENU_ITEM;
		...
		/**
		 * ########## ############ ############# ### ######## #### {BPMSoft.ViewItemType.MENU_ITEM}
		 * @protected
		 * @virtual
		 * @param {Object} config ############ ######## ####
		 * @return {Object} ########## ############### ############# ######## ####
		...
		/**
		 * ########## ############ ############# ########### # #### {BPMSoft.ViewItemType.MENU_SEPARATOR}
		 * @protected
		 * @virtual
		 * @param {Object} config ############ ########### # ####
		 * @return {Object} ########## ############### ############# ########### # ####
		...
		generateInformationButton: function(config) {
			var tipEnums = BPMSoft.controls.TipEnums;
		...
		generateItemTips: function(config, viewItem) {
			...
			viewItem.tips = [];
			BPMSoft.each(config.tips, function(tipConfig) {
				tipConfig.itemType = BPMSoft.ViewItemType.TIP;
		...
		generateTipTools: function(config) {
			var tools = [];
			if (config.tools) {
				BPMSoft.each(config.tools, function(tool) {
		...
		/**
		 * Generate view configuration for external widget {BPMSoft.ViewItemType.EXTERNAL_WIDGET}
		 * @protected
		 * @virtual
		 * @param {Object} config
		 * @returns {Object}
		...
		generateExternalWidgetControl: function(config) {
			var className = "BPMSoft.ExternalWidget";
		...
		getTipConfig: function(config, defTipConfig) {
			...
			if (tipConfig.contextIdList) {
				defTipConfig.tip.className = "BPMSoft.ContextTip";
			}
		...
		getDefaultTipConfig: function() {
			var config = {
				tip: {
					...
					displayMode: BPMSoft.TipDisplayMode.WIDE
				},
				settings: {
		...
		generateTipLabel: function(config) {
			...
			var id = this.getControlId(config, "BPMSoft.TipLabel");
			var tipLabelConfig = {
		...
		/**
		 * ########## ############ ############# ### {BPMSoft.ViewItemType.MODEL_ITEM}
		 * @protected
		 * @param {Object} config ############ ######## ############# #####
		 * @return {Object} ########## ############### ############# ########
		 */
		generateModelItem: function(config) {
			var modelItemWrapId = this.getControlId(config, "BPMSoft.Container");
		...
		getModelItemContainerClasses: function(config) {
			...
			if (columnConfig && columnConfig.dataValueType === BPMSoft.DataValueType.BOOLEAN) {
		...
		_getLabelWrapClass: function(config) {
			var columnConfig = this.findViewModelColumn(config);
			var labelWrapClass = BPMSoft.deepClone(this.defaultLabelWrapClass);
			if (columnConfig && columnConfig.dataValueType === BPMSoft.DataValueType.BOOLEAN) {
		...
		generateControlLabelWrap: function(config) {
			var labelWrapId = this.getControlId(config, "BPMSoft.Container");
			if (labelWrapId) {
		 ...
		 * @virtual
		 * @param {Object} config The configuration of the circuit element representations.
		 * @return {BPMSoft.DataValueType|null} Returns element data type.
		 */
		...
		getAutoBindings: function(config) {
			...
			if (this.schemaType === BPMSoft.SchemaType.EDIT_VIEW_MODEL_SCHEMA) {
				result.focus = {
		...
		updateColumnCaption: function(config) {
			...
			if (labelCaption) {
				var column = this.findViewModelColumn(config);
				if (column && !BPMSoft.isEqual(labelCaption, column.caption)) {
		...
		generateTextEdit: function(config) {
			...
			switch (contentType) {
				case BPMSoft.ContentType.LONG_TEXT:
					className = "BPMSoft.MemoEdit";
					this._addTextLengthConstraint(config);
					break;
				case BPMSoft.ContentType.RICH_TEXT:
					className = "BPMSoft.HtmlEdit";
					break;
				case BPMSoft.ContentType.SEARCHABLE_TEXT:
			...
			if (config.contentType === BPMSoft.ContentType.RICH_TEXT) {
		...
		generateIntegerEdit: function(config) {
			var id = this.getControlId(config, "BPMSoft.IntegerEdit");
			var integerEdit = {
		...
		generateFloatEdit: function(config) {
			var id = this.getControlId(config, "BPMSoft.FloatEdit");
			var floatEdit = {
		...
		generateDateEdit: function(config) {
			var id = this.getControlId(config, "BPMSoft.DateEdit");
			var dateEdit = {
		...
		generateTimeEdit: function(config) {
			var id = this.getControlId(config, "BPMSoft.TimeEdit");
			var timeEdit = {
		...
		generateEnumEdit: function(config) {
			var id = this.getControlId(config, "BPMSoft.ComboBoxEdit");
			var enumEdit = {
		...
		_getIsPredictableColumn: function(config) {
			...
			return BPMSoft.getIsPredictableColumn(entitySchemaName, column);
		},
		...
		generateSearchableTextEdit: function(config) {
			...
			var id = this.getControlId(config, "BPMSoft.SearchableTextEdit");
			var searchableTextEdit = {
				"className": "BPMSoft.SearchableTextEdit",
				"list": {
		...
		generateLookupEdit: function(config) {
			var contentType = config.contentType;
			if ((contentType && (contentType !== BPMSoft.ContentType.LOOKUP)) || !contentType) {
				var viewModelColumn = this.findViewModelColumn(config);
				if (viewModelColumn && viewModelColumn.isSimpleLookup) {
					contentType = BPMSoft.ContentType.ENUM;
				}
			}
			if (contentType && contentType === BPMSoft.ContentType.ENUM) {
				return this.generateEnumEdit(config);
			}
			var id = this.getControlId(config, "BPMSoft.LookupEdit");
			var lookupEdit = {
		...
		generateMappingEdit: function(config) {
			var id = this.getControlId(config, "BPMSoft.MappingEdit");
			var mappingEdit = {
		...
		generateBooleanEdit: function(config) {
			var id = this.getControlId(config, "BPMSoft.CheckBoxEdit");
			var checkBoxEdit = {
		...
		generateStageIndicator: function(config) {
			var id = this.getControlId(config, "BPMSoft.BaseProgressBar");
			var stageIndicator = {
		...
		generateEditControlWrap: function(config) {
			var controlWrapId = this.getControlId(config, "BPMSoft.Container");
			if (controlWrapId) {
		...
		getHintConfig: function(config) {
			...
			var hintConfig = {
				tip: {
					displayMode: config.displayMode || BPMSoft.TipDisplayMode.NARROW,
		...
		generateEditControl: function(config) {
			...
			switch (itemDataValueType) {
				...
				case BPMSoft.DataValueType.LOOKUP:
					...
					break;
				case BPMSoft.DataValueType.MAPPING:
		...
		/**
		 * ########## ############ ############# ### BPMSoft.ViewItemType.SECTION_VIEWS
		 * @protected
		 * @virtual
		 * @param {Object} config ######## ######## #############
		 * @return {Object} ############ #############
		...
		/**
		 * ########## ############ ############# ### BPMSoft.ViewItemType.SECTION_VIEW
		 * @protected
		 * @virtual
		 * @param {Object} config ######## ######## #############
		 * @return {Object} ############ #############
		...
		generateTiledCellCaption: function(gridConfig, columnConfig) {
			...
			if (gridConfig.type !== BPMSoft.GridType.TILED || !captionConfig.visible) {
				return null;
			}
			...
			return {
				type: gridConfig.isVertical ? BPMSoft.GridKeyType.LABEL : BPMSoft.GridKeyType.CAPTION,
				name: name
			};
		},
		...
		generateGridCellValue: function(config) {
			...
			switch (type) {
				case BPMSoft.GridCellType.LINK:
					cellValue.name = {
						bindTo: "on" + config.bindTo + "LinkClick"
					};
					cellValue.type = BPMSoft.GridKeyType.LINK;
					break;
		...
		getGridColumnCaption: function(config) {
			...
			var needUseEntityCaption = !BPMSoft.ColumnUtilities.isProfileCultureEqualsUserCulture(this.schemaProfile);
		...
		generateVerticalGridCellConfig: function(cellCaption, cellValue, colsCount) {
			var config = [];
			if (cellCaption) {
				var captionColsCount = parseInt(colsCount / 100 * 50, 10);
				config.push({
		 ...
		 * @param {BPMSoft.BaseViewModel} viewModelClass Custom view model.
		 */
		addLinks: function(gridConfig, isTiled, viewModelClass) {
			gridConfig.columnsConfig.forEach(function(row) {
				...
				BPMSoft.each(row, function(cell) {
					var item = Ext.isArray(cell) ? cell : [cell];
					BPMSoft.each(item, function(cellItem) {
						...
						var moduleUtils = BPMSoft.ModuleUtils;
						if (entitySchema) {
						...
						if (Ext.isEmpty(cellItem.link) && (columnType !== BPMSoft.GridKeyType.LINK)) {
							cellItem.link = this._getLinkBindToColumn(columnName);
						}
					}, this);
				}, this);
		...
		generateGrid: function(config) {
			...
			if (profile && profile.listedConfig && profile.tiledConfig) {
				...
				config.type = profile.isTiled ? BPMSoft.GridType.TILED : BPMSoft.GridType.LISTED;
				config.listedConfig = {
			...
			if (config.activeRowActions) {
				BPMSoft.each(config.activeRowActions, function(item) {
		...
		/**
		 * ########## ############ ############# ### BPMSoft.ViewItemType.SCHEDULE_EDIT
		 * @protected
		 * @virtual
		 * @param {Object} config ############ ######## #############
		 * @return {Object} ########## ############ ##########
		...
		generateScheduleEdit: function(config) {
			var className = "BPMSoft.ScheduleEdit";
			var result = {
				className: className,
		...
		/**
		 * ########## ############ ############# ### ###### ###### ##### {BPMSoft.ViewItemType.COLOR_BUTTON}
		 * @protected
		 * @virtual
		 * @param {Object} config ############ ###### #####
		 * @return {Object} ########## ############### ############# ###### #####
		...
		generateColorButton: function(config) {
			var id = this.getControlId(config, "BPMSoft.ColorButton");
			var result = {
		...
		/**
		 * @inheritdoc BPMSoft.configuration.BaseGenerator#generate
		 * @override
		 */
