define("SocialFeedUtilities", [
	...
	/**
	 * @class BPMSoft.configuration.SocialFeedUtilities
	 */
	Ext.define("BPMSoft.configuration.mixins.SocialFeedUtilities", {
		alternateClassName: "BPMSoft.SocialFeedUtilities",
		 ...
		 */
		init: function() {
			this.set("SchemasCollection", this.Ext.create("BPMSoft.Collection"));
			this.set("SocialMessageColumnNames", [
				"Id", "CreatedBy", "CreatedOn", "Message", "EntitySchemaUId",
				"EntityId", "Parent", "LikeCount", "CommentCount"
			]);
			this.isUseEsnRightEnabled = this.BPMSoft.Features.getIsEnabled(useEsnRightFeatureName);
		...
		_createSocialMessageFromServiceResponse: function(response) {
			...
			const viewModels = Ext.create("BPMSoft.BaseViewModelCollection");
			BPMSoft.each(messages, function (message) {
		...
		getPrimaryImageValue: function(entity, entitySchemaName, callback, scope) {
			const moduleStructure = this.BPMSoft.configuration.ModuleStructure[entitySchemaName];
		...
		getImageValue: function(entity) {
			...
			if (entity) {
				...
				if (this.Ext.isObject(image) || this.BPMSoft.isGUID(image)) {
					imageValue = image.value || image;
				} else {
					return this.BPMSoft.ImageUrlBuilder.getUrl(this.get("Resources.Images.DefaultCreatedBy"));
				}
			}
		...
		/**
		 * @deprecated Moved to BPMSoft.SocialMessageViewModel#onCreateByClick
		 */
		onCreateByClick: function(e) {
			const mouseButton = BPMSoft.getMouseButton(e);
			if (mouseButton === BPMSoft.MouseButton.LEFT) {
		...
		/**
		 * @deprecated Moved to BPMSoft.SocialMessageViewModel#getCreatedUrlContact
		 */
		...
		/**
		 * @deprecated Moved to BPMSoft.SocialMessageViewModel#getCreatedPublishUrl
		 */
		...
		getEntityImage: function() {
			...
			if (this.Ext.isObject(image) || BPMSoft.isGUID(image)) {
				imageValue = image.value || image;
			}
			return imageValue ? this.getImageValue(entity)
				: this.BPMSoft.ImageUrlBuilder.getUrl(resources.localizableImages.NoChannel);
		},
		...
		/**
		 * @deprecated Moved to BPMSoft.SocialMessageViewModel#onEntityClick
		 */
		...
		getShowCommentsText: function() {
			...
			if (commentCount > 0) {
				text = Ext.String.format("{0}", commentCount);
			}
		...
		getPostCommentEditDeleteVisible: function() {
			...
			if (this.get("CreatedBy").value === BPMSoft.SysValue.CURRENT_USER_CONTACT.value) {
				visible = true;
			}
			return visible;
		},
		...
		/**
		 * @inheritdoc BPMSoft.BaseViewModel#showConfirmationDialog
		 * @private
		...
		_showConfirmationDialog: function(dialogText) {
			return new Promise(function(resolve) {
				this.showConfirmationDialog(dialogText, function(dialogResult) {
						if (dialogResult === BPMSoft.MessageBoxButtons.YES.returnCode) {
							resolve();
						}
					},
					[
						BPMSoft.MessageBoxButtons.YES.returnCode,
		...
		_showErrorInfo: function(msg) {
			return Promise.resolve(BPMSoft.showErrorMessage(msg));
		},
		 ...
		 * @param {BPMSoft.BaseViewModel} post ViewModel post element.
		 * @private
		 */
		...
		getNewCommentButtonVisibility: function() {
			...
			if (entitySchemaUId === ESNConstants.SysSchema.CaseUId) {
				result = !isCaseFinalStatus && BPMSoft.isCurrentUserSsp();
			}
		...
		onCommentPublishClick: function() {
			...
			this.insertSocialMessage(post, function(item) {
				this.set("commentMessage", "");
				const loadedComments = this.get("LoadedComments");
				this.BPMSoft.each(loadedComments.getItems(), function(comment) {
		...
		getLikeImage: function() {
			...
			return {
				...
				url: this.BPMSoft.ImageUrlBuilder.getUrl(isLikedMe
		...
		insertLike: function(callback, scope) {
			if (this.isUseEsnRightEnabled) {
				...
				insert.setParameterValue("User", BPMSoft.SysValue.CURRENT_USER.value, BPMSoft.DataValueType.GUID);
				insert.setParameterValue("SocialMessage", socialMessageId, BPMSoft.DataValueType.GUID);
		...
		deleteLike: function(callback, scope) {
			...
			if (this.isUseEsnRightEnabled) {
				...
				const userIdFilter = this.BPMSoft.createColumnFilterWithParameter(
					this.BPMSoft.ComparisonType.EQUAL, "User", this.BPMSoft.SysValue.CURRENT_USER.value);
				const entityIdFilter = BPMSoft.createColumnFilterWithParameter(
					this.BPMSoft.ComparisonType.EQUAL, "SocialMessage", socialMessageId);
		...
		getLikedUsersViewModel: function() {
				...
				methods: {
					...
					getLikedUserItems: function() {
						...
						if (this.get("IsUseEsnRightEnabled")) {
							BPMSoft.each(likedUsersCollection, function(item) {
								var entityForImage = {
									"primaryImageValue": null
								};
								if (item.contactPrimaryImageId && item.contactPrimaryImageId !== BPMSoft.GUID_EMPTY) {
					...
					showLikedUsers: function() {
						...
						const likedUsersConfig = Ext.create("BPMSoft.Container", {
					...
					loadLikedUsers: function(selectedMessage) {
						if (this.get("IsUseEsnRightEnabled")) {
							...
							esq.filters.add("currentMessageFilter",
								BPMSoft.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL, "SocialMessage",
									selectedMessage));
			...
			return this.Ext.create("BPMSoft.BaseViewModel", config);
		},
		...
		getEditImage: function() {
			return {
				source: this.BPMSoft.ImageSources.URL,
				url: this.BPMSoft.ImageUrlBuilder.getUrl(resources.localizableImages.Edit)
			};
		},
		...
		getDeleteImage: function() {
			return {
				source: this.BPMSoft.ImageSources.URL,
				url: this.BPMSoft.ImageUrlBuilder.getUrl(resources.localizableImages.Delete)
			};
		},
		...
		createSocialMessageInsert: function(data) {
			...
			insert.setParameterValue("Message", data.message, BPMSoft.DataValueType.TEXT);
			if (data.entityId) {
				insert.setParameterValue("EntitySchemaUId", data.entitySchemaId, BPMSoft.DataValueType.GUID);
				insert.setParameterValue("EntityId", data.entityId, BPMSoft.DataValueType.GUID);
			}
			if (data.parentId) {
				insert.setParameterValue("Parent", data.parentId, BPMSoft.DataValueType.GUID);
			}
			return insert;
		},
		...
		_applySortColumnFilters: function(filters, config) {
			...
			if (sortColumnLastValue) {
				const sortDirection = config.sortDirection || BPMSoft.OrderDirection.DESC;
				const sortColumnName = config.sortColumnName;
				filters.addItem(this.BPMSoft.createColumnFilterWithParameter(
					sortDirection === BPMSoft.OrderDirection.DESC
		...
		_applyParentIdFilters: function(filters, config) {
			...
			if (parentId) {
				filters.addItem(this.BPMSoft.createColumnFilterWithParameter(
					BPMSoft.ComparisonType.EQUAL, "Parent", parentId));
			} else {
				filters.addItem(this.BPMSoft.createIsNullFilter(
					this.Ext.create("BPMSoft.ColumnExpression", {columnPath: "Parent"})
				));
			}
		},
		...
		_updateIsLikedMeForLoadedMessages: function(likes, loadedMessages) {
			this.BPMSoft.each(likes, function(like) {
		 ...
		 * @param {BPMSoft.Collection} loadedMessages
		...
		loadMessages: function(config, callback, scope) {
			...
			requestConfig.timeout = BPMSoft.BaseServiceProvider.getRequestTimeout(requestConfig);
			...
			var data = {
				"readOptions": {
					...
					"OffsetDate": sortColumnLastValue
						? BPMSoft.encodeDate(sortColumnLastValue)
						: undefined,
		...
		_applyNotLoadedMessagesFilter: function(esq) {
			...
			const filter = BPMSoft.createColumnInFilterWithParameters("Id", ids);
		...
		getSocialMessageViewModelName: function() {
			return "BPMSoft.SocialMessageViewModel";
		},
		 ...
		 * @param {Array|BPMSoft.Collection}loadedMessages
		 * @protected
		 */
		loadMessageLikesForUser: function(loadedMessages) {
			let likesCollection = new this.BPMSoft.Collection();
			if (this.isUseEsnRightEnabled) {
				ServiceHelper.callService({
					"serviceName": this.esnServiceName,
				...
				const socialLikeEsq = this.Ext.create("BPMSoft.EntitySchemaQuery", {
					rootSchemaName: "SocialLike"
				});
				socialLikeEsq.addColumn("SocialMessage");
				socialLikeEsq.filters.add("socialMessagesFilter",
					this.BPMSoft.createColumnInFilterWithParameters("SocialMessage", loadedMessages.getKeys()));
				socialLikeEsq.filters.add("currentUserFilter",
					this.BPMSoft.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL, "User",
		...
		/**
		 * Builds {BPMSoft.EntitySchemaQuery} for message/comments from request configuration.
		 * @param {Object} config Request configuration.
		...
		getSocialMessageEsq: function(config) {
			...
			const options = this.Ext.apply({
				rootSchemaName: "SocialMessage",
				rowViewModelClassName: "BPMSoft.SocialMessageViewModel"
			}, rowCount);
			const esq = this.Ext.create("BPMSoft.EntitySchemaQuery", options);
			esq.on("createviewmodel", this.createSocialMessage, this);
			const sortColumnName = config.sortColumnName;
			const socialMessageColumnNames = this.get("SocialMessageColumnNames");
			const columnNames = this.BPMSoft.deepClone(socialMessageColumnNames);
			...
			if (BPMSoft.Features.getIsEnabled("LinkPreview")) {
				esq.addColumn("[LinkPreviewData:EntityId:Id].Data", "LinkPreviewData");
			}
			if (config.extendedColumns) {
				this.BPMSoft.each(config.extendedColumns, function(item) {
			...
			if (BPMSoft.Features.getIsEnabled("UseLoadedMessagesFilterInESN")) {
