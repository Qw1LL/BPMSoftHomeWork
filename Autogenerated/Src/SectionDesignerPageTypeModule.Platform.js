define("SectionDesignerPageTypeModule", [
	"ext-base", "BPMSoft", "sandbox", "SectionDesignerPageTypeModuleResources",
	"SectionDesignerUtils", "SectionDesignDataModule", "ConfigurationEnums"
],
	function(Ext, BPMSoft, sandbox, resources, SectionDesignerUtils, SectionDesignDataModule, ConfigurationEnums) {
		 ...
		 */
		var availableEntityTypeColumns = Ext.create("BPMSoft.Collection");
		...
		function getAvailableEntityTypeColumns(entitySchema) {
			var columns = Ext.create("BPMSoft.Collection");
		...
		function getViewModel() {
			var config = {
				...
				values: {
					pages: new BPMSoft.Collection(),
					pagesView: new BPMSoft.Collection(),
					typeColumnList: new BPMSoft.Collection(),
					addButtonMenuItemsCollection: new BPMSoft.Collection(),
					addButtonMenuFirstItem: Ext.create("BPMSoft.BaseViewModel", {
						values: {
							Id: "addNewItem",
					...
					addButtonMenuSeparator: Ext.create("BPMSoft.BaseViewModel", {
						values: {
							Id: "separator",
					...
					addNotExistedTypesMenuItems: function(pages, entitySchemaData) {
						...
						BPMSoft.each(entitySchemaData, function(item) {
							if (!pages.contains(item.Id) && !menuItemsCollection.contains(item.Id)) {
								menuItemsCollection.add(item.Id, Ext.create("BPMSoft.BaseViewModel", {
									values: {
										Caption: item.Name,
										Id: item.Id,
										Name: item.Name,
							...
							SectionDesignDataModule.getDesignData(function(data) {
								SectionDesignDataModule.getEntities(value.referenceSchemaName, ["Name"], false,
									function(entitySchemaData) {
										...
										BPMSoft.each(sortedPages, function(editPage) {
											var page = {
...
//								typeColumn.referenceSchemaName, callback);
		...
		function addItemEdit(config) {
			...
			if (itemEditViewModel) {
				...
				itemEditViewModel.set("value", value.value || BPMSoft.generateGUID());
				...
				itemEditViewModel = Ext.create("BPMSoft.BaseViewModel", {
					columns: {
						displayValue: {
					...
					values: {
						...
						value: value.value || BPMSoft.generateGUID(),
