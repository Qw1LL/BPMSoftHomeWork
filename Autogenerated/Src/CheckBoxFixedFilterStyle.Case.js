define("CheckBoxFixedFilterStyle", ["CheckBoxFixedFilterStyleResources", "ext-base", "BPMSoft",
		"css!CheckBoxFixedFilterStyle"],
	function(resources, Ext, BPMSoft) {
		/**
		 * ############### ##### ### ########## # ############## #########
		 * @param {Object} scope
		 */
		...
		function setCheckBoxBackground(imageName, scope) {
			var scopeIdParts = scope.BPMSoft.id.split("_");
			...
			if (checkBox && imageConfig) {
				checkBox.setStyle("background-image", "url(" + BPMSoft.ImageUrlBuilder.getUrl(imageConfig) + ")");
			}
		}
		/**
