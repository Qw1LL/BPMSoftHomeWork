namespace BPMSoft.Configuration
{
	...
	public static class ActivityUtils {
		...
		[Obsolete("Use GetEmailHash(sender, senDate, title, body, timeZoneInfo)")]
        ...
        /// <returns>Unique hash for email instance.</returns>
        public static string GetEmailHash(string sender, DateTime sendDate, string title, string body, TimeZoneInfo timeZoneInfo,
                bool deleteWhiteSpaces = true, string dateTimeFormat = "yyyy-MM-dd HH:mm", bool fixTitleWhitespaces = true)
        {
            ...
            return FileUtilities.GetMD5Hash(Encoding.Unicode.GetBytes(sender + plainTextBody + fixedTitle +
                utcSendDate.ToString(dateTimeToStringFormat)));
        }
		...
		public static string GetEmailHash(UserConnection userConnection, string sender, DateTime sendDate, string subject, string body,
			TimeZoneInfo timeZoneInfo)
		{
            var key = BPMSoft.Core.Configuration.SysSettings.GetValue(userConnection, "MailHashComposer", "BaseEmailHashComposer");
			var composer = ClassFactory.Get<IEmailHashComposer>(key);
			return composer.GetDefaultHash(userConnection, new EmailHashDTO
			{
				Sender = userConnection.GetIsFeatureEnabled("UseSenderForEmailHash") ? sender : string.Empty,
				SendDate = sendDate,
				Subject = subject,
				Body = body,
		...
		public static List<Guid> GetExistingEmaisIds(UserConnection userConnection, string sender, DateTime sendDate, string subject, string body,
			TimeZoneInfo timeZoneInfo) {
