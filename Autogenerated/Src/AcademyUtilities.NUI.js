define("AcademyUtilities", ["BPMSoft"], function(BPMSoft) {
	Ext.define("BPMSoft.configuration.AcademyUtilities", {
		extend: "BPMSoft.BaseObject",
		alternateClassName: "BPMSoft.AcademyUtilities",
		...
		getUrl: function(config) {
			...
			var callback = config.callback || BPMSoft.emptyFn;
		...
		_mergeHelpConfigs: function(config, defaultConfig) {
			BPMSoft.each(defaultConfig, function(propertyValue, propertyName) {
		...
		getHelpConfigFromDb: function(contextHelpCode, callback, scope) {
			...
			esq.filters.add("ContextHelpCode", esq.createColumnFilterWithParameter(
				BPMSoft.ComparisonType.EQUAL, "Code", contextHelpCode));
		...
		getLocalDocumentationUrl: function(config) {
			var url = BPMSoft.workspaceBaseUrl + "/WebHelpNui/";
			url += config.productEdition + "/";
			var userCulture = BPMSoft.SysValue.CURRENT_USER_CULTURE.displayValue;
		...
		getAcademyUrlFromLookup: function(config) {
			var cacheItemName = "AcademyURL_" + BPMSoft.currentUserCultureName;
			var getLmsUrlQuery = Ext.create("BPMSoft.EntitySchemaQuery", {
	...
	return BPMSoft.AcademyUtilities;
});
