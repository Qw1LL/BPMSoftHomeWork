define("CompletenessParameterDetailV2", ["CompletenessParameterDetailV2Resources", "BusinessRulesApplierV2",
	...
	function(resources, BusinessRulesApplierV2) {
		...
		Ext.define("BPMSoft.configuration.CompletenessParameterGridRowViewModel", {
			extend: "BPMSoft.BaseViewModel",
			alternateClassName: "BPMSoft.CompletenessParameterGridRowViewModel",
			 ...
			 */
			init: function() {
				this.set("AttributeList", new BPMSoft.Collection());
		...
		return {
				...
				"values": {
					...
					"listedConfig": {
						...
						"items": [{
							...
							"bindTo": "Attribute",
							"type": BPMSoft.GridCellType.LOOKUP,
							"position": {
								"column": 1,
								"colSpan": 16
							}
			...
			methods: {
				...
				initData: function(callback, scope) {
					...
					this.set("AttributeCollection", new BPMSoft.Collection());
					this.set("AttributeTypesCollection", new BPMSoft.Collection());
					this.set("FilteredAttributeCollection", new BPMSoft.Collection());
					this.set("LocalCollection", this.Ext.create("BPMSoft.BaseViewModelCollection"));
				...
				generateAttributeList: function(schemaName, callback, scope) {
					var schemaBuilder = this.Ext.create("BPMSoft.configuration.SchemaBuilder");
					var config = {
				...
				addModulesFields: function(columns, modules) {
					BPMSoft.each(modules, function(moduleConfig) {
				...
				findMasterColumn: function(moduleConfig, columns) {
					...
					var foundColumns = BPMSoft.where(columns, {
				...
				loadTypedDetail: function(config) {
					...
					var selectTypeDetail = Ext.create("BPMSoft.EntitySchemaQuery", {
						rootSchemaName: completeness.typeSchemaName
					});
					selectTypeDetail.addMacrosColumn(BPMSoft.QueryMacrosType.PRIMARY_COLUMN, "Id");
					selectTypeDetail.addMacrosColumn(BPMSoft.QueryMacrosType.PRIMARY_DISPLAY_COLUMN, "displayValue");
				...
				selectFromSysDetail: function(details, callback, scope) {
					...
					var selectSysDetail = Ext.create("BPMSoft.EntitySchemaQuery", {
						rootSchemaName: "SysDetail"
					});
					selectSysDetail.addMacrosColumn(BPMSoft.QueryMacrosType.PRIMARY_COLUMN, "Id");
				...
				/**
				 * @inheritdoc BPMSoft.ConfigurationGridUtilities#initEditableGridRowViewModel
				 * @overridden
				 */
				...
				/**
				 * @inheritdoc BPMSoft.GridUtilities#getEditableGridRowViewModelClassName
				 * @overridden
				 */
				getEditableGridRowViewModelClassName: function() {
					return "BPMSoft.CompletenessParameterGridRowViewModel";
				},
				...
				subscribeSandboxEvents: function() {
					...
					sandbox.subscribe("GetTotalPercentage", function() {
						var gridData = this.getGridData();
						var totalValue = 0;
						this.BPMSoft.each(gridData.getItems(), function(item) {
				...
				validateDetail: function() {
					...
					gridData.each(function(item) {
						var isChanged = !this.BPMSoft.isEmptyObject(item.oldChangedValues);
				...
				getSaveQuery: function(item) {
					...
					for (var columnName in item.changedValues) {
						if (item.changedValues.hasOwnProperty(columnName)) {
							...
							if (!column || column.type !== this.BPMSoft.ViewModelColumnType.ENTITY_COLUMN) {
								continue;
							}
				...
				getSaveItemsQueries: function() {
					...
					collection.each(function(item) {
						...
						if (isChanged) {
							item.changedValues = this.BPMSoft.deepClone(item.oldChangedValues);
				...
				save: function() {
					...
					this.BPMSoft.each(queries, function(query) {
				...
				saveRowChanges: function(row, callback, scope) {
					scope = scope || this;
					callback = callback || this.BPMSoft.emptyFn;
					...
					if (row && this.getIsRowChanged(row)) {
						...
						var clonedChangedValues = this.BPMSoft.deepClone(row.changedValues);
				...
				onDeleteAccept: function() {
					...
					var gridData = this.getGridData();
					var localData = this.get("LocalCollection");
					this.BPMSoft.each(selectedItems, function(selectedItem) {
				...
				/**
				 * @inheritdoc BPMSoft.GridUtilites#onDataChanged
				 * @overridden
				 */
				...
				/**
				 * @inheritdoc BPMSoft.ConfigurationGridUtilities#addRow
				 * @overridden
				 */
				addRow: function() {
					var activeRow = this.getActiveRow();
					...
					if (totalValue >= 100) {
						...
						this.BPMSoft.showInformation(warningMessage);
						return;
					}
				...
				/**
				 * @inheritdoc BPMSoft.ConfigurationGridUtilities#setDefaultFocus
				 * @overridden
				 */
