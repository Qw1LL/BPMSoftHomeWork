define("GridProfileHelper", ["ext-base", "BPMSoft", "GridProfileHelperResources", "GridUtilities"],
	function(Ext, BPMSoft, resources, GridUtilities) {
		function getLoadedColumnsWithSortedColumnsInitialization(select, columnsSettingsProfile, profileSortedColumns) {
			...
			BPMSoft.each(sectionSchemaColumns, function(sectionSchemaColumn) {
				var addItem = true;
				var columnPath = sectionSchemaColumn.columnPath;
				BPMSoft.each(loadedColumns, function(loadedColumn) {
				...
				if (addItem) {
					...
					if (!(sectionSchemaColumn instanceof BPMSoft.BaseQueryColumn)) {
		...
		function initSelectSorting(select, columnsSettingsProfile, profileSortedColumns) {
			...
			if (!BPMSoft.isEmptyObject(profileSortedColumns)) {
				BPMSoft.each(profileSortedColumns, function(profileSortedColumn, key) {
				...
				BPMSoft.each(collectionColumns, function(item) {
					var column;
		...
		function updateSortColumnsCaptions(columnsSettingsProfile) {
			...
			if (gridsColumnsConfig) {
				...
				BPMSoft.each(columnsConfig, function(item) {
		...
		function changeSorting(config) {
			...
			if (!column) {
				...
				BPMSoft.each(loadedColumns, function(item, key) {
					if (key === tag) {
						column = item;
						column.name = tag;
					}
		...
		function setSortMenu(columnsSettingsProfile) {
			...
			if (sortMenu) {
				...
				sortMenu = Ext.create("BPMSoft.BaseViewModelCollection");
					...
					for (var j = 0; j < row.length; j++) {
						...
						if (sortMenu.contains(name)) {
							this.log(Ext.String.format(resources.localizableStrings.ColumnAlreadyAddedToSortedList,
								name), BPMSoft.LogMessageType.WARNING);
							continue;
						}
						sortMenu.add(name, Ext.create("BPMSoft.BaseViewModel", {
							values: {
								Caption: {
