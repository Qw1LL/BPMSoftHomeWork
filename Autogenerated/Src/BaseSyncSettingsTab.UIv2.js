define("BaseSyncSettingsTab", ["ExchangeNUIConstants", "BaseSyncSettingsTabResources", "ConfigurationConstants",
	...
	return {
		...
		methods: {
			...
			/**
			 * Creates {@link BPMSoft.data.queries.BatchQuery} instance.
			 * @private
			 */
			createBatchQuery: function() {
				return this.Ext.create("BPMSoft.BatchQuery");
			},
				...
				if (this.get("IsReady")) {
					...
					var isEntityColumnChanged = changedProperties.some(function(item) {
						return (!this.Ext.isEmpty(this.entitySchema.columns[item]) ||
							BPMSoft.contains(mailboxSettingsColumns, item));
					}, this);
			...
			initFolders: function(callback, scope) {
				...
				BPMSoft.chain.apply(this, chainSteps);
			},
			...
			initFolder: function(config, callback, scope) {
				var collection = this.get(config.foldersCollectionName) || this.Ext.create("BPMSoft.Collection");
			 ...
			 */
			getFolderParameters: this.BPMSoft.emptyFn,
			...
			loadFolders: function(config, callback, scope) {
				if (BPMSoft.Features.getIsEnabled("NewMeetingIntegration")) {
			...
			onFoldersLoadedCallback: function(config, folders, callback, scope) {
				...
				for (var i = 0; i < folders.length; i++) {
					...
					var rowModel = this.Ext.create("BPMSoft.BaseViewModel", {
						rowConfig: {
							Id: {dataValueType: this.BPMSoft.DataValueType.TEXT},
							Name: {dataValueType: this.BPMSoft.DataValueType.TEXT},
							ParentId: {dataValueType: this.BPMSoft.DataValueType.TEXT},
							Path: {dataValueType: this.BPMSoft.DataValueType.TEXT}
						},
						values: exchangeFolder
					});
					rowModel.onNameLinkClick = this.BPMSoft.emptyFn;
			 ...
			 * @param {BPMSoft.Collection} exchangeFolders Folders collection.
			 * @param {Function} callback Callback function.
			 * @param {Object} scope Callback scope.
			 */
			...
			setDefaultValues: function(values) {
				this.BPMSoft.each(values, function(value, key) {
					this.set(key, value);
				}, this);
			},
			 ...
			 * @protected
			 * @param {BPMSoft.BaseViewModel} folder Folder item model.
			 * @param {BPMSoft.Collection} selectedRows Folder collection selected rows.
			...
			isFolderSelected: function(folder, selectedRows) {
				...
				return (this.BPMSoft.contains(selectedRows, itemId) || this.Ext.isEmpty(itemParentId));
			},

			/**
			 * Updates ParentId property for folders.
			 * @param {BPMSoft.Collection} collection Folders collection.
			 ...
			 * @param {BPMSoft.Collection} collection All folder collection.
			 * @param {String} selectedRowsProperty Selected rows property name.
			 * @return {BPMSoft.Collection} Selected folders with actualized ParentId.
			 */
			getSelectedFoldersTree: function(collection, selectedRowsProperty) {
				var foldersCollection = this.Ext.create("BPMSoft.Collection");
				...
				var result = this.Ext.create("BPMSoft.Collection");
			 ...
			 * @param {BPMSoft.Collection} collection All folders collection.
			 * @param {Guid} parentId Parent folder id.
			 * @return {BPMSoft.Collection} Child folders with folder parentId.
			 */
			 ...
			 * @param {BPMSoft.Collection} rootFoldersInfo.oldFolders Collection of old folders.
			 */
			 ...
			 * @param {BPMSoft.Collection} config.selectedFolders Collection of selected folders.
			 * @param {Guid} config.rootSelectedFolder Root folder id.
			 * @param {BPMSoft.Collection} config.oldFolders Collection of old folders.
			 * @param {BPMSoft.Collection} childFolders Collection of child elements of folder.
			 */
			saveNewFoldersCollection: function(config, childFolders) {
				...
				var nextLevel = this.Ext.create("BPMSoft.Collection");
			 ...
			 * @virtual
			 */
			setAdditionalFolderInsert: this.BPMSoft.emptyFn,

			/**
			 * Save new folders collection if child folders exists, else delete old folders if it needs.
			 * @param {Object} config Folders parameters.
			 * @param {BPMSoft.Collection} childFolders The collection of child folders.
			 */
			...
			getRootActivityFolderSelect: function() {
				var rootActivityFolderSelect = this.Ext.create("BPMSoft.EntitySchemaQuery", {
					rootSchemaName: "ActivityFolder",
					rowCount: 1
				});
				rootActivityFolderSelect.addColumn("Id");
				rootActivityFolderSelect.filters.addItem(
					rootActivityFolderSelect.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL,
			 ...
			 * @param {BPMSoft.Collection} params.selectedFolders Collection of selected folders.
			 ...
			 * @param {BPMSoft.Collection} params.oldFolders Collection of old folders.
			 ...
			 * @return {BPMSoft.InsertQuery} InsertQuery for ActivityFolder.
			 */
			getActivityFolderInsert: function(activityFolderId, name, parentId) {
				var insert = this.Ext.create("BPMSoft.InsertQuery", {
					rootSchemaName: "ActivityFolder"
				});
				insert.setParameterValue("Id", activityFolderId, this.BPMSoft.DataValueType.GUID);
				insert.setParameterValue("Parent", parentId, this.BPMSoft.DataValueType.GUID);
				insert.setParameterValue("FolderType", ConfigurationConstants.Folder.Type.SubEmail,
					BPMSoft.DataValueType.GUID);
				insert.setParameterValue("Name", name, this.BPMSoft.DataValueType.TEXT);
				return insert;
			},
			 ...
			 * @virtual
			 */
			getFolderForDeleteBySchemaName: this.BPMSoft.emptyFn,
			...
			getMailboxRootParameters: function(folderParameter, callback, scope) {
				var rootParametersBQ = this.Ext.create("BPMSoft.BatchQuery");
			 ...
			 * @return {BPMSoft.Collection|Array}
			 */
			...
			getUpdateQueryMailbox: function() {
				...
				update.setParameterValue("ExchangeAutoSynchronization",
					this.get("ExchangeAutoSynchronization"), this.BPMSoft.DataValueType.BOOLEAN);
				return update;
			},
			...
			setEsqFilters: function(esq) {
				...
				var filterMailboxSyncSettings = esq.createColumnFilterWithParameter(
					this.BPMSoft.ComparisonType.EQUAL, "MailboxSyncSettings", mailboxSyncSettingsId);
			...
			getDynamicGroups: function(config, callback, scope) {
				...
				select.getEntityCollection(function(response) {
					var collection = response.collection;
					var resultItems = [];
					var rootId = this.BPMSoft.utils.generateGUID();
					resultItems.push({
						Id: rootId,
						Name: this.get("MailboxName"),
						ParentId: "",
