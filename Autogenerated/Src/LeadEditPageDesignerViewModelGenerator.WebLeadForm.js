define('LeadEditPageDesignerViewModelGenerator', ['ext-base', 'BPMSoft', 'GeneratedWebForm',
	...
	function(Ext, BPMSoft, GeneratedWebForm, resources, GeneratedWebFormUtilities, viewGenerator,
		...
		function generate(viewModelSchema, info) {
				...
				select.getEntityCollection(function(result) {
					if (result.success) {
						...
						if (result.collection.getCount() > 0) {
							...
							item.columns.FormFields.type = BPMSoft.ViewModelColumnType.ENTITY_COLUMN;
							item.set('FormFields', LeadColumns, BPMSoft.DataValueType.TEXT);
			...
			fullViewModelSchema.methods.getNewColumnConfig = function(name, columnPath, dataType,
				...
				return {
					...
					EditStructureContainerId: (!!Id) ? Id : BPMSoft.generateGUID(),
			...
			fullViewModelSchema.methods.addColumn = function(columnName) {
				...
				var columnConfig = this.getNewColumnConfig(columnName, columnName,
					BPMSoft.DataValueType.TEXT, column.caption);
				this.addLeadColumn(BPMSoft.deepClone(columnConfig));
