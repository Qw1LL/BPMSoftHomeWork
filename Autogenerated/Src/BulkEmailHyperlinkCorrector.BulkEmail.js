define("BulkEmailHyperlinkCorrector",
	function() {
		Ext.define("BPMSoft.BulkEmailHyperlinkCorrector", {
			/**
			...
			_getMaxBpmTrackIdFromContent: function (content) {
				...
				BPMSoft.each(hrefs, function (href) {
			...
			_getBpmTrackIdFromLink: function (url) {
				var idText = BPMSoft.getQueryParameter(url, this._trackIdParameterName);
			...
			_setBpmTrackIdToLink: function(url, bpmTrackId, callback) {
				...
				var matchUrl = BPMSoft.isUrl(lowerCaseUrl);
				if (matchUrl) {
					bpmTrackId++;
			...
			extractHyperlinks: function (html) {
				...
				do {
					...
					if (regexResult && regexResult.length === 6) {
						...
						if (url) {
							let urlWithoutAmpersandSanitization = url.replaceAll('&amp;', '&');
							allLinks.push({
								"Url": urlWithoutAmpersandSanitization,
								"Caption": caption || urlWithoutAmpersandSanitization
							});
						}
					}
				} while (regexResult);
			...
			getDistinctHyperlinks: function (allLinks) {
				var result = [];
				var map = new Map();
				BPMSoft.each(allLinks, function(item) {
					if (!map.has(item.Url)){
		...
		return Ext.create("BPMSoft.BulkEmailHyperlinkCorrector");
	}
);
