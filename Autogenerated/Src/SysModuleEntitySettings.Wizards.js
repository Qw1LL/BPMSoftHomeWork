define("SysModuleEntitySettings", ["SysModuleEntitySettingsResources", "ConfigurationGridUtilities",
	...
	return {
		...
		methods: {
			 ...
			 * @protected
			 * @param {BPMSoft.SysModuleEditManagerItem} managerItem SysModuleEditManagerItem.
			 ...
			 * Generate caption for SysModuleEditManagerItem.
			 * @protected
			 * @param {BPMSoft.BaseEntitySchema} entitySchema Model.
			 * @param {Object} [typeValue] Type.
			 * @return {String} Caption.
			 */
			...
			actualizeSysModuleEditCaption: function() {
				...
				BPMSoft.each(gridData.getItems(), function(item) {
					...
					var managerItem = BPMSoft.SysModuleEditManager.getItem(id);
					var caption = this.generateSysModuleEditManagerItemCaption(typeValue);
					managerItem.setPageCaption(caption);
					managerItem.setActionKindCaption(caption);
				}, this);
			...
			getCellControlsConfig: function(entitySchemaColumn) {
				...
				if (entitySchemaColumn.controlConfig) {
					config.controlConfig = BPMSoft.deepClone(entitySchemaColumn.controlConfig);
				}
				return config;
			},
			...
			init: function(callback, scope) {
				...
				this.set("GridData", Ext.create("BPMSoft.BaseViewModelCollection"));
				this.set("TypeColumnList", Ext.create("BPMSoft.Collection"));
			...
			getReferenceEntitySchemaConfigChain: function(next) {
				...
				var name = BPMSoft.EntitySchemaManager.getUniqueNameByTemplate(referenceEntitySchemaNameMask);
				var config = {
					"name": name,
			...
			createReferenceEntitySchemaChain: function(next, config) {
				var name = Ext.String.format("{0}{1}",
					BPMSoft.EntitySchemaManager.getSchemaNamePrefix(), config.name);
				var caption = config.caption;
				var packageUId = config.packageUId;
				var newEntityUId = BPMSoft.generateGUID();
				var newSchema = BPMSoft.EntitySchemaManager.createSchema({
					"uId": newEntityUId,
					"name": name,
				...
				var baseLookupUId = BPMSoft.DesignTimeEnums.BaseSchemaUId.BASE_LOOKUP;
				newSchema.setParent(baseLookupUId, function() {
					var entitySchema = BPMSoft.EntitySchemaManager.addSchema(newSchema);
			 ...
			 * @protected
			 * @param {Function} next Callback function.
			 * @param {BPMSoft.EntitySchemaManagerItem} referenceEntitySchemaManagerItem New lookup managerItem.
			 */
			defineReferenceEntitySchemaChain: function(next, referenceEntitySchemaManagerItem) {
				referenceEntitySchemaManagerItem.getInstance(function(instance) {
					...
					var className = Ext.String.format("BPMSoft.data.models.{0}", schemaName);
					var alternateClassName = Ext.String.format("BPMSoft.{0}", schemaName);
					...
					define(schemaName, ["BPMSoft"], function(BPMSoft) {
			 ...
			 * @protected
			 * @param {BPMSoft.EntitySchema} instance Instance of EntitySchemaManagerItem.
			...
			getReferenceEntitySchemaColumnList: function(instance) {
				...
				if (instance && instance.columns) {
					instance.columns.each(function(column) {
						...
						var isLookup = BPMSoft.isLookupDataValueType(dataValueType);
						...
						if (isLookup && columnReferenceSchemaUId) {
							...
							var managerItem = BPMSoft.EntitySchemaManager.getItem(columnReferenceSchemaUId);
			...
			createReferenceEntityColumnChain: function(next, referenceEntitySchemaManagerItem) {
				var newColumn = Ext.create("BPMSoft.EntitySchemaColumn");
				...
				newColumn.name = Ext.String.format("{0}{1}",
					BPMSoft.EntitySchemaManager.getSchemaNamePrefix(), this.get("DefaultTypeColumnName"));
				newColumnCaption.setValue(caption);
				newColumn.dataValueType = BPMSoft.DataValueType.LOOKUP;
			...
			setEntitySchemaManagerItemModificateStatis: function() {
				var moduleEntitySchema = this.getModuleEntitySchema();
				var entitySchemaManagerItem = BPMSoft.EntitySchemaManager.getItem(moduleEntitySchema.uId);
				if (!entitySchemaManagerItem.isModified()) {
					entitySchemaManagerItem.setStatus(BPMSoft.ModificationStatus.UPDATED);
				}
			},
			...
			getIsNewTypeEntitySchema: function() {
				...
				if (typeColumn && typeColumn.column) {
					...
					if (BPMSoft.EntitySchemaManager.contains(referenceSchemaUId)) {
						var referenceSchema = BPMSoft.EntitySchemaManager.getItem(referenceSchemaUId);
						isNew = referenceSchema.getStatus() === BPMSoft.ModificationStatus.NEW;
					}
				}
				return isNew;
			},
			...
			addTypeRecordInDataManager: function(displayValue) {
				...
				if (rowViewModel) {
					var dataManagerItem = Ext.create("BPMSoft.DataManagerItem", {"viewModel": rowViewModel});
			...
			getTypeRecordsFromDataManager: function(filterValue, callback, scope) {
				...
				BPMSoft.DataManager.select(config, function(dataCollection) {
			 ...
			 * @protected
			 * @return {BPMSoft.EntitySchemaManagerItem} Reference entitySchemaManagerItem.
			 */
			 ...
			 * @param {BPMSoft.Collection} pageCollection Collection of active edit pages.
			 */
			...
			showSelectSinglePageModalBox: function(getInfoHandler, pushResponseHandler) {
				var modalBoxId = this.getSelectSinglePageModalBoxId(BPMSoft.generateGUID());
			 ...
			 * @protected
			 * @param {BPMSoft.Collection} pageCollection Collection of edit page manager items.
			 * @param {BPMSoft.Collection} typeCollection Collection of type values.
			 * @return {BPMSoft.BaseViewModelCollection} Collection of edit page view model.
			 */
			generatePageViewModelCollection: function(pageCollection, typeCollection) {
				var collection = Ext.create("BPMSoft.BaseViewModelCollection");
				var typeMap = {};
				BPMSoft.each(typeCollection.getItems(), function(item) {
					typeMap[item.get("value")] = item.get("displayValue");
				}, this);
				this.set("TypeListMap", typeMap);
				BPMSoft.each(pageCollection.getItems(), function(sysModuleEditManagerItem) {
			 ...
			 * @protected
			 * @param {BPMSoft.SysModuleEditManagerItem} sysModuleEditManagerItem Manager item.
			 * @return {BPMSoft.BaseViewModel} Collection of edit page view model.
			 */
			 ...
			 * @protected
			 * @param {BPMSoft.BaseViewModel} viewModel Page view model.
			 */
			 ...
			 * Get list of type.
			 * @protected
			 * @param {BPMSoft.BaseViewModelCollection} pageCollection pageCollection parameter.
			 * @param {Function} callback Callback method.
			 * @param {Object} scope Callback method context.
			 */
			getTypeList: function(pageCollection, callback, scope) {
				...
				if (typeColumn && typeColumn.column) {
					if (this.getIsNewTypeEntitySchema()) {
						...
						if (pageCollection) {
							...
							if (!items.isEmpty()) {
								...
								var filterById = BPMSoft.createColumnInFilterWithParameters("Id", itemIds);
					...
					var emptyCollection = Ext.create("BPMSoft.BaseViewModelCollection");
			...
			getGridRowViewModelClassName: function() {
				return "BPMSoft.configuration.SysModuleEntitySettingsItemModel";
			},
			...
			generateGridEntitySchema: function() {
				...
				this.entitySchema = Ext.create("BPMSoft.BaseEntitySchema", {
					"columns": columns
				});
			...
			validateModel: function() {
				...
				BPMSoft.each(validateList, function(validateFn) {
			...
			onActualizeSysModuleEntitySettings: function(callback) {
				...
				var managerItem = BPMSoft.EntitySchemaManager.getItem(referenceSchemaUId);
			 ...
			 * @private
			 * @param {BPMSoft.manager.EntitySchemaManagerItem} managerItem EntitySchema manager item.
			 */
			_removeEntitySchemaDataCollection: function(managerItem) {
				var entitySchemaDataCollection = BPMSoft.DataManager.getEntitySchemaData(managerItem.name);
			...
			_removeLookupManagerItem: function(managerItem) {
				var lookupManagerItem = BPMSoft.LookupManager.findItemBySysEntitySchemaUId(managerItem.uId);
			 ...
			 * @private
			 * @param {BPMSoft.manager.EntitySchemaColumn} column EntitySchemaColumn to remove.
			 */
			...
			getEntitySchemaLookupColumnCollection: function(filter, callback, scope) {
				...
				var collection = new BPMSoft.Collection();
				var generalUsageType = BPMSoft.EntitySchemaColumnUsageType.General;
				BPMSoft.eachAsync(columns, function(column, next) {
			...
			isLookupSchema: function(referenceSchemaUId, callback, scope) {
				var item = BPMSoft.EntitySchemaManager.findItem(referenceSchemaUId);
			...
			isBaseLookupSchema: function(referenceSchemaUId) {
				var managerItem = BPMSoft.EntitySchemaManager.findItem(referenceSchemaUId);
				var response = false;
				if (managerItem) {
					var hierarchyInfo = managerItem.getHierarchyInfo() || [];
					BPMSoft.each(hierarchyInfo, function(item) {
						if (item && item.uId === BPMSoft.DesignTimeEnums.BaseSchemaUId.BASE_LOOKUP) {
							response = true;
							return false;
						}
					}, this);
			...
			afterTypeColumnChange: function(typeColumn, singlePageId) {
				if (typeColumn && typeColumn.column) {
					...
					var referenceSchemaUId = column.referenceSchemaUId;
					gridData.clear();
					column.referenceItem = BPMSoft.EntitySchemaManager.findItem(referenceSchemaUId);
			...
			requireReferenceEntitySchema: function(callback, scope) {
				...
				BPMSoft.chain(
					...
					function(next) {
						BPMSoft.require([entitySchemaName], next);
					},
			 ...
			 * @protected
			 * @param {BPMSoft.EntitySchemaColumn} column Column.
			 */
			...
			/**
			 * @inheritdoc BPMSoft.GridUtilities#deleteRecords
			 * @override
			 */
			...
			/**
			 * @inheritdoc BPMSoft.GridUtilities#onDeleteAccept
			 * @override
			 */
			 ...
			 * @protected
			 * @param {BPMSoft.Collection} removePageList Collection of removed pages.
			 */
			removePage: function(removePageList) {
				BPMSoft.each(removePageList, function(pageId) {
					if (BPMSoft.SysModuleEditManager.contains(pageId)) {
						var managerItem = BPMSoft.SysModuleEditManager.getItem(pageId);
						managerItem.remove();
					}
				}, this);
			...
			_registerFileDetail: function(fileSchemaName, callback, scope) {
				...
				var detail = BPMSoft.DetailManager.getItems().findByPath("entitySchemaUId", fileSchemaUId);
				if (detail) {
					callback.call(scope);
				} else {
			...
			_createFilesDetail: function(page, config, callback, scope) {
				...
				var entitySchemaItems = BPMSoft.EntitySchemaManager.getItems();
			...
			createPageSchema: function(schemaConfig, callback, scope) {
				...
				BPMSoft.chain(
					function(next) {
						...
						config.schemaName = BPMSoft.ClientUnitSchemaManager
			...
			addPrefixIfNeed: function(schemaName) {
				return BPMSoft.ApplicationStructureWizardUtils.getEntityFullName(schemaName);
			},
			...
			getClientUnitSchemaBaseConfig: function(schemaConfig) {
				var mvmSchema = BPMSoft.SchemaType.EDIT_VIEW_MODEL_SCHEMA;
				var entitySchema = schemaConfig.entitySchema;
				return {
					"bodyTemplate": BPMSoft.ClientUnitSchemaBodyTemplate[mvmSchema],
			...
			createClientUnitSchema: function(config, isExtendedParent) {
				...
				var caption = {};
				caption[BPMSoft.SysValue.CURRENT_USER_CULTURE.displayValue] = config.schemaCaption;
				var clientUnitSchema = BPMSoft.ClientUnitSchemaManager.createSchema({
					"uId": BPMSoft.generateGUID(),
					"name": schemaName,
					...
					"parentSchemaUId": BPMSoft.DesignTimeEnums.BaseSchemaUId.BASE_MODULE_PAGE
				});
				if (!isExtendedParent) {
