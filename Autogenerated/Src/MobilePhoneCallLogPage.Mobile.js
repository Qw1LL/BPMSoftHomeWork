...
Ext.define("MobilePhoneCallLogPage.View", {
	...
	initialize: function() {
		...
		navigationPanel.setTitle(BPMSoft.LocalizableStrings.MobilePhoneCallLogPageTitle);
		var detailedResult = Ext.getCmp("MobilePhoneCallLogPageDetailedResult");
		detailedResult.setPlaceHolder(BPMSoft.LocalizableStrings.MobilePhoneCallLogPageDetailResultPlaceholder);
		...
		planActivityToggle.setLabel(BPMSoft.LocalizableStrings.MobilePhoneCallLogPageArrangeNextActivity);
		var cancelButton = Ext.getCmp("MobilePhoneCallLogPage_cancelButton");
		cancelButton.setText(BPMSoft.LocalizableStrings.MobilePhoneCallLogPageCancelButtonCaption);
		var saveButton = Ext.getCmp("MobilePhoneCallLogPage_saveButton");
		saveButton.setText(BPMSoft.LocalizableStrings.MobilePhoneCallLogPageSaveButtonCaption);
	},
...
Ext.define("MobilePhoneCallLogPage.Controller", {
	...
	getCallDuration: function() {
		...
		var minutesText = Ext.String.format(formatTpl,
			min, BPMSoft.LocalizableStrings.MobilePhoneCallLogPageMinutesText);
		return minutesText;
	},
	...
	updateCallInfo: function() {
		...
		var dateFormat = BPMSoft.Date.getDateTimeFormat();
		var startDate = Ext.Date.format(this.callStartDate, dateFormat);
		var info = (this.callFinishDate) ?
			Ext.String.format("{0} {1} {2}", startDate,
				BPMSoft.LocalizableStrings.MobilePhoneCallLogPageCallInfoSeparator, this.getCallDuration()) :
					startDate;
...
/**
	...
	initializeView: function() {
		this.callParent(arguments);
		var historyItem = BPMSoft.PageHistory.getItemByPosition(BPMSoft.PagePositions.Current);
		...
		var outgoingCallText = BPMSoft.LocalizableStrings.MobilePhoneCallLogPageOutgoingCall;
		userInfoLabel.setHtml(outgoingCallText + ": " + BPMSoft.CurrentUserInfo.contactName);
		...
		BPMSoft.DocumentEventManager.subscribe("resume", onAppChangeStateBinded, false);
		BPMSoft.DocumentEventManager.subscribe("pause", onAppChangeStateBinded, false);
	...
	onSaveButtonButtonTap: function() {
		var modelConfig = BPMSoft.ApplicationConfig.getModelConfig("Activity");
				...
				this.createCallActivity(function() {
					...
					if (toArrange) {
						setTimeout(function() {
							...
							BPMSoft.util.openEditPage("Activity", config);
						}.bind(this), 500);
					}
				}.bind(this));
			},
	...
	createCallActivity: function(callback) {
		var title = BPMSoft.LocalizableStrings.MobilePhoneCallLogPageActivityTitle;
		...
		model.createWithDefaultValues(function(activityRecord) {
			...
			activityRecord.set("ActivityCategory", BPMSoft.Configuration.ActivityCategory.Call);
			activityRecord.set("Status", BPMSoft.Configuration.ActivityStatus.Finished);
			activityRecord.set("Result", BPMSoft.Configuration.ActivityResult.Call.Completed);
			activityRecord.set("DetailedResult", detailedResultText);
			activityRecord.set("ShowInScheduler", false);
			activityRecord.set("CallDirection", BPMSoft.CallDirectionId.Outgoing);
