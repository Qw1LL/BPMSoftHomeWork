define('SummaryModule', ['ext-base', 'BPMSoft', 'sandbox', 'SummaryView', 'SummaryViewModel',
	'SummaryModuleResources', 'performancecountermanager'],
	function(Ext, BPMSoft, sandbox, SummaryView, SummaryViewModel, resources, performanceCounterManager) {
				...
				switch (profile[i][1]) {
					case 'COUNT':
						select.addAggregationSchemaColumn(profile[i][0],
							BPMSoft.AggregationType.COUNT, profile[i][0] + profile[i][1]);
						break;
					case 'SUM':
						select.addAggregationSchemaColumn(profile[i][0],
							BPMSoft.AggregationType.SUM, profile[i][0] + profile[i][1]);
						break;
					case 'AVG':
						select.addAggregationSchemaColumn(profile[i][0],
							BPMSoft.AggregationType.AVG, profile[i][0] + profile[i][1]);
						break;
					case 'MIN':
						select.addAggregationSchemaColumn(profile[i][0],
							BPMSoft.AggregationType.MIN, profile[i][0] + profile[i][1]);
						break;
					case 'MAX':
						select.addAggregationSchemaColumn(profile[i][0],
							BPMSoft.AggregationType.MAX, profile[i][0] + profile[i][1]);
						break;
					default :
						break;
				}
			...
			if (profile && profile.length > 0) {
				getSummaries(profile, filters,
					function(result) {
						...
						BPMSoft.each(entities[0].values, function(value) {
							var count = itemsList.getCount();
							var key = 'item_' + count;
							var viewModel = Ext.create('BPMSoft.BaseViewModel', SummaryViewModel.generateItem());
							var view = Ext.create("BPMSoft.Container", SummaryView.generateItem(container, key));
							viewModel.key = key;
							...
							value = BPMSoft.utils.getTypedStringValue(value, columnDataValueType);
		...
		function init() {
			this.itemsCollection = new BPMSoft.Collection();
			this.firstLoad = true;
		}
		...
		function render(renderTo) {
			...
			require(['profile!' + profileKey], function(loadedProfile) {
				...
				var view = Ext.create("BPMSoft.Container", SummaryView.generate(container));
				var summariesContainer = Ext.get('summariesContainer');
				var viewModel = Ext.create('BPMSoft.BaseViewModel', SummaryViewModel.generate());
