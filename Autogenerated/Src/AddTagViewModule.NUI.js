define('AddTagViewModule', ['ext-base', 'BPMSoft', 'AddTagViewModuleResources'],
	function(Ext, BPMSoft, resources) {
		...
		function generate(config) {
			return {
				...
				values: {
					tags: new BPMSoft.Collection(),
					filters: new BPMSoft.Collection(),
					tagsViews: new BPMSoft.Collection(),
					currentTagName: null,
					tagColumnList: new BPMSoft.Collection(),
					editVisible: false,
                    config: config || {}
				},
				methods: {
        ...
        function loadItemsFromState() {
            ...
            if (!Ext.isEmpty(filterState)) {
                BPMSoft.each(filterState, function(item) {
		...
		function showTagEdit(valueEditConfig, afterDestroy, renderIndex) {
			...
			if (addConditionView) {
				...
				addConditionView = this.Ext.create("BPMSoft.Container", addConditionViewConfig);
        ...
        function loadItemsFromRecord(recordId) {
            ...
            tagNameColumn.orderDirection = BPMSoft.OrderDirection.ASC;
		...
		function insertToTagAndSave(existingItems, allItems, recordId) {
            ...
            BPMSoft.each(allItems, function(item) {
                var insertTag = Ext.create('BPMSoft.InsertQuery', {
                    rootSchemaName: this.getTagSchemaName()
                });
                insertTag.setParameterValue('Name', item, BPMSoft.DataValueType.TEXT);
                bq.add(insertTag);
            }, this);

            if (bq.queries.length) {
                bq.execute(function(response) {
                    var inserItemsBQ = Ext.create('BPMSoft.BatchQuery');
                    BPMSoft.each(response.queryResults, function(item, index) {
		...
		function getInsertToTagDecouplingQuery(recordId, tagValue) {
			...
			insert.setParameterValue(this.getRootTagSchemaName(), recordId, BPMSoft.DataValueType.GUID);
			insert.setParameterValue('Tag', tagValue, BPMSoft.DataValueType.GUID);
			return insert;
		}
		...
		function saveTags(recordId) {
			...
			deleteQuery.filters.addItem(deleteQuery.createColumnFilterWithParameter(
				BPMSoft.ComparisonType.EQUAL, this.getRootTagSchemaName(), this.recordId));
            bq.add(deleteQuery);
            ...
            update.setParameterValue('Keywords', tagCollection.join(', '), BPMSoft.DataValueType.TEXT);
		...
		function updateFilterItems() {
			...
			var filterItems = new BPMSoft.Collection();
			var tagsCollectionLength = tags.collection.length;
			if (tagsCollectionLength > 0) {
				BPMSoft.each(tags.getItems(), function(tag) {
					...
					select.addColumn('Id');
					select.addColumn('Name');
					select.filters.addItem(select.createColumnFilterWithParameter(BPMSoft.ComparisonType.CONTAIN,
                        'Name', tagName));
					select.getEntityCollection(function(response) {
                        if (response.success) {
				...
				this.set('filters', new BPMSoft.Collection());
				this.filterChanged();
			}
		}
		...
		function getFilters() {
			...
			if (filters && filters.collection.length > 0) {
				...
				filterCollection.add('tagsFilter', BPMSoft.createColumnInFilterWithParameters(
