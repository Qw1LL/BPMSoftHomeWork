define("MappingMenuItemConfigBuilder", ["SysSchemaUIdEnums", "MappingEnums"],
		function() {
	return Ext.define("BPMSoft.configuration.MappingMenuItemConfigBuilder", {
		extend: "BPMSoft.ObjectBuilder",
		alternateClassName: "BPMSoft.MappingMenuItemConfigBuilder",
		...
		getIconConfig: function(icon) {
			if (icon) {
				return {
					...
					url: BPMSoft.ImageUrlBuilder.getUrl(icon)
				};
			}
		},

		/**
		 * @inheritdoc BPMSoft.ObjectBuilder#getResult
		 * @overridden
		 */
		getResult: function() {
			this.end();
