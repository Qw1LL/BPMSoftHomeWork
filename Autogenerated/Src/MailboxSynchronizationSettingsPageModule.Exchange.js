define("MailboxSynchronizationSettingsPageModule", [
		"BPMSoft", "ext-base", "sandbox", "ConfigurationConstants",
	...
	function(BPMSoft, Ext, sandbox, ConfigurationConstants, LookupUtilities, resources, RightUtilities, MaskHelper,
			ExchangeNUIConstants, EmailHelper) {
		/**
		 * @class BPMSoft.configuration.MailboxSynchronizationSettingsPageViewModel
		 * Mailbox synchronization settings page viewmodel class.
		 */
		Ext.define("BPMSoft.configuration.MailboxSynchronizationSettingsPageViewModel", {
			extend: "BPMSoft.BaseViewModel",
			alternateClassName: "BPMSoft.MailboxSynchronizationSettingsPageViewModel",
			ajaxProvider: BPMSoft.AjaxProvider,
			...
			onOkBtnClick: function() {
				...
				if (Ext.isEmpty(invalidMessage)) {
					...
					BPMSoft.utils.showInformation(invalidMessage);
				}
			},
			...
			createBPMCRMEmailFolder: function() {
				...
				this.requestService(data, "CreateBPMCRMFolder", function(request, success) {
					if (success) {
						sandbox.publish("PushHistoryState", {hash: "MailboxSynchronizationSettingsModule"});
					} else {
						BPMSoft.utils.showInformation(resources.localizableStrings.ImapExceptionMessageEx,
			...
			AddEmailToContactCommunications: function(callback, innerCallback) {
				BPMSoft.utils.showConfirmation(resources.localizableStrings.AddContactEmailMessageEx,
					function(returnCode) {
			...
			saveSettings: function() {
				...
				if (settingsId) {
					...
					update.setParameterValue("EnableMailSynhronization",
						this.get("enableMailSynchronization"), BPMSoft.DataValueType.BOOLEAN);
					...
					update.setParameterValue("SysAdminUnit",
						BPMSoft.SysValue.CURRENT_USER.value, BPMSoft.DataValueType.GUID);
					...
					update.setParameterValue("MailSyncPeriod",
						this.get("mailSyncPeriodValue").value, BPMSoft.DataValueType.GUID);
					update.setParameterValue("ExchangeAutoSynchronization",
						this.get("exchangeAutoSynchronization"), BPMSoft.DataValueType.BOOLEAN);
					update.setParameterValue("ExchangeSyncInterval",
						this.get("exchangeSyncInterval"), BPMSoft.DataValueType.INTEGER);
					if (this.get("emailsLastSyncDate") !== undefined) {
						update.setParameterValue("LastSyncDate",
							this.get("emailsLastSyncDate"), BPMSoft.DataValueType.DATE);
					}
			...
			processContactSyncSettings: function(action, callback) {
				...
				switch (action) {
					...
					case "update":
						...
						break;
					case "delete":
						query = Ext.create("BPMSoft.DeleteQuery", {
							rootSchemaName: "ContactSyncSettings"
						});
						query.filters.add("IdFilter", idFilter);
						break;
				...
				select.getEntityCollection(function(response) {
					if (response.success) {
						...
						if (collection.length === 0) {
							...
							BPMSoft.utils.showInformation(resources.localizableStrings.settingsExistsEx);
						}
					}
				}, scope);
			},
			...
			onPrepareMailSyncPeriodList: function(filter, list) {
				...
				function fillList(list, collection) {
					...
					if (collection && collection.getCount() > 0) {
						BPMSoft.each(collection.getItems(), function(item) {
							obj[item.get("value")] = {
			...
			initializeMailSyncPeriodCollection: function(callback) {
				...
				positionColumn.orderDirection = BPMSoft.core.enums.OrderDirection.ASC;
			...
			/**
			 *
			 * @param {BPMSoft.InsertQuery|BPMSoft.UpdateQuery} query
			 */
			fillActivitySyncSettingsQuery: function(query) {
				...
				query.setParameterValue("ImportActivitiesFrom",
					this.get("importActivitiesFrom"), BPMSoft.DataValueType.DATE);
				query.setParameterValue("AppointmentLastSyncDate", null, BPMSoft.DataValueType.DATE);
				query.setParameterValue("TaskLastSyncDate", null, BPMSoft.DataValueType.DATE);
				...
				query.setParameterValue("ImportAppointmentsCalendars",
					this.get("importAppointmentsCalendars"), BPMSoft.DataValueType.TEXT);
				...
				query.setParameterValue("ExportActivitiesFromScheduler",
					this.get("exportActivitiesFromSheduler"), BPMSoft.DataValueType.BOOLEAN);
				query.setParameterValue("ExportActivitiesFromGroups",
					this.get("exportActivitiesFromGroups"), BPMSoft.DataValueType.BOOLEAN);
			...
			getInsertMailboxSyncSettings: function(scope) {
				...
				insert.setParameterValue("AutomaticallyAddNewEmails",
					scope.get("automaticallyAddNewEmails"), BPMSoft.DataValueType.BOOLEAN);
				...
				insert.setParameterValue("EmailsCyclicallyAddingInterval",
					scope.get("emailsCyclicallyAddingInterval"), BPMSoft.DataValueType.INTEGER);
				...
				insert.setParameterValue("IsAnonymousAuthentication",
					scope.get("isAnonymousAuthentication"), BPMSoft.DataValueType.BOOLEAN);
				...
				insert.setParameterValue("ExchangeAutoSynchronization",
					scope.get("exchangeAutoSynchronization"), BPMSoft.DataValueType.BOOLEAN);
			...
			getMailboxFolderCorrespondenceInsert: function(params) {
				...
				insert.setParameterValue("Mailbox", params.mailboxId, BPMSoft.DataValueType.GUID);
				insert.setParameterValue("ActivityFolder", params.activityFolderId,
					BPMSoft.DataValueType.GUID);
				insert.setParameterValue("FolderPath", params.path, BPMSoft.DataValueType.TEXT);
				return insert;
			},
			...
			requestService: function(data, methodName, callback, timeout) {
				...
				var requestUrl = BPMSoft.workspaceBaseUrl + "/rest/MailboxSettingsService/" + methodName;
				provider.request({
					url: requestUrl,
			...
			init: function(callback, scope) {
				this.set("mailSyncPeriodList", Ext.create("BPMSoft.Collection"));
				this.set("mailServerList", Ext.create("BPMSoft.Collection"));
				this.set("mailServerCollection", {});
				this.set("loadEmailsFromDate", new Date());
				this.set("bindAccountList", Ext.create("BPMSoft.Collection"));
		...
		/**
		 * @class BPMSoft.configuration.MailboxSynchronizationSettingsPageModule
		 * Mailbox synchronization settings page module class.
		 */
		Ext.define("BPMSoft.configuration.MailboxSynchronizationSettingsPageModule", {
			alternateClassName: "BPMSoft.MailboxSynchronizationSettingsPageModule",
			 ...
			 * @return {BPMSoft.MailboxSynchronizationSettingsPageViewModel} instance.
			 */
			getViewModel: function() {
				var model = Ext.create("BPMSoft.MailboxSynchronizationSettingsPageViewModel", {
					values: {
		...
		return BPMSoft.MailboxSynchronizationSettingsPageModule;
	});
