			...
			methods: {
				...
				_getExpressionTypes: function() {
					...
					BPMSoft.each(BPMSoft.TimerExpressionTypes, function(value, name) {
						if (value !== BPMSoft.TimerExpressionTypes.Empty &&
								value !== BPMSoft.TimerExpressionTypes.MinuteHour &&
								value !== BPMSoft.TimerExpressionTypes.Year) {
							result[value] = {
								value: value,
				...
				* */
				_isExpressionTypeValid: function() {
					...
					return typeNotEmpty && expressionType.value !== BPMSoft.TimerExpressionTypes.Empty &&
						expressionType.value !== BPMSoft.TimerExpressionTypes.MinuteHour &&
						expressionType.value !== BPMSoft.TimerExpressionTypes.Year;
				},
				...
				_initCronExpression: function(element) {
					...
					if (!BPMSoft.CronExpression.validate(value).isValid) {
						value = null;
					}
				...
				_saveCronExpression: function(element) {
					if (this._isSectionActive(BPMSoft.TimerExpressionTypes.SingleRun) ||
							this._isSectionActive(BPMSoft.TimerExpressionTypes.Day)) {
				...
				_cronExpressionValidator: function(value) {
					...
					if (sections.some(this._isSectionActive, this)) {
						var cron = BPMSoft.CronExpression.from(value);
				...
				_periodStartTimeValidator: function(value) {
					...
					if (this.getIsStartEndTimeGroupActive() && this._getRunTimeIsPeriod() && Ext.isEmpty(value)) {
						message = BPMSoft.Resources.BaseViewModel.columnRequiredValidationMessage;
					}
					return {invalidMessage: message};
				},
				...
				_endDateTimeValidator: function(value) {
					...
					if (this.getIsStartEndTimeGroupActive() && this.$IsEndDateTimeActive) {
						...
						if (!Ext.isEmpty(startDateTime) && !Ext.isEmpty(value)) {
							...
							var clearEnd = BPMSoft.clearSeconds(value);
				...
				_singleRunStartDateTimeValidator: function(value) {
					var message;
					if (this._isSectionActive(BPMSoft.TimerExpressionTypes.SingleRun) && Ext.isEmpty(value)) {
						message = BPMSoft.Resources.BaseViewModel.columnRequiredValidationMessage;
					}
					return { invalidMessage: message };
				},
				 ...
				 * @private
				 * @param {Object} filter Filter object.
				 * @param {BPMSoft.Collection} list Time zone list.
				 */
