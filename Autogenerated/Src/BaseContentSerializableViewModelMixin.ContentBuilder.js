define("BaseContentSerializableViewModelMixin", [], function() {
	Ext.define("BPMSoft.configuration.mixins.BaseContentSerializableViewModelMixin", {
		extend: "BPMSoft.BaseViewModel",
		alternateClassName: "BPMSoft.BaseContentSerializableViewModelMixin",
		 ...
		 * @protected
		 */
		serializableChildrenCollectionSource: BPMSoft.emptyString,
		...
		_sliceViewModel: function(serializedParameters) {
			var result = {};
			BPMSoft.each(serializedParameters, function(parameterName) {
		...
		_serializeChildViewModels: function() {
			if (this.serializableChildrenCollectionSource === BPMSoft.emptyString) {
				return undefined;
			}
			...
			if(childViewModels) {
				childViewModels.each(function (itemViewModel) {
					if (itemViewModel instanceof BPMSoft.BaseContentItemViewModel) {
		...
		prepareValues: function(config) {
			...
			values.Id = config.Id || BPMSoft.generateGUID();
			return values;
		},
		...
		removeChildItem: function(itemId) {
			var itemToDelete = !BPMSoft.isEmpty(this.$Items) && this.$Items.find(itemId);
			if (itemToDelete) {
	...
	return BPMSoft.BaseContentSerializableViewModelMixin;
});
