define("ChartDrillDownProvider", [
	 ...
	 */
	Ext.define("BPMSoft.configuration.ChartDrillDownProvider", {
		extend: "BPMSoft.BaseObject",
		alternateClassName: "BPMSoft.ChartDrillDownProvider",

		mixins: {
			EntityStructureHelper: "BPMSoft.EntityStructureHelperMixin",
			ChartDrillDownProviderUtils: "BPMSoft.ChartDrillDownProviderUtils",
		...
		constructor: function() {
			...
			this.chartModuleHelper = Ext.create("BPMSoft.ChartModuleHelper");
			this.addEvents(
					"historyChanged"
			);
		},
		...
		*/
		_trySetColumnInfo: function(column) {
			...
			let entitySchemaColumns = BPMSoft[entitySchemaName] && BPMSoft[entitySchemaName].columns;
		...
		*/
		_getColumnInfo: function(columnPath, entitySchemaColumns) {
			...
			BPMSoft.each(columnPaths, function(path) {
				...
				entitySchemaColumns = BPMSoft[referenceSchemaName] && BPMSoft[referenceSchemaName].columns;
			}, this);
		 ...
		 * @return {BPMSoft.BaseEntitySchema} Returns entity schema by name.
		 */
		 ...
		 * @param {String} entitySchemaName Name of entity schema.
		 * @param {String} columnPath Column path.
		 * @return {BPMSoft.DataValueType} Returns data value type of entity schema column.
		 */
		...
		getCategoryItemXAxisColumnPath: function(categoryItem) {
			const rowConfig = BPMSoft.deepClone(categoryItem.rowConfig);
		...
		addItemFilter: function(categoryItem) {
			...
			if (!BPMSoft.isDateDataValueType(dataValueType)) {
				...
				BPMSoft.each(categoryItems, function(category) {
		...
		initState: function(rawState) {
			...
			BPMSoft.each(rawState.seriesConfig, function(rawSeries) {
		...
		_getInitialSeriesState: function(seriesNumber) {
			return Ext.apply(BPMSoft.deepClone(this.initialGeneralState), this.initialSeriesState[seriesNumber]);
		},

		/**
		 *
		 * Returns the current view type.
		 * @return {BPMSoft.DashboardEnums.ChartDisplayMode} Returns the current view type.
		 */
		...
		isOrderedByGroupColumn: function() {
			...
			return orderBy.orderBy === BPMSoft.DashboardEnums.ChartOrderBy.GROUP_BY_FIELD;
		},
		...
		getDrillDownFilters: function() {
			const filters = {};
			BPMSoft.each(this.drillDownHistory, function(history) {
		...
		_getSectionSchemaBoundColumnPath: function(sectionBindingColumnPath, columnPath) {
			const sectionEntitySchema = BPMSoft[this._getSectionEntitySchemaName()];
		 ...
		 * @protected
		 * @param {BPMSoft.FilterGroup} quickFilter Quick section filter object.
		 ...
		 * @return {BPMSoft.FilterGroup} Returns quick filters group.
		 */
		getQuickFilters: function(sectionBindingColumn) {
			...
			if (seriesState.primaryColumnValue) {
				...
				filter.addItem(BPMSoft.createColumnFilterWithParameter(
						BPMSoft.ComparisonType.EQUAL, "Id", seriesState.primaryColumnValue));
			...
			if (quickFilter) {
				if (!quickFilter.getIsEnabled()) {
					quickFilter.addItem(BPMSoft.createColumnIsNotNullFilter("Id"));
				}
				const sectionEntitySchema = BPMSoft[this._getSectionEntitySchemaName()];
			...
			return quickFilter || this.Ext.create("BPMSoft.FilterGroup");
		},

		/**
		 * Returns chart filters group.
		 * @return {BPMSoft.FilterGroup} Returns chart filters group.
		 */
		 ...
		 * @return {BPMSoft.FilterGroup} Returns group of chart filters.
		 */
		 ...
		 * @protected
		 * @param {BPMSoft.EntitySchemaQuery} entitySchemaQuery Entity schema query.
		 */
		addFilters: function(entitySchemaQuery) {
			...
			if (categoryItem) {
				...
				BPMSoft.each(seriesConfig, function(config) {
					var serializedFilterData = this.getSerializedFilters(config.filterData);
					serializedFilters.logicalOperation = BPMSoft.LogicalOperatorType.OR;			
					serializedFilters.addItem(serializedFilterData);
				}, this);
			}
		...
		initColumnsInfo: function(config, callback, scope) {
			...
			BPMSoft.each(config, function(configItem) {
			...
			this.getColumnPathCaption(Ext.JSON.encode(callParameters), function(responseObject) {
				if (responseObject && responseObject[0]) {
					BPMSoft.each(responseObject, function(responseItem) {
		...
		getDrillDownCaptions: function() {
			...
			BPMSoft.each(filters, function(filterValue, filterColumn) {
				...
				if (filterValue && filterValue.datePart) {
					columnValue = [];
					BPMSoft.each(filterValue.datePart, function(value) {
		...
		fillPeriodsForDateTimeSeries: function(seriesData) {
			...
			if (BPMSoft.isDateDataValueType(dataValueType) && this.getUseEmptyValue()) {
		...
		fillFixedPeriodsForDateTimeSeries: function(seriesData, dateTimeFormat) {
			...
			if (periodPart) {
				...
				BPMSoft.each(periodList, function(periodPartValue) {
		...
		fillSeriesData: function(seriesData, itemValues) {
			BPMSoft.each(seriesData, function(seriesItem) {
		...
		generateEmptySeriesItemPoint: function(values, seriesQueryConfig) {
			const itemViewModel = Ext.create("BPMSoft.BaseViewModel", {"values": values});
		...
		getSeriesOrderValueList: function(seriesData, selector) {
			const result = [];
			BPMSoft.each(seriesData, function(seriesItem) {
				BPMSoft.each(seriesItem.data, function(item) {
		...
		getUnitedCategories: function(seriesData) {
			...
			BPMSoft.each(seriesData, function(seriesDataItem) {
				...
				BPMSoft.each(seriesCategoryItems, function(seriesCategoryItem) {
					if (!BPMSoft.contains(existingValues, seriesCategoryItem.value)) {
						if (this.isOrderedByGroupColumn()) {
							const insertInto = BPMSoft.sortedIndex(categoryItems, seriesCategoryItem, function(item) {
		...
		getSeriesDataItemOrderValue: function(item) {
			...
			if (this.isOrderedByGroupColumn()) {
				const categoryItem = item.categoryItem;
				const categoryItemValues = BPMSoft.deepClone(categoryItem.values);
		...
		getOrderDirectionIndex: function() {
			return (this.getOrderDirection() === BPMSoft.DashboardEnums.ChartOrderDirection.DESCENDING) ? -1 : 1;
		},
		...
		sortSeriesData: function(seriesData) {
			...
			if (this.isOrderedByChartEntityColumn()) {
				...
				BPMSoft.each(orderSeriesItemData, function(item, index) {
					item.x = index;
				});
				result[seriesOrderIndex] = orderSeriesItem;
				BPMSoft.each(seriesData, function(seriesItem, seriesIndex) {
					const seriesItemData = seriesItem.data;
					if (seriesIndex !== seriesOrderIndex) {
						result[seriesIndex] = seriesItem;
						const orderedData = [];
						BPMSoft.each(orderSeriesItemData, function(orderDataItem) {
		...
		getDataByCategories: function(data) {
			...
			BPMSoft.each(data, function(item) {
		...
		getSeriesYAxis: function(seriesConfig) {
			...
			const textStyle = {
				"color": BPMSoft.DashboardEnums.StyleColors[seriesConfig.styleColor] || "#999999",
			...
			return {
				...
				min: yAxisConfig.min,
				max: yAxisConfig.max,
				opposite: yAxisConfig.position === BPMSoft.DashboardEnums.ChartAxisPosition.RIGHT
			};
		},
		 ...
		 * @param {BPMSoft.BaseFilter} config.filters.quickFilters Section quick filter object.
		 * @param {BPMSoft.BaseFilter} config.filters.chartFilters Chart filter object.
		 * @param {BPMSoft.BaseFilter} config.filters.serializedFilters Filter serialized object.
		 * @param {BPMSoft.BaseFilter} config.filters.drillDownFilters Drill down filter chart.
		...
		getSeriesQuery: function(config) {
			if (config) {
				...
				esq.isBatchable = BPMSoft.Features.getIsEnabled("BatchableDashboardQueryFeature");
			...
			const getSeriesDataFunction = function(next) {
				...
				query.getEntityCollection(function(response) {
					...
					if (seriesConfig.yAxisConfig &&
							seriesConfig.yAxisConfig.position !== BPMSoft.DashboardEnums.ChartAxisPosition.NONE) {
					...
					if (seriesConfig.styleColor) {
						const color = BPMSoft.DashboardEnums.StyleColors[seriesConfig.styleColor];
						seriesData._colorIndex = BPMSoft.DashboardEnums.WidgetColorSet.indexOf(color);
					}
					if (seriesData.isDateTime === BPMSoft.isDateDataValueType(xAxis && xAxis.dataValueType)) {
		...
		isOrderedByChartEntityColumn: function() {
			const orderBy = this.getOrderBy();
			return orderBy.orderBy === BPMSoft.DashboardEnums.ChartOrderBy.CHART_ENTITY_COLUMN;
		},
		...
		getOrderByConfig: function() {
			...
			switch (chartOrderDirection) {
				case BPMSoft.DashboardEnums.ChartOrderDirection.ASCENDING:
					esqOrderDirection = BPMSoft.OrderDirection.ASC;
					break;
				case BPMSoft.DashboardEnums.ChartOrderDirection.DESCENDING:
					esqOrderDirection = BPMSoft.OrderDirection.DESC;
					break;
				default:
					esqOrderDirection = BPMSoft.OrderDirection.NONE;
			}
			const result = {
			...
			switch (orderBy) {
				...
				case BPMSoft.DashboardEnums.ChartOrderBy.CHART_ENTITY_COLUMN:
					...
					break;
				default:
					result.chartOrderType = BPMSoft.DashboardEnums.ChartOrderBy.CUSTOM_COLUMN;
		...
		getMainSeriesData: function(callback, scope) {
			...
			BPMSoft.chain.apply(this, chainParameters);
		},
		 ...
		 * @returns {BPMSoft.FilterGroup} Filters collection.
		 */
		...
		getAdditionalSeriesData: function(callback, scope) {
			...
			if (this.isNotDrilledDown()) {
				BPMSoft.each(seriesConfig, function(config, seriesNumber) {
		...
		canGroupByEmptyValue: function(dataValueType, useEmptyValue) {
			return useEmptyValue && (
					BPMSoft.isTextDataValueType(dataValueType) ||
					BPMSoft.isLookupDataValueType(dataValueType) ||
					BPMSoft.isDateDataValueType(dataValueType));
		},
		...
		getSeriesDataItemConfig: function(item, index, seriesQueryConfig) {
			...
			const isDateType = BPMSoft.isDateDataValueType(xAxis.dataValueType);
		...
		getFormatDateName: function(item, categoryItems) {
			...
			switch (dateTimeFormatStringValue) {
				case "DayMonth":
					name = this.Ext.Date.format(date, BPMSoft.Resources.CultureSettings.shortMonthDayPattern);
					break;
				case "MonthYear":
					name = this.Ext.Date.format(date, BPMSoft.Resources.CultureSettings.shortYearMonthPattern);
					break;
				case "DayMonthYear":
					name = this.Ext.Date.format(date, BPMSoft.Resources.CultureSettings.dateFormat);
					break;
				default:
		 ...
		 * @protected
		 * @param {BPMSoft.BaseViewModel} viewModel Series data item viewModel.
		 * @param {Object} seriesQueryConfig Series query config.
		 */
		updateSeriesDataItemViewModel: function(viewModel, seriesQueryConfig) {
			...
			viewModel.seriesValue =
					this.isOrderedByChartEntityColumn() && BPMSoft.isDateDataValueType(xAxis && xAxis.dataValueType)
		...
		getItemViewModelOrderValue: function(viewModel, xAxis) {
			...
			if (this.isOrderedByGroupColumn() && BPMSoft.isDateDataValueType(xAxis && xAxis.dataValueType)) {
		 ...
		 * @protected
		 * @param {BPMSoft.BaseViewModel} item Series data item viewModel.
		...
		getSeriesDataItemCombinedProperty: function(item, displayFormats, propertyName, valueModificator) {
			...
			BPMSoft.each(displayFormats, function(part) {
		...
		convertOrderValueToNumber: function(xAxis, orderValue) {
			...
			if (BPMSoft.contains(numberDataValueTypes, xAxis && xAxis.dataValueType)) {
