define("LeadManagementUtilities", ["LeadManagementUtilitiesResources", "LeadConfigurationConst",
		...
		function(resources, LeadConfigurationConst, ProcessModuleUtilities, DesktopPopupNotification, RightUtilities) {
			var leadManagementUtilitiesClass = Ext.define("BPMSoft.configuration.mixins.LeadManagementUtilities", {
				alternateClassName: "BPMSoft.LeadManagementUtilities",
				defaultProcessName: "LeadManagement10",
				 ...
				 * @private
				 * @return {BPMSoft.BaseViewModel} Entity.
				 */
				getEntity: function() {
					var entity = this;
				...
				executeLeadManagementProcess: function(config) {
					this.checkLeadEditRights(function() {
						...
						if (leadManagementProcessId && continueExecuting) {
							...
							BPMSoft.SysSettings.querySysSettingsItem("LeadManagementProcess", function(sysSetting) {
				...
				getModuleIconUrl: function() {
					var module = BPMSoft.configuration.ModuleStructure.Lead;
				...
				getIsQualificationOnActiveStage: function(qualifyStatusId) {
					...
					return BPMSoft.contains(activeStageStatuses, qualifyStatusId);
				},
				...
				initParameters: function(callback, scope) {
					BPMSoft.SysSettings.querySysSettingsItem("UseProcessLeadManagement", function(value) {
				 ...
				 * @param {BPMSoft.BaseViewModel} entity ViewModel for initialization.
				 * @param {Function} callback Callback function.
				 ...
				 * @returns {BPMSoft.EntitySchemaQuery} Query object.
				 */
				 ...
				 * @returns {BPMSoft.EntitySchemaQuery} Query that selects identifier of qualification process.
				 */
				 ...
				 * @private
				 * @param {String} primaryColumnValue Value of primary column.
				 * @returns {BPMSoft.BatchQuery} Batch query.
				 */
				...
				updateValues: function(config, callback, scope) {
					...
					bq.execute(function(response) {
						if (response.success) {
							BPMSoft.each(config, function(item, index) {
				...
				initDisqualifyButtonItems: function(callback, scope) {
					...
					if (Ext.isEmpty(menuItems)) {
						menuItems = Ext.create("BPMSoft.BaseViewModelCollection");
				 ...
				 * @param {BPMSoft.BaseViewModelCollection} collection Collection for initialization.
				 * @param {Function} callback Callback function.
				 * @param {Object} scope Execution contexxt.
				 * @private
				 */
				initDisqualifyReasonsButtonItems: function(collection, callback, scope) {
					...
					if (Ext.isEmpty(disqualificationMenuItems)) {
						disqualificationMenuItems = Ext.create("BPMSoft.BaseViewModelCollection");
				...
				getDisqualifyButtonItems: function(collection) {
					...
					if (collection instanceof BPMSoft.BaseViewModelCollection) {
