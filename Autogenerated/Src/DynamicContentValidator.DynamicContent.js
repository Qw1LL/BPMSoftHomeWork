...
define("DynamicContentValidator", ["DynamicContentValidatorResources"],
	function(resources) {
		/**
		 * @class BPMSoft.configuration.DynamicContentValidator
		 */
		Ext.define("BPMSoft.configuration.DynamicContentValidator", {
			extend: "BPMSoft.BaseViewModel",
			alternateClassName: "BPMSoft.DynamicContentValidator",
			 ...
			 * @private
			 * @param {BPMSoft.BaseFilter} filters Deserialized filter object.
			 ...
			 * @private
			 * @param {BPMSoft.BaseFilter} filter Filter item instance.
			...
			_validateFilterItem: function(filter) {
				...
				if (filter.filterType === BPMSoft.FilterType.FILTER_GROUP) {
					...
					BPMSoft.each(filter, function(el) {
			 ...
			 * @private
			 * @param {BPMSoft.Collection} items Collection of template items.
			 * @param {Function} callback Callback function.
			 */
			...
			showBodyMask: function(config) {
				...
				BPMSoft.MaskHelper.showBodyMask(config);
			},
			...
			validateDCAttributeFilter: function(attribute) {
				if (!attribute || attribute.Value === BPMSoft.emptyString) {
					return false;
				}
				var filters = BPMSoft.deserialize(attribute.Value);
		...
		return BPMSoft.configuration.DynamicContentValidator;
	}
);
