define("CardProcessViewModelGenerator", ["ext-base", "BPMSoft", "sandbox", "CardProcessViewModelGeneratorResources",
	"BusinessRuleModule", "ConfigurationEnums", "ReportUtilities", "GeneralDetails", "ResponseExceptionHelper",
	"RightUtilities", "ProcessHelper"],
	function(Ext, BPMSoft, sandbox, resources, BusinessRuleModule, ConfigurationEnums, ReportUtilities,
	...
	function internalSetColumnValues(entity, info, scope) {
		...
		BPMSoft.each(entityColumns, function(item, key) {
	...
	function getFullViewModelSchema(sourceViewModelSchema) {
		return BPMSoft.utils.common.deepClone(sourceViewModelSchema);
	}

	function generateViewModelColumn(schemaItem) {
		return {
			type: schemaItem.columnType ? schemaItem.columnType : BPMSoft.ViewModelColumnType.ENTITY_COLUMN,
	...
	function needIncludeToQuery(entityColumn) {
		return (entityColumn.dataValueType !== BPMSoft.DataValueType.IMAGE &&
			entityColumn.dataValueType !== BPMSoft.DataValueType.BLOB);
	}

	function getViewModelColumns(config, containerCollection) {
		var groupedColumns = [];
		var columns = config.columns;
		BPMSoft.each(containerCollection, function(schemaItem) {
			var itemType = schemaItem.type;
			switch (itemType) {
				case BPMSoft.ViewModelSchemaItem.IMAGE:
					break;
	...
	function prepareInfo(schema) {
		BPMSoft.each(schema.entitySchemaInfo.info, function(info) {
	...
	function generate(viewModelSchema, info, sandbox) {
		...
		var config = {
			...
			className: "BPMSoft." + fullViewModelSchema.name,
			entitySchema: fullViewModelSchema.entitySchema,
		...
		fullViewModelSchema.methods.getActionsButtonVisible = function() {
			var visible = true;
			BPMSoft.each(this.visibilityBindings, function(item) {
		...
		fullViewModelSchema.methods.moveToAction = function(tag) {
			var containerId;
			var detailId;
			BPMSoft.each(this.entitySchemaInfo.groups, function(group) {
				if (group.id === tag) {
					...
					if (value.hasOwnProperty("bindTo")) {
						...
						if (!Ext.isEmpty(column)) {
							...
							if (value.hasOwnProperty("converter")) {
								...
								cValue = BPMSoft.getTypedStringValue.apply(this, [cValue, type]);
							}
						}
						infoText = cValue;
					}
		...
		fullViewModelSchema.methods.hasChangedValues = function() {
			if (BPMSoft.isEmptyObject(this.changedValues)) {
				return false;
			}
		...
		fullViewModelSchema.methods.getHeader = function() {
			var header;
			var entityStructure = BPMSoft.configuration.EntityStructure[this.entitySchema.name];
			if (childGetHeader) {
		...
		fullViewModelSchema.methods.edit = function() {
			...
			var config = BPMSoft.configuration.ModuleStructure[this.entitySchema.name];
		...
		fullViewModelSchema.methods.getLookupQuery = function(filterValue, columnName) {
			...
			BPMSoft.each(info.columns[columnName], function(column) {
			...
			if (primaryOrderColumnName) {
				...
				if (primaryColumn) {
					primaryColumn.orderDirection = BPMSoft.OrderDirection.ASC;
		...
		fullViewModelSchema.methods.getEntitySchemaQuery = function() {
			...
			if (this.entitySchema && this.entitySchema.name && BPMSoft[this.entitySchema.name]) {
				var entityColumns = BPMSoft[this.entitySchema.name].columns;
				BPMSoft.each(esqColumns, function(columnName) {
		...
		fullViewModelSchema.methods.getEntityColumnItemsList = function(referenceSchemaName, nameFilter, list) {
			...
			sandbox.requireModuleDescriptors(["force!" + referenceSchemaName], function() {
				require([referenceSchemaName], function(selectedEntitySchema) {
					...
					esq.filters.add("typeFilter", BPMSoft.createColumnFilterWithParameter(
						BPMSoft.ComparisonType.START_WITH, selectedEntitySchema.primaryDisplayColumnName,
						nameFilter));
		...
		fullViewModelSchema.methods.setReadOnlyDetails = function(detailsList, isReadOnly) {
			...
			BPMSoft.each(this.entitySchemaInfo.details, function(detailInfo) {
