 namespace BPMSoft.Configuration
{
	...
	#region Class: MailoutsValidationResult

	/// <summary>
	/// Base validation result.
	/// </summary>
	public class MailoutsValidationResult
	{
	...
	#region Class: MailoutsHelper

	/// <summary>
	/// Class for work with active contacts.
	/// </summary>
	public class MailoutsHelper
	{
		...
		/// </summary>
		private static readonly string[] _licensePackageNamePatterns = { "BpmCrm Marketing BPMStudio.*1000.*", 
			"bpmcrm marketing mailouts.*","marketing bpmstudio.*mailouts.*", "marketing.*mailouts.*",
			"Marketing.*mailouts.*", "bpmonline.*marketing.*mailouts.*", "Bpmonline.*marketing.*mailouts.*",
			"Bpmonline.*Marketing.*mailouts.*", "BPMSoft.*marketing.*mailouts.*", "BPMSoft.*Marketing.*mailouts.*" };
		private const string _mailoutsWarningThresholdSettingsCode = "MailoutsWarningThreshold";
		...
		private readonly Guid _actualizeMailoutsProcessUId = new Guid("adbe7656-d144-44a0-870e-c27944d7148a");
		...
		private static Select GetMailoutsCountSelect(Select selectCurrentBulkEmailRecipients,
		...
		private static int GetMailoutsCount(DateTime startDate, int bulkEmailRId,
				UserConnection userConnection) {
			...
			var selectCount = GetMailoutsCountSelect(selectCurrentBulkEmailRecipients,
		...
		private void CreateRemindings(UserConnection userConnection, List<Guid> recipientIds,
			...
			foreach (var recipientId in recipientIds) {
				...
				reminding.Description = GetLczString(userConnection, "MailoutsNotificationDescription");
				reminding.LoaderId = _actualizeMailoutsProcessUId;
		...
		private string GetLczString(UserConnection userConnection, string lczName) {
			...
			return new LocalizableString(userConnection.Workspace.ResourceStorage, "MailoutsHelper",
		...
		public static MailoutsValidationResult ValidateContactsCount(int bulkEmailRId,
				UserConnection userConnection) {
			...
			var currentMailoutsCount = GetMailoutsCount(startDate, bulkEmailRId, userConnection);
			var maxMailoutsCount = userConnection.LicHelper
				.GetLicPackageMaxConditionCount(licensePackageName);
			var validationResult = new MailoutsValidationResult();
			if (currentMailoutsCount <= maxMailoutsCount) {
				validationResult.Code = MailoutsValidationResult.Success;
				validationResult.Message = string.Empty;
			} else {
				validationResult.Code = MailoutsValidationResult.Error;
		...
		public static MailoutsValidationResult ValidatePermissionEditBulkEmail(UserConnection userConnection) {
			var validationResult = new MailoutsValidationResult();
		...
		/// </summary>
		/// <param name="userConnection">User connection instance.</param>
		public void ActualizeMailoutsWithReminding(UserConnection userConnection) {
			var activeContactsWarningThreshold =
				(int)CoreSysSettings.GetValue(userConnection, _mailoutsWarningThresholdSettingsCode);
			...
			var leftMailouts = maxValue - newValue;
			var leftPercent = leftMailouts / maxValue * 100;
			if ((leftPercent <= activeContactsWarningThreshold) & (newValue != lastValue)) {
				...
				var licenseExceededSubject = GetLczString(userConnection, "MailoutsExceededNotificationSubject");
				var licenceAlmostOverSubject = GetLczString(userConnection, "MailoutsAlmostOverNotificationSubject");
