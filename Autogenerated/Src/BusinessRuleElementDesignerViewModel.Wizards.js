define("BusinessRuleElementDesignerViewModel", ["BusinessRuleElementDesignerViewModelResources", "ExpressionEnums",
	 ...
	 */
	Ext.define("BPMSoft.Designers.BusinessRuleElementDesignerViewModel", {
		extend: "BPMSoft.model.BaseViewModel",
		alternateClassName: "BPMSoft.BusinessRuleElementDesignerViewModel",
		 ...
		 * @type {BPMSoft.BusinessRuleElementUtils}
		 */
		 ...
		 * @protected
		 * @return {BPMSoft.BusinessRuleElementUtils} Business rule element util class instance.
		 */
		getBusinessRuleElementUtils: function() {
			if (!this.businessRuleElementUtils) {
				this.businessRuleElementUtils = Ext.create("BPMSoft.BusinessRuleElementUtils");
			}
		 ...
		 * @return {BPMSoft.BaseViewModel} Instance.
		 */
		createItemViewModel: function(className, columnValues) {
			...
			var viewModel = this.Ext.create(className, {
				...
				values: this.Ext.apply({Id: BPMSoft.generateGUID()}, columnValues)
			});
		 ...
		 * Create meta item.
		 * @protected
		 * @return {BPMSoft.BaseBusinessRuleElement} Meta item.
		 */
		...
		getPathByMetaPath: function(config, callback, scope) {
			...
			BPMSoft.SchemaDesignerUtilities.getEntitySchemaColumnPathByMetaPath(config, function(response) {
				const dataValueType = BPMSoft.ServerDataValueType[response.columnDataValueTypeUId];
				const baseDataValueType = BPMSoft.getBaseDataValueType(dataValueType);
				callback.call(scope, {
		...
		validateEmptyValue: function(config) {
			...
			if (!isValid) {
				...
				var message = Ext.String.format("{0}: {1}",
					config.columnCaption, BPMSoft.Resources.BaseViewModel.columnRequiredValidationMessage);
		...
		asyncSysSettingExpressionValidator: function(config, callback, scope) {
			...
			BPMSoft.ServiceProvider.executeRequest("QuerySysSettings", querySysSettingsRequest, function(responce) {
		...
		getFirstEntitySchemaName: function() {
			const config = BPMSoft.BusinessRuleSchemaManager.getDataSourcesConfig(this.pageSchemaUId);
		...
		getEntitySchemaUId: function(dataModelName) {
			...
			const entitySchema = BPMSoft.BusinessRuleSchemaManager.getEntitySchemaByDataModel(
		 ...
		 * @param {Object} config Configuration object.
		 * @param {String} config.actionType Changing action type name.
		 * @param {BPMSoft.BusinessRuleElementDesignerViewModel} config.sourceElement Source element.
		 */
		 ...
		 * @protected
		 */
		unsubscribePullChangesEvent: BPMSoft.emptyFn,
		 ...
		 * @return {*|BPMSoft.BaseBusinessRuleElement}
		 */
	...
	return BPMSoft.BusinessRuleElementDesignerViewModel;
});
