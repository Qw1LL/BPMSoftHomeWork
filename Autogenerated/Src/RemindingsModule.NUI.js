define('RemindingsModule', ['ext-base', 'BPMSoft', 'sandbox', 'Reminding', 'RemindingsModuleResources',
	'ConfigurationConstants', 'profile!remindingsModuleKey', 'RemindingsUtilities', 'MaskHelper', 'VisaHelper',
	'css!VisaHelper'],
	function(Ext, BPMSoft, sandbox, Reminding, resources, ConfigurationConstants, profile, RemindingsUtilities,
		...
		var getViewModel = function() {
			var viewModelConfig = {
				...
				values: {
					remindingsGridData: new BPMSoft.BaseViewModelCollection(),
					emailsGridData: new BPMSoft.BaseViewModelCollection(),
					approvalsGridData: new BPMSoft.BaseViewModelCollection(),
					selectedGrid: '',
					remindingCount: 0,
					emailCount: 0,
					visaCount: 0,
				...
				methods: {
					...
					setReadEmailAll: function() {
						...
						var question = Ext.String.format(
							...
							emailCount);
						var scope = this;
						BPMSoft.utils.showConfirmation(question,
							function getSelectedButton(returnCode) {
								if (returnCode === BPMSoft.MessageBoxButtons.YES.returnCode) {
									...
									var participantIdFilter = BPMSoft.createColumnFilterWithParameter(
										BPMSoft.ComparisonType.EQUAL,
										'Participant.Id', BPMSoft.SysValue.CURRENT_USER_CONTACT.value);
									filters.add('participantIdFilter', participantIdFilter);
									update.setParameterValue('ReadMark', true, BPMSoft.DataValueType.BOOLEAN);
		...
		function additionalSetLink(item, config, scope) {
			...
			select.getEntity(item.get('SubjectId'), function(response) {
				...
				if (Ext.isEmpty(response.entity)) {
					...
					var sectionUrl = BPMSoft.workspaceBaseUrl + '/Nui/ViewModule.aspx#' + sectionToken.join('/');
				...
				if (!Ext.isEmpty(pageTypeId)) {
					...
					if (!Ext.isEmpty(pageObjects) && !Ext.isEmpty(pageObjects[0])) {
						...
						var url = BPMSoft.workspaceBaseUrl + '/Nui/ViewModule.aspx#' + token.join('/');
						row.set('goToLink', {
							name: 'captionLink',
							title: title,
							url: url,
			...
			select.getEntityCollection(function(response) {
				...
				if (response && response.success) {
					...
					if (entities.collection.length > 0) {
						...
						var contactConfig = BPMSoft.configuration.ModuleStructure.Contact;
		...
		function setLinks(item, scope) {
			...
			switch (currentType) {
				case 'remindingsGridData':
					...
					var config = BPMSoft.configuration.ModuleStructure[sysEntitySchemaName];
					var cardSchemaName = config.cardSchema;
				...
				case 'emailsGridData':
					...
					var goToLinkURL = BPMSoft.workspaceBaseUrl + '/Nui/ViewModule.aspx#' + linkEditURL.join('/');
					item.set('goToLink', {
						name: 'captionLink',
						title: '',
						url: goToLinkURL,
				...
				case 'approvalsGridData':
					...
					var approvalsGoToLinkURL = BPMSoft.workspaceBaseUrl +
						'/Nui/ViewModule.aspx#' +
		...
		function getEditPage(entitySchemaName, pageTypeId) {
			var config = BPMSoft.configuration.ModuleStructure[entitySchemaName];
		...
		function deleteRemindingItem(activeRowId, scope, callback) {
			...
			var workspaceIdFilter = BPMSoft.createColumnFilterWithParameter(
				BPMSoft.ComparisonType.EQUAL,
		...
		function setReadEmailItem(emailId, scope, callback) {
			...
			var idFilter = BPMSoft.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL,
				'Activity', emailId);
			filters.add('IdFilter', idFilter);
			var participantIdFilter = BPMSoft.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL,
				'Participant.Id', BPMSoft.SysValue.CURRENT_USER_CONTACT.value);
			filters.add('participantIdFilter', participantIdFilter);
			update.setParameterValue('ReadMark', true, BPMSoft.DataValueType.BOOLEAN);
			update.execute(function(response) {
		...
		function delayItem(remindingId, value, scope, callback) {
			...
			var workspaceIdFilter = BPMSoft.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL,
				'SysEntitySchema.SysWorkspace.Id', BPMSoft.SysValue.CURRENT_WORKSPACE.value);
			filters.add('workspaceIdFilter', workspaceIdFilter);
			update.setParameterValue('RemindTime', value, BPMSoft.DataValueType.DATE_TIME);
			update.execute(function(response) {
				if (response.success && callback) {
					callback.call(this, this, response);
				}
		...
		function getGeneralItemsCongif() {
			...
			BPMSoft.each(customButtons, function(item) {
