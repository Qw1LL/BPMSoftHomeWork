define("PivotGrid", ["ConfigurationGrid", "ConfigurationGridGenerator", "css!PivotGrid",
	...
	Ext.define("BPMSoft.controls.PivotGrid", {
		extend: "BPMSoft.ConfigurationGrid",
		alternateClassName: "BPMSoft.PivotGrid",
		...
		getContentColumnsConfig: function() {
			...
			BPMSoft.each(this.rowsCaptionsColumnsConfig, function(rowsCaptionsColumn) {
			...
			var contentColumnsConfig = Ext.Array.filter(columnsConfig, function(column) {
				return !BPMSoft.contains(rowsCaptionsColumns, this.getColumnName(column));
			}, this);
		...
		getNodeWidth: function(node) {
			var sum = 0;
			if (node.children) {
				BPMSoft.each(node.children, function(element) {
		...
		renderCell: function(result, options) {
			...
			var key = cell.key;
			if (Ext.isArray(key)) {
				BPMSoft.each(key, function(column) {
		...
		getColumnsCaptionRows: function(captions) {
			...
			while (queue.length > 0) {
				...
				BPMSoft.each(children, setChildSettings, this);
			}
			captionRows.shift();
			return captionRows;
		},
		...
		getColumnsCaptionRowsHtmlConfig: function(captionRows) {
			...
			BPMSoft.each(captionRows, function(element, row) {
				...
				BPMSoft.each(captionRowConfig, function(element, column) {
		...
		/**
		 * ########## ############ ######### ############## ######## ###### # ######## ######.
		 * @inheritDoc BPMSoft.ConfigurationGrid#getRowControls
		 * @overridden
		 * @param {String} id ############# ######.
		 * @return {Array} ############ ######### ##############.
		 */
		...
		/**
		 * @inheritDoc BPMSoft.Grid#renderRowCaptionActions
		 * @overridden
		 */
		...
		/**
		 * @inheritDoc BPMSoft.ConfigurationGrid#onUpdateItem
		 * @overridden
		 */
		...
		initDomEvents: function() {
			this.callParent(arguments);
			this.debounceWindowResize = BPMSoft.debounce(this.updateSize, 250);
		...
		renderListedGrid: function(result, options, contentColumnsConfig) {
			...
			for (var index = 0, length = rows.length; index < length; index += 1) {
				...
				if (this.multiSelect) {
					...
					if (BPMSoft.contains(this.selectedRows, id)) {
		...
		renderListedHierarchicalGrid: function(result, options, contentColumnsConfig) {
			...
			for (var index = 0, length = rows.length; index < length; index++) {
				...
				if ((!row.hasOwnProperty(this.hierarchicalColumnName) && !parent) ||
					...
					if (BPMSoft.contains(this.expandHierarchyLevels, id.toString())) {
					...
					if (level > 0) {
						...
						if (!BPMSoft.contains(this.expandHierarchyLevels, parent.toString())) {
