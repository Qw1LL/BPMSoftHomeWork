define("NetworkUtilities", ["MaskHelper", "ConfigurationConstants", "ConfigurationEnumsV2", "ModuleUtils"
	 ...
	 */
	Ext.define("BPMSoft.utilities.NetworkUtilities", {
		extend: "BPMSoft.core.BaseObject",
		alternateClassName: "BPMSoft.NetworkUtilities",
		...
		getEntitySchemaName: function(entitySchemaUId) {
			return BPMSoft.ModuleUtils.getEntitySchemaNameByUId(entitySchemaUId);
		},

		/**
		 * Creates a relative URL to open the card of the entity.
		 * @throws {BPMSoft.ArgumentNullOrEmptyException} Throws an exception if not specified the schema name or
		 * the entity ID.
		...
		getPageUrl: function(pageSchemaName, operation, primaryColumnValue) {
			return BPMSoft.combinePath("CardModuleV2", pageSchemaName, operation, primaryColumnValue);
		},

		/**
		 * Creates a configuration object values for the U R L to a card entity.
		 * @throws {BPMSoft.ArgumentNullOrEmptyException} Throws an exception if not specified
		...
		getEntityConfigUrl: function(config) {
			...
			var entitySchemaName = BPMSoft.isGUID(entitySchema)
				? this.getEntitySchemaName(entitySchema)
				: entitySchema;
			if (!entitySchemaName) {
				throw Ext.create("BPMSoft.ArgumentNullOrEmptyException");
			}
			var moduleStructure = BPMSoft.ModuleUtils.getModuleStructureByName(entitySchemaName);
			var entityStructure = BPMSoft.ModuleUtils.getEntityStructureByName(entitySchemaName);
			if (!entityStructure && !moduleStructure) {
				return null;
			}
			let pages = BPMSoft.ModuleUtils.getPages(entityStructure);
			if (typeColumnValue && moduleStructure && moduleStructure.attribute && !Ext.isEmpty(pages)) {
				var typePage = BPMSoft.where(pages, {"UId": typeColumnValue});
			...
			var configUrl = {
				...
				operation: operation || BPMSoft.ConfigurationEnums.CardOperation.EDIT,
		...
		prepareOpenConfig: function(config, callback, scope) {
			if (!Ext.isObject(config)) {
				throw Ext.create("BPMSoft.UnsupportedTypeException");
			}
			...
			if (Ext.isEmpty(config.sandbox) || Ext.isEmpty(entitySchemaName) ||
				(Ext.isEmpty(entityId) && config.operation !== BPMSoft.ConfigurationEnums.CardOperation.ADD)) {
				throw Ext.create("BPMSoft.ArgumentNullOrEmptyException", {
		 ...
		 * @throws {BPMSoft.ArgumentNullOrEmptyException} Throws exception,
		...
		openEntitySection: function(config) {
			...
			var hash = BPMSoft.combinePath(sectionModule, sectionSchemaName);
		 ...
		 * BPMSoft.ConfigurationEnums.CardOperation.EDIT] Operation, that will pass to card.
		...
		getTypeColumn: function(schemaName) {
			var entityStructure = BPMSoft.ModuleUtils.getEntityStructureByName(schemaName);
		...
		openSectionInNewUI: function (config) {
			...
			const url = BPMSoft.workspaceBaseUrl + "/ClientApp/#/" + entitySchemaName;
			window.open(url, "_blank");
		}
	});

	return Ext.create("BPMSoft.NetworkUtilities");
});
