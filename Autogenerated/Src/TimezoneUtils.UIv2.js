define("TimezoneUtils", ["ServiceHelper", "BaseModule"], function(ServiceHelper) {
	/**
	 * @class BPMSoft.configuration.TimezoneUtils
	 */
	Ext.define("BPMSoft.configuration.TimezoneUtils", {
		alternateClassName: "BPMSoft.TimezoneUtils",
		...
		getAdjustmentRuleResposeHandler: function(response) {
			...
			if (response && response.GetAdjustmentRuleResult) {
				adjustmentRule = BPMSoft.decode(response.GetAdjustmentRuleResult);
			}
		...
		getPreparedContactTimeInfo: function(contactTimeInfo) {
			...
			var result = BPMSoft.decode(contactTimeInfo);
			if (result.time) {
				result.time = BPMSoft.parseDate(result.time);
			}
	...
	return Ext.create("BPMSoft.TimezoneUtils");
});
