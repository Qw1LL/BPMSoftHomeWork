define("CallExtendedMenu", ["ConfigurationConstants", "CtiConstants", "BaseExtendedMenu", "HistoryStateUtilities", "Call"],
	function(ConfigurationConstants, CtiConstants) {
		...
		Ext.define("BPMSoft.configuration.mixins.CallExtendedMenu", {
			extend: "BPMSoft.BaseExtendedMenu",
			alternateClassName: "BPMSoft.CallExtendedMenu",
			 ...
			 * @return {BPMSoft.EntitySchemaQuery} Entity schema query for getting phones communication.
			 */
			...
			addPhonesQueryFilters: function(esq, recipientInfo) {
				esq.filters.add(recipientInfo.SchemaName + "Filter", this.BPMSoft.createColumnFilterWithParameter(
					this.BPMSoft.ComparisonType.EQUAL, recipientInfo.SchemaName, recipientInfo.Id));
				esq.filters.add("CommunicationCode", this.BPMSoft.createColumnFilterWithParameter(
			 ...
			 * @private
			 * @param {BPMSoft.BaseViewModelCollection} collection Collection in which the value is set.
			 ...
			 * @return {BPMSoft.Collection} Call relation fields.
			 */
			getCallRelationFields: function(dataItem) {
				var relationFields = this.Ext.create("BPMSoft.Collection");
			...
			extendedCallMenuItemClick: function(callInfo, callback) {
				...
				if (!this.Ext.isEmpty(entitySchemaName)) {
					...
					if (!this.Ext.isEmpty(callEntityConnectionsColumns[entitySchemaName])) {
						...
						this.BPMSoft.each(this.BPMSoft.Call.columns, function (column) {
				...
				if (!this.Ext.isEmpty(columnName) && (schemaName !== "Account" || columnName !== "Contact")) {
					collection.push({
						name: columnName,
						value: columnValue,
						type: this.BPMSoft.DataValueType.GUID
					});
				}
