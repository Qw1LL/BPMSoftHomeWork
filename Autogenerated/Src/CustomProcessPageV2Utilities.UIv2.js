define("CustomProcessPageV2Utilities", [
		"ext-base", "BPMSoft", "ProcessHelper", "MaskHelper", "ProcessModuleUtilities"
	], function(Ext, BPMSoft, ProcessHelper, MaskHelper) {

	/**
	 * Base class for autogenerated and pre-configured pages.
	 */
	Ext.define("BPMSoft.configuration.mixins.CustomProcessPageV2Utilities", {
		extend: "BPMSoft.BaseObject",
		alternateClassName: "BPMSoft.CustomProcessPageV2Utilities",
		 ...
		 */
		showProcessPage: BPMSoft.abstractFn,
		...
		saveProcessParameters: function() {
			...
			BPMSoft.each(processData.parameters, function(parameterValue, name) {
				if (BPMSoft.ProcessModuleUtilities.isSystemParameter(name)) {
					return;
				}
				let value = BPMSoft.deepClone(this.get(name));
				value = Ext.isDate(value)
					? BPMSoft.encodeDate(value)
		...
		_getParameterValues: function() {
			...
			BPMSoft.each(this.processParameters, function(processParameter) {
		...
		completeExecution: function(code) {
			...
			if (this._canGetNextStepsViaResponse()) {
				...
				BPMSoft.ProcessEngineUtilities.completeExecuting(config, this.onCompleteExecution, this);
				return;
			}
		...
		isParametersChanged: function() {
			if (BPMSoft.Features.getIsEnabled("BasePageV2UseForceUpdateInProcessMode")){
				return false;
			}
			const processParameterColumnNames = this._getProcessParameterColumnNames();
			const changedKeys = BPMSoft.keys(this.changedValues);
			return BPMSoft.intersect(processParameterColumnNames, changedKeys).length > 0;
		}

	});

	Object.defineProperty(BPMSoft.CustomProcessPageV2Utilities.prototype, "processParameters", {
		get: function() {
			return [];
		}
	});
