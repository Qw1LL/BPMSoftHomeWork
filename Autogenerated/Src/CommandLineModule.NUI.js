define("CommandLineModule", ["CommandLineModuleResources", "StorageUtilities", "ProcessModuleUtilities",
	...
	Ext.define("BPMSoft.configuration.CommandLineModuleViewModel", {
		extend: "BPMSoft.BaseViewModel",
		alternateClassName: "BPMSoft.CommandLineModuleViewModel",
		mixins: {
		...
		valueChanged: function(item) {
			if (BPMSoft.isEmptyObject(item) || !item.value) {
				return;
			}
	...
	function createConstructor(context) {
		...
		function getIsGlobalSearchEnabled() {
			return BPMSoft.Features.getIsEnabled("GlobalSearch") &&
		 ...
		 * Safe adds to list.
		 * @private
		 * @param {BPMSoft.Collection} list List for adding.
		 * @param {BPMSoft.Collection} tempList Temporary list.
		 */
		 ...
		 * @param {BPMSoft.Collection} commandList Command list.
		...
		/**
		 * Returns entity schema parameters, registered in BPMSoft.configuration.ModuleStructure.
		 * @param {String} entitySchemaName Entity schema name.
		 * @return {Object|null} Entity schema parameters, registered in BPMSoft.configuration.ModuleStructure.
		 */
		...
		/**
		 * Returns page parameters, registered in BPMSoft.configuration.ModuleStructure.
		 * @param {Object} config Search config.
		 * @param {String} config.cardType Entity type parameter. Can be set as page shema name or type id.
		 * @param {Object} module Entity schema parameters, registered in BPMSoft.configuration.ModuleStructure.
		...
		function getOpenCardConfig(schemaName, config) {
			...
			var values = {
				TypeColumnName: "",
				TypeColumnValue: BPMSoft.GUID_EMPTY,
			...
			return Ext.create("BPMSoft.BaseViewModel", {values: values});
		}
		...
		function getSectionSessionFiltersKey(entityName) {
			...
			var moduleStructure = BPMSoft.configuration.ModuleStructure[entityName];
		...
		function executeGoToCommand(config) {
			...
			if (addParam.value && addParam.valueId) {
				var filtersStorage = BPMSoft.configuration.Storage.Filters =
				...
				var filter = BPMSoft.deserialize(addParam.value);
				sessionStorageFilters.FolderFilters = [filter];
				state = sandbox.publish("GetHistoryState");
				currentState = state.state || {};
				newState = BPMSoft.deepClone(currentState);
		...
		function getModuleLoadedSearchParams(historyState) {
			...
			return {
				sectionEntityName: (searchParams && searchParams.sectionEntityName) ||
					BPMSoft.ModuleUtils.getEntityNameBySchemaName(stateSchemaName),
		...
		function getCurrentEntityName(historyState) {
			...
			return BPMSoft.ModuleUtils.getEntityNameBySchemaName(hashEntityName);
		}
		...
		function generateTip(config) {
			...
			var tip = viewGenerator.generatePartial(Ext.merge({
				...
				schema: {},
				viewModelClass: Ext.ClassManager.get("BPMSoft.CommandLineModuleViewModel")
			})[0];
			return tip;
		}
				...
				items: [
					{
						...
						bigSize: true,
						placeholder: resources.localizableStrings.RunTheCommand,
		...
		/**
		 * Creates view model.
		 * @return {BPMSoft.CommandLineModuleViewModel}
		 */
		function createViewModel() {
			...
			var viewModel = Ext.create("BPMSoft.CommandLineModuleViewModel", {
				values: {
					...
					hintList: new BPMSoft.Collection(),
					commands: new BPMSoft.Collection(),
					commandsCollection: new BPMSoft.Collection(),
					mainParamCollection: new BPMSoft.Collection()
				}
			});
			return viewModel;
		}
		...
		function callServiceMethod(ajaxProvider, methodName, callback, dataSend) {
			...
			ajaxProvider = ajaxProvider || BPMSoft.AjaxProvider;
		...
		function initCommandList(scope) {
			var commandList = scope.$commands = new BPMSoft.Collection();
			var commandsCollection = scope.$commandsCollection = new BPMSoft.Collection();
			var mainParamCollection = scope.$mainParamCollection = new BPMSoft.Collection();
		...
		function getViewModel() {
			...
			viewModel.getCommandList = function(filter) {
				var filteredCommandListByCaption = function(caption) {
					...
					filteredList.sort("MainParamCaption", BPMSoft.OrderDirection.ASC);
			...
			viewModel.initCommandList = function() {
				if (this.$commands.isEmpty() && !BPMSoft.isCurrentUserSsp()) {
			...
			viewModel.executeCommand = function(input) {
				...
				if (macros) {
					...
					if (macros.HintType === ConfigurationConstants.CommandLineHintType.Macros &&
						...
						switch (command) {
							...
							case "startbp":
								...
								esq.filters.add("recordId", BPMSoft.createColumnFilterWithParameter(
									BPMSoft.ComparisonType.EQUAL, "Id", valueId));
						...
						if (tempList.getCount() === 1) {
							...
							callServiceMethod(ajaxProvider, "GetHintList", function(response) {
								...
								if (hintArray.length === 1) {
									...
									try {
										var params = BPMSoft.deserialize(value);
					...
					var searchSchemaName = BPMSoft.isCurrentUserSsp() ? "KnowledgeBase" : "Contact";
