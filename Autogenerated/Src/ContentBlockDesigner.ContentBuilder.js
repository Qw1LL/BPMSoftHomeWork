define("ContentBlockDesigner", ["css!ContentBlockDesignerCSS", "GridLayoutEditItemModel", "ContainerList",
	...
	return {
		...
		attributes: {
			/**
			 * Collection of view model elements (BPMSoft.ContentBlockElementItemViewModel).
			...
			/**
			 * Collection of view model tools (BPMSoft.ContentBlockToolItemViewModel).
			 * @type {BPMSoft.Collection}
			 */
			Tools: {
		...
		properties: {
			 ...
			 * @type {BPMSoft.ContentElementManager}
			 */
			 ...
			 * @private
			 */
			_contentBlockElementGroupsHelper: this.Ext.create("BPMSoft.ContentBlockElementGroupsHelper")
		},
		methods: {
			 ...
			 */
			_initTools: function() {
				this.$Tools = new BPMSoft.BaseViewModelCollection();
				const items = this.contentElementManager.getItems();
				items.each(function(item) {
					const config = item.DesignTimeConfig;
					const itemViewModel = new BPMSoft.ContentBlockToolItemViewModel({
			...
			_initElements: function() {
				this.$Elements = this.Ext.create("BPMSoft.BaseViewModelCollection");
			...
			_addElement: function(config) {
				...
				config.Values.Id = id;
				const viewModel = new BPMSoft.ContentBlockElementItemViewModel({
			...
			_canUngroup: function(selectedElements, elements) {
				return selectedElements.length > 0 && Boolean(elements.firstOrDefault(function(item) {
					return BPMSoft.contains(selectedElements, item.$Id) && Boolean(item.$GroupName);
				}, this));
			},
			...
			constructor: function() {
				this.contentElementManager = new BPMSoft.ContentElementManager();
			 ...
			 * @param {BPMSoft.ContentBlockToolItemViewModel}toolItem Tools item view model.
			 */
		...
		diff: /**SCHEMA_DIFF*/[
			...
			},
			{
				...
				"propertyName": "items",
				"values": {
					"itemType": BPMSoft.ViewItemType.GRID_LAYOUT_EDIT,
					"id": "GridLayoutEdit",
