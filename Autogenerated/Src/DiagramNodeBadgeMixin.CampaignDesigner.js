Ext.define("BPMSoft.diagram.DiagramNodeBadgeMixin", {
	alternateClassName: "BPMSoft.DiagramNodeBadgeMixin",
	...
	renderBadge: function(badge, node, svg, parent) {
		...
		if (BPMSoft.getIsRtlMode() && !Ext.isIE) {
			badge.point.x += width;
		}
		if (x !== badge.point.x) {
	...
	renderBadges: function(svg, parent) {
		...
		if (badges.length) {
			...
			BPMSoft.each(nodes, function(node){
				BPMSoft.each(badges, function(badge){
	...
	getBadgeSide: function(isDefaultSide) {
		if (!BPMSoft.getIsRtlMode() === isDefaultSide) {
			return "right";
		}
		return "left";
	},
	...
	getBadgeConfig: function() {
		return {
			...
			content: "",
			fontSize: 13,
			fontFamily: "'Golos Regular', serif",
			fontColor: "#fff",
			textDecoration: "none",
			fillColor: "#000",
			borderColor: "#fff",
