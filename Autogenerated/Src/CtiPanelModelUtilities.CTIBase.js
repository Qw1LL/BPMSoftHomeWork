define("CtiPanelModelUtilities", ["ConfigurationConstants", "CtiConstants", "NetworkUtilities",
	...
	function(ConfigurationConstants, CtiConstants, NetworkUtilities, CtiNotificationUtilities) {
		/**
		 * @class BPMSoft.configuration.mixins.CtiPanelModelUtilities
		 * Mixin of ViewModel of cti panel.
		 * @type {BPMSoft.BaseObject}
		 */
		Ext.define("BPMSoft.configuration.mixins.CtiPanelModelUtilities", {
			extend: "BPMSoft.core.BaseObject",
			alternateClassName: "BPMSoft.CtiPanelModelUtilities",
			...
			initializeSubscribersCollections: function() {
				...
				BPMSoft.each(events, function(eventName) {
			...
			createPopupNotification: function(tag, lifeTime, body, currentCall) {
				var self = this;
				if (currentCall && (currentCall.direction === BPMSoft.CallDirection.IN)) {
			...
			initOnConnected: function() {
				...
				this.set("IsVideoSupported",
					(capabilities.callCapabilities & BPMSoft.CallFeaturesSet.CAN_VIDEO) !== 0);
				/*jshint bitwise:true */
			},
			...
			getEmptySearchResultImageUrl: function() {
				...
				return BPMSoft.ImageUrlBuilder.getUrl(emptySearchResultImage);
			},
			...
			getCurrentDatabaseUId: function() {
				...
				if (BPMSoft.isEmpty(currentCall)) {
					return BPMSoft.emptyString;
				}
				return currentCall.databaseUId || BPMSoft.emptyString;
			},

			/**
			 * @inheritdoc BPMSoft.CtiModel#callStarted
			 * @protected
			 */
			...
			callByNumber: function(number) {
				if (BPMSoft.Features.getIsDisabled("NotReplaceNonDigitsFromMakeCallNumber")) {
				...
				if (this.isPhoneNumberValid(number)) {
					if (this.getCanCall() && this.activeCalls.isEmpty()) {
						...
						const connectionParams = BPMSoft.SysValue.CTI.connectionParams;
			...
			getIsInternalTelephonyNotAvailable: function() {
				var buildType = this.BPMSoft.SysSettings.cachedSettings.BuildType;
			...
			/**
			 * @inheritdoc BPMSoft.CtiModel#commutationStarted
			 * @private
			 */
			 ...
			 * @private
			 * @param {Backbone.Model} model Cti-model.
			 * @param {BPMSoft.integration.telephony.Call} call Call object.
			 */
			onChangeCurrentCall: function(model, call) {
				if (call) {
					...
					if (call.state === BPMSoft.GeneralizedCallState.CONNECTED && !this.$CommutationStartedOn) {
			...
			/**
			 * @inheritdoc BPMSoft.CtiModel#connected
			 */
			onConnected: function() {
				//TODO Implement a client status request when connecting/restart the browser
			...
			/**
			 * @inheritdoc BPMSoft.BaseCtiProvider#callSaved
			 */
			 ...
			 * @private
			 * @param {BPMSoft.Collection} subscribersCollection Collection of identified subscribers.
			 ...
			 * @private
			 * @param {BPMSoft.Collection} subscribersCollection Collection of found subscribers.
			 */
			...
			/**
			 * Fires on adding call to the list of active calls @link BPMSoft.CtiModel#activeCalls.
			 * @private
			...
			/**
			 * Fires on removing call from the list of active calls @link BPMSoft.CtiModel#activeCalls.
			 * @private
			 * @param {BPMSoft.integration.telephony.Call} call The removed call.
			 */
			...
			/**
			 * Collection of active calls clearing event handler @link BPMSoft.CtiModel#activeCalls.
			 * @private
			 */
			...
			onWebRtcStarted: function(sessionId, config) {
				...
				////TODO wb. Video calls. Don`t use DOM directly
			...
			onWebRtcDestroyed: function(sessionId) {
				////TODO wb. wb. Video calls. Don`t use DOM directly
				var videoTag = Ext.get(sessionId);
				if (videoTag) {
					videoTag.remove();
				}
