...
define("SectionDesignerUtils", [
	"ext-base", "BPMSoft", "SectionDesignerUtilsResources", "SectionDesignerEnums",
...
], function(Ext, BPMSoft, resources, SectionDesignerEnums, BaseEntity, BaseFolder, File, BaseItemInFolder,
	...
	function useMultilanguageData() {
		return !BPMSoft.Features.getIsEnabled("DoNotuseMultilanguageData");
	}
	 ...
	 * @param {BPMSoft.SchemaType} schemaType ### #####
	 * @param {String} entitySchemaName ######## ####### #######
	 * @param {BPMSoft.SchemaPackageStatus} schemaPackageStatus ###### #####
	...
	function getSchemaBody(schemaName, schemaType, entitySchemaName, schemaPackageStatus, callback, scope) {
		BPMSoft.chain({}, [
			...
			function(context) {
				...
				if (!body) {
					switch (schemaType) {
						case BPMSoft.SchemaType.EDIT_VIEW_MODEL_SCHEMA:
							body = getPageSchemaBody(schemaName, entitySchemaName);
							break;
						case BPMSoft.SchemaType.GRID_DETAIL_VIEW_MODEL_SCHEMA:
	 ...
	 * @param {BPMSoft.SchemaType} schemaConfig.schemaType ### #####
	...
	function createControlInput(renderTo, dataValueType, value) {
		var inputConfig = BPMSoft.getControlConfigByDataValueType(dataValueType);
		if (dataValueType !== BPMSoft.DataValueType.BOOLEAN) {
	...
	function postServiceRequest(config) {
		...
		var serviceUrl = BPMSoft.workspaceBaseUrl + "/rest/DesignService/";
		var requestOptions = {
		...
		BPMSoft.AjaxProvider.request(requestOptions);
	}
	...
	function createInputControls(config) {
		...
		var controls = {};
		BPMSoft.each(config, function(controlConfigItem, key) {
	...
	/**
	 * ####### ######### ######## ##########
	 * @param {BPMSoft.TextEdit} textEdit ######## ##########
	...
	function createButtons(config) {
		...
		BPMSoft.each(config, function(controlConfigItem, key) {
			...
			var button = Ext.create("BPMSoft.Button", controlConfigItem);
			controls[key] = button;
		}, this);
		return controls;
	}
	...
	/**
	 * ####### ######## reference ####### entity #####
	 * @param {BPMSoft.BaseEntitySchema} entity ###### BPMSoft.BaseEntitySchema
	 * @param {Object} config ############## ############ #######
	 ...
	 * @return {BPMSoft.BaseEntitySchema} entity ###### BPMSoft.BaseEntitySchema
	 */
	function createEntitySchema(config) {
		...
		Ext.define("BPMSoft.data.models." + name, {
			extend: "BPMSoft.BaseEntitySchema",
			alternateClassName: "BPMSoft." + name,
			caption: caption,
			name: name,
			singleton: true,
			uId: BPMSoft.generateGUID(),
		...
		BPMSoft[name].rootEntitySchema = baseEntity;
		return BPMSoft[name];
	}

	/**
	 * ####### ######## ###### ######### #####
	 * @param {BPMSoft.BaseEntitySchema} entity ######### #####
	 ...
	 * @return {BPMSoft.BaseViewModel} ###### BPMSoft.BaseViewModel
	 */
	function createClientUnitSchema(config) {
		var result = {
			...
			structure: {
				...
				schemaUId: BPMSoft.generateGUID(),
		...
		if (config.type === BPMSoft.SchemaType.EDIT_VIEW_MODEL_SCHEMA) {
			Ext.apply(result.schema, {
				entitySchemaName: config.code,
				details: {},
				rules: {}
			});
		} else if (config.type === BPMSoft.SchemaType.GRID_DETAIL_VIEW_MODEL_SCHEMA) {
			Ext.apply(result.schema, {
				entitySchemaName: config.code
			});
		}
	...
	function createModuleScheme(config) {
		...
		var defaultModuleConfig = {
			...
			sectionSchemaName: "",
			typeColumnId: null,
			sysModuleEntityId: BPMSoft.generateGUID()
		};
		...
		entities[entityFolder.name] = getEntitySchemaDesignData(entityFolder, [], BPMSoft.BaseFolder.uId);
		...
		entities[entityTag.name] = getEntitySchemaDesignData(entityTag, [], BPMSoft.BaseTag.uId);
		...
		var tagColumnConfig = BPMSoft.deepClone(BaseEntityInTag.columns.Tag);
		tagColumnConfig.referenceSchemaName = entityTag.name;
		var entityColumnConfig = BPMSoft.deepClone(BaseEntityInTag.columns.Entity);
		...
		var folderColumnConfig = BPMSoft.deepClone(BaseItemInFolder.columns.Folder);
		...
		var schemaConfig = {
			...
			code: code,
			type: BPMSoft.SchemaType.MODULE_VIEW_MODEL_SCHEMA,
		...
		var detailSchemaConfig = {
			...
			code: code,
			type: BPMSoft.SchemaType.GRID_DETAIL_VIEW_MODEL_SCHEMA,
	...
	function getDefaultGridSettings(sectionSchemaName, isTypedModule, moduleDesignData) {
		...
		var typeColumnName;
		if (!Ext.isEmpty(typeColumnId) && !BPMSoft.isEmptyGUID(typeColumnId)) {
			BPMSoft.each(entityDesignData.columns, function(column, key) {
				if (key === typeColumnId) {
	...
	function setEntityPrimaryDisplayColumn(designData, sectionCode, entityPage) {
		...
		if (!BPMSoft.isArray(entityPage.schema.diff)) {
	...
	function createEditPage(designData, config) {
		...
		var pageConfig = {
			...
			code: sectionCode,
			type: BPMSoft.SchemaType.EDIT_VIEW_MODEL_SCHEMA,
	...
	function addModuleEditLcz(value, columnId, pageId) {
		var newRecordId;
		newRecordId = BPMSoft.generateGUID();
		SectionDesignDataModule.addEntity("SysModuleEditLczOld", {
			Id: newRecordId,
			ColumnUId: columnId,
			Record: pageId,
			SysCulture: BPMSoft.Resources.CultureSettings.currentCultureId,
			Value: value
		});
		return newRecordId;
	}
				...
				callback: function(request, success, response) {
					...
					if (success) {
						...
						if (responseObject.GetModuleStructureInfoResult) {
							existingModuleConfig = BPMSoft.decode(responseObject.GetModuleStructureInfoResult);
	...
	function getSectionSchemaNames(moduleConfig, config) {
		...
		var propertyValuesArray = BPMSoft.getPropertyValuesArray(schemaNameProperties, moduleConfig);
		var moduleScheme = BPMSoft.deleteEmptyItems(propertyValuesArray);
		var pages = moduleConfig.pages;
		BPMSoft.each(pages, function(pageConfig) {
			...
			callback: function(request, success, response) {
				...
				if (success) {
					...
					if (responseObject[serviceMethodName + "Result"]) {
						schemaInfo = BPMSoft.decode(responseObject[serviceMethodName + "Result"]);
					}
				}
		...
		SectionDesignDataModule.getEntitySchemaByName({
			...
			callback: function(entitySchema) {
				...
				var entityType = createEntitySchema({
					...
					rootEntitySchema: BPMSoft.BaseLookup
				});
					...
					callback: function(entitySchema) {
						...
						SectionDesignDataModule.getEntities(typeColumn.referenceSchemaName, ["Name"], false,
								function(entitySchemaData) {
									var currentEditPage;
									BPMSoft.each(entitySchemaData, function(item) {
	...
	function clearEditPages(designData) {
		...
		BPMSoft.each(editPages, function(page) {
		...
		BPMSoft.each(typesForDelete, function(typeId) {
			removeEditPage(designData, typeId);
		});
		BPMSoft.each(sysModuleEditRecords, function(sysModuleEditId) {
			SectionDesignDataModule.deleteEntity("SysModuleEdit", sysModuleEditId);
		}, this);
		BPMSoft.each(sysModuleEditLczRecords, function(sysModuleEditLczId) {
		...
		postServiceRequest({
			...
			callback: function(request, success, response) {
				var result = {
					...
					currentPackageUId: BPMSoft.GUID_EMPTY,
				...
				if (success) {
					...
					if (responseObject.GetAvailablePackagesResult) {
						...
						if (responseResultObject.AvailablePackages) {
							...
							BPMSoft.each(responseResultObject.AvailablePackages, function(pack) {
	...
	Ext.define("BPMSoft.configuration.SectionDesignerUtils", {
		extend: "BPMSoft.BaseObject",
		alternateClassName: "BPMSoft.SectionDesignerUtils",
	...
	return Ext.create("BPMSoft.SectionDesignerUtils");
});
/*jshint maxparams: 10 */
