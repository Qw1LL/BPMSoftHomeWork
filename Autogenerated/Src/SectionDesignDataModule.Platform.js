define("SectionDesignDataModule", ["ext-base", "BPMSoft", "SectionDesignerEnums", "SectionDesignerUtils",
		"SectionDesignDataModuleResources", "SchemaBuilderV2", "ModalBox"],
	function(Ext, BPMSoft, SectionDesignerEnums, SectionDesignerUtils, resources, SchemaBuilder, ModalBox) {
		...
		var utils = {
			 ...
			 * @private
			 * @param {Object} schema ###### #####
			 * @return {BPMSoft.BaseEntitySchema} ##### # ###########
			 */
			mergeEntitySchema: function(schema) {
				...
				if (schema.metaInfo.modifiedData) {
					var modifiedData = BPMSoft.deepClone(schema.metaInfo.modifiedData);
					var modifiedColumns = modifiedData.columns;
					if (modifiedColumns) {
						BPMSoft.each(entitySchemaColumns, function(column, columnName) {
						...
						BPMSoft.each(modifiedColumns, function(column) {
			...
			getClientUnitOriginSchema: function(schema) {
				var clientUnitSchema = BPMSoft.deepClone(schema.schema.originData);
				var clientUnitStructureSchema = BPMSoft.deepClone(schema.structure.originData);
				var clientUnitResourcesSchema = BPMSoft.deepClone(schema.resources.originData);
				return {
			...
			mergeClientUnitSchema: function(schemaData) {
				var clientUnitSchema = BPMSoft.deepClone(schemaData.schema.originData);
				var clientUnitStructureSchema = BPMSoft.deepClone(schemaData.structure.originData);
				var clientUnitResourcesSchema = BPMSoft.deepClone(schemaData.resources.originData);
				if (!BPMSoft.isEmptyObject(schemaData.schema.metaInfo.modifiedData)) {
					clientUnitSchema = BPMSoft.deepClone(schemaData.schema.metaInfo.modifiedData);
				}
				if (schemaData.structure.metaInfo.modifiedData) {
					var modifiedStructureData = BPMSoft.deepClone(schemaData.structure.metaInfo.modifiedData);
					Ext.merge(clientUnitStructureSchema, modifiedStructureData);
				}
				if (schemaData.resources.metaInfo.modifiedData) {
					var modifiedResourcesData = BPMSoft.deepClone(schemaData.resources.metaInfo.modifiedData);
			...
			getEntitySchemaData: function(schemaName, isOriginal) {
				...
				if (data) {
					for (var i = 0, dataLength = data.length; i < dataLength; i++) {
						var entity = BPMSoft.deepClone(data[i]);
			...
			getEntitySchemaDataById: function(schemaName, isOriginal, entityId) {
				...
				var entity;
				if (data) {
					entity = BPMSoft.deepClone(this.getInnerEntityById(schemaName, entityId));
					if (entity) {
			...
			getSchemaNameById: function(id, isClientUnit) {
				...
				BPMSoft.each(schemaBranch, function(schema) {
			...
			getEntitySchemaPostData: function(schemaNameSequence) {
				var schemaManager = BPMSoft.deepClone(designData.schemaManager);
			...
			getIsSchemaChanged: function(schemaMetaInfo) {
				var modifiedData = BPMSoft.deepClone(schemaMetaInfo.modifiedData);
			...
			mergeExistingProperties: function(destinationObject, modifiedDataObject, originDataObject) {
				var result = {};
				BPMSoft.each(destinationObject, function(propertyValue, propertyName) {
					...
					} else if (BPMSoft.isArray(destinationObjectPropertyValue)) {
						result[propertyName] = modifiedDataObject[propertyName] || [];
					} else {
				...
				BPMSoft.chain({
					...
					function(context) {
						...
						var diff = (schemaViewConfig.length === 0)
							? schemaDiff
							: BPMSoft.JsonDiffer.getJsonDiff(parentSchemaViewConfig, schemaViewConfig);
						var result = {
			...
			getClientUnitSchemaPostData: function(callback, scope) {
				...
				BPMSoft.each(clientUnits, function(schemaInfo) {
					...
					if (this.isClientUnitSchemaModified(modificationType, schemaInfo)) {
						...
						BPMSoft.each(clientUnitPostDataItem, function(property, propertyName) {
						...
						BPMSoft.each(designData.module, function(sectionInfo) {
			...
			getGridSettingPostData: function() {
				var result;
				var gridSettingsModifiedData = BPMSoft.deepClone(designData.gridSettings.modifiedData);
				if (!Ext.Object.isEmpty(gridSettingsModifiedData)) {
					result = {
						key: gridSettingsModifiedData.key,
						data: BPMSoft.encode(gridSettingsModifiedData),
					...
					function(context) {
						...
						if (innerEntity.metaInfo &&
							...
							getEntityById("SysModule", sectionId, ["Image32", "Logo"], false, function(data) {
								...
								context.existingImageId = BPMSoft.GUID_EMPTY;
								context.currentLogoId = data.Logo;
								context.existingLogoId = BPMSoft.GUID_EMPTY;
								context.next();
							});
						}
					},
			...
			getSchemaNameSequence: function(callback) {
				...
				var schemaNameSequence = [];
				BPMSoft.each(data, function(schemaData, schemaName) {
				...
				BPMSoft.chain({}, [
					...
					function(context) {
						...
						BPMSoft.each(designedSchemaItems, function(schemaMetaInfo, schemaName) {
							schemaNameSequence.push(schemaName);
						});
			...
			getSchemeByName: function(config) {
				...
				var innerSchemaNames = BPMSoft.deepClone(schemaNames);
		...
		function getDesignDataStrorageItemKey() {
			...
			return Ext.String.format(designDataKeyTemplate, BPMSoft.SysValue.CURRENT_USER.value);
		}
		...
		function saveDesignDataToStorage() {
			// TODO UI: SectionDesigner. ######## ######### ###### # LocalStorage
			//if (Ext.isEmpty(storage)) {
			//return;
			//}
			/*var storageDesignData = BPMSoft.deepClone(designData);
			var schemaManager = storageDesignData.schemaManager;
			var entitySchemaInfo = schemaManager.entity;
			BPMSoft.each(entitySchemaInfo, function(item) {
				delete item.schema;
			});
			var clientUnitsSchemaInfo = schemaManager.clientUnit;
			BPMSoft.each(clientUnitsSchemaInfo, function(item) {
		...
		function loadDesignDataFromStorage(callback, scope) {
			...
			var storageDesignData = BPMSoft.decode(stringifiedDesignData);
			var schemaManager = storageDesignData.schemaManager;
			resetDesignData();
			BPMSoft.chain({
				...
				function(context) {
					context.maskId = BPMSoft.Mask.show();
					var entitySchemaInfo = schemaManager.entity;
					var entitySchemaNames = [];
					BPMSoft.each(entitySchemaInfo, function(item, key) {
						var metaInfo = item.metaInfo;
						if (metaInfo.modificationType === SectionDesignerEnums.ModificationType.MODIFIED) {
							entitySchemaNames.push(key);
						} else {
							var modifiedData = BPMSoft.deepClone(metaInfo.modifiedData);
							delete modifiedData.columns;
				...
				function(context) {
					...
					BPMSoft.each(clientUnitSchemaInfo, function(item, key) {
				...
				function(context) {
					...
					BPMSoft.Mask.hide(context.maskId);
					callback.call(scope);
				}
			]);
...
*		}
		...
		var getEntityById = function(schemaName, id, columns, isOriginal, callback) {
			if (Ext.isEmpty(id) || !BPMSoft.isGUID(id)) {
				callback(null);
				return;
			}
			...
			if ((schemaData) || (designedSchema && designedSchema.metaInfo.modificationType ===
				...
				if (columns && columns.length > 0) {
					...
					select.filters.add("entityId", BPMSoft.createColumnFilterWithParameter(
		...
		function revertClientUnitSchemaChanges(names) {
			...
			var deletedNames = [];
			BPMSoft.each(scheme, function(schema) {
				if (!names || BPMSoft.contains(names, schema.name)) {
...
*	}
		...
		var addEntity = function(schemaName, columnValues) {
			columnValues.Id = columnValues.Id || BPMSoft.generateGUID();
			var entity = {
				metaInfo: {
					modificationType: SectionDesignerEnums.ModificationType.NEW,
...
*	}
		...
		var modifyEntity = function(schemaName, columnValues, primaryColumnValue) {
			if (Ext.isEmpty(primaryColumnValue) || !BPMSoft.isGUID(primaryColumnValue)) {
				return;
			}
		...
		var loadLikeSchemaNames = function(sectionCode, callback) {
			...
			select.filters.add("AcountIdFilter", BPMSoft.createColumnFilterWithParameter(
				BPMSoft.ComparisonType.CONTAIN, "Name", sectionCode));
			var result = {
				likeSchemaNames: []
			};
			BPMSoft.chain({
		...
		var setModuleStructureInfo = function(config) {
			var defaultModuleConfig = {
				code: "",
				entityTagId: BPMSoft.generateGUID(),
				entityTagName: "",
				entityInTagId: BPMSoft.generateGUID(),
				entityInTagName: "",
				entityFolderId: BPMSoft.generateGUID(),
				entityFolderName: "",
				entityId: BPMSoft.generateGUID(),
				entityInFolderId: BPMSoft.generateGUID(),
				entityInFolderName: "",
				entityName: "",
				id: BPMSoft.generateGUID(),
				pages: [],
				sectionSchemaId: BPMSoft.generateGUID(),
				sectionSchemaName: "",
				typeColumnId: BPMSoft.generateGUID(),
				sectionIconId: null,
				sectionLogoId: null
			};
		...
		function setModuleStructureData(moduleStructure) {
			...
			var doNotUseMultilanguageData = BPMSoft.Features.getIsEnabled("DoNotUseMultilanguageData");
			var data = {
				SysModuleEntity: [{
			...
			BPMSoft.each(moduleStructure.pages, function(page) {
		...
		var getDesignData = function(callback, scope) {
			scope = scope || this;
			var returnDesingData = BPMSoft.deepClone(designData);
						...
						callback: function(request, success, response) {
							...
							if (success) {
								...
								BPMSoft.utils.showMessage({
									...
									style: BPMSoft.MessageBoxStyles.ORANGE,
									handler: function() {
										BPMSoft.DomainCache.setItem("SectionDesigner_DetailsInfo", null);
										BPMSoft.DomainCache.setItem("DetailsInfo", null);
										window.close();
									}
								});
							}
		...
		function loadSchema(config) {
			...
			if (!isClientUnit) {
				...
				require(schemaNames, function() {
					...
					BPMSoft.each(arguments, function(requiredSchema) {
						var schema = BPMSoft.deepClone(requiredSchema);
		...
		function setLoadedSchemeDesignData(schemaConfig, isClientUnit) {
			...
			BPMSoft.each(schemaConfig, function(schema, key) {
				if (schema) {
					removeMethod(key);
