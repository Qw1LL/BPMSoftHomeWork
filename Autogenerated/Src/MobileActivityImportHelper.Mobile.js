...
/**
 * @class BPMSoft.configuration.ActivityImportHelper
 */
Ext.define("BPMSoft.configuration.ActivityImportHelper", {
	mixins: {
	...
	eachDate: function(startDate, dueDate, iterator, scope) {
		var diff = BPMSoft.Date.diff(startDate, dueDate, Ext.Date.DAY);
		...
		for (var i = 0; i <= total; i++) {
			...
			currentDate = BPMSoft.Date.add(currentDate, Ext.Date.DAY, sign);
		}
	},
	...
	addPeriodBitMask: function(sourceMask, mask) {
		BPMSoft.each(mask, function(monthMask, monthKey) {
			sourceMask[monthKey] = BPMSoft.BitMask.or(sourceMask[monthKey], monthMask);
		});
	},
	...
	removePeriodBitMask: function(sourceMask, mask) {
		BPMSoft.each(mask, function(monthMask, monthKey) {
			sourceMask[monthKey] = BPMSoft.BitMask.xor(sourceMask[monthKey], monthMask);
		});
	},
	...
	applyDiffBitMask: function(sourceMask, mask) {
		BPMSoft.each(sourceMask, function(monthMask, monthKey) {
			if (mask[monthKey]) {
				var tmpMask = BPMSoft.BitMask.and(sourceMask[monthKey], mask[monthKey]);
				sourceMask[monthKey] = BPMSoft.BitMask.xor(tmpMask, sourceMask[monthKey]);
			}
		});
	},
	...
	getBitMaskByPeriod: function(startDate, dueDate) {
		...
		this.eachDate(startDate, dueDate, function(currentDate) {
			...
			periodBitMask[cacheKey.monthKey] = BPMSoft.BitMask.add(days, cacheKey.dayMask);
		}, this);
	...
	getPeriodByBitMask: function(periodBitMask) {
		var startDate;
		var dueDate;
		BPMSoft.each(periodBitMask, function(mask, monthKey) {
			...
			for (var i = 1; i <= daysInMonth; i++) {
				...
				if (BPMSoft.BitMask.isIntersected(tmpBitMask, mask)) {
	...
	calculateImportPeriod: function(startDate, dueDate) {
		startDate = BPMSoft.Date.add(startDate, Ext.Date.DAY, -this.additionalDays);
		dueDate = BPMSoft.Date.add(dueDate, Ext.Date.DAY, this.additionalDays);
		var periodBitMask = this.getBitMaskByPeriod(startDate, dueDate);
	...
	getEmbeddedDetails: function() {
		if (!Ext.isArray(this.detailConfigs)) {
			this.detailConfigs = [];
			var columnSets = BPMSoft.sdk.RecordPage.getColumnSets("Activity");
			columnSets.each(function(columnSetConfig) {
				if (BPMSoft.sdk.RecordPage.getIsEmbeddedDetail(columnSetConfig)) {
	...
	importActivity: function(config, success, failure) {
		...
		var syncConfig = {
			...
			pageSize: config.isMain ? BPMSoft.AllRecords : this.self.ImportPageSize,
			pagesPerLoad: 1,
		...
		var operationConfig = {
			priority: config.isMain ? BPMSoft.SyncPriority.Normal : BPMSoft.SyncPriority.Minimum
		};
		if (!BPMSoft.Platform.isWebKit && BPMSoft.Features.getIsEnabled("UseMobileNativeImport")) {
			syncConfig.pageSize = BPMSoft.AllRecords;
			...
			BPMSoft.SyncManager.runNativeImport(syncConfig, operationConfig);
		} else {
			BPMSoft.SyncManager.runImport(syncConfig, operationConfig);
		}
	},
	...
	deleteUnexistingActivities: function(activityIds, activityFilters, callback) {
		...
		var sql = BPMSoft.Sql.DeleteBuilder.build({
			model: Activity,
	...
	importActivityDetails: function(config) {
		return new Promise(function(resolve, reject) {
			...
			for (var i = 0, ln = config.detailConfigs.length; i < ln; i++) {
				...
				specifiedFilters[modelName] = Ext.create("BPMSoft.Filter", {
	...
	deleteDefaultParticipants: function(activityIds) {
		var deleteSql = BPMSoft.Sql.DeleteBuilder.build({
			filter: Ext.create("BPMSoft.Filter", {
				type: BPMSoft.FilterTypes.Group,
				subfilters: [
					{
						property: "Activity",
					...
					{
						property: BPMSoft.SystemColumnName.ModifiedOn,
	...
	createDefaultParticipants: function(loadedActivityIds, callback) {
		var sqls = [];
		var saveQueryConfig = Ext.create("BPMSoft.QueryConfig", {
		...
		for (var i = 0, ln = loadedActivityIds.length; i < ln; i++) {
			...
			var sql = BPMSoft.Sql.InsertBuilder.build({
