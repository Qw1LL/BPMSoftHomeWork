define("SummarySettingsModule", ["ext-base", "BPMSoft", "sandbox", "SummarySettingsViewGenerator",
	"SummarySettingsViewModelGenerator", "SummarySettingsModuleResources",
	"MaskHelper", "ModalBox", "StructureExplorerUtilities"],
	function(Ext, BPMSoft, sandbox, SummarySettingsViewGenerator, SummarySettingsViewModelGenerator,
		...
		function addRow(container, functionCollection, rowsCollection, array, columnDataValueType) {
			...
			var view = Ext.create("BPMSoft.Container", SummarySettingsViewGenerator.generateRow(renderTo, key));
			viewModel.key = key;
		...
		function saveDataToProfile(rowsList, showCount) {
			...
			if (save) {
				...
				BPMSoft.saveUserProfile(profileKey, profile, false);
				sandbox.publish("BackHistoryState");
			}
			else {
				BPMSoft.showInformation(resources.localizableStrings.NotUniqueColumns);
			}
		}
		...
		function init() {
			if (!this.rowsCollection) {
				this.rowsCollection = new BPMSoft.Collection();
			}
		...
		function render(renderTo) {
			...
			if (viewModel) {
				...
				rowsCollection.each(function(item) {
					...
					var genView = Ext.create(generatedItemViewConfig.className ||
						"BPMSoft.Container", generatedItemViewConfig);
			...
			if (schemaName) {
				require([schemaName, "profile!" + profileKey], function(schema, profile) {
					viewModel = Ext.create("BPMSoft.BaseViewModel", SummarySettingsViewModelGenerator.generate());
					...
					var view = Ext.create(generatedViewConfig.className || "BPMSoft.Container", generatedViewConfig);
