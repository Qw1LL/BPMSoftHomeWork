define("GridSettingsDesignerContainer", ["GridSettingsDesignerContainerResources", "ConfigurationConstants", 
	...
	Ext.define("BPMSoft.controls.GridSettingsDesignerContainer", {
		alternateClassName: "BPMSoft.GridSettingsDesignerContainer",
		extend: "BPMSoft.GridLayoutTableEdit",
		 ...
		 * @type {String}
		 */
		itemClassName: "BPMSoft.GridSettingsDesignerItem",

		/**
		 * @inheritdoc BPMSoft.GridLayoutEdit#rows
		 * @overridden
		 */
		rows: 1,

		/**
		 * @inheritdoc BPMSoft.GridLayoutEdit#maxRows
		 * @overridden
		 */
		minRows: 1,
		...
		/**
		 * @inheritdoc BPMSoft.GridLayoutEdit#columns
		 * @overridden
		 */
		columns: 24,

		/**
		 * @inheritdoc BPMSoft.GridLayoutEdit#minColumns
		 * @overridden
		 */
		minColumns: 24,

		/**
		 * @inheritdoc BPMSoft.GridLayoutEdit#maxColumns
		 * @overridden
		 */
		maxColumns: 24,
		...
		/**
		 * @inheritdoc BPMSoft.GridLayoutEdit#itemActions
		 * @overridden
		 */
		itemActions: [
			{
		...
		/**
		 * @inheritdoc BPMSoft.GridLayoutEdit#fixedCellWidth
		 * @overridden
		 */
		fixedCellWidth: false,

		/**
		 * @inheritdoc BPMSoft.GridLayoutEdit#cellWidth
		 * @overridden
		 */
		cellWidth: 100 / 24,

		/**
		 * @inheritdoc BPMSoft.GridLayoutEdit#cellHeight
		 * @overridden
		 */
		cellHeight: 2.9,
		 ...
		 * @return {BPMSoft.BaseViewModel} Target item.
		 */
		 ...
		 * @return {BPMSoft.BaseViewModel} Nearest neighbour from right.
		 */
		 ...
		 * @private
		 * @param {BPMSoft.BaseViewModel} targetItem Drop zone item view model.
		 * @param {BPMSoft.GridSettingsDesignerItem} eventItem Drag&drop element.
		 */
		 ...
		 * @private
		 * @param {BPMSoft.BaseViewModel} destinationCellViewModel Destination column view model.
		 * @param {BPMSoft.BaseViewModel} sourceCellViewModel Source column view model.
		 */
		 ...
		 * @private
		 * @param {BPMSoft.BaseViewModel} destinationCellViewModel Destination cell view model.
		 * @param {BPMSoft.BaseViewModel} sourceCellViewModel Source column view model.
		 */
		 ...
		 * @private
		 * @param {Number} row Row index.
		 * @param {BPMSoft.BaseViewModelCollection} collection Items.
		 */
		 ...
		 * @private
		 * @param {BPMSoft.BaseViewModel} item1 Collection item.
		 * @param {BPMSoft.BaseViewModel} item2 Collection item.
		...
		_removeItemById: function(itemId) {
			...
			if (this.gridType === BPMSoft.GridType.LISTED) {
		...
		_canChangeColumn: function(itemId, msqTpl) {
			let canChange = true;
			if (BPMSoft.Features.getIsDisabled("UseColumnExpression")) {
				return canChange;
			}
			...
			BPMSoft.each(this.items, function(item) {
				...
				if (this._isColumnUsedInExpression(targetItem, columnExpression)) {
					...
					BPMSoft.showInformation(errorMsg);
					return false;
				}
			}, this);
			return canChange;
		 ...
		 * @private
		 * @param {BPMSoft.BaseViewModel} item Item viewModel.
		 ...
		 * @param {BPMSoft.BaseViewModel} elementViewModel Element viewmodel.
		...
		_getMaxColumns: function() {
			return this.gridType === BPMSoft.GridType.TILED ? this.maxColumnsTiled : this.maxColumns;
		},

		/**
		 * Returns flag, should revert item position after drop.
		 * @private
		 * @param {BPMSoft.BaseViewModel} targetItem Drop item view model.
		 * @param {BPMSoft.GridSettingsDesignerItem} eventItem Item, that fired event.
		 ...
		 * @private
		 * @param {BPMSoft.BaseViewModel} item Column setting item view model.
		 * @return {Bollean}
		 */
		 ...
		 * @private
		 * @param {BPMSoft.BaseViewModel} item Added item view model.
		 ...
		 * @private
		 * @param {BPMSoft.BaseViewModel} item Item view model.
		...
		/**
		 * @inheritdoc BPMSoft.GridLayoutEdit#getCellWidth
		 * @overridden
		 */
		...
		init: function() {
			...
			this._debounceOnScrollValueChanged = BPMSoft.debounce(this.onScrollValueChanged, 100);
			this.addEvents(
				/**
		...
		/**
		 * @inheritdoc BPMSoft.GridLayoutEdit#onItemsLoad
		 * @overridden
		 */
		onItemsLoad: function(collection) {
			...
			if (this.gridType === BPMSoft.GridType.TILED) {
		...
		/**
		 * @inheritdoc BPMSoft.GridLayoutEdit#onItemAdd
		 * @overridden
		 */
		onItemAdd: function(item, index, key) {
			...
			if (this.itemActions) {
				itemBindingConfig.items = BPMSoft.deepClone(this.itemActions);
			}
		...
		scrollToItem: function(item) {
			...
			const gridEl = this.getGridEl();
			if (BPMSoft.getIsRtlMode()) {
				const scrollTypeInRtl = BPMSoft.getScrollTypeInRtl();
				const scrollWidth = gridEl.dom.scrollWidth - gridEl.dom.clientWidth;
				const columnDistance = this._getColumnScrollDistance(item);
				if (scrollTypeInRtl === BPMSoft.ScrollTypeInRtl.DEFAULT) {
					scrollPosition = scrollWidth - columnDistance;
				}
				if (scrollTypeInRtl === BPMSoft.ScrollTypeInRtl.REVERSE) {
					scrollPosition = columnDistance;
				}
				if (scrollTypeInRtl === BPMSoft.ScrollTypeInRtl.NEGATIVE) {
					scrollPosition = -columnDistance;
				}
			}
		 ...
		 * @param {BPMSoft.GridType} gridType Grid type.
		 */
		...
		/**
		 * @inheritdoc BPMSoft.GridLayoutEdit#setColumns
		 * @overridden
		 */
		 ...
		 * @param {BPMSoft.GridLayoutTableEditItem} resizeItem Resize item.
		...
		/**
		 * @inheritdoc BPMSoft.GridLayoutEdit#updateSize
		 * @overridden
		 */
		updateSize: function() {
			if (this.isCollectionLoading) {
