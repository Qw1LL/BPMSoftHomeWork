define("ImageSectionWizardCustomGenerator", ["ext-base", "BPMSoft", "ImageCustomGeneratorV2"],
	function(Ext, BPMSoft) {
		Ext.define("BPMSoft.configuration.ImageSectionWizardCustomGenerator", {
			alternateClassName: "BPMSoft.ImageSectionWizardCustomGenerator",
			extend: "BPMSoft.ImageCustomGenerator",
			...
			prepareCustomImage: function(imageControl) {
				return this.removeExtraParameters(BPMSoft.deepClone(imageControl), ["labelConfig"]);
			},
			...
			prepareCustomLabel: function(label) {
				return this.removeExtraParameters(BPMSoft.deepClone(label), ["defaultImageSrc", "generator",
			...
			generateControlWrapper: function(config, imageControl, label) {
				var wrapperId = this.getControlId(config, "BPMSoft.Container");
		...
		return Ext.create("BPMSoft.ImageSectionWizardCustomGenerator");
	});
