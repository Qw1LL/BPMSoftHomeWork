/**
 * @class BPMSoft.configuration.ProcessSchemaUserTaskUtilities
 * ProcessSchemaUserTaskUtilities class.
 */
define("ProcessSchemaUserTaskUtilities", ["ext-base", "BPMSoft", "ProcessUserTaskConstants",
	"ProcessSchemaUserTaskUtilitiesResources", "Activity"],
	function(Ext, BPMSoft, ProcessUserTaskConstants, resources, activityEntitySchema) {
		const definedClass = Ext.ClassManager.get("BPMSoft.ProcessSchemaUserTaskUtilities");
		if (definedClass) {
			return Ext.create(definedClass);
		}
		const processSchemaUserTaskUtilities = Ext.define("BPMSoft.configuration.ProcessSchemaUserTaskUtilities", {
			extend: "BPMSoft.BaseObject",
			alternateClassName: "BPMSoft.ProcessSchemaUserTaskUtilities",
			...
			findReadDataUserTaskSchemaResultInfo: function(element) {
				...
				if (resultParameter) {
					...
					if (resultParameter.dataValueType === BPMSoft.DataValueType.ENTITY) {
			...
			getReadDataUserTaskSchemaResultParametersInfo: function(element, callback, scope) {
				...
				const aggregationType = BPMSoft.convertToServerAggregationType(BPMSoft.AggregationType.COUNT);
				...
				BPMSoft.EntitySchemaManager.findBundleSchemaInstance(config, function(schema) {
					...
					if (dataValueType === BPMSoft.DataValueType.INTEGER) {
						resultParameter = element.findParameterByName("ResultIntegerFunction");
					} else if (BPMSoft.isNumberDataValueType(dataValueType)) {
						resultParameter = element.findParameterByName("ResultFloatFunction");
					} else if (BPMSoft.isDateDataValueType(dataValueType)) {
			...
			_getObjectFileProcessingUserTaskResultParametersInfo: function(element, callback, scope) {
				...
				if (resultActionType && resultActionType.getValue() === BPMSoft.ProcessUserTaskConstants
			 ...
			 * @param {Array|BPMSoft.Collection} parameters Parameters of the element.
			...
			findResultParameter: function(parameters) {
				...
				let resultParameter = null;
				const items = parameters instanceof BPMSoft.Collection ? parameters.getItems() : parameters;
			...
			_getEntityConnectionSchemaQuery: function() {
				...
				esq.filters.addItem(esq.createColumnFilterWithParameter(
					BPMSoft.ComparisonType.EQUAL, "SysEntitySchemaUId", activityEntitySchema.uId));
				return esq;
			},
			...
			loadEntityConnectionColumns: function(callback, scope) {
				const entityConnectionColumns = Ext.create("BPMSoft.Collection");
			...
			_removeTechnicalActivityParameters: function(resultParameters, callback, scope) {
				this.loadEntityConnectionColumns(function(columns) {
					resultParameters = BPMSoft.filter(resultParameters, function(parameter) {
						if (BPMSoft.contains(this._activityParameterNames, parameter.name)) {
							return false;
						}
			 ...
			 * @param {BPMSoft.Collection} collection of items.
			...
			validateRowsCountRange: function(value) {
				let invalidMessage;
				const minMessage = BPMSoft.Resources.BaseViewModel.columnIncorrectMinNumberValidationMessage;
				const maxMessage = BPMSoft.Resources.BaseViewModel.columnIncorrectMaxNumberValidationMessage;
				const minValue = 1;
				const maxValue = 5000;
				if (value > maxValue) {
			...
			validateSelectValue: function(value) {
				let isValid = true;
				let invalidMessage = "";
				if (BPMSoft.isEmptyObject(value) || Ext.isEmpty(value.value)) {
					isValid = false;
