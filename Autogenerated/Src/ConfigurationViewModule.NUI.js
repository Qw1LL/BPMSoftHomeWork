define("ConfigurationViewModule", [
			"ext-base", "sandbox", "BPMSoft", "ConfigurationViewModuleResources", "LookupUtilities",
		...
		function(Ext, sandbox, BPMSoft, resources, LookupUtilities,
				ViewModuleHelper, ConfigurationConstants, LeftPanelUtilities, ModalBox) {

			/**
			 * @class BPMSoft.configuration.DemoButtons
			 * Class creates demo buttons.
			 */
			Ext.define("BPMSoft.configuration.DemoButtons", {
				extend: "BPMSoft.BaseViewModule",
				alternateClassName: "BPMSoft.DemoButtons",
				...
				createOnlineHelpButtonParameters: function() {
					...
					BPMSoft.each(parameters, function(parameter, name) {
				...
				createDemoButton: function(config) {
					...
					var element = Ext.create("BPMSoft.Button", {
				...
				initButtons: function() {
					BPMSoft.chain(
							...
							function(next) {
								BPMSoft.each(this.buttonsConfig, this.createDemoButton, this);
								next();
							},
							function(next) {
				...
				prepareButtonsConfig: function(callback) {
					...
					BPMSoft.each(this.buttonsSysSettingConfig, function(item) {
						BPMSoft.chain(
								function(next) {
									BPMSoft.SysSettings.querySysSettingsItem(item.sysSetting, function(value) {
										next(value);
									});
								},
								function(next, value) {
				...
				updateButtonsPosition: function() {
					...
					BPMSoft.each(buttons, function(button) {
			 ...
			 */
			var viewModule = Ext.define("BPMSoft.configuration.ConfigurationViewModule", {
				extend: "BPMSoft.BaseViewModule",
				alternateClassName: "BPMSoft.ConfigurationViewModule",
				...
				/**
				 * Demo buttons.
				 * @type {BPMSoft.configuration.DemoButtons}
				 */
				demoButtons: null,
				...
				prepareDemoLinkButtons: function() {
					this.demoButtons = this.Ext.create("BPMSoft.DemoButtons");
				...
				isNeedToShowReadOnlyButton: function() {
					return this.BPMSoft.CurrentUser.userType === this.BPMSoft.UserType.SSP &&
						this.BPMSoft.isSspInReadonlyMode;
				},
				...
				/**
				 * @inheritdoc BPMSoft.BaseSchemaModule#getMessages
				 * @protected
				 * @overridden
				 */
				...
				/**
				 * @inheritDoc BPMSoft.configuration.BaseViewModule#loadNonVisibleModules
				 * @overridden
				 */
				...
				/**
				 * @inheritDoc BPMSoft.configuration.BaseViewModule#subscribeMessages
				 * @overridden
				 */
				...
				initIntroPage: function() {
					this.defaultIntroPage = BPMSoft.configuration.defaultIntroPageName || this.defaultIntroPage;
				},
				...
				onShowHideRightPanel: function(config) {
					...
					if (Ext.isEmpty(mainHeaderEl) || Ext.isEmpty(centerPanelContainerEl) || Ext.isEmpty(communicationPanelEl)) {
						BPMSoft.delay(this.onShowHideRightPanel, this, 500, [config]);
						return;
					}
				...
				/**
				 * @inheritDoc BPMSoft.configuration.BaseViewModule#onLoadModule
				 * @overridden
				 */
				...
				loadProcessModule: function() {
					...
					var sysWorkspaceFilter = BPMSoft.createColumnFilterWithParameter(
					...
					var isMaxVersionFilter = BPMSoft.createColumnFilterWithParameter(
							BPMSoft.ComparisonType.EQUAL, "IsMaxVersion", 1);
				...
				loadWelcomeScreen: function() {
					var isFirstLogin = this.BPMSoft.isFirstLogin;
					if (!isFirstLogin) {
						return;
					}
					this.BPMSoft.SysSettings.querySysSettings([
						...
						if (buildType.value === ConfigurationConstants.BuildType.Public) {
							...
							this.BPMSoft.require(["profile!WelcomeScreenModule"], function(profile) {
								if (profile && profile.isShown) {
									return;
								}
								this.loadWelcomeScreenModule();
								this.BPMSoft.saveUserProfile("WelcomeScreenModule", {isShown: true});
							}, this);
						}
					}, this);
				},
				...
				/**
				 * @inheritDoc BPMSoft.configuration.BaseViewModule#getHomeModulePath
				 * @overridden
				 */
				getHomeModulePath: function() {
					return this.BPMSoft.combinePath(this.homeModule, this.defaultIntroPage);
				}
			});
			return viewModule;
