define('SchemaDesignerModule',
	['ext-base', 'BPMSoft', 'sandbox', 'EditPageDesignerViewGenerator', 'SchemaDesignerModuleResources',
		'MaskHelper', 'ConfigurationEnums', 'SectionDesignDataModule'],
	function(Ext, BPMSoft, sandbox, designerViewGenerator, resources, maskHelper, configurationEnums, designData) {
		...
		function render(renderTo) {
			...
			BPMSoft.chain({}, [
				...
				function(context) {
					...
					designViewModel = Ext.create('BPMSoft.SchemaDesignerViewModel', {
						values: {												// TODO: create VM columns
							NewColumns: new BPMSoft.Collection(),				// TODO: ##### ## ###, #### ## ###### ####### ########
							schema: designSchema,
		...
		function createDesignSchema(schemaConfig) {
			...
			var schema = BPMSoft.deepClone(schemaConfig);
		...
		function setSchemaContainerIds(itemsContainer) {
			BPMSoft.each(itemsContainer, function(schemaItem) {
				if (schemaItem instanceof Array) {
					...
					if (Ext.isEmpty(schemaItem.EditStructureContainerId)) {
						...
						schemaItem.EditStructureContainerId = BPMSoft.generateGUID();
					}
					if (schemaItem.items) {
		...
		function createDesignViewModelClass() {
			...
			Ext.define('BPMSoft.Configuration.SchemaDesignerViewModel', {
				extend: 'BPMSoft.BaseViewModel',
				alternateClassName: 'BPMSoft.SchemaDesignerViewModel',
				...
				isActionVisible: function(tag) {
					...
					if (schemaItem.type === BPMSoft.ViewModelSchemaItem.DETAIL && tag === 'edit') {
						actionVisible = false;
					}
				...
				onActionItemClick: function(param1, param2, param3, tag) {
					switch (tag) {
						case 'upField':
							//this.upField();
							var schemaItem = BPMSoft.deepClone(this.getCurrentColumnInfo());
				...
				moveSchemaItemUp: function(schemaItem) {
					...
					if (newItemVisibleIndex >= 0) {
						...
						if (prevVisibleSchemaItem.type === BPMSoft.ViewModelSchemaItem.GROUP) {
				...
				moveSchemaItemDown: function(schemaItem) {
					...
					if (nextVisibleSchemaItemInfo) {
						...
						if (nextVisibleSchemaItem.type === BPMSoft.ViewModelSchemaItem.GROUP) {
				...
				addGroup: function() {
					var callback = function(returnCode, controlData) {
						if (returnCode === 'ok' && controlData.name.value) {
							...
							var editStructureContainerId = BPMSoft.generateGUID();
				...
				addNewColumn: function(tag) {
					...
					var handler = function(args) {
						...
						var schemaItemConfig = BPMSoft.deepClone(args.itemConfig);
				...
				editColumn: function() {
					...
					if (this.isContainer(item)) {
						var callback = function(returnCode, controlData) {
							if (returnCode === 'ok' && controlData.name.value) {
								...
								this.modifySchemaItem(item.name, BPMSoft.deepClone(columnConfig));
								/*
								this.insertModification({
									type: 'modify',
									value: BPMSoft.deepClone(columnConfig),
					...
					if (this.isColumn(item)) {
						...
						var handler = function(args) {
							...
							this.modifySchemaItem(prevName, BPMSoft.deepClone(args.itemConfig));
							/*this.insertModification({
								type: 'modify',
				...
				findSchemaItemIndexByName: function(config) {
					...
					var itemIndex = 0;
					var visibleIndex = 0;
					BPMSoft.each(items, function(schemaItem) {
