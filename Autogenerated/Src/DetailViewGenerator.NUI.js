define('DetailViewGenerator', ['ext-base', 'BPMSoft', 'DetailViewGeneratorResources', 'GridUtilities'],
	...
	function getGridConfig(fullViewModelSchema, isTiled) {
		...
		var config = {
			id: entitySchema.instanceId + '-' + BPMSoft.generateGUID() + '-grid',
	...
	function getEditPages(fullViewModelSchema) {
		var config = BPMSoft.configuration.EntityStructure[fullViewModelSchema.entitySchema.name];
	...
	function GenerateUnitsButtonConfig(fullViewModelSchema, columnsSettingsProfile) {
		...
		if (actions && Ext.isArray(actions)) {
			...
			utilsCollection.push(getContainerConfig(BPMSoft.generateGUID(), ['clear-both']));
			return utilsCollection;
		}
		...
		if (fullViewModelSchema.modifyUtilsButton && Ext.isFunction(fullViewModelSchema.modifyUtilsButton)) {
			utilsButton = fullViewModelSchema.modifyUtilsButton(BPMSoft.deepClone(utilsButton));
		}
		utilsCollection.unshift(utilsButton);
		return utilsCollection;
	}
