define("ServiceOAuthAuthenticatorEndpointHelper", ["ServiceOAuthAuthenticatorEndpointHelperResources"],
	function(resources) {
		Ext.define("BPMSoft.ServiceOAuthAuthenticatorEndpointHelper", {
			singleton: true,
			...
			_getOAuthServiceClientSecretUrl: function(OAuthAppId) {
				return BPMSoft.workspaceBaseUrl +
			...
			onLoggedIn: function(config) {
				const handler = function(scope, message) {
					...
					if (isEndpointMessage) {
						...
						if (!response.success) {
							...
							BPMSoft.utils.showInformation(msg, null, this, { useHtmlContent: true });
				...
				BPMSoft.ServerChannel.on(BPMSoft.EventName.ON_MESSAGE, handler, config.scope);
			},
			...
			validateApplication: function(appId, callback, scope) {
				...
				select.addFunctionColumn("ClientId", BPMSoft.FunctionType.LENGTH, "ClientIdLen");
				select.addFunctionColumn("Name", BPMSoft.FunctionType.LENGTH, "NameLen");
				select.addFunctionColumn("AuthorizeUrl", BPMSoft.FunctionType.LENGTH, "AuthorizeUrlLen");
				select.addFunctionColumn("TokenUrl", BPMSoft.FunctionType.LENGTH, "TokenUrlLen");
				select.filters.logicalOperation = BPMSoft.LogicalOperatorType.AND;
		...
		return BPMSoft.ServiceOAuthAuthenticatorEndpointHelper;
	});
