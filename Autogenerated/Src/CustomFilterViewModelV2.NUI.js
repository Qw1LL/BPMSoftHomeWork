define("CustomFilterViewModelV2", ["ext-base", "BPMSoft", "CustomFilterViewModelV2Resources", "ConfigurationEnums",
		"CustomFilterViewV2", "QuickFilterViewV2Resources", "ConfigurationConstants", "SearchableTextEdit",
		"FolderFilterItemViewModel"],
	function(Ext, BPMSoft, resources, ConfigurationEnums, CustomFilterView, quickFilterViewResources,
		...
		function generate(config) {
			var values = {
				...
				lookupValueColumnList: new BPMSoft.Collection(),
				...
				ActiveFolder: null,
				ActionsMenu: Ext.create("BPMSoft.BaseViewModelCollection"),
				IsSeparateMode: true,
			...
			var viewModelConfig = {
				...
				methods: {
					...
					createActiveFolderFilter: function(callback, scope) {
						...
						if (Ext.isEmpty(folderFilter)) {
							...
							this.requireFolderSchema(folderFilter.sectionEntityScheamName, function(entityFolderSchema) {
								var activeFolder = Ext.create("BPMSoft.FolderFilterItemViewModel", {
									entitySchema: entityFolderSchema,
									columns: BPMSoft.deepClone(entityFolderSchema.columns),
									isNew: false
								});
								activeFolder.set(entityFolderSchema.primaryColumnName, BPMSoft.generateGUID());
								...
								Ext.callback(callback, scope, [{
									folder: activeFolder,
									filter: BPMSoft.deserialize(folderFilter.filter)
								}]);
							}, this);
						}
					},
					...
					requireFolderSchema: function(rootSchemaName, callback, scope) {
						...
						BPMSoft.require([folderSchemaName], function(entityFolderSchema) {
					...
					_setFilterValue: function(simpleFilter, filterValue, useValidate, dataValueType) {
						var useNewLookupComparison = BPMSoft.Features.getIsEnabled("UseNewLookupComparison");
					...
					getFilterDisplayValue: function(filterColumn, filterValue) {
						...
						switch (dataValueType) {
							...
							case BPMSoft.DataValueType.BOOLEAN:
								...
								break;
							default:
								displayValue = BPMSoft.getTypedStringValue(
					...
					_getLookupColumnDisplayValue: function(filterValue) {
						var displayValue;
						if (BPMSoft.Features.getIsEnabled("UseNewLookupComparison")) {
					...
					_getDateColumnDisplayValue: function(filterValue) {
						...
						displayValue = BPMSoft.getTypedStringValue(date, BPMSoft.DataValueType.DATE);
						return displayValue;
					},
					...
					getSimpleFilter: function() {
						...
						var equalComparisonType = BPMSoft.ComparisonType.EQUAL;
						...
						switch (dataValueType) {
							...
							case BPMSoft.DataValueType.ENUM:
								if (BPMSoft.Features.getIsEnabled("UseNewLookupComparison")) {
									var columnPath = BPMSoft.isGUID(value) ? columnName : columnName + "." +
										this.get("referenceSchemaPrimaryDisplayColumnName");
									var comparisonType = BPMSoft.isGUID(value) ? equalComparisonType :
					...
					editSimpleFilter: function(editedFilterName) {
						...
						if (editedFilter) {
							...
							if (currentDataValueType === BPMSoft.DataValueType.BOOLEAN) {
					...
					getLookupValueColumnList: function(filters, list, callback, scope) {
						...
						var esq = Ext.create(BPMSoft.EntitySchemaQuery, queryConfig);
						if (BPMSoft.Features.getIsEnabled("UseNewLookupComparison")) {
					...
					addFilterValue: function(filteredColumnName, filterValue) {
						...
						if (column) {
							...
							if (column.dataValueType === BPMSoft.DataValueType.LOOKUP ||
									column.dataValueType === BPMSoft.DataValueType.BOOLEAN) {
								if (BPMSoft.Features.getIsEnabled("UseNewLookupComparison")) {
									value = filterValue;
								} else {
		...
		function updateFiltersOnReturn() {
			...
			if (this.get("filters") !== null) {
				...
				var filterNames = BPMSoft.deepClone(filters.getKeys());
				var scope = this;
				BPMSoft.each(filterNames, function(filterName) {
			...
			select.execute(function(response) {
				if (response && response.success) {
					response.collection.each(function(folderItem) {
						...
						var menuItem = Ext.create("BPMSoft.BaseViewModel", itemConfig);
		...
		function selectFavoriteFolders(folderEntitySchemaName, folderSchemaUId) {
			var select = Ext.create("BPMSoft.EntitySchemaQuery", {
				...
				rowViewModelClassName: "BPMSoft.FolderFilterItemViewModel"
			});
			...
			if (!isShortFilterVisible) {
				var filterMenuItems = [
					...
					},
					{
						Id: BPMSoft.generateGUID(),
						Caption: "",
						Type: BPMSoft.MenuItemType.SEPARATOR
					},
					{
						Id: BPMSoft.generateGUID(),
				...
				BPMSoft.each(filterMenuItems, function(items) {
					var itemConfig = {
						values: items
					};
					var menuItem = Ext.create("BPMSoft.BaseViewModel", itemConfig);
		...
		function setCustomFilters(filtersConfig) {
			var filters = this.get("filters");
			filters.clear();
			BPMSoft.each(filtersConfig, function(filterConfig) {
		...
		function isColumnDeprecated(column) {
			...
			if (allowedColumns && column && column.uId) {
				var isColumnNotInAllowedColumns = Ext.isEmpty(
					BPMSoft.findWhere(allowedColumns, {uId: column.uId}));
		...
		function getSimpleFilterColumnList(filters, list) {
			...
			var filteredColumnNames = this.filterColumns(this.entitySchema.uId, columnNames);
		...
		function getFilterValue(filterName) {
			...
			filters.each(function(filterInfo) {
				...
				if (!filterName || currentFilterName === filterName) {
					...
					} else if (filterInfo.get("dataValueType") === BPMSoft.DataValueType.DATE ||
						filterInfo.get("dataValueType") === BPMSoft.DataValueType.DATE_TIME) {
		...
		function showSimpleFilterEdit(valueEditConfig, afterDestroy, renderIndex) {
			...
			addConditionView = Ext.create("BPMSoft.Container", addConditionViewConfig);
		...
		function getFilterViewModel(filterName, config) {
			var Ext = this.getExt();
			var filterViewModel = Ext.create("BPMSoft.BaseViewModel", config);
			...
			var filterView = Ext.create("BPMSoft.Container", this.generateFilterViewConfig(filterName));
						...
						items: [
							...
							},
							{
								...
								caption: {
									...
									bindConfig: {
										converter: function(x) {
											if (x && BPMSoft.Features.getIsEnabled("UseNewLookupComparison")) {
												if (BPMSoft.isGUID(x)) {
													return resources.localizableStrings.EqualComparisonLabel;
												} else if (BPMSoft.isJsonObject(x)) {
													return "";
												} else {
													var comparisonType = BPMSoft.SysSettings
		...
		function getFilterView(filterViewModel, filterName) {
			var Ext = this.getExt();
			var customFiltersViewsContainer = Ext.get(this.get("customFilterContainerName"));
			var filterView = Ext.create("BPMSoft.Container",
		...
		function showFolderManager(cfg) {
			...
			var config = {isFoldersContainerExpanded: this.get("isFoldersContainerExpanded")};
			var activeFolder = this.getFolderFilter();
			this.set("activeFolder", activeFolder);
			if (activeFolder) {
				config = Ext.apply(config, {activeFolderId: activeFolder.value});
				this.set("activeFolder", null);
			}
