 ...
 */
define("BasePageV2", ["performancecountermanager", "ChangeLogUtilities", "BroadcastChannelServiceUtility",
...
], function(performanceManager, changeLogUtilities, broadcastService) {
	return {
		mixins: {
			DelayExecutionUtilities: "BPMSoft.DelayExecutionUtilities",
			RecommendationModuleUtilities: "BPMSoft.RecommendationModuleUtilities",
			...
			QuickAddMixin: "BPMSoft.QuickAddMixin",
			...
			EntityRelatedColumnsMixin: "BPMSoft.EntityRelatedColumnsMixin",
			DcmPageMixin: "BPMSoft.DcmPageMixin",
		...
		messages: {
			 ...
			 * @param {BPMSoft.ConfigurationEnums.CardOperation} config.operation Operation type.
		...
		methods: {
			...
			/**
			 * @inheritdoc BPMSoft.BaseSchemaViewModel#initOnRestored
			 * @overridden
			 */
			...
			getMasterDisplayValue: function() {
				...
				this.BPMSoft.each(defaultValues, function(defaultValue) {
			 ...
			 * @protected
			 * @param {BPMSoft.BaseViewModelCollection} viewOptions Menu items of the button "View".
			 */
			...
			addSectionDesignerViewOptions: function(viewOptions) {
				this.addSectionWizardMenuItem(viewOptions, this.getButtonMenuItem({
					...
					"Tag": {"pageName": BPMSoft.SectionWizardEnums.PageName.PAGE_DESIGNER},
			...
			callServiceMethod: function(serviceName, methodName, callback, data, scope) {
				var obsoleteMessage = this.BPMSoft.Resources.ObsoleteMessages.ObsoleteMethodMessage;
			...
			getDefaultValues: function() {
				...
				this.set("DefaultValues", this.BPMSoft.deepClone(defaultValues));
			...
			getDefaultValueByName: function(valueName) {
				...
				var defaultValue = BPMSoft.findItem(defaultValues, {name: valueName});
				if (!defaultValue) {
					return null;
				}
			...
			initEntity: function(callback, scope) {
				...
				switch (operation) {
					case BPMSoft.ConfigurationEnums.CardOperation.ADD:
						this._initEntityAdd(callback, scope);
						break;
					case BPMSoft.ConfigurationEnums.CardOperation.EDIT:
						this._initEntityEdit(callback, scope);
						break;
					case BPMSoft.ConfigurationEnums.CardOperation.COPY:
			...
			/**
			 * @inheritdoc BPMSoft.BaseViewModel#isChanged
			 * @override
			 */
			isChanged: function() {
			 ...
			 */
			isAddMode: function() {
				return this.get("Operation") === BPMSoft.ConfigurationEnums.CardOperation.ADD;
			},
			 ...
			 */
			isEditMode: function() {
				return this.get("Operation") === BPMSoft.ConfigurationEnums.CardOperation.EDIT;
			},
			 ...
			 */
			isCopyMode: function() {
				return this.get("Operation") === BPMSoft.ConfigurationEnums.CardOperation.COPY;
			},
			 ...
			 * @return {BPMSoft.FilterGroup} Group of the filters for reports.
			 */
			getReportFilters: function() {
				var filters = this.Ext.create("BPMSoft.FilterGroup");
				filters.name = "primaryColumnFilter";
				var filter = this.BPMSoft.createColumnInFilterWithParameters(this.entitySchema.primaryColumnName,
			...
			_getActionsDashboardModuleConfig: function() {
				return BPMSoft.get(this, "modules.ActionsDashboardModule.config.parameters.viewModelConfig") || {};
			},
			...
			initHasAnyDcm: function(callback, scope) {
				...
				BPMSoft.chain(
					function(next) {
						BPMSoft.DcmUtilities.hasAnyDcmSchema(this, next, this);
					},
					function(next, hasAnyDcmSchema) {
						...
						if (hasAnyDcm || config.actionsConfig || config.dashboardConfig) {
							...
							BPMSoft.ActionsDashboardUtils.getTabPanelCollapsed(entitySchemaName, next, this);
						} else {
							Ext.callback(callback, scope);
						}
					},
			...
			_showActionsDashboardMask: function() {
				...
				var config = this._getActionsDashboardModuleConfig();
				if (hasAnyDcm || config.actionsConfig || config.dashboardConfig) {
					BPMSoft.ActionsDashboardUtils.showMask({
						showSpinnerEl: false,
			 ...
			 * @private
			 */
			_debouncedInitRunProcess: BPMSoft.emptyFn,

			/**
			 * @private
			 */
			_prepareRunProcessMenu: function() {
				if (this._debouncedInitRunProcess === BPMSoft.emptyFn) {
					this._debouncedInitRunProcess = BPMSoft.debounce(this.initRunProcessButtonMenu.bind(this), 1000);
				}
			...
			init: function(callback, scope) {
				this.AngularRenderItemsCollection = [];
				this.IsAngularModulesLoaded = false;
				...
				this.callParent([function() {
					...
					BPMSoft.chain(
						...
						function(next) {
							...
							if (BPMSoft.Features.getIsEnabled("DcmOptimizationFeature")) {
			...
			resetBodyAttributes: function() {
				BPMSoft.utils.dom.setAttributeToBody(this.get("IsCardOpenedAttribute"), false);
				BPMSoft.utils.dom.setAttributeToBody(this.get("IsMainHeaderVisibleAttribute"), true);
			},
			...
			onRender: function() {
				...
				this.callParent(arguments);
				if (this.getIsFeatureDisabled("OldUI")) {
					BPMSoft.utils.dom.setAttributeToBody(this.get("IsCardOpenedAttribute"), this.get("IsPageHeaderVisible"));
					var sectionContainer = Ext.get("SectionContainer");
					var value = false;
					if (sectionContainer && !sectionContainer.hasCls("display-none")) {
						value = true;
					}
					BPMSoft.utils.dom.setAttributeToBody(this.get("IsMainHeaderVisibleAttribute"), value);
			...
			setEmptyClasses: function () {
				const emptyContainerClassName = 'empty-container';
				const containers =
					document.body.querySelectorAll('#CardContentContainer.center-main-container > [id$="Container"]');
				...
				containers.forEach(container => {
					if (container.hasChildNodes())
						container.classList.remove(emptyContainerClassName);
					else
						container.classList.add(emptyContainerClassName);
				});
			},
			...
			/**
			 * @inheritdoc BPMSoft.BaseEntityPage#getUpdateDetailConfig
			 */
			...
			getSchemaName: function () {	
				var entityInfo = this.onGetEntityInfo();
				if (entityInfo) {
					var entitySchemaNameUId = entityInfo.entitySchemaUId;
					return entitySchemaNameUId;
				}
			},

			getAngularComponent: function(callback) {
				// const schemaName = this.getSchemaName();
				var select = Ext.create("BPMSoft.EntitySchemaQuery", {
					rootSchemaName: "AppWrapper",
				});
				select.addColumn("FileName");
				select.addColumn("Content");
				select.addColumn("PageSchemaUId");
				// if (schemaName) {
				// 	select.filters.addItem(this.BPMSoft.createColumnFilterWithParameter(
				// 	this.BPMSoft.ComparisonType.EQUAL, "[SysSchema:UId:PageSchemaUId].UId", schemaName));
				// }

				select.getEntityCollection(function(response) {
					if (response.success && response.collection.getCount() > 0) {
						var collection = response.collection;
							this.set('AngularModules', collection);
								let objects = this.get('AngularModules');
								console.log(this.get('AngularModules'))
								console.log('AngularModules')
								if (objects) {
									this.insertAngularComponent(objects.collection.items);								
								  }
			...
			findAngularContainers: function () {
				var containers = document.querySelectorAll('.angular-grid-layout-item');
				var markerValues = [];
				containers.forEach(function(container) {
				var component = Ext.getCmp(container.id);
   			    
				if (component && component.ownerCt && component.ownerCt.markerValue) {
   			         var classes = component.classes.wrapClassName;

   			         var tabNameClass = classes.find(function(className) {
   			             return className.startsWith("tabName-");
   			         });

   			         var tabName = tabNameClass ? tabNameClass.split('-')[1] : null;
				 
   			         markerValues.push({
   			             component: component, 
   			             layoutName: component.ownerCt.markerValue,
   			             tabName: tabName
   			         });
   			     }
   			 });
			...
			insertAngularComponent: function(items) {
				var angularContainers = this.findAngularContainers();
				if (this.AngularItemsOnDestroyArray) {
					for (let i = 0; i < this.AngularItemsOnDestroyArray.length; i++) {
						broadcastService.sendMessageToAngularByChannel("destroy",this.AngularItemsOnDestroyArray[i].values.FileName.toLowerCase());
					}
				}
				
				this.AngularItemsOnDestroyArray = [];
				for (let i = 0; i < items.length; i++) {
					const item = items[i];
					const containerId = item.$FileName;
					const containerEl = document.getElementById(containerId);
					const angularSelector = containerId;
					const containerInfo = angularContainers.find(c => c.component.id === containerId);
					const layoutName = containerInfo ? containerInfo.layoutName: null;
					
					if (containerEl) {
						this.AngularRenderItemsCollection.push(angularSelector);
						
						if (layoutName && (layoutName !== 'Header' && layoutName !== 'ProfileContainer')) {
							this.AngularItemsOnDestroyArray.push(item);
						}
					
						try {
							const angularElement = document.createElement(angularSelector);
							containerEl.appendChild(angularElement);
							
							const scriptEl = document.createElement('script');
							scriptEl.type = 'text/javascript';
							scriptEl.innerHTML = item.$Content;
							scriptEl.async = true;
							containerEl.appendChild(scriptEl);
							
						} catch (error) {
							console.error(`Error while inserting Angular component for container ${containerId}:`, error);
						}
					} else {
						console.warn('Container with ID:', containerId, 'not found or Angular component already added!');
					}
				}
				this.IsAngularModulesLoaded = true;
			...
			subscribeSandboxEvents: function() {
				...
				var openCardSubscribers = this.BPMSoft.deepClone(subscribersIds);
				openCardSubscribers.push(this.getLookupModuleId());
				openCardSubscribers = openCardSubscribers.concat(moduleIds);
				var saveRecordSubscribers = this.BPMSoft.deepClone(subscribersIds);
				...
				sandbox.subscribe("OnAngularValueChange",this.entityColumnChangedAngularAdapter, this, ['angularSandboxParams']);
				sandbox.subscribe("SendToAngularData",this.sendToAngularColumnValueByValue, this, ['angularSandboxParams']);
				...
				sandbox.subscribe("AngularMessage", this.getAngularMessage, this, ['angularSandboxParams']);
			...
			getAngularMessage: function(config) {
				//Здесь получаем конфиг из sandbox AngularMessage
				console.log(config);
				console.log('config from Angular data');
			},
			...
			onUpdateParentLookupDisplayValue: function(config) {
					BPMSoft.each(this.columns, function(column) {
			...
			getParameters: function(parameters) {
				...
				BPMSoft.each(parameters, function(value) {
			...
			setParameters: function(parameters) {
				...
				BPMSoft.each(parameters, function(value, key) {
					this.set(key, value);
				}, this);
			},
			...
			insertAngularScript: function(htmlItel,modules){
				const item = modules.find(el => el.values.FileName === htmlItel.getAttribute('id'));
				const containerEl = document.getElementById(htmlItel.getAttribute('id'));
				const angularElement = document.createElement(htmlItel.getAttribute('id'));
				containerEl.appendChild(angularElement);
			...
			businessRuleAngularModuleReRender: function() {
				if(this.IsAngularModulesLoaded ) {
					const htmlCollection = document.querySelectorAll(".angular-grid-layout-item");
					const modules = this.get('AngularModules').collection.items;

					for(const htmlItel of htmlCollection) {
						if(!this.AngularRenderItemsCollection.includes(htmlItel.getAttribute('id')) && htmlItel.children.length === 0) {
							try {
								this.insertAngularScript(htmlItel,modules);
							
							} catch (error) {
								console.error(`Error while inserting Angular component`, error);
							}
						}
					}
				}
			...
			entityColumnChanged: function(columnName, columnValue) {
				this.businessRuleAngularModuleReRender();

				const replacer = this.getCircularReplacer();
				broadcastService.sendMessageToAngularByChannel(JSON.stringify({
			...
			entityColumnChangedAngularAdapter: function(value) {
				this.set(value.columnName, value.columnValue);
				this.entityColumnChanged(value.columnName, value.columnValue);
			},

			sendToAngularColumnValueByValue: function(name) {
				let typeName = '';
				let value = '';
				if (name) {
					// получаем тип поля источника.
					const dataValueType  = this.getColumnByName(name).dataValueType;
					// Находим текстовое представление для типа данных
					typeName = Object.keys(BPMSoft.DataValueType).find(key => BPMSoft.DataValueType[key] === dataValueType);
					value = this.get(name);
				}
				broadcastService.sendMessageToAngularByChannel(JSON.stringify({
					columnName: name,
			...
			/**
			 * @inheritdoc BPMSoft.BaseSchemaViewModel#openCardInChain
			 * @override
			 */
			 ...
			 * @param {BPMSoft.ConfigurationEnums.CardOperation} config.operation ### ########.
			...
			mergeDefaultValues: function(defaultValues) {
				...
				defaultValues = this.BPMSoft.deepClone(defaultValues);
				...
				this.BPMSoft.each(currentDefaultValues, function(currentDefaultValue) {
					var searchResult = this.BPMSoft.findItem(defaultValues, {name: currentDefaultValue.name});
					if (!searchResult) {
			...
			_reloadDashboardItems() {
				if (BPMSoft.Features.getIsDisabled("DisableReloadDashboardItemsPTP")) {
			...
			onSaved: function(response, config) {
				BPMSoft.ProcessExecutionDataCollector.add(response.processExecutionData);
				this.hideBodyMask();
				...
				this.set("Operation", BPMSoft.ConfigurationEnums.CardOperation.EDIT);
			...
			/**
			 * @inheritdoc BPMSoft.EntityDataModel#getSaveQuery#getSaveQuery
			 * @override
			 */
			...
			tryShowNextPrcElCard: function(isSilentSave) {
				if (isSilentSave) {
					this.log(Ext.String.format(this.BPMSoft.Resources.ObsoleteMessages.MethodFormatObsolete,
			 ...
			 * @protected
			 * @return {BPMSoft.BaseViewModelCollection} Collection actions card.
			 */
			getActions: function() {
			...
			getViewOptionsButtonVisible: function() {
				...
				var menuUtilities = this.Ext.create("BPMSoft.MenuUtilities");
			...
			changeCloseButtonColorIfSaveButtonExist: function () {
				if(this.get("ShowSaveButton")) {
			...
			changeActionButtonColorIfSaveButtonExist: function () {
				if(this.get("ShowSaveButton")) {
			...
			reloadDetail: function(detailName, args) {
				...
				this.BPMSoft.each(this.entitySchemaInfo.details, function(detailInfo) {
			...
			/**
			 * @inheritdoc BPMSoft.BaseEntityPage#validateSaveEntityResponse
			 * @override
			 */
			...
			/**
			 * ########## ######## ####### ## #########.
			 * ####### ######## #### ######## ######## {@link BPMSoft.BasePageV2#DefaultTabName DefaultTabName},
			 * #### ######## ###### ####### # ######### {@link BPMSoft.BasePageV2#TabsCollection TabsCollection}
			 * @protected
			 * @return {String|null} ######## ####### ## #########.
			 */
			...
			isOverridenGetHeaderMethod: function() {
				return !Ext.String.startsWith(this.getHeader.$owner.$className, "BPMSoft.model.BasePageV2");
			},
			 ...
			 * @protected
			 */
			initHeaderCaption: this.BPMSoft.emptyFn,
			...
			initControlGroupsProfile: function() {
				...
				if (profile && profile.controlGroups) {
					this.BPMSoft.each(profile.controlGroups, function(controlGroup, controlGroupId) {
			...
			activeTabChange: function(activeTab) {
				...
				var tabsCollection = this.get("TabsCollection");
				var modules = this.get('AngularModules');
				...
				if (modules && modules.collection) {
					var angularContainers = this.findAngularContainers();
			
					var filteredModules = modules.collection.items.filter(function(module) {
						return angularContainers.some(function(container) {
							var layoutName = container.layoutName;
                			var tabName = container.tabName; 

                			if ((layoutName === 'Header' || layoutName === 'ProfileContainer')) {
                			    return false;
                			}
							return (tabName && tabName === activeTabName);
						});
					});
					this.AngularRenderItemsCollection = [];
					this.insertAngularComponent(filteredModules);
				}
			},

			/**
			 * @inheritdoc BPMSoft.model.BaseViewModel#onLookupDataLoaded
			 * @override
			 */
			 ...
			 * @param columnName ### #######.
			 */
			getLookupValuePairs: BPMSoft.emptyFn,
			...
			subscribeModulesEvents: function() {
				this.BPMSoft.each(this.modules, this.subscribeModuleEvents, this);
			},
			 ...
			 * @return {Object}
			 */
			onGetPageTips: BPMSoft.emptyFn,

			/**
			 * @inheritdoc BPMSoft.LookupQuickAddMixin#getLookupQueryFilters
			 * @override
			 */
			 ...
			 * @return {BPMSoft.Collection} Section view.
			 */
			...
			onCanBeDestroyed: function(cacheKey) {
				var config = this.BPMSoft.ClientPageSessionCache.getItem(cacheKey);
		...
		diff: /**SCHEMA_DIFF*/[
			...
			},
			{
				...
				"values": {
					"itemType": BPMSoft.ViewItemType.CONTAINER,
					"wrapClass": ["run-process-wrapClass"],
					"items": []
				}
			},
			{
			...
			},
			{
				...
				"values": {
					"itemType": BPMSoft.ViewItemType.BUTTON,
					"style": {"bindTo": "changeCloseButtonColorIfSaveButtonExist"},
			...
			},
			{
				...
				"values": {
					...
					"style": {"bindTo": "changeActionButtonColorIfSaveButtonExist"},
					"classes": {
			...
			},
			{
				...
				"values": {
					...
					"classes": {
						...
						"wrapperClass": ["addbutton-buttonClass email-page-add-button"]
					},
