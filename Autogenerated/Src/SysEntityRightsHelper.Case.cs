	...
	public static class SysEntityRightsHelper
	{
		...
		public static int CopyEntityAdministrateByRecordsRights(UserConnection userConnection, CopyEntityRightsParams copyParams)
		{
			...
			var targetRightsSelect = new Select(userConnection)
				...
				.Where("RecordId").IsEqual(Column.Parameter(copyParams.TargetEntityId));

			var sourceRightsSelect = new Select(userConnection)
				.Column(Column.Parameter(DateTime.UtcNow)).As("CreatedOn")
				.Column(Column.Parameter(userConnection.CurrentUser.ContactId)).As("CreatedById")
				.Column(Column.Parameter(DateTime.UtcNow)).As("ModifiedOn")
				.Column(Column.Parameter(userConnection.CurrentUser.ContactId)).As("ModifiedById")
				.Column(Column.Parameter(copyParams.TargetEntityId)).As("RecordId")
				.Column("srt", "SysAdminUnitId")
				.Column("srt", "Operation")
				.Column("srt", "RightLevel")
				.Column("srt", "Position")
				.Column(Column.Parameter(ManualRightSourceId)).As("SourceId")
				...
				.Where("srt", "RecordId").IsEqual(Column.Parameter(copyParams.SourceEntityId))
				.And("trt", "Id").IsNull();
