...
define("QuestionUserTaskPropertiesPage", ["Contact", "QuestionUserTaskPropertiesPageResources",
	...
	return {
		...
		methods: {
			 ...
			 * @protected
			 * @param {BPMSoft.BaseModel} [control] Answer list item.
			 * @return {Object}
			 */
			getAnswerViewModelConfig: function(control) {
				...
				var config = {
					ItemUId: control ? control.get("ItemUId") : BPMSoft.generateGUID(),
					Id: {
						value: control ? control.get("Id") : BPMSoft.generateGUID()
					},
					Name: {
			...
			getItemName: function(index) {
				...
				var controlList = this.get("RecordAnswerValues");
				var namesIndexes = [];
				BPMSoft.each(controlList.getItems(), function(item) {
			...
			getAnswers: function(parameterValues) {
				var answers = [];
				var scope = this;
				BPMSoft.each(parameterValues, function(answerItem, index) {
			...
			getRecordAnswerViewModel: function(answerItem, index) {
				...
				var viewModel = Ext.create("BPMSoft.BaseModel", config);
				return viewModel;
			},
			...
			getDecisionModeList: function() {
				var decisionModeList = Ext.create("BPMSoft.Collection");
			 ...
			 * @protected
			 * @param {BPMSoft.ProcessSchemaUserTask} element User task element.
			 */
