namespace BPMSoft.Mail.Sender {
	...
	public class EmailSender: IEmailSender
	{
		...
		/// <summary>
		/// "bpmcrm test message" localizable string.
		/// </summary>
		...
		/// <summary>
		/// "This message was send automatically from bpmcrm due to user settings check." localizable string.
		/// </summary>
			...
			if (matchCollection.Count > 0) {
				foreach (Match match in matchCollection) {
					if (match.Groups.Count > 5) {
						...
						Guid contentId = Guid.Parse(contentGroup.ToString());
						contentIds.Add(contentId);
						string oldValue = string.Format("./BPMSoft.axd?{0}", match.Groups[0]);
