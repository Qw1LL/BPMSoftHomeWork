define("MobileDesignerUtils", ["sandbox", "MobileDesignerUtilsResources",
	...
	function(sandbox) {
		...
		/**
		 * @class BPMSoft.MobileDesignerUtils
		 * @public
		 * Mobile designer utilities.
		 */
		Ext.define("BPMSoft.MobileDesignerUtils", {
			...
			initializeSchemaManager: function() {
				...
				var schemaManager = this.schemaManager = Ext.create("BPMSoft.MobileDesignerSchemaManager");
			...
			buildManifestByHierarchy: function(manifestSchemaInstance, baseManifestSchemaInstances) {
				BPMSoft.SchemaDesignerUtilities.buildPackageHierarchy({
					packageUId: manifestSchemaInstance.packageUId,
					useCache: true
				}, function(packageHierarchy) {
					BPMSoft.SchemaDesignerUtilities.getSchemaHierarchy(manifestSchemaInstance, function(manifestSchemaInstances) {
						...
						this.manifest = Ext.create("BPMSoft.MobileDesignerApplicationManifest", {
							manifests: manifests,
			...
			initializeManifest: function() {
				...
				schemaManager.readManifestSchema({
					callback: function(manifestSchemaInstance) {
						if (this.initialConfig.workplaceTypeId === BPMSoft.MobileDesignerEnums.WorkplaceType.Portal) {
			...
			loadSettings: function(config) {
				this.schemaManager.readSettingsSchema({
					...
					callback: function(schemaInstance) {
						var designConfigManager = Ext.create("BPMSoft.MobileDesignerConfigManager", {
			...
			saveSettings: function(config) {
				...
				designConfigManager.applyDesignerConfig({
					...
					callback: function(diff) {
						var converter = Ext.create("BPMSoft.MobileDesignerMetadataToManifestConverter");
			...
			openDesignerModule: function(workplaceName) {
				var url = Ext.String.format("{0}/Nui/ViewModule.aspx?vm=MobileDesignerModule#{1}",
					BPMSoft.workspaceBaseUrl, workplaceName);
				window.open(url);
			}

		});

		return BPMSoft.MobileDesignerUtils;
