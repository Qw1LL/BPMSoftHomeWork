define("FixedFilterViewModelV2", ["ext-base", "BPMSoft", "FixedFilterViewModelV2Resources", "LookupUtilities",
		"QuickFilterViewV2Resources", "ConfigurationConstants"],
	function(Ext, BPMSoft, resources, LookupUtilities, quickFilterViewResources, ConfigurationConstants) {
		...
		function defGetFilter(filterInfo) {
			...
			if (filterInfo.dataValueType === BPMSoft.DataValueType.DATE) {
				...
				if (filterInfo.isLookup) {
					filter = BPMSoft.createColumnInFilterWithParameters(filterInfo.columnName, filterInfo.value);
				} else {
		...
		function generate(config) {
			...
			var viewModelConfig = {
				...
				methods: {
					...
					getAddedFixedFilter: BPMSoft.emptyFn,
			...
			BPMSoft.each(config.filters, function(filterConfig) {
				if (filterConfig.dataValueType === BPMSoft.DataValueType.DATE) {
					...
					if (filterConfig.dataValueType === BPMSoft.DataValueType.LOOKUP ||
						...
						viewModelConfig.values[column.name + "List"] = Ext.create(BPMSoft.Collection);
						viewModelConfig.values[getSelectedLookupValuesPropertyName(column.name)] =
							Ext.create(BPMSoft.Collection);
						...
						viewModelConfig.values[filterConfig.name + "Views"] = Ext.create(BPMSoft.Collection);
					}
				}
			}, this);
		...
		function updateFiltersOnReturn() {
			...
			BPMSoft.each(this.config.filters, function(filterValue) {
				...
				if (filterDataValueType === BPMSoft.DataValueType.LOOKUP ||
					filterDataValueType === BPMSoft.DataValueType.ENUM) {
				...
				} else if (filterDataValueType === BPMSoft.DataValueType.DATE) {
		...
		function setPeriod(tag) {
			...
			switch (periodName) {
				case "CurrentQuarter":
					startDate = BPMSoft.startOfQuarter(startDate);
					dueDate = BPMSoft.endOfQuarter(startDate);
					break;
				case "NextQuarter":
					startDate = BPMSoft.startOfQuarter(Ext.Date.add(startDate, Ext.Date.MONTH, 3));
					dueDate = BPMSoft.endOfQuarter(startDate);
					break;
				case "PrevQuarter":
					startDate = BPMSoft.startOfQuarter(Ext.Date.add(startDate, Ext.Date.MONTH, -3));
		...
		function periodChanged(value, tag) {
			...
			if (!this.suspendUpdate) {
				this.suspendUpdate = true;
				var clonedValue = BPMSoft.deepClone(value);
		...
		const multiSelectLookupCollection=
		[
			"Owner"
		];
		...
		function lookupSelecting(tag, currentFilter) {
			...
			LookupUtilities.Open(sandbox, {
				...
				multiSelect: multiSelectLookupCollection.includes(tag) ? true : false,
				columnName: tag,
