define("DuplicatesModuleViewModel", ["ext-base", "BPMSoft", "sandbox", "DuplicatesModuleViewModelResources",
	"MaskHelper", "RightUtilities"], function(Ext, BPMSoft, sandbox, resources, MaskHelper, RightUtilities) {
	...
	function removeItemsFromGrid(itemsToRemove, parentId) {
		...
		BPMSoft.each(removeCollection, function(removeKey) {
	...
	function loadDuplicates(responce) {
		...
		esq.filters.addItem(BPMSoft.createColumnInFilterWithParameters("Id", responce));
	...
	function loadDuplicatesChilds(response) {
		...
		esq.filters.addItem(BPMSoft.createColumnInFilterWithParameters("Entity1", response));
	...
	function onLoadDuplicates(response, isChilds) {
		checkCanMergeDuplicates.call(this, function(result) {
			...
			if (response.success) {
				...
				BPMSoft.each(loadItems, function(item) {
					if (!isChilds) {
	...
	function onMergeClick() {
		...
		var parentId = null;
		var selectedRows = this.get("selectedRows") || [];
		BPMSoft.each(selectedRows, function(id) {
	...
	function onRowsSelectionChanged() {
		...
		BPMSoft.each(BPMSoft.deepClone(selectedRows), function(item) {
