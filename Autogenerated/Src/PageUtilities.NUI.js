define("PageUtilities", [], function() {
	/**
	 * @class BPMSoft.configuration.mixins.PageUtilities
	 * Contains utility methods used by page.
	 */
	Ext.define("BPMSoft.configuration.mixins.PageUtilities", {
		alternateClassName: "BPMSoft.PageUtilities",
		...
		excludeMediaHeaderForUrlSectionPart: ['ConstructorHomePage', 'ActivitySectionV2'],
		...
		openPage: function(config) {
			...
			var path = this.BPMSoft.combinePath(cardModule, cardSchema,
					"edit", value);
		...
		getCardSchemaName: function(entitySchemaName, columnName) {
			...
			var modulePages = BPMSoft.deepClone(structure.pages);
		...
		getModuleStructure: function(moduleName) {
			return BPMSoft.ModuleUtils.getModuleStructureByName(moduleName || this.entitySchemaName);
		},
		...
		getEntityStructure: function(entitySchemaName) {
			return BPMSoft.ModuleUtils.getEntityStructureByName(entitySchemaName || this.entitySchemaName);
		},

		/**
		 * Активирует динамический заголовок в зависимости от разрешения.
		 */
		activateMediaHeader: function() {
			if(this.excludeMediaHeaderForUrlSectionPart.find(partOfUrl => window.location.href.toLowerCase().includes(`/${partOfUrl.toLowerCase()}`)))
				document.body.classList.remove('media-header__allow');
			else
				document.body.classList.add('media-header__allow');
		},

	//endregion
	});

	return BPMSoft.PageUtilities;
});
