define("GridUtilitiesV2", [
	"GridUtilitiesV2Resources", "DataUtilities", "RightUtilities", "FixedSectionGridCaptionsPlugin",
...
], function(resources, DataUtilities, RightUtilities, fixedSectionGridCaptionsPlugin, ConfigurationConstants, ConfigurationEnums, 
		LookupUtilities, LinkColumnHelper, BusinessRulesApplier, performanceManager, DefaultProfileHelper) {

	/**
	 * Mixin for grid utilities.
	 */
	Ext.define("BPMSoft.configuration.mixins.GridUtilities", {
		alternateClassName: "BPMSoft.GridUtilities",
		 ...
		 * @type {String}
		 */
		queryClassName: "BPMSoft.EntitySchemaQuery",
		 ...
		 * @private
		 * @type {BPMSoft.RightUtilities}
		 */
		rightUtilitiesInstance: null,

		/**
		 * Type of operation with data
		 * @type {BPMSoft.QueryOperationType}
		 */
		queryOperationType: BPMSoft.QueryOperationType.SELECT,
		...
		getEsqOperationType: function () {
			return BPMSoft.Features.getIsEnabled("UseSeparatedSelectMethods")
				? this.queryOperationType
				: BPMSoft.QueryOperationType.SELECT;
		},
		 ...
		 * @public
		 * @return {BPMSoft.EntitySchema} Grid entity schema.
		 */
		 ...
		 * @return {BPMSoft.data.queries.EntitySchemaQuery} Initialized entity schema query.
		 */
		 ...
		 * @param {BPMSoft.data.queries.EntitySchemaQuery} esq Report entity schema query.
		 */
		...
		checkNotFoundColumns: function(response) {
			...
			this.notFoundColumns = this.BPMSoft.ColumnUtilities.findNotFoundColumns(rowConfig);
		},
		...
		_getSelectedRecordsFilter: function(entitySchema, selectedRecordsConfig) {
			...
			const inFilter = this.BPMSoft.createColumnInFilterWithParameters(
		 ...
		 * @private
		 * @return {BPMSoft.BusinessRulesApplier} Instance of BusinessRulesApplier.
		 */
		getBusinessRulesApplier: function() {
			return this.Ext.create("BPMSoft.BusinessRulesApplier");
		},
		 ...
		 * @private
		 * @return {BPMSoft.data.queries.EntitySchemaQuery} ########## ######### ###### BPMSoft.EntitySchemaQuery
		 */
		...
		loadGridDataRecord: function(primaryColumnValue, callback, scope) {
			...
			if (gridData.contains(primaryColumnValue)) {
				...
				esq.getEntity(primaryColumnValue, function(response) {
					...
					BPMSoft.each(entity?.columns, function(column, columnName) {
		 ...
		 * @protected
		 * @param {BPMSoft.BaseViewModel} row - grid row.
		 * @param {BPMSoft.BaseViewModel} entity - loaded entity.
		 ...
		 * @virtual
		 */
		beforeLoadGridDataRecord: this.BPMSoft.emptyFn,
		 ...
		 * @virtual
		 */
		afterLoadGridDataRecord: this.BPMSoft.emptyFn,
		 ...
		 * @virtual
		 */
		afterLoadGridDataUserFunction: this.BPMSoft.emptyFn,
		 ...
		 * @virtual
		 */
		onDataChanged: this.BPMSoft.emptyFn,
		 ...
		 * @protected
		 * @virtual
		 * @param {BPMSoft.BaseViewModelCollection} responseCollection
		 */
		initCanLoadMoreData: function(responseCollection) {
			const collectionCount = responseCollection.getCount();
			const offsetFetchPaging = this.BPMSoft.useOffsetFetchPaging;
		...
		onGridDataLoaded: function(response) {
			const dataCollection = response.collection || Ext.create("BPMSoft.Collection");
			...
			const isContinueExecution = this.BPMSoft.findValueByPath(response, "errorInfo.response.timedout") &&
		 ...
		 * @param {BPMSoft.core.collections.Collection} dataCollection Grid data.
		 * @private
		 */
		 ...
		 * @protected
		 * @param {BPMSoft.data.queries.EntitySchemaQuery} esq A query to initialize.
		 */
		...
		_initOrderColumnConditionValue: function(lastRecord, esq) {
			...
			esq.columns.each(function(column) {
				if (this.BPMSoft.OrderDirection.ASC === column.orderDirection ||
						this.BPMSoft.OrderDirection.DESC === column.orderDirection) {
					...
					if (columnType === BPMSoft.DataValueType.LOOKUP) {
						columnValue = columnValue.displayValue;
						columnType = BPMSoft.DataValueType.TEXT;
					}
		...
		_initPageableConditionValues: function(esq) {
			...
			esq.conditionalValues = this.Ext.create("BPMSoft.ColumnValues");
		 ...
		 * @protected
		 * @param {BPMSoft.data.queries.EntitySchemaQuery} esq The query instance to initialize.
		 */
		 ...
		 * @protected
		 * @param {BPMSoft.data.queries.EntitySchemaQuery} esq The query in which columns will be initialized.
		 */
		...
		/**
		 * Add's all columns into entity schema query instance {@link BPMSoft.EntitySchemaQuery}.
		 * @protected
		 * @param {BPMSoft.data.queries.EntitySchemaQuery} esq Entity schema query in which we will add all column.
		 */
		addAllColumns: function(esq) {
			BPMSoft.each(this.columns, function(column, columnName) {
				if (column.type === BPMSoft.ViewModelColumnType.ENTITY_COLUMN && !esq.columns.contains(columnName)) {
		...
		/**
		 * Add's default columns into entity schema query instance {@link BPMSoft.EntitySchemaQuery}.
		 * @protected
		 * @param {BPMSoft.data.queries.EntitySchemaQuery} esq Entity schema query in which we will add default column.
		 */
		addGridDataColumns: function(esq) {
			const gridDataColumns = this.getGridDataColumns();
			BPMSoft.each(gridDataColumns, function(column, columnName) {
		...
		/**
		 * Returns column config for {@link BPMSoft.EntitySchemaQuery}.
		 * @protected
		...
		/**
		 * Add column from user profile into entity schema query instance {@link BPMSoft.EntitySchemaQuery}.
		 * @protected
		 * @param {BPMSoft.data.queries.EntitySchemaQuery} esq Entity schema query in which we will add new column.
		 */
		...
		addColumnToEsq: function(esq, column, columnName) {
			if (!esq.columns.contains(columnName) && !this.Ext.isEmpty(column)) {
				...
				if (this.isAggregationColumn(column)) {
					...
					const columnByExpression = BPMSoft.EsqExpressionConverter.createEsqColumn(column.expression, columnConfig);
					esq.addColumn(columnByExpression, columnName);
				} else {
					const columnConfig = this.getColumnConfig(column);
					const newColumn = Ext.create("BPMSoft.EntityQueryColumn", columnConfig);
		...
		isAggregationColumn: function(column) {
			...
			return Ext.isNumber(aggregationType) && aggregationType !== BPMSoft.AggregationType.NONE;
		},

		/**
		 * Add's aggregated column from user profile into entity schema query
		 * instance {@link BPMSoft.EntitySchemaQuery}.
		 * @param {BPMSoft.data.queries.EntitySchemaQuery} esq Entity schema query in which we will add new column.
		...
		addProfileAggregationColumn: function(esq, column, columnName) {
			...
			const newColumn = this.Ext.create("BPMSoft.AggregationQueryColumn", columnConfig);
			...
			if (column.subFilters) {
				...
				aggregationColumn.expression.subFilters = BPMSoft.deserialize(filters.serialize(serializationInfo));
			}
		},

		/**
		 * Returns column config for {@link BPMSoft.AggregationQueryColumn}.
		 * @private
		 * @param {Object} column Column, the configuration of which we create.
		 ...
		 * @protected
		 * @param {BPMSoft.data.queries.EntitySchemaQuery} esq Query, to add columns to.
		 */
		...
		getColumnByPath: function(columnPath) {
			...
			const entity = BPMSoft[entityName];
		...
		getProfileColumns: function() {
			...
			profile = propertyName ? profile[propertyName] : profile;
			if (!profile) {
				profile = this.applyDefaultGridProfile()
			}
		...
		convertProfileColumns: function(profileColumns, profile) {
			if (profile && profile.isTiled !== undefined) {
				...
				if (gridsColumnsConfig) {
					...
					this.BPMSoft.each(columnsConfig.items, function(item) {
						...
						if (metaPath && !profileColumns[metaPath]) {
							...
							profileColumns[metaPath] = {
								...
								path: path || metaPath,
								subFilters: this.BPMSoft.deserialize(item.serializedFilter),
								type: item.type || this.BPMSoft.GridCellType.TEXT,
		...
		getTypeColumn: function(schemaName) {
			const schemaConfig = this.BPMSoft.configuration.EntityStructure[schemaName];
		 ...
		 * ############## ####### ##########.
		 * @protected
		 * @param {BPMSoft.data.queries.EntitySchemaQuery} esq ######, # ####### ##### ################ ##### ##########.
		 */
		 ...
		 * @param {BPMSoft.data.queries.EntitySchemaQuery} esq Grid's esq.
		 */
		 ...
		 * @protected
		 * @param {BPMSoft.BaseViewModel} item The row to process the cell from.
		...
		shouldAddLink: function(column) {
			...
			const isProfiledLinkColumn =
				profileColumn && (profileColumn.type === this.BPMSoft.GridCellType.LINK);
		...
		addLookupColumnLink: function(item, column) {
			...
			const schemaConfig = BPMSoft.configuration.ModuleStructure[referenceSchemaName];
			if (!schemaConfig) {
				return;
			}
		...
		addPrimaryColumnLink: function(item, column) {
			...
			const entitySchemaConfig = BPMSoft.configuration.ModuleStructure[entitySchemaName];
		...
		createLink: function(entitySchemaName, columnPath, displayValue, recordId) {
			const moduleStructure = BPMSoft.configuration.ModuleStructure[entitySchemaName];
			const entityStructure = BPMSoft.configuration.EntityStructure[entitySchemaName];
			...
			const link = BPMSoft.workspaceBaseUrl + "/Nui/ViewModule.aspx#" + URL.join("/");
			return {
		...
		applyColumnDefaults: function(column) {
			...
			column.type = BPMSoft.ViewModelColumnType.VIRTUAL_COLUMN;
			...
			if (viewModelColumn) {
				...
				if (column.dataValueType === BPMSoft.DataValueType.FLOAT) {
		 ...
		 * @protected
		 * @return {BPMSoft.FilterGroup} Current grid filters.
		 */
		 ...
		 * @protected
		 * @return {BPMSoft.data.queries.EntitySchemaQuery} Export to excel entity schema query.
		 */
		getExportToExcelEsq: function() {
			...
			if ((!this.Ext.isEmpty(selectedRows) && this.$MultiSelect) || selectAllModeEnabled) {
				...
				esq.filters.addItem(selectAllModeEnabled
					...
					: this.BPMSoft.createColumnInFilterWithParameters(this.primaryColumnName, selectedRows));
			}
			return esq;
		},
		...
		_getDownloadFileNameSuffix: function() {
			return BPMSoft.toCultureDateTime(new Date());
		},
		...
		openStaticGroupLookup: function() {
			...
			if (records && records.length) {
				const config = {
					...
					filters: this.BPMSoft.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL,
		...
		createRecordsInFoldersSelectQuery: function(folders, records) {
			...
			select.filters.add("recordsFilter", this.BPMSoft.createColumnInFilterWithParameters(
				recordId.columnPath, records));
			select.filters.add("foldersFilter", this.BPMSoft.createColumnInFilterWithParameters(
		...
		getNotExistingsRecordsInFolders: function(folders, records, callback, scope) {
			this.getExistingsRecordsInFolders(folders, records, function(existingEntities) {
				...
				BPMSoft.each(folders, function(folderId) {
		...
		_getIncludeInFolderSelectAllQuery: function() {
			...
			if ((!Ext.isEmpty(selectedRows) && this.$MultiSelect) || this.$SelectAllMode) {
				esq.filters.addItem(this.$SelectAllMode
						? this._getSelectAllModeFilters()
						: BPMSoft.createColumnInFilterWithParameters(this.primaryColumnName, selectedRows));
			}
			return esq;
		},
		...
		_getNotExistingRecordsBatch: function(notExistingRecords) {
			...
			BPMSoft.each(notExistingRecords, function(records, folder) {
				BPMSoft.each(records, function(record) {
		...
		includeEntitiesInFolder: function(args, callback, scope) {
			...
			BPMSoft.chain.apply(this, commandChain);
		},

		/**
		 * Returns {@link BPMSoft.FilterGroup IN} filter for provided record ids array.
		 * @protected
		 * @param {Array} records Record Ids.
		 * @param {BPMSoft.ComparisonType} [comparisonType] Filter comparison type.
		 * @return {BPMSoft.FilterGroup}
		 */
		getFolderRecordsInFilter: function(records, comparisonType) {
			const entityColumnNameInFolderEntity = this.getEntityColumnNameInFolderEntity();
			const filter = BPMSoft.createColumnInFilterWithParameters(entityColumnNameInFolderEntity, records);
			filter.comparisonType = this.isEmpty(comparisonType) ? BPMSoft.ComparisonType.EQUAL : comparisonType;
			return filter;
		},
		...
		excludeAllFromFolder: function() {
			...
			let filter = null;
			if (records.length) {
				filter = this.getFolderRecordsInFilter(records, BPMSoft.ComparisonType.NOT_EQUAL);
			}
		...
		excludeRecordsFromFolder: function(records, folder, callback, scope) {
			...
			const entityColumnNameInFolderEntity = this.getEntityColumnNameInFolderEntity();
			query.filters.add("recordFilter", BPMSoft.createColumnInFilterWithParameters(
		 ...
		 * @return {BPMSoft.InsertQuery} Insert query for record in folder.
		 */
		includeEntityInFolder: function(record, folder) {
			const insert = this.Ext.create("BPMSoft.InsertQuery", {
				rootSchemaName: this.getInFolderEntityName()
			});
			insert.setParameterValue(this.folderColumnName, folder, BPMSoft.DataValueType.GUID);
			const entityColumnNameInFolderEntity = this.getEntityColumnNameInFolderEntity();
			insert.setParameterValue(entityColumnNameInFolderEntity, record, BPMSoft.DataValueType.GUID);
			return insert;
		},
		 ...
		 * @private
		 * @return {BPMSoft.RightUtilities} Right utilities instance.
		 */
		getRightUtilities: function() {
			return this.rightUtilitiesInstance ||
				(this.rightUtilitiesInstance = this.Ext.create("BPMSoft.RightUtilities"));
		},
		...
		checkCanDeleteCallback: function(result) {
			...
			this.getDeleteConfirmationMessage(items, function(message){
				...
				}, [BPMSoft.MessageBoxButtons.YES, BPMSoft.MessageBoxButtons.NO], null);
			}, this);
		},
		...
		_getSelectAllModeFilters: function() {
			...
			if (unselectedItems.length) {
				const notInFilter = this.BPMSoft.createColumnInFilterWithParameters(entitySchema.primaryColumnName,
					unselectedItems);
		...
		onMultiDeleteAccept: function() {
			const operationKey = this.BPMSoft.generateGUID();
			const params = {
		...
		handlerAfterMultiDelete: function(responseObject) {
			...
			const result = this.BPMSoft.decode(responseObject.DeleteRecordsAsyncResult);
		...
		getDeleteFromFolderQuery: function(folderId) {
			...
			query.filters.add("folderFilter", BPMSoft.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL,
				"Folder", folderId));
			return query;
		},
		...
		_getConvertedListedConfig: function(listedConfig, viewGenerator) {
			const columnConfig = {};
			const gridType = BPMSoft.GridType.LISTED;
			const gridConfig = {
		...
		_excludeAllItemsFromFolder: function(filter, folder, callback, scope) {
			...
			if (!query.filters.contains("folderFilter")) {
				throw new BPMSoft.ArgumentNullOrEmptyException("folderFilter");
			}
			query.execute(callback, scope);
		},
		...
		_getConvertedTiledConfig: function(tiledConfig, viewGenerator, propertyName) {
			const columnConfig = {};
			const gridType = BPMSoft.GridType.TILED;
			const gridConfig = {
		 ...
		 * @virtual
		 */
		onDeleted: this.BPMSoft.emptyFn,
		 ...
		 * @protected
		 * @param {BPMSoft.data.queries.EntitySchemaQuery} select Entity schema query.
		 */
		 ...
		 * @private
		 * @return {BPMSoft.data.queries.EntitySchemaQuery} Entity schema query.
		 */
		getCountESQ: function() {
			...
			select.addAggregationSchemaColumn(entitySchema.primaryColumnName, this.BPMSoft.AggregationType.COUNT,
				"Count");
			return select;
		},
		...
		/**
		 * @deprecated 7.8.4 Use {@link BPMSoft.GridUtilities#deselectRows deselectRows} instead.
		 * @protected
		 */
		 ...
		 * @protected
		 * @param {BPMSoft.core.collections.Collection} gridData Grid data for selected.
		 */
		...
		_initSortColumnsCollection: function () {
			...
			if (!sortColumns) {
				this.set("SortColumns", Ext.create("BPMSoft.BaseViewModelCollection"));
			}
		},
		...
		initSortActionItems: function() {
			...
			this.BPMSoft.each(gridColumns, function(column, columnName) {
		...
		getProfileColumnCaption: function(config) {
			...
			if (this.BPMSoft.ColumnUtilities.isProfileCultureEqualsUserCulture(profile)) {
				return config.caption;
			}
			const viewModelColumn = this.getColumnByName(config.bindTo || config.name);
			const defaultCaption = config.caption || BPMSoft.emptyString;
		 ...
		 * @param {BPMSoft.OrderDirection} orderDirection ########### ##########.
		 * @return {String} ######### #######.
		 */
		...
		setColumnsProfile: function(viewColumnsSettingsProfile, notSaveToProfile) {
			...
			if (notSaveToProfile !== true) {
				...
				if (profile[gridName]) {
					...
					BPMSoft.utils.saveUserProfile(profileKey, viewColumnsSettingsProfile, false);
				}
			}
		...
		changeSorting: function(config) {
			...
			if (!column) {
				column = this.Ext.create("BPMSoft.EntityQueryColumn", {
					columnPath: tag
				});
				column.name = tag;
			}
		...
		getSortColumnByIndex: function(index, columnsSettingsProfile) {
			let column;
			let columnName;
			const columnsConfig = columnsSettingsProfile && (columnsSettingsProfile.isTiled
			? columnsSettingsProfile.tiledConfig
			: columnsSettingsProfile.listedConfig);
		 ...
		 * ######## ########### ########## # ########### ## ####.
		 * @protected
		 * @param {BPMSoft.core.enums.OrderDirection} orderDirection ########### ##########.
		...
		getColumnSortDirection: function(orderDirection, dataValueType) {
			if (orderDirection && orderDirection !== "") {
				orderDirection = (orderDirection === BPMSoft.OrderDirection.ASC)
					? BPMSoft.OrderDirection.DESC
					: BPMSoft.OrderDirection.ASC;
			} else if (dataValueType === BPMSoft.DataValueType.TEXT ||
					dataValueType === BPMSoft.DataValueType.LOOKUP) {
				orderDirection = BPMSoft.OrderDirection.ASC;
			} else if (dataValueType === BPMSoft.DataValueType.INTEGER ||
		 ...
		 */
		applyDefaultGridProfile: BPMSoft.emptyFn,

		/**
		 * Resets grid column bindings.
		 * @param {BPMSoft.controls.Grid} grid Grid item instance.
		 * @private
		 */
		_resetGridColumnBindings: function(grid) {
			...
			if (!BPMSoft.isEmptyObject(gridProfile)) {
				const viewGenerator = Ext.create("BPMSoft.ViewGenerator", {schemaProfile: profile});
				viewGenerator.viewModelClass = this;
				const columnConfig = gridProfile.type === BPMSoft.GridType.LISTED
					? this._getConvertedListedConfig(BPMSoft.decode(gridProfile.listedConfig), viewGenerator)
					: this._getConvertedTiledConfig(BPMSoft.decode(gridProfile.tiledConfig), viewGenerator,
						propertyName);
		...
		generateEntityProfile: function() {
			...
			if (entitySchema) {
				return DefaultProfileHelper.getEntityProfile(this.getProfileKey(),
						entitySchema.name, this.BPMSoft.GridType.LISTED);
			} else {
				return {};
			}
		},
		 ...
		 * @param {BPMSoft.configuration.ViewGenerator} viewGenerator ######### #############.
		...
		ensureActiveRowVisible: function() {
			...
			if (grid && activeRow) {
				...
				if (el) {
					BPMSoft.scrollIntoView(el, false);
				}
			}
		},
		...
		getGridSettingsInfo: function() {
			...
			return {
				baseGridType: isEditable ? this.BPMSoft.GridType.LISTED : this.BPMSoft.GridType.TILED,
		...
		getEditableGridRowViewModelClassName: function(config) {
			...
			let className = "BPMSoft.BaseConfigurationGridRowViewModel";
			const entitySchemaName = this.getGridEntitySchemaName();
			const entityStructure = BPMSoft.configuration.EntityStructure[entitySchemaName];
			...
			this.BPMSoft.each(pages, function(page) {
				if (typeColumn) {
			...
			if (schemaName) {
				className = "BPMSoft." + schemaName + "ConfigurationGridRow" + entitySchemaName + "ViewModel";
			}
			return className;
		},
		 ...
		 * @protected
		 */
		prepareEmptyGridMessageConfig: BPMSoft.emptyFn,
		 ...
		 * @virtual
		 */
		afterFiltersUpdated: this.BPMSoft.emptyFn,
		...
		initSelectAllCheckBox: function() {
			if (this.isSingleSelectVisible()) {
				if (this.isTiledDataGrid()) {
					this.set("SelectAllTiledItemVisible", true);
					this.set("SelectAllListedItemVisible", false);
				} else {
					this.set("SelectAllListedItemVisible", true);
		...
		*/
		isSingleSelectVisible: function() {
			return this.get("MultiSelect") && !this.get("IsGridEmpty");
		},

		/**
		* Determines if grid is tiled.
		* @return {Boolean}
		*/
		isTiledDataGrid: function() {
			return this.get("Profile")?.DataGrid?.isTiled;
		},
	...
	return Ext.create("BPMSoft.configuration.mixins.GridUtilities");
});
