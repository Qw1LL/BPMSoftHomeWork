define("EntityRelatedColumnsMixin", ["ext-base"],
	function(Ext) {
		 ...
		 */
		Ext.define("BPMSoft.configuration.mixins.EntityRelatedColumnsMixin", {
			alternateClassName: "BPMSoft.EntityRelatedColumnsMixin",
			...
			initLookupListConfig: function(column) {
				...
				if (typeColumn) {
					...
					if (!this.BPMSoft.contains(columns, path)) {
						columns.push(path);
					}
				}
			},
			 ...
			 * @private
			 * @param {BPMSoft.BaseViewModel} entity Entity instance.
			 */
			setLookupColumnValues: function(entity) {
				this.BPMSoft.each(this.columns, function(column) {
					...
					if (column.type === BPMSoft.ViewModelColumnType.ENTITY_COLUMN && lookupListConfig &&
						lookupListConfig.columns) {
						var columnName = column.name;
						var lookupValue = entity.get(columnName);
						if (!Ext.isEmpty(lookupValue)) {
							this.BPMSoft.each(lookupListConfig.columns, function(lookupColumnName) {
				...
				this.BPMSoft.each(columns, function(column, columnName) {
					if (this._canGetRelatedColumnValue(column)) {
						...
						if (!Ext.isEmpty(primaryColumnValue)) {
							...
							batch.addNamedQuery(esq, columnName, BPMSoft.emptyFn, this);
						}
					}
				}, this);
				return batch;
			...
			_getColumnValue: function(column) {
				...
				return !Ext.isEmpty(value) && column.dataValueType === BPMSoft.DataValueType.LOOKUP
					? value.value
					: value;
			},
			...
			findTypeColumn: function(schemaName) {
				var schemaConfig = this.BPMSoft.configuration.EntityStructure[schemaName];
				...
				if (Ext.isEmpty(batch.queries)) {
					...
					batch.execute(function(response) {
						if (response && response.success) {
							BPMSoft.each(batch.queriesMap, function(esqIndex, columnName) {
