define("HistoryStateUtilities", ["ConfigurationEnums", "ConfigurationEnumsV2"], function(ConfigurationEnums) {
	/**
	 * @class BPMSoft.configuration.mixins.HistoryStateUtilities
	 * ######, ########### ###### # ####### ########## ####### ########.
	 */
	Ext.define("BPMSoft.configuration.mixins.HistoryStateUtilities", {
		alternateClassName: "BPMSoft.HistoryStateUtilities",
		...
		parse: function(historyState, result) {
			...
			var cardOperations = BPMSoft.ConfigurationEnums.CardOperation;
			...
			while (++currentItemIndex < historyStateItemsLength && !result.operation) {
				...
				if (BPMSoft.contains(cardOperationsValues, currentItem) || (isLast && schemas.length)) {
					if (this.BPMSoft.isGUID(currentItem)) {
		...
		getFilteredSectionInfo: function(filters) {
			...
			this.BPMSoft.each(filters, function(filter) {
				if (!this.Ext.isEmpty(filter)) {
					this.BPMSoft.each(BPMSoft.configuration.ModuleStructure, function(moduleStructureItem) {
	...
	return BPMSoft.HistoryStateUtilities;
});
