define('FolderLookupPageViewModel', ['ext-base', 'BPMSoft', 'FolderLookupPageViewModelResources',
		'ConfigurationConstants'], function(Ext, BPMSoft, resources, ConfigurationConstants) {
	...
	function selectButton() {
		...
		if (this.get('simpleSelectMode')) {
			var currentFolders = BPMSoft.deepClone(this.get('currentFolders'));
		...
		var viewModelConfig = {
			...
			methods: {
				createNewFolder: function(folderType, parent) {
					...
					filterManager.setFilters(Ext.create('BPMSoft.FilterGroup'));
				...
				onSelectedFilterChange: function() {
					...
					this.set('unGroupButtonVisible', notRootFilter &&
						filter.$className === 'BPMSoft.data.filters.FilterGroup');
				...
				this.currentEditElement.loadEntity(rowId, function() {
					if (this.get('FolderType').value === ConfigurationConstants.Folder.Type.Search) {
						...
						var deserializedFilters = Ext.isEmpty(searchData) ?
							null : BPMSoft.deserialize(searchData);
						if (filterModel.methods.assignEntitySchema) {
							...
							searchData = Ext.isEmpty(deserializedFilters) ?
								Ext.create('BPMSoft.FilterGroup') : deserializedFilters;
