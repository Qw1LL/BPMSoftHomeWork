define("ViewModelSchemaValidationMixin", ["ViewModelSchemaValidationMixinResources"], function(resources) {
		/**
		 * @class BPMSoft.configuration.mixins.ViewModelSchemaValidationMixin
		 * Client schema validation mixin
		 */
		const schemaValidationMixin = Ext.define("BPMSoft.configuration.mixins.ViewModelSchemaValidationMixin", {
			alternateClassName: "BPMSoft.ViewModelSchemaValidationMixin",
			...
			getViewSchemaRootItemName: function(schemaType) {
				switch (schemaType) {
					...
					case BPMSoft.SchemaType.MODULE_VIEW_MODEL_SCHEMA:
						return "SectionWrapContainer";
					case BPMSoft.SchemaType.GRID_DETAIL_VIEW_MODEL_SCHEMA:
					case BPMSoft.SchemaType.EDIT_CONTROLS_DETAIL_VIEW_MODEL_SCHEMA:
					case BPMSoft.SchemaType.MODULE:
			...
			viewSchemaUniqueNamesValidator: function(schema) {
				...
				const result = {success: true};
				const itemsNames = [];
				BPMSoft.iterateChildItems(viewSchema, function(iterationConfig) {
			...
			viewSchemaModelItemsValidator: function(schema) {
				const viewSchema = BPMSoft.deepClone(schema.viewConfig);
				BPMSoft.iterateChildItems(viewSchema, function(iterationConfig) {
					const item = iterationConfig.item;
				...
				BPMSoft.iterateChildItems(viewSchema, function(iterationConfig) {
					...
					if ((Ext.isEmpty(itemType) || (itemType === BPMSoft.ViewItemType.MODEL_ITEM)) &&
						(parentItemType !== BPMSoft.ViewItemType.GRID_LAYOUT &&
						parentItemType !== BPMSoft.ViewItemType.TAB_PANEL)) {
						result.success = false;
						result.message = Ext.String.format(
			...
			viewSchemaJsonStructureValidator: function(schema) {
				...
				BPMSoft.each(viewSchema, function(propertyValue) {
			...
			getSchemaDuplicates: function(schema) {
				...
				const messageConfig = {};
				const result = [];
				const pathDelimiter = BPMSoft.JsonDiffer.pathDelimiter;
				const flatViewConfig = BPMSoft.JsonDiffer.getFlatObject(viewConfig, {identifyItemByPath: true});
			...
			validateSchema: function(schema, callback, scope) {
				...
				BPMSoft.each(validators, function(validator) {
