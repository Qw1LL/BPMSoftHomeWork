/**
 * @class BPMSoft.configuration.ChartDashboardItemConfigGenerator
 * Config generator of chart dashboard item.
 */
Ext.define("BPMSoft.configuration.ChartDashboardItemConfigGenerator", {
	extend: "BPMSoft.BaseDashboardItemConfigGenerator",
	alternateClassName: "BPMSoft.ChartDashboardItemConfigGenerator",
		...
		return [
			{
				...
				color: chartConfig.orderDirection === 2 ?
					BPMSoft.DashboardGaugeScaleColor.max : BPMSoft.DashboardGaugeScaleColor.min
			},
			{
				from: chartConfig.middleFrom,
				to: chartConfig.middleTo,
				color: BPMSoft.DashboardGaugeScaleColor.middle
			},
			{
				from: chartConfig.middleTo,
				to: chartConfig.max,
				color: chartConfig.orderDirection === 2 ?
					BPMSoft.DashboardGaugeScaleColor.min : BPMSoft.DashboardGaugeScaleColor.max
			}
		];
	},
	...
	getSortableValueByItem: function(item, config) {
		...
		if (config.orderBy === BPMSoft.DashboardOrderBy.GroupByField) {
	...
	sortChartData: function(chartData, config) {
		var coefficient = (config.orderDirection === BPMSoft.OrderTypes.DESC) ? -1 : 1;
	...
	getSerieData: function(config) {
		...
		var serieData = {
			...
			color: BPMSoft.DashboardStyleColor[serieConfig.style],
			data: []
		};
	...
	convertDateTimeValue: function(value, formatConfig) {
		if (formatConfig.dateTime === BPMSoft.DashboardDateTimeFormatType.DayMonth) {
			return Ext.Date.format(this.getDateByFormat(formatConfig.dateTime, value),
				BPMSoft.CurrentUserInfo.shortMonthDayFormat);
		} else if (formatConfig.dateTime === BPMSoft.DashboardDateTimeFormatType.MonthYear) {
			return Ext.Date.format(this.getDateByFormat(formatConfig.dateTime, value),
				BPMSoft.CurrentUserInfo.shortYearMonthFormat);
		} else if (formatConfig.dateTime === BPMSoft.DashboardDateTimeFormatType.DayMonthYear) {
			return BPMSoft.String.getTypedValue(this.getDateByFormat(formatConfig.dateTime, value),
			...
			if (valueIsNotEmpty) {
				return result.join(BPMSoft.LS.MobileChartDashboardItemDatePartSeparator);
			}
		}
		return null;
	},
	...
	getColors: function() {
		var colors = [];
		for (var colorName in BPMSoft.DashboardStyleColor) {
			if (BPMSoft.DashboardStyleColor.hasOwnProperty(colorName)) {
				colors.push(BPMSoft.DashboardStyleColor[colorName]);
			}
		}
		return colors;
	},
	...
	getSeriesConfig: function(categories) {
		...
		if (chartConfig.orderBy &&
			BPMSoft.String.startsWith(chartConfig.orderBy, BPMSoft.DashboardOrderBy.ChartEntityColumn)) {
	...
	applyChartConfigByType: function(config) {
		...
		for (var i = seriesConfig.length - 1; i >= 0; i--) {
			...
			switch (chartType) {
				case BPMSoft.ChartType.Pipeline:
					var chartConfig = {
						plotOptions: {},
						customConfig: {
							legend: {
	...
	convertValue: function(value, type, formatConfig) {
		...
		if (Ext.isEmpty(caption)) {
			caption = BPMSoft.LS.MobileChartDashboardItemEmptyValueText;
		}
		return caption;
	},
