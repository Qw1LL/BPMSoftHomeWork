define("SectionModule", ["ext-base", "BPMSoft", "sandbox", "SectionModuleResources", "SectionViewGenerator",
	...
	function(Ext, BPMSoft, sandbox, resources, SectionViewGenerator, SectionViewModelGenerator, GridUtilities,
		...
		var viewCollection = new BPMSoft.Collection();
		var viewModel;
		var currentView;
		...
		function loadView(renderTo) {
			...
			if (viewModel && viewModel.name === entityName && !viewModel.isObsolete) {
				var viewsConfig = BPMSoft.deepClone(baseViewsConfig);
				...
				if (!Ext.isEmpty(openCardConfig)) {
					...
					if (!Ext.isEmpty(cardModuleResponse) &&
						cardModuleResponse.success && BPMSoft.isGUID(cardModuleResponse.uId)) {
								...
								var getColumnsProfilesCallback = function(columnsSettingsProfile) {
									...
									BPMSoft.each(this.columns, function(column) {
										if (column.isCollection === true) {
											this.set(column.name, new BPMSoft.BaseViewModelCollection());
										}
									}, this);
									...
									if (selectedAnalyticsId) {
										...
										selectedAnalyticsId = "";
										BPMSoft.utils.saveUserProfile(selectedAnalyticsElementProfileKey, {}, false);
									}
									...
									var defView = {};
									BPMSoft.each(sectionViews, function(sectionView) {
									...
									var reportsSelectedCallback = function(reports) {
										...
										BPMSoft.each(viewsConfig.views, function(tabView) {
											var itemViewSchema = BPMSoft.deepClone(tabView.viewConfig);
									...
									if (BPMSoft.SysSettings.cachedSettings.BuildType !==
		...
		function getColumnsProfiles(profileKey, tabNames, callback, scope) {
			var profileData = {};
			var requireSchemas = [];
			BPMSoft.each(tabNames, function(tabName) {
		...
		function createViewModelClassBySchema(schema) {
			...
			var viewModelConfigCopy = BPMSoft.deepClone(viewModelConfig);
		...
		function changeView(view, renderOnly) {
			...
			var config = BPMSoft.deepClone(itemViewConfig);
			currentView = Ext.create(config.className, config);
			var currentViewItems = currentView.items;
			if (currentViewItems) {
				if (currentViewItems.items) {
					BPMSoft.each(currentViewItems.items, function(item) {
						if (item.className === "BPMSoft.Grid") {
