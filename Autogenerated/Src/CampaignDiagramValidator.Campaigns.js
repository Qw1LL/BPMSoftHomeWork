define("CampaignDiagramValidator", ["CampaignDiagramValidatorResources", "CampaignEnums", "ServiceHelper"],
	function(resources, CampaignEnums, ServiceHelper) {
		...
		/**
		 * @class BPMSoft.configuration.CampaignDiagramValidator
		 * Class validate campaign diagram.
		 */
		Ext.define("BPMSoft.configuration.CampaignDiagramValidator", {

			extend: "BPMSoft.BaseObject",

			alternateClassName: "BPMSoft.CampaignDiagramValidator",
			...
			stepNeedsContainEntity: function(stepType) {
				...
				return BPMSoft.contains(stepTypes, stepType);
			},
			...
			hasStartElement: function() {
				...
				for (var i = 0; i < length; i++) {
					var node = nodes[i];
					if (BPMSoft.contains(startElements, node.stepType)) {
						result = true;
						break;
					}
				}
			...
			getLandingEntitySchemaQuery: function(recordId) {
				...
				filterGroup.add("IdFilter", BPMSoft.createColumnFilterWithParameter(
					BPMSoft.ComparisonType.EQUAL, "Id", recordId));
				filterGroup.addItem(BPMSoft.createColumnInFilterWithParameters(
					"Type.SchemaUid.Name", [
						"EventTarget",
						"Lead"
					]));
		...
		return BPMSoft.CampaignDiagramValidator;
	});
