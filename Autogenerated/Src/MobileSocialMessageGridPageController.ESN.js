...
Ext.define("BPMSoft.configuration.controller.SocialMessageGridPage", {
	...
	shouldOpenPageOnItemTap: function(targetElement) {
		...
		var shouldOpenPageOnItemTap = !BPMSoft.String.contains(className, "cf-feed-list-item-buttons-container");
	...
	launch: function() {
		var args = arguments;
		var requiredModels = [];
		var modules = BPMSoft.ApplicationConfig.manifest.Modules;
		for (var moduleName in modules) {
			var module = modules[moduleName];
			var modelName = module.Model;
			if (BPMSoft.StructureLoader.getPathBySchemaName(modelName)) {
	...
	openPreviewPage: function(recordId, focusCommentInput) {
		if (!focusCommentInput) {
			...
			BPMSoft.util.openPreviewPage(this.self.Model, {
				recordId: recordId,
				isStartRecord: true,
	...
	initializeSearchField: function() {
		/* HACK: Implement simple container instead of filter panel */
		var view = this.getView();
	...
	getFilters: function() {
		...
		var detailConfig = this.getDetailConfig();
		if (detailConfig) {
			var parentIsNullFilter = Ext.create("BPMSoft.Filter", {
				property: "Parent",
				isNot: true,
	...
	onFilterPanelTap: function() {
		...
		if (lastPageController instanceof BPMSoft.controller.BaseEditPage) {
			return;
		}
		this.openEditPage();
	},
	...
	onActionSelected: function(actionConfig) {
		...
		if (actionClassName === "BPMSoft.ActionDelete" && BPMSoft.Configuration.isFeedInHybridMode()) {
			var executionConfig = {
	...
	onActionExecutionEnd: function(action, success) {
		...
		if (action.config.name === "Delete" && success && BPMSoft.Configuration.isFeedInHybridMode()) {
	...
	refreshDirtyDataByOperationConfig: function(operationConfig) {
		...
		if ("isLiked" in operationConfig) {
			...
			if (storeRecord) {
				...
				var updater = Ext.create("BPMSoft.RecordUpdater");
