define('FolderLookupPage', ['ext-base', 'BPMSoft', 'sandbox', 'FolderLookupPageResources', 'ContactFolder',
		'ConfigurationConstants', 'ResponseExceptionHelper', 'MaskHelper'], function(Ext, BPMSoft, sandbox, resources,
	...
	function initializeModule() {
		require([config.folderFilterViewId, config.folderFilterViewModelId], function(filterView, filterViewModel) {
			...
			view = Ext.create('BPMSoft.Container', viewConfig);
			view.bind(viewModel);
			var folderViewConfig = FolderLookupPageView.getFolderView();
			folderView = Ext.create('BPMSoft.ControlGroup', folderViewConfig);
			...
			if (sectionSchema) {
				var currentModule = BPMSoft.configuration.ModuleStructure[sectionSchema.name];
				if (currentModule) {
	...
	function getTopLevelRecord(rowConfig) {
		if (!topLevelRecord) {
			topLevelRecord = Ext.create("BPMSoft.BaseViewModel", {
	...
	function getFolderEditViewModel(folderType) {
		...
		var folderViewModel = Ext.create('BPMSoft.BaseViewModel', viewModelConfig);
	...
	function initializeProvider(renderTo) {
		...
		require(['EntitySchemaFilterProviderModule'], function(EntitySchemaFilterProviderModule) {
				...
				filterManager = Ext.create('BPMSoft.FilterManager', {
	...
	function changeFolderParent(folders, parentId, callback) {
		...
		var folder = Ext.create('BPMSoft.BaseViewModel',
