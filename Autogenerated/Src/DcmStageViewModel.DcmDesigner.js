define("DcmStageViewModel", ["ext-base", "BPMSoft", "DcmStageViewModelResources", "ProcessSchemaUserTaskUtilities",
	"DcmStage", "DcmStageElementViewModel", "DcmReorderableContainer", "DcmStageViewConfigBuilder"
], function(Ext, BPMSoft, resources, UserTaskUtilities) {
	/**
	 * @class BPMSoft.Designers.DcmStageViewModel
	 */
	Ext.define("BPMSoft.Designers.DcmStageViewModel", {
		extend: "BPMSoft.BaseViewModel",
		alternateClassName: "BPMSoft.DcmStageViewModel",
		...
		columns: {
			...
			AddButtonStyle: {
				...
				value: BPMSoft.enums.DcmStage.AddButtonStyle.INNER_ARROW
			},
			 ...
			 * @type {BPMSoft.data.model.BaseViewModelCollection}
			 */
		...
		/**
		 * Parent view model.
		 * @type {BPMSoft.Designers.DcmSchemaDesignerViewModel}
		 */
		 ...
		 * @return {BPMSoft.enums.DcmStage.AddButtonStyle}
		 */
		getStageAddButtonStyle: function(isLast) {
			...
			if (isLast) {
				addButtonStyle = BPMSoft.enums.DcmStage.AddButtonStyle.INNER_ROUNDED;
			} else if (isAlternativeStage && !isLastStageInGroup) {
				addButtonStyle = BPMSoft.enums.DcmStage.AddButtonStyle.INNER_ROUNDED;
			} else {
				addButtonStyle = BPMSoft.enums.DcmStage.AddButtonStyle.INNER_ARROW;
			}
		...
		onDefaultDcmStageElementTypeUIdChanged: function() {
			this.setAddDcmStageElementTypeButtonImageConfig(BPMSoft.emptyFn);
		},
		...
		initDcmStageElementTypesCollection: function(callback, scope) {
			this.set("DcmStageElementTypesCollection", Ext.create("BPMSoft.BaseViewModelCollection"));
			this.set("AvailableDcmStageElementTypesCollection", Ext.create("BPMSoft.BaseViewModelCollection"));
		 ...
		 * @private
		 * @param {BPMSoft.configuration.DcmSchemaElement} dcmStageElement Dcm schema element.
		 ...
		 * @private
		 * @param {BPMSoft.configuration.DcmSchemaElement} dcmSchemaElement Dcm schema element.
		 ...
		 * @private
		 * @return {BPMSoft.manager.DcmElementSchemaManager}
		 */
		 ...
		 * @private
		 * @param {BPMSoft.configuration.DcmSchemaElement} dcmStageElement Dcm element.
		 * @return {BPMSoft.DcmStageElementViewModel}
		 */
		createDcmStageElementViewModel: function(dcmStageElement) {
			var schema = this.get("Schema");
			var items = this.get("ViewModelItems");
			var dcmStageElementViewModel = Ext.create("BPMSoft.DcmStageElementViewModel", {
				values: {
		 ...
		 * @param {BPMSoft.Designers.DcmStageElementViewModel[]} dependentElementsViewModels An array of
		...
		getResetDependentElementsTransitionsMessage: function(elementCaption, dependentElementsViewModels) {
			...
			var dependentItemsCaptions = dependentElementsViewModels.map(function(viewModel) {
				return BPMSoft.getFormattedString("\"{0}\"", viewModel.get("Caption"));
			}, this);
			return BPMSoft.getFormattedString(
		 ...
		 * @private
		 * @param {String} elementCaption Drag DcmSchemaElement caption.
		 * @param {BPMSoft.Designers.DcmStageElementViewModel} previousExecutedElementViewModel ViewModel of
		...
		applyMoveDataForChainElement: function(moveData) {
			...
			var isElementExecuteInChain =
				!BPMSoft.DcmSchemaElementTransitionFactory.getIsDefaultTransitionByTypeUId(transitionTypeUId);
		...
		getUnselectedStageColor: function(stageColor) {
			...
			return hsla;  //https://jira-ax.norbit.ru/browse/CREAT-8439
		},
		...
		updateDataItemMarker: function() {
			...
			BPMSoft.each(templateValues, function(value, placeholder) {
		...
		getViewConfig: function() {
			var builder = Ext.create("BPMSoft.DcmStageViewConfigBuilder", {viewModel: this});
		 ...
		 * @return {BPMSoft.BaseViewModelCollection|null}
		 */
		 ...
		 * @return {BPMSoft.manager.DcmSchemaElement}
		 */
		addDcmStageElement: function() {
			...
			var dcmElement = Ext.create("BPMSoft.DcmSchemaElement", {
			...
			processElement.uId = BPMSoft.generateGUID();
			processElement.name = this.generateElementUniqueName(defaultDcmProcessElementName);
			processElement.caption = Ext.create("BPMSoft.LocalizableString", elementCaption);
	...
	return BPMSoft.Designers.DcmStageViewModel;
});
