  ...
  function createVideoContainer(renderTo, useVideo) {
    ...
    if (useVideo) {
      Ext.create("Ext.Component", {
        ...
        listeners: {
          click: function () {
            Ext.global.Wb.video.setVisible(!Ext.global.Wb.video.getVisible());
          },
          element: "el",
        },
      });
      var ControlImage = Ext.get("videoBtn").dom;
      var imageUrl = BPMSoft.ImageUrlBuilder.getUrl(
        resources.localizableImages.imgVideoVisible
      );
      ControlImage.style.setProperty(
        "background",
        "url(" + imageUrl + ") 50% 50% no-repeat"
      );
    }
