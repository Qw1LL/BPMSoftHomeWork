define("BulkEmailAnalytics", ["MarketingEnums", "SimpleIndicatorModule", "HighchartsWrapper"],
	function(MarketingEnums) {
		var BulkEmailAnalyticsClass = Ext.define("BPMSoft.configuration.mixins.BulkEmailAnalytics", {

			alternateClassName: "BPMSoft.BulkEmailAnalytics",
			...
			_createPieces: function(allIndicators) {
				var result = [];
				BPMSoft.each(allIndicators, function(item) {
			...
			_loadNotDeliveredChart: function() {
				...
				this.notDeliveredChart = this.Ext.create("BPMSoft.Chart", {
			...
			_loadCanceledChart: function() {
				...
				var chartConfig = this._getPieChartConfig(title, seriesConfig);
				this.canceledChart = this.Ext.create("BPMSoft.Chart", {
			...
			_getIndicatorsConfig: function(tabName) {
				...
				if (tabName === "AnalyticsTab") {
					...
					indicators = {
						Recipient: {style: "widget-orange", textDecorator: "{0}", showBottomLabel : false },
						PreparingToSendRecipients: {style: "widget-orange", textDecorator: "{0}%", showBottomLabel : true },
						SendingRecipients: {style: "widget-orange", textDecorator: "{0}%", showBottomLabel : true},
						ReceivedInitialResponse: {style: "widget-orange", textDecorator: "{0}%", showBottomLabel : true },
						StoppedRecipients: {style: "widget-orange", textDecorator: "{0}%", showBottomLabel : true },
						Canceled: {style: "widget-orange", textDecorator: "{0}%", showBottomLabel : true}
					};
				}
				return indicators;
			},
				...
				BPMSoft.each(indicators, function(item, key) {
					...
					this.sandbox.subscribe("GetIndicatorConfig", function() {
						...
						if (item.showBottomLabel) {
							...
							box.format.type = BPMSoft.DataValueType.INTEGER;
						}
			...
			loadClicksMap: function() {
				if (this.get("ActiveTabName") !== "ClicksAnalysisTab" ||
					this.BPMSoft.Features.getIsEnabled("DynamicContentClickHeatmap")) {
					return;
				}
				var linksData = [];
				this.getHyperlinksClicksData(function(collection) {
					...
					this.BPMSoft.each(collection, function (item) {
						...
						linksData.push(this.BPMSoft.deepClone(linkDto));
					}, this);
			...
			getHyperlinksClicksData: function(callback, scope) {
				...
				clicksCountColumn.orderDirection = BPMSoft.OrderDirection.DESC;
				esq.filters.addItem(esq.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL, "BulkEmail",
					this.get("Id")));
			 ...
			 * @private
			 * @param {BPMSoft.BaseViewModelCollection} collection Collection of data for the chart.
			 */
			showHyperlinksClicksChart: function(collection) {
				...
				this.hyperlinksClicksChart = this.Ext.create("BPMSoft.Chart", {
				...
				return {
					...
					tooltip: {
						...
						formatter: function(tooltip) {
							...
							var url = "<a href=\"" + hyperlink + "\" target=\"_blank\" style=\"color: #F9763D;\">" + hyperlink + "</a>";
							return Ext.String.format("<span style=\"display: inline-block; font-size: 14px; color: #F9763D; font-weight: 700; margin-bottom: 8px;\">{0}</span><br/>" +
									"<span style=\"color:{1}; display: inline-block; margin-bottom: 8px;\">{2}</span>: <span style=\"color: #252F40; font-weight: 400; font-size: 14px;\">{3}</span><br/>" +
			...
			loadSendProcessDiagram: function() {
				...
				this.sendProcessDiagram = this.Ext.create("BPMSoft.Chart", {
			...
			getSendProcessDiagramConfig: function() {
				...
				return {
					chart: {
						...
						type: "bar",
						height: BPMSoft.Features.getIsEnabled("UseHighCharts") ? 86 : 36,
						style: {
							width: "100%"
						},
						marginLeft: 28,
					...
					tooltip: {
						...
						formatter: function() {
							...
							var message = "";
							BPMSoft.each(this.datasets, function(dataset) {
								if (dataset.data[0]) {
									var valueNumber = BPMSoft.getFormattedNumberValue(dataset.data[0],
										{type: BPMSoft.DataValueType.INTEGER});
									var percentNumber = BPMSoft.getFormattedNumberValue(
										dataset.data[0] / this.total * 100,
										{type: BPMSoft.DataValueType.FLOAT, decimalPrecision: 2});
			...
			showOpensClicksChart: function() {
				this.loadClicksOpensChartData(function(clicks, opens) {
					...
					BPMSoft.SysSettings.querySysSettingsItem("BulkEmailHourlyStatisticPeriod",
						function(hours) {
							var chartConfig = this.getOpensClicksChartConfig(clicks, opens, hours);
							this.opensClicksChart = this.Ext.create("BPMSoft.Chart", {
			 ...
			 * @return {BPMSoft.EntitySchemaQuery} Returns query.
			 */
			getClicksOpensEsq: function(indicatorName) {
				...
				dateMarkColumn.orderDirection = BPMSoft.OrderDirection.ASC;
			 ...
			 * @private
			 * @param {BPMSoft.BaseViewModelCollection} clicks Clicks collection.
			 * @param {BPMSoft.BaseViewModelCollection} opens Opens collection.
			 ...
			 * @private
			 * @param {BPMSoft.BaseViewModelCollection} events Collection of data for the chart.
			...
			getDateMarks: function(clicks, opens, hoursToShow) {
				...
				if (!clicks.isEmpty() || !opens.isEmpty()) {
					...
					BPMSoft.each(dateMarks, function(item, index) {
			 ...
			 * @private
			 * @returns {BPMSoft.EntitySchemaQuery} Query to counters of send process.
			 */
