define("ViewModelGeneratorV2", [
		"ext-base", "BPMSoft", "ViewModelGeneratorV2Resources",
		"BaseSchemaViewModel", "BaseGeneratorV2", "ViewConfigBuilderMixin"
	],
	function(Ext, BPMSoft, resources) {
		/**
		 * @class BPMSoft.configuration.ViewModelGenerator
		 * #####, ############ ###### #############
		 */
		Ext.define("BPMSoft.configuration.ViewModelGenerator", {
			alternateClassName: "BPMSoft.ViewModelGenerator",
			extend: "BPMSoft.BaseGenerator",
			 ...
			 * @type {String}
			 */
			baseViewModelClassName: "BPMSoft.BaseSchemaViewModel",
			 ...
			 * @type {Function}
			 */
			preprocessBusinessRules: BPMSoft.emptyFn,
			...
			generateSchemaClass: function(hierarchy) {
				...
				BPMSoft.each(hierarchy, function(schema) {
			...
			generateClass: function(parentClassName, schema, viewConfig) {
				...
				if (schema.type === BPMSoft.SchemaType.EDIT_VIEW_MODEL_SCHEMA) {
			...
			applyColumnDefaults: function(column) {
				...
				column.type = Ext.isEmpty(column.columnPath)
					? BPMSoft.ViewModelColumnType.VIRTUAL_COLUMN
			...
			applyColumnBusinessRules: function(rulesConfig, columnRules, columnName) {
				...
				if (columnRulesConfig) {
					BPMSoft.each(columnRules, function(columnRule, columnRuleName) {
						this.applyColumnBusinessRule(columnRulesConfig, columnRule, columnRuleName);
					}, this);
				} else {
					rulesConfig[columnName] = BPMSoft.deepClone(columnRules);
				}
			},
			...
			applyColumnBusinessRule: function(columnRulesConfig, columnRule, columnRuleName) {
				...
				if (columnRuleConfig) {
					...
					columnRulesConfig[columnRuleName] = BPMSoft.deepClone(columnRule);
				}
			},
			...
			applySchemaModules: function(modulesConfig, parentModules, modules) {
				BPMSoft.each(parentModules, function(module, moduleName) {
					this.applyModule(modulesConfig, module, moduleName);
				}, this);
				BPMSoft.each(modules, function(module, moduleName) {
					this.applyModule(modulesConfig, module, moduleName);
				}, this);
			},
			...
			applyModule: function(modulesConfig, module, moduleName) {
				...
				modulesConfig[moduleName] = BPMSoft.deepClone(module);
			},

			/**
			 * Applies details config to generated class.
			 * @private
			 * @deprecated 7.8 Use {@link BPMSoft.ViewModelGenerator#applySchemaModules}
			...
			applyResource: function(columnsConfig, name, value, type) {
				const resourceColumn = {
					...
					type: BPMSoft.ViewModelColumnType.RESOURCE_COLUMN,
					value: value
				};
			...
			addViewItemColumns: function(columnsConfig, config) {
				if ((config.itemType === BPMSoft.ViewItemType.TAB_PANEL) ||
					(config.itemType === BPMSoft.ViewItemType.IMAGE_TAB_PANEL)) {
			...
			_setEntityColumnsPath: function(dataModels) {
				dataModels.each(function(dataModel) {
					...
					if (entitySchema) {
						...
						BPMSoft.each(schemaColumns, function(column, name) {
			...
			_reorderTabs: function(tabs) {
				if (!BPMSoft.Features.getIsEnabled("UseTabOrderProperty")) {
					return tabs;
				}
			...
			initDataModels: function(classConfig, schema, parentClassPrototype) {
				...
				if (BPMSoft.isEmptyObject(dataModelsConfig) || !Ext.isEmpty(entitySchemaName)) {
					if (entitySchemaName) {
			...
			applyMessages: function(messagesConfig, schemaMessages) {
				BPMSoft.each(schemaMessages, function(schemaMessage, messageName) {
					const message = messagesConfig[messageName];
					if (message) {
						Ext.apply(message, schemaMessage);
					} else {
						messagesConfig[messageName] = BPMSoft.deepClone(schemaMessage);
					}
				}, this);
			},
			...
			applyMethods: function(config, methods) {
				Ext.apply(config, BPMSoft.deepClone(methods));
			},
			...
			applyProperties: function(config, properties) {
				Ext.apply(config, BPMSoft.deepClone(properties));
			},
			...
			applyColumn: function(columnsConfig, column, columnName) {
				...
				if (parentColumn) {
					...
					const columnConfig = BPMSoft.deepClone(column);
					column.value = columnValue;
					if (columnValue !== undefined) {
			...
			applySchemaBusinessRules: function(rulesConfig, parentRules, rules) {
				BPMSoft.each(parentRules, function(columnRules, columnName) {
			...
			applyResources: function(columns, resources) {
				BPMSoft.each(resources.localizableStrings, function(value, name) {
					this.applyResource(columns, name, value, this.resourceType.STRING);
				}, this);
				BPMSoft.each(resources.localizableImages, function(value, name) {
			...
			getTabCaption: function(columnsConfig, config) {
				const caption = BPMSoft.Bindable.getBindingRuleBy$(config.caption, columnsConfig);
		...
		return Ext.create("BPMSoft.configuration.ViewModelGenerator");
	});
