define("ContextTipManager", ["ext-base", "BPMSoft", "css!ContextTipManager"], function(Ext, BPMSoft) {
	/**
	 * @class BPMSoft.configuration.ContextTipManager
	 * Class "ContextTipManager"
	 */
	Ext.define("BPMSoft.configuration.ContextTipManager", {
		extend: "BPMSoft.BaseObject",
		alternateClassName: "BPMSoft.ContextTipManager",
		singleton: true,
		 ...
		 * @type {String}
		 */
		collectionClassName: "BPMSoft.Collection",
		 ...
		 *	BPMSoft.ContextTipManager.setTools([
		 *		{
		...
		initToolsContainer: function() {
			this.toolsContainer = Ext.create("BPMSoft.Container", {
		 ...
		 * @private
		 * @param {BPMSoft.Container} container Tools container instance.
		 * @param {BPMSoft.Component[]} tools Added tools.
		 */
		onToolsAdd: function(container, tools) {
			BPMSoft.each(tools, function(tool) {
		...
		initTools: function() {
			...
			var tools = [];
			BPMSoft.each(this.tools, function(tool) {
		...
		renderContextModeView: function() {
			...
			this.contextModeContainer = Ext.create("BPMSoft.Container", {
		...
		tryEnterContextMode: function(visibleTipsCount) {
			if (visibleTipsCount >= this.maskVisibleTipsCount) {
				...
				if (Ext.isEmpty(this.maskId)) {
					...
					this.maskId = BPMSoft.Mask.show(maskConfig);
				}
			}
		},
		 ...
		 * @protected
		 * @param {BPMSoft.Tip} tip Tip instance.
		...
		iterateTipGroups: function(tip, callback) {
			var tipInfo = this.getTipInfo(tip);
			BPMSoft.each(tipInfo.groupIds, callback, this);
		},
		 ...
		 * return {BPMSoft.Collection} Returns collections of tips.
		 */
		 ...
		 * @throws {BPMSoft.UnsupportedTypeException} Throws BPMSoft.UnsupportedTypeException if passed tip
		 * is not instance of BPMSoft.Tip.
		 */
		register: function(tip, groupIdList) {
			if (!(tip instanceof BPMSoft.Tip)) {
			...
			BPMSoft.each(groupIdList, function(groupId) {
		 ...
		 */
		clear: function() {
			BPMSoft.each(this.tipGroupsStore, function(tipsCollection, groupId) {
	...
	return BPMSoft.ContextTipManager;
