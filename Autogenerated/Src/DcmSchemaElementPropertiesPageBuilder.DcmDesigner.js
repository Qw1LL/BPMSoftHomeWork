define("DcmSchemaElementPropertiesPageBuilder", ["SchemaBuilderV2"], function() {
	/**
	 * @class BPMSoft.configuration.DcmSchemaElementPropertiesPageBuilder
	 * Class, which generates the view and view model for the DCM element properties page.
	 */
	Ext.define("BPMSoft.configuration.DcmSchemaElementPropertiesPageBuilder", {
		extend: "BPMSoft.SchemaBuilder",
		alternateClassName: "BPMSoft.DcmSchemaElementPropertiesPageBuilder",
			...
			if (index !== -1) {
				this.callParent([initialHierarchy, initialConfig, function(hierarchy, config) {
					...
					this.requireAllSchemaHierarchy(baseDcmSchemaConfig, function(baseHierarchy) {
						BPMSoft.each(baseHierarchy, function(baseSchema) {
	...
	return BPMSoft.DcmSchemaElementPropertiesPageBuilder;
});
