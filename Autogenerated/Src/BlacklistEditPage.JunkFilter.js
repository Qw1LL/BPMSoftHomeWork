				...
				methods: {
					...
					onNameChange: function() {
						...
						if (EmailHelper.isEmailAddress(name)) {
							recordId = BPMSoft.JunkFilterConstants.junkFilterTypeOfField.Email;
						}
						else if (EmailHelper.isEmailDomain(name)) {
							recordId = BPMSoft.JunkFilterConstants.junkFilterTypeOfField.Domain;
						}
						else if (EmailHelper.isEmailLogin(name)){
							recordId = BPMSoft.JunkFilterConstants.junkFilterTypeOfField.Entry;
						}
					...
					nameHasDuplicatesValidate: function(callback, scope, result, name) {
						...
						query.addColumn("Id");
						var duplicatesFilter = query.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL,
								"Name", name);
						var notSelfFilter = query.createColumnFilterWithParameter(BPMSoft.ComparisonType.NOT_EQUAL,
								"Id", id);
