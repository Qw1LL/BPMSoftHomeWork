...
function(CtiConstants, ConfigurationConstants) {
	...
	/**
	 * @class BPMSoft.configuration.mixins.CtiPanelIdentificationUtilities
	 * ###### ############# ######## CTI ######.
	 * @type {BPMSoft.BaseObject}
	 */
	Ext.define("BPMSoft.configuration.mixins.CtiPanelIdentificationUtilities", {
		extend: "BPMSoft.core.BaseObject",
		alternateClassName: "BPMSoft.CtiPanelIdentificationUtilities",
		...
		fillSubscribersCollection: function(queryResultSubscribers, collectionName, subscriberKeyName) {
			...
			BPMSoft.each(queryResultSubscribers, function(queryResultSubscriber) {
		...
		fillSearchResultCollection: function(querySearchResults, isNextPageLoaded) {
			var searchResultCollection = new BPMSoft.Collection();
			BPMSoft.each(querySearchResults, function(querySearchResultItem) {
		 ...
		 * @private
		 * @param {BPMSoft.Collection} searchResultCollection Collection of data for found subscribers that should be
		 * updated.
		 ...
		 * @private
		 * @param {BPMSoft.Collection} subscribers Collection of the identified subscribers.
		...
		addCallRelationFields: function(call, subscriberId) {
			var advisedIdentifiedSubscriberInfo = this.get("AdvisedIdentifiedSubscriberInfo");
 			var isNeedAddFields = !Ext.isEmpty(advisedIdentifiedSubscriberInfo) &&
 				(call.direction === BPMSoft.CallDirection.OUT) &&
 				!Ext.isEmpty(advisedIdentifiedSubscriberInfo.callRelationFields);
 			if (isNeedAddFields) {
				this.BPMSoft.each(advisedIdentifiedSubscriberInfo.callRelationFields, function(field) {
					var name = field.name;
		 ...
		 * ######### ################# ########## ######## ######.
		 * @private
		 * @param {BPMSoft.integration.telephony.Call} call ######.
		 */
		...
		getSearchResultPanelConfig: function(queryResultSubscriber) {
			...
			Ext.apply(panelConfig, {
				SubscriberCommunications: new BPMSoft.Collection(),
				CtiModel: this
			});
			return panelConfig;
		},
		...
		getSearchNumberComparisonType: function(searchNumber) {
			if (Ext.isEmpty(searchNumber)) {
				...
				throw new BPMSoft.NullOrEmptyException("PhoneNumberCantBeEmpty");
			}
			var isInternal = this.getIsNumberInternal(searchNumber);
			return (isInternal) ? BPMSoft.ComparisonType.EQUAL : BPMSoft.ComparisonType.START_WITH;
		},

		/**
		 * ############ ####### ####### ##### ######### ############.
		 * @param {BPMSoft.EntitySchemaQuery} query ####### ####### ##### #########.
		 */
		addEmployeeFilter: function(query) {
			...
			filterGroup.add("IsUser", BPMSoft.createExistsFilter("[SysAdminUnit:Contact:Contact].Id"));
			filterGroup.add("IsEmployeeType", BPMSoft.createColumnFilterWithParameter(
				BPMSoft.ComparisonType.EQUAL, "Contact.Type", ConfigurationConstants.ContactType.Employee));
		 ...
		 * @param {BPMSoft.ComparisonType} [comparisonType=BPMSoft.ComparisonType.START_WITH] ### #########.
		 * @return {BPMSoft.EntitySchemaQuery}
		 */
		getContactQuery: function(searchFieldName, searchValue, comparisonType) {
			...
			var esq = Ext.create("BPMSoft.EntitySchemaQuery", {rootSchemaName: "ContactCommunication"});
			esq.isPageable = true;
			...
			contactNameColumn.orderDirection = BPMSoft.OrderDirection.ASC;
			var contactIdColumn = esq.addColumn("Contact.Id", "SubscriberId");
			contactIdColumn.orderPosition = 1;
			contactIdColumn.orderDirection = BPMSoft.OrderDirection.ASC;
			...
			communicationTypeNameColumn.orderDirection = BPMSoft.OrderDirection.ASC;
			...
			esq.addParameterColumn(CtiConstants.SubscriberTypes.Contact, BPMSoft.DataValueType.TEXT,
				"SubscriberType");
			const typeColumnName = this.getTypeColumnName("Contact");
			if (typeColumnName) {
				esq.addColumn("Contact." + typeColumnName, "TypeColumn");
			}
			esq.filters.addItem(BPMSoft.createIsNotNullFilter(
				Ext.create("BPMSoft.ColumnExpression", {columnPath: "Contact.Id"})));
			esq.filters.add("Search", BPMSoft.createColumnFilterWithParameter(
				comparisonType, searchFieldName, searchValue));
			esq.filters.add("CommunicationCode", BPMSoft.createColumnFilterWithParameter(
		...
		getAccountQuery: function(searchFieldName, searchValue, comparisonType) {
			comparisonType = comparisonType || this.getStringColumnComparisonType();
			var esq = Ext.create("BPMSoft.EntitySchemaQuery", {rootSchemaName: "AccountCommunication"});
			esq.isPageable = true;
			esq.rowsOffset = this.$SearchSubscribersRowsOffset;
			esq.rowCount = CtiConstants.IdentificationMaxRowCount;
			esq.addColumn("Id");
			...
			accountNameColumn.orderDirection = BPMSoft.OrderDirection.ASC;
			var accountIdColumn = esq.addColumn("Account.Id", "SubscriberId");
			accountIdColumn.orderPosition = 1;
			accountIdColumn.orderDirection = BPMSoft.OrderDirection.ASC;
			var communicationTypeNameColumn = esq.addColumn("CommunicationType", "CommunicationType");
			communicationTypeNameColumn.orderPosition = 2;
			communicationTypeNameColumn.orderDirection = BPMSoft.OrderDirection.ASC;
			...
			esq.addColumn("Number");
			esq.addParameterColumn(CtiConstants.SubscriberTypes.Account, BPMSoft.DataValueType.TEXT,
				"SubscriberType");
			...
			esq.filters.addItem(BPMSoft.createIsNotNullFilter(
				Ext.create("BPMSoft.ColumnExpression", {columnPath: "Account.Id"})));
			esq.filters.add("Search", BPMSoft.createColumnFilterWithParameter(
				comparisonType, searchFieldName, searchValue));
			esq.filters.add("CommunicationCode", BPMSoft.createColumnFilterWithParameter(
				BPMSoft.ComparisonType.EQUAL,
		...
		identifySubscriber: function(number, collectionName, subscriberKeyName, callback) {
			...
			var reverseNumber = BPMSoft.utils.common.reverseStr(number);
			var ctiSettings = this.get("CtiSettings");
			...
			this.querySubscribersByPhoneNumber(reverseNumber, function(phoneNumber, response) {
				...
				BPMSoft.each(response.queryResults, function(queryResult) {
		...
		getCallFieldValuesBySubscriber: function(subscribersCollectionName, subscriberId) {
			var updateFields = new BPMSoft.Collection();
