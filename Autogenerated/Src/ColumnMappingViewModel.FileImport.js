define("ColumnMappingViewModel", ["StructureExplorerUtilities", "ColumnMappingViewModelResources", "RightUtilities",
	...
	Ext.define("BPMSoft.configuration.ColumnMappingViewModel", {
		alternateClassName: "BPMSoft.ColumnMappingViewModel",
		 ...
		 * @type {String}
		 */
		defaultDestinationViewModelClassName: "BPMSoft.configuration.ColumnDestinationViewModel",
		 ...
		 * @type {String}
		 */
		lookupDestinationViewModelClassName: "BPMSoft.configuration.LookupColumnDestinationViewModel",
		...
		findDestinationInfo: function(uId) {
			...
			var destinationInfo = this.BPMSoft.findWhere(destinationsInfo, {uId: uId});
		...
		findDestinationType: function(info, values) {
			...
			var attributeColumnValue = this.BPMSoft.findWhere(info.attributeColumnValues, {value: typeValue});
		...
		getTypeValue: function(values) {
			var property = this.BPMSoft.findWhere(values.Properties, {Key: "AttributeColumnValueName"});
			var attributeName = property.Value;
			var attribute = this.BPMSoft.findWhere(values.Attributes, {Key: attributeName});
		...
		findDestinationColumnCaption: function(info, columnName) {
			var column = this.BPMSoft.findWhere(info.columns, {name: columnName});
		...
		getDestinationClassName: function(values) {
			...
			if (properties && BPMSoft.findWhere(properties, {Key: "ImportColumnName"})) {
		...
		getDestinationConfig: function(columnConfig) {
			...
			var properties = [
				{
					Key: "TypeUId",
					Value: this.BPMSoft.convertToServerDataValueType(column.dataValueType)
				}
			];
		...
		addDestinations: function(destinationConfigs, callback, scope) {
			var rootSchema = this.get("Schema");
			this.BPMSoft.each(destinationConfigs, function(config) {
		...
		getEmptyDestinationImageConfig: function() {
			return {
				...
				url: BPMSoft.ImageUrlBuilder.getUrl(resources.localizableImages.EmptyDestinationImage)
			};
		},
