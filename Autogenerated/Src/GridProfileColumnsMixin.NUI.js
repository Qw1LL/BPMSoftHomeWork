/**
 * @class BPMSoft.configuration.GridProfileColumnsMixin
 * Mixin which manages grid columns in the profile.
 */
define("GridProfileColumnsMixin", [], function() {
	Ext.define("BPMSoft.configuration.GridProfileColumnsMixin", {
		alternateClassName: "BPMSoft.GridProfileColumnsMixin",
		...
		_sortColumnInListed: function(profile, columnIndex) {
			...
			listedConfig.items.forEach(function(column) {
				column.orderDirection = BPMSoft.emptyString;
				column.orderPosition = BPMSoft.emptyString;
			});
			if (column)  {
