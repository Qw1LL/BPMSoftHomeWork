define("BaseDataView", ["BPMSoft", "ConfigurationEnums", "ConfigurationConstants", "RightUtilities",
...
], function(BPMSoft, ConfigurationEnums, ConfigurationConstants, RightUtilities, ProcessModuleUtilities,
	...
	return {
		messages: {
			 ...
			 * ######## ###### #### ###### "###" ########
			 * @param {BPMSoft.BaseViewModelCollection} ###### #### ###### "###"
			 */
			"GetCardViewOptions": {
				mode: BPMSoft.MessageMode.PTP,
				direction: BPMSoft.MessageDirectionType.SUBSCRIBE
		...
		mixins: {
			 ...
			 *
			 */
			ProcessEntryPointUtilities: "BPMSoft.ProcessEntryPointUtilities",
			/**
		...
		attributes: {
			...
			"ActionsButtonStyle": {
				dataValueType: BPMSoft.DataValueType.TEXT,
				value: BPMSoft.controls.ButtonEnums.style.DEFAULT
			},
			...
			/**
			 * Section select all check-box state.
			 */
			 "SelectAllCheckBox": {
			...
			/**
			 * Sign of visibility section select all tiled.
			 * @type {Boolean}
			 */
			"SelectAllTiledItemVisible": {
				dataValueType: BPMSoft.DataValueType.BOOLEAN,
				value: false
			},
			/**
			 * Sign of visibility section select all listed.
			 * @type {Boolean}
			 */
			"SelectAllListedItemVisible": {
				dataValueType: BPMSoft.DataValueType.BOOLEAN,
				value: false
			}
		},
		properties: {
			 ...
			 */
			folderManagerViewModelClassName: "BPMSoft.BaseFolderManagerViewModel",
			/**
			 ...
			 */
			queryOperationType: BPMSoft.QueryOperationType.LISTSELECT
		},
		methods: {
			...
			getGridDataESQ: function() {
				...
				if (this.isNotEmpty(currentFolder)) {
					...
					esq.queryKind = BPMSoft.QueryKind.LIMITED;
					esq.useMetrics = this.isNeedToUseQueryMetrics();
					esq.querySource = BPMSoft.QuerySource.FILTER;
			...
			/**
			 * @inheritDoc BPMSoft.GridUtilities#destroyQueryEvents
			 * @override
			 */
			 ...
			 * @param {BPMSoft.FolderFilterItemViewModel} [folder] Folder filter.
			...
			getFolderFilterOptimizationOptions: function(folder) {
				...
				switch (optimizationType) {
					case BPMSoft.Folder.OptimizationType.TryOptimize:
						useReExecuteData = true;
						useReExecuteCount = true;
						break;
					case BPMSoft.Folder.OptimizationType.Data:
						queryOptimizeData = true;
						break;
					case BPMSoft.Folder.OptimizationType.Count:
						queryOptimizeCount = true;
						break;
					case BPMSoft.Folder.OptimizationType.CountAndData:
						queryOptimizeData = true;
						queryOptimizeCount = true;
						break;
					case BPMSoft.Folder.OptimizationType.AppliedDataTryCount:
						queryOptimizeData = true;
						useReExecuteCount = true;
						break;
					case BPMSoft.Folder.OptimizationType.NotAppliedDataTryCount:
						useReExecuteCount = true;
						break;
					case BPMSoft.Folder.OptimizationType.AppliedCountTryData:
						queryOptimizeCount = true;
						useReExecuteData = true;
						break;
					case BPMSoft.Folder.OptimizationType.NotAppliedCountTryData:
						useReExecuteData = true;
						break;
					case BPMSoft.Folder.OptimizationType.CountOver:
						useCountOver = true;
						break;
					case BPMSoft.Folder.OptimizationType.NotApplied:
						break;
					default:
						break;
				}
			 ...
			 * @return {BPMSoft.FolderFilterItemViewModel | null} Folder filter.
			 * @private
			 */
			...
			onGridDataLoaded: function(response) {
				...
				if (!response.success) {
					const isTimedout = this.BPMSoft.findValueByPath(response, "errorInfo.response.timedout");
			...
			_saveFolderFilterQueryOptimization: function(isSuccessResponse, predicate) {
				...
				if (!this.get("IsExtendedFiltersVisible")) {
					folder.saveEntity(this.BPMSoft.emptyFn, this);
				}
			},
			...
			_setOptimizationTypeByData: function(folder, isSuccess) {
				const optimizationType = BPMSoft.Folder.OptimizationType;
				const transitions = [{
			...
			_setOptimizationType: function(transitions, folder, isSuccess) {
				...
				const type = this.BPMSoft.findWhere(transitions, {type: currentType});
			...
			initSectionHeaderContainerVisibility: function() {
				if (BPMSoft.Features.getIsEnabled("OldUI")) {
			...
			getSectionHeaderCaption: function() {
				if (BPMSoft.Features.getIsDisabled("OldUI")) {
			...
			getCardModuleSandboxIdentifiers: function() {
				...
				BPMSoft.each(this.getTypeColumnValues(), function(typeColumnValue) {
			...
			showCard: function() {
				...
				BPMSoft.utils.dom.setAttributeToBody("is-card-opened", true);
				BPMSoft.utils.dom.setAttributeToBody("is-main-header-visible", true);
			...
			addSectionHistoryState: function() {
				...
				historyState = BPMSoft.combinePath(module, sectionSchema, cardSchema, operation,
					primaryColumnValue);
			...
			_getFormattedHistoryState: function(historyState, schemaName, operation, primaryColumnValue) {
				...
				if (hashItems.length === 6) {
					...
					if (historyStateInfo.workAreaMode === ConfigurationEnums.WorkAreaMode.CARD) {
						hState = BPMSoft.combinePath("CardModuleV2", schemaName, operation,
							primaryColumnValue);
					} else {
						hState = hState.replace(hashItems[2], schemaName);
						hState = hState.replace(hashItems[3], operation);
			...
			getCardHistoryStateConfig: function(config) {
				...
				if (this.getIsSectionVisible()) {
					...
					hash = BPMSoft.combinePath("CardModuleV2", schemaName, operation, primaryColumnValue);
				}
				return {
					stateObj: BPMSoft.deepClone(currentHistoryState.state),
					silent: true,
					hash: hash
				};
			},
			...
			addRecord: function(typeColumnValue) {
				if (!typeColumnValue) {
					...
					typeColumnValue = tag || BPMSoft.GUID_EMPTY;
				}
			 ...
			 * @protected
			 */
			editCurrentRecord: function(recordID) {
				this.closeMiniPage();
				const primaryColumnValue = this.getPrimaryColumnValue() ?? recordID;
			...
			openCard: function(schemaName, operation, primaryColumnValue) {
				...
				BPMSoft.delay(this.initTagButtonCaption, this, 1000, [primaryColumnValue]);
				if (!isCardVisible) {
					this.loadCardModule();
			...
			getDefaultSectionGridProfile: function() {
				...
				const defType = profilePropertyName.indexOf(this.verticalProfileKeySuffix) > -1
					? BPMSoft.GridType.TILED
					: BPMSoft.GridType.LISTED;
			...
			applyDefaultGridProfile: function() {
				...
				BPMSoft.utils.saveUserProfile(key, profile, false, BPMSoft.emptyFn, this);
				return defProfile;
			},
			...
			_initCollections: function() {
				this.set("SeparateModeActionsButtonMenuItems", this.Ext.create("BPMSoft.BaseViewModelCollection"));
				this.set("CombinedModeActionsButtonMenuItems", this.Ext.create("BPMSoft.BaseViewModelCollection"));
			...
			init: function(callback, scope) {
				...
				if (BPMSoft.Features.getIsDisabled("SectionLoadingMask")) {
			...
			_afterParentInit: function(callback, scope) {
				...
				this.initHelpUrl(BPMSoft.emptyFn, this);
				this.initPrintButtonsMenu(BPMSoft.emptyFn, this);
			...
			_initSectionAttributes: function(callback) {
				...
				this.updateSortMenuVisible();
				Ext.callback(callback, this);
			},

			checkCanManageAnalytics: BPMSoft.emptyFn,
			/**
			...
			/**
			 * @inheritdoc BPMSoft.GridUtilities#onDeleted
			 * @overridden
			 */
			onDeleted: function() {
				this.reloadSummaryModule();
			...
			initResetAction: function() {
				this.sandbox.subscribe("ResetSection", function() {
					const storage = BPMSoft.configuration.Storage.Filters =
						BPMSoft.configuration.Storage.Filters || {};
					storage[this.name] = {};

					const profile = this.get("Profile");
					profile.Filters = {};
					BPMSoft.utils.saveUserProfile(this.getProfileKey(), profile, false);
			...
			initDataViews: function() {
				const dataViews = this.updateDataViews();
			...
			/**
			 * Updates data views.
			 * @return {BPMSoft.Collection} Return data views collection.
			 */
			updateDataViews: function() {
				const defaultDataViews = this.getDefaultDataViews();
				const dataViews = this.Ext.create("BPMSoft.Collection");
				const savedActiveViewName = this.getActiveViewNameFromProfile();
				BPMSoft.each(defaultDataViews, function(dataView, dataViewName) {
			 ...
			 * @protected
			 * @virtual
			 * @param {BPMSoft.Collection} itemsList New summary items.
			 */
			handleSummaryItemsUpdate: function(itemsList) {
				...
				if (this._isCanUseCountOver()) {
					...
					const summaryCollection = this.BPMSoft.map(itemsList.getItems(), function(item) {
						return item.array;
					});
			...
			initAddRecordButtonParameters: function() {
				...
				let tag = BPMSoft.GUID_EMPTY;
				const editPages = this.get("EditPages");
				const editPagesCount = editPages.getCount();
				if (editPagesCount === 1) {
					const entitySchemaName = this.getEntitySchemaName();
					const sectionEntity = BPMSoft.configuration.EntityStructure[entitySchemaName];
			...
			_filterValueEncode: function(filterValue, serializableFilter) {
				BPMSoft.each(filterValue, function(item) {
					...
					if (!isSerializedFilter) {
						item.filter = BPMSoft.encode(serializableFilter);
					}
				});
			},
			...
			saveFilter: function(filterKey, filterValue, filter) {
				...
				switch (filterKey) {
					...
					case "FolderFilters":
						...
						break;
					case "FixedFilters":
						filterValue.filter = BPMSoft.encode(serializableFilter);
			...
			_saveUserProfile: function(profileFilters, isDefault, isSetProfileFilters) {
				BPMSoft.saveUserProfile(this.getFiltersKey(), profileFilters, isDefault);
			...
			getSessionFilters: function() {
				const storage = BPMSoft.configuration.Storage.Filters = BPMSoft.configuration.Storage.Filters || {};
			...
			subscribeGetRunProcessesProperties: function() {
				this.sandbox.subscribe("GetRunProcessesProperties", function(properties) {
					BPMSoft.each(properties, function(property) {
						...
						if (viewModelItem instanceof BPMSoft.Collection) {
			...
			OpenCardInChain: function(config) {
				this.log(Ext.String.format(BPMSoft.Resources.ObsoleteMessages.ObsoleteMethodMessage,
			...
			saveCardScroll: function() {
				if (this.get("IsCardVisible")) {
					...
					const historyState = this.sandbox.publish("GetHistoryState");
					const state = BPMSoft.deepClone(historyState.state);
			...
			initSectionFiltersCollection: function() {
				...
				this.set("SectionFilters", this.Ext.create("BPMSoft.FilterGroup"));
				this.set("SectionFiltersValue", this.Ext.create("BPMSoft.Collection"));
			},
			...
			_tryGetFiltersFromHistoryState: function() {
				...
				if (state.state && state.state.filterStates) {
					filters = BPMSoft.deepClone(state.state.filterStates);
				}
				return filters;
			},
			...
			loadProfileFilters: function(callback, scope) {
				...
				BPMSoft.require([profileKey], function(profile) {
			 ...
			 */
			onLoadProfileFilters: async function(callback, scope, profile) {
				await this.updateProfileFilters(profile);
			...
			updateProfileFilters: async function(profile) {
				if (Ext.isEmpty(profile.CustomFilters)) return;
				
				for (const filter of Object.values(profile.CustomFilters))
				  	await this.updateFilter(filter);
			},

			updateFilter: async function(profileFilter) {
				let filters;
				try {
					filters = JSON.parse(profileFilter.value);
					for (const filter of Object.values(filters?.items))
						if (this.isLookupValue(filter.rightExpressions))
							await this.updateLookupValues(
								filter.rightExpressions, 
								filter.referenceSchemaName
							);
				} catch (error) {
					console.error(error);
					return;
			...
			updateLookupValues: async function(expressions, schemaName) {
				if(expressions) {
					for (const expression of expressions) {
						if (!this.isLookupValue(expression)) continue;
						const { parameter } = expression;
						const schema = await this.requireReferenceSchema(schemaName);
						const displayValue = await this.fetchDisplayValue(
							schema,
							parameter.value.value
						);
						expression.parameter.value[schema.primaryDisplayColumn.name]
							= expression.parameter.value.displayValue
							= displayValue;
					}
				}
			},
			  
			isLookupValue: async function(expression) {
				return (
					expression
				  	&& expression.className === "BPMSoft.ParameterExpression"
				  	&& expression.parameter.dataValueType === BPMSoft.DataValueType.LOOKUP
				);
			},

			fetchDisplayValue: async function(schema, id){
				return new Promise((resolve, reject) => {
					const primaryDisplayColumnName = schema.primaryDisplayColumn.name;
					...
					esq.getEntity(id, function(result) {
						if (result.success) {
							resolve(result.entity.get(primaryDisplayColumnName));
						}
					}, this);
				}) 
			},

			requireReferenceSchema: async function(schemaName){
				return new Promise((resolve, reject) => {
					require([schemaName], (schema) => {
						resolve(schema)
					});
				})
			},
			...
			onLoadHistoryStateFilters: function(callback, scope, filters) {
				...
				BPMSoft.each(filters, this.applyFilters, this);
				this.set("IsSectionFiltersLoaded", true);
			...
			initFilterAttributes: function(profile) {
				...
				const profileFilters = BPMSoft.deepClone(profile);
				const fixedSessionFilters = this._getFixedSessionFilters();
				BPMSoft.each(profileFilters, this.applyFilters, this);
				BPMSoft.each(fixedSessionFilters, this.applyFilters, this);
				this.set("ProfileFilters", profileFilters);
			},
			...
			_chooseFilterProcessing: function(filterValue, key) {
				const sectionFilters = this.get("SectionFilters");
				BPMSoft.each(filterValue, function(item, propName) {
					...
					if (item.filter) {
						const filter = BPMSoft.deserialize(item.filter);
			...
			_applyFixedFilter: function(key, filterValueFixed) {
				...
				if (this.isNotEmpty(filterValueFixed.filter)) {
					...
					const filter = BPMSoft.deserialize(filterValueFixed.filter);
					if (!sectionFilters.contains(key)) {
						sectionFilters.add(key, filter);
					}
				}
			...
			_createTagFilter: function(inTagSchemaName, tagId) {
				...
				const existsFilter = BPMSoft.createExistsFilter(columnPath);
				existsFilter.subFilters.addItem(BPMSoft.createColumnFilterWithParameter(
					BPMSoft.ComparisonType.EQUAL, "Tag", tagId));
				return existsFilter;
			},
			...
			applyTagFilter: function(filterKey, filterConfig) {
				...
				if (filterConfig.key === TagConstantsV2.TagFilterKey) {
					...
					const filterCopy = BPMSoft.deepClone(filterConfig);
					delete filterCopy.tags;
					const filter = BPMSoft.deserialize(filterCopy);
			...
			applyFolderFilter: function(filterKey, filterConfig) {
				...
				if (folderType.value === ConfigurationConstants.Folder.Type.General) {
					const folderFilter = BPMSoft.createFilterGroup();
					...
					folderFilter.add("filterStaticFolder",
						BPMSoft.createColumnInFilterWithParameters(Ext.String.format(
			...
			addDynamicFolder: function(sectionFilters, item) {
				...
				const filters = this.Ext.create("BPMSoft.FilterGroup");
				filters.addItem(select.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL, "Id",
					item.folderId));
					...
					if (response && response.success) {
						response.collection.each(function(responseItem) {
							sectionFilters.add("FolderFilters",
								BPMSoft.deserialize(responseItem.get("SearchData")));
			...
			showCustomFilterExtendedMode: function(args) {
				...
				this.sandbox.subscribe("GetExtendedFilter", function() {
					...
					if (folderEditMode) {
						...
						const useNewLookupComparison = BPMSoft.Features.getIsEnabled("UseNewLookupComparison");
			...
			showFolderTree: function(args) {
				...
				const isSeparateMode = !this.get("IsCardVisible");
				this.setLeftSectionContainerVisibility(isSeparateMode);
				if (args && args.initialization && !args.isFoldersContainerExpanded) {
					this.onHideFoldersAndFilters();
					this.changeDataViewsContainerClasses({
						showFolders: false
					});
			...
			saveFiltersContainersVisibility: function(folderEditMode) {
				...
				BPMSoft.saveUserProfile(this.getFiltersKey(), profileFilters, false);
				this.changeGridUtilitiesContainerSize();
			},
			 ...
			 */
			getExtendedFilterDisplayValue: BPMSoft.FilterUtilities.getExtendedFilterDisplayValue,
			...
			getDefaultGridDataViewIcon: function() {
				if (this.isTiledDataGrid())
					return this.get("Resources.Images.GridDataViewTiledIcon");
				else
			...
			getFilters: function() {
				...
				serializationInfo.serializeFilterManagerInfo = true;
				const deserializedFilters = BPMSoft.deserialize(sectionFilters.serialize(serializationInfo));
			...
			getReportFilters: function(recordId) {
				...
				if (this.isAnySelected()) {
					...
					const filter = BPMSoft.createColumnInFilterWithParameters("Id", selectedRows);
			 ...
			 * @param {BPMSoft.BaseViewModelCollection} actionMenuItems
			 */
			initActionButtonMenu: function(modeType, actionMenuItems) {
				...
				if (actionMenuItems.getCount()) {
					...
					const newCollection = this.Ext.create("BPMSoft.BaseViewModelCollection");
			 ...
			 * @param {BPMSoft.BaseViewModelCollection} viewOptions ###### ####
			 */
			...
			/**
			 * ######### ### ######## ###### #############, ############ ######## #########.
			 * @param {BPMSoft.BaseViewModel} originalItem ###### #############.
			 * @return {BPMSoft.BaseViewModel|*} ##### ###### ############# # ############## ##########.
			 */
			cloneBaseViewModel: function(originalItem) {
				const newItem = this.Ext.create("BPMSoft.BaseViewModel");
				BPMSoft.each(originalItem.values, function(itemValue, valueName) {
					if (!(itemValue instanceof BPMSoft.Collection)) {
						newItem.set(valueName, BPMSoft.deepClone(itemValue));
					} else {
						const newItemsCollection = this.Ext.create("BPMSoft.BaseViewModelCollection");
			...
			/**
			 * Determines if multi select action is active.
			 * @return {Boolean}
			 */
			...
			isSingleSelectActionButtonVisible: function() {
				return this.get("MultiSelect");
			},
			...
			getFolderMenuItems: function() {
				if (this.get("UseStaticFolders")) {
					const addFolderButtonMenuItems = this.Ext.create("BPMSoft.BaseViewModelCollection");
			...
			/**
			 * Update IsSortMenuVisible
			 * @protected
			 */
			updateSortMenuVisible: function() {
				const viewName = this.getActiveViewName();
				const gridViewName = this.$GridDataViewName;
				this.set("IsSortMenuVisible", viewName == gridViewName);
			},
			...
			createSelectOneRecordButton: function() {
				return this.getButtonMenuItem({
					...
					"Visible": {"bindTo": "isSingleSelectActionButtonVisible"},
			...
			getViewOptions: function() {
				const viewOptions = this.Ext.create("BPMSoft.BaseViewModelCollection");
			...
			getCanDesignSection: function() {
				const isSchemaRegisteredInModuleStructure =
					!Ext.isEmpty(BPMSoft.configuration.ModuleStructure[this.entitySchemaName]);
			...
			initActiveViewSettingsProfile: function(callback, scope) {
				...
				BPMSoft.require(["profile!" + profileKey], function(profile) {
			...
			_hasSummaryCountFunction: function(summaryCollection) {
				const hasCountSummary = this.isNotEmpty(
					this.BPMSoft.findWhere(summaryCollection, {1: "COUNT"}));
			...
			getActiveViewGridSettingsProfile: function(callback, scope) {
				this.requireProfile(function(profile) {
					profile = BPMSoft.ColumnUtilities.updateProfileColumnCaptions({
						profile: profile,
			...
			rowSelected: function(primaryColumnValue) {
				...
				const actionMenuElement = Ext.select(`.grid-row-actions[id*=${primaryColumnValue}]`).item(0);
        		this._actionMenuPosition(actionMenuElement);
			...
			// todo apply when a way is found to hang a handler on a click event
			/**
			 * Add listeners for element to hide action menu
			 */
			// menuClickListeners: function () {
			// 	const el = document.querySelector('.grid.grid-listed')
			// 		? document.querySelector('.grid.grid-listed')
			// 		: document.querySelector('.grid.grid-tiled');
			// 	let isActionMenuOpen = false;
			// 	let currentGridRowId;
			// 	let actionMenuElement;
			// 	let closestParent;
			//
			// 	el?.addEventListener('click', (event) => {
			// 		closestParent = event.target.closest('.grid-active-selectable');
			// 		actionMenuElement = closestParent.querySelector('.grid-row-actions');
			//
			// 		if (event.target.closest('a')) {
			// 			isActionMenuOpen = this._actionMenuClose(closestParent, actionMenuElement);
			// 			return;
			// 		}
			//
			// 		this._actionMenuPosition(actionMenuElement);
			//
			// 		if (closestParent && isActionMenuOpen && closestParent.id === currentGridRowId) {
			// 			isActionMenuOpen = this._actionMenuClose(closestParent, actionMenuElement);
			// 		} else {
			// 			isActionMenuOpen = this._actionMenuOpen(closestParent, actionMenuElement);
			// 			currentGridRowId = closestParent.id;
			// 		}
			// 	});
			//
			// 	window.addEventListener('scroll', () => {
			// 		if (isActionMenuOpen && closestParent && actionMenuElement) {
			// 			isActionMenuOpen = this._actionMenuClose(closestParent, actionMenuElement);
			// 		}
			// 	});
			// },
			...
			/**
			 * Shows the action menu on an element
			 * @param actionMenuEl it's window action menu DOM element
			 * @private
			 */
			_actionMenuPosition: (element) => {
				
				if(!element) { 
					return;
				}

				const {dom: actionMenuEl } = element;

				if (!actionMenuEl.offsetParent) {
					return;
				}

				const docHeight = document.documentElement.clientHeight;

				const actionMenuOffsetParent = actionMenuEl.offsetParent.offsetParent;
				const containerBottom = actionMenuOffsetParent.getBoundingClientRect().bottom;
				const containerTopWithBias = actionMenuOffsetParent.getBoundingClientRect().top + actionMenuEl.offsetHeight;

				const resultHeight = docHeight < actionMenuOffsetParent.clientHeight ? docHeight : containerBottom;
				const elementBottom = actionMenuEl.getBoundingClientRect().bottom;
				const elementTop = actionMenuEl.getBoundingClientRect().top;
				if(resultHeight - elementBottom < 0 && containerTopWithBias < elementTop) {
					actionMenuEl.style.top = `-${actionMenuEl.offsetHeight}px`;
					actionMenuEl.style.bottom = 0; 
					actionMenuEl.style.height = 'max-content';
				}
			},

			/**
			 * open action menu
			 * @param closestParent parent element for action menu
			 * @param menuEl action menu element
			 * @returns {boolean}
			 * @private
			 */
			// _actionMenuOpen: (closestParent, menuEl) => {
			// 	closestParent.classList.add('grid-row-selected');
			// 	menuEl.classList.remove('grid-row-hidden');
			//
			// 	return true;
			// },
			 ...
			 * @private
			 */
			// _actionMenuClose: (closestParent, menuEl) => {
			// 	closestParent.classList.remove('grid-row-selected');
			// 	menuEl.classList.add('grid-row-hidden');
			//
			// 	return false;
			// },
			...
			saveMultiSelectState: function() {
				...
				this.set("MultiSelectState", {
					...
					selectedRows: BPMSoft.deepClone(selectedRows)
				});
			},
			...
			restoreMultiSelectState: function() {
				...
				if (storage) {
					if (storage.multiSelect) {
						this.setMultiSelect();
						const rows = BPMSoft.deepClone(storage.selectedRows);
			...
			/**
			 * @inheritdoc BPMSoft.GridUtilities#setMultiSelect
			 * @overridden
			 */
			...
			/**
			 * @inheritdoc BPMSoft.GridUtilities#unSetMultiSelect
			 * @overridden
			 */
			...
			/**
			 * @inheritdoc BPMSoft.GridUtilities#clearSelection
			 * @overridden
			 */
			...
			/**
			 * @inheritdoc BPMSoft.GridUtilities#setSelectAllMode
			 * @overridden
			 */
			...
			createScrollTopBtn: function() {
				...
				Ext.EventManager.addListener(window, "scroll", function() {
					...
					if (historyStateInfo.workAreaMode === ConfigurationEnums.WorkAreaMode.SECTION) {
						const contentContainer = Ext.getBody().select("#ContentContainer").elements[0] || null;
						const contentContainerStyleTop = contentContainer ? window.getComputedStyle(contentContainer, null).getPropertyValue("top").replace(/[^0-9]/g,"") : null;
						scope.set("ScrollTop", top);
						scope.updateScrollTopBtnPosition(scope, top + Ext.getBody().getHeight() - 200 - contentContainerStyleTop);
				...
				Ext.EventManager.addListener(Ext.get(this.getScrollContainerId()), "scroll", function() {
					...
					if (historyStateInfo.workAreaMode === ConfigurationEnums.WorkAreaMode.COMBINED) {
						...
						if (scrollContainer) {
							scope.updateScrollTopBtnPosition(scope, scrollContainer.getHeight());
						}
					}
				});
			},
			...
			getScrollTopButtonImageConfig: function() {
				return {
					...
					url: BPMSoft.ImageUrlBuilder.getUrl(this.get("Resources.Images.scrollTopImage"))
				};
			},
			...
			afterLoadGridDataUserFunction: function(primaryColumnValue) {
				if (this.get("MultiSelect")) {
					const selectedRows = BPMSoft.deepClone(this.get("SelectedRows"));
			...
			runProcessWithParameters: function(config) {
				...
				BPMSoft.ProcessModuleUtilities.runProcessWithParameters(config);
			},
			...
			getLastFilterKey: function() {
				...
				filters = filters.filter(function(filter) {
					...
					const isFilterGroup = (filter instanceof BPMSoft.FilterGroup);
					const notEmptyFilterGroup = (isFilterGroup && !filter.isEmpty() && !(filter.getByIndex(0).key === "undefined" &&
						(filter.getByIndex(0) instanceof BPMSoft.FilterGroup) && filter.getByIndex(0).isEmpty()));
					...
					items: [
						{
							...
							imageSrc: BPMSoft.ImageUrlBuilder.getUrl(emptyGridMessageProperties.image)
						}
					]
				});
				config.items.push({
			...
			getExportToFileMenuItem: function() {
				return this.getButtonMenuItem({
					...
					"Visible": BPMSoft.Features.getIsEnabled("ExportToCSV")
				});
			},
			 ...
			 * @protected
			 * @return {BPMSoft.BaseViewModel} "Export to excel file" menu item.
			 */
			 ...
			 * @protected
			 * @return {BPMSoft.BaseViewModel} Function button drop-down list item that is responsible
			 ...
			 * @return {BPMSoft.BaseViewModel} "Add tags" menu item.
			 */
			getAddTagsMenuItem: function() {
				return this.getButtonMenuItem({
					"Caption": {"bindTo": "Resources.Strings.MultiTagButtonCaption"},
					"Visible": BPMSoft.Features.getIsEnabled("MultipleTagging"),
			...
			onMultiSelectChange: function() {
				const styleButton = BPMSoft.controls.ButtonEnums.style.DEFAULT
			...
			onSelectedRowsChange: function() {
				if (this.get("MultiSelect")) {
					const selectedRows = this.get("SelectedRows") || [];
					const gridDataRows = this.get("GridData").getKeys() || [];
					if (selectedRows.length !== gridDataRows.length && gridDataRows.length !== 0 ) {
						this.set("SelectAllCheckBox", undefined);
					}
			...
			getToggleSectionButtonIsVisible: function() {
				return BPMSoft.Features.getIsDisabled("OldUI");
			},

			/**
			 * @inheritdoc BPMSoft.configuration.mixins.GridUtilities#unSetSelectAllMode
			 * @overridden
			 */
			 ...
			 * @protected
			 * @param {BPMSoft.BaseViewModel} menuItem Menu item.
			 */
			...
			onLeftSectionContainerSizeChanged: function(currentSize) {
				...
				BPMSoft.saveUserProfile(profileKey, this.get("LeftSectionContainerSize"), false);
			},
			...
			changeGridUtilitiesContainerSize: function() {
				if (!this.throttledChangeGridUtilitiesContainerSize) {
					this.throttledChangeGridUtilitiesContainerSize = BPMSoft.throttle(function() {
			...
			initLeftSectionContainerSize: function(callback, scope) {
				BPMSoft.require(["profile!" + this.getLeftSectionContainerSizeProfileKey()],
			 ...
			 */
			 onChangeSelectAllCheckBox: function(value) {
				if (this.get("IsSectionVisible")) {
					this.set("SelectAllCheckBox", value);
					if (value) {
			...
			/**
			 * Get caption to sort by column.
			 * @protected
			 */
			getSortCaption: function() {
				const propertyName = this.getDataGridName();
				const columnsSettingsProfile = propertyName ? this.get("Profile")[propertyName] : this.get("Profile");
				...
				const sortCaption = this.get(this.name + this.getSortColumnByIndex(this.get("SortColumnIndex"), columnsSettingsProfile) + "_SortedColumnCaption");
				if (gridsColumnsConfig && this.get("GridSortDirection") && sortCaption) {
					return sortCaption;
				} else {
					return this.get("Resources.Strings.SortMenuCaption");
				}
			}
		...
		diff: [
			...
			},
			{
				...
				"values": {
					...
					"itemType": BPMSoft.ViewItemType.SECTION_VIEWS,
			...
			},
			{
				...
				"values": {
					"generateId": false,
					"itemType": BPMSoft.ViewItemType.CONTAINER,
					"wrapClass": ["selectAll-container-wrapClass"],
					"visible": {"bindTo": "isMultiSelectActive"},
					"items": []
				}
			},
			{
				"operation": "insert",
				"name": "SelectAllTiledContainer",
				"parentName": "SelectAllContainer",
				"propertyName": "items",
				"values": {
					"id": "SelectAllTiledContainer",
					"selectors": {wrapEl: "#SelectAllTiledContainer"},
					"itemType": BPMSoft.ViewItemType.CONTAINER,
					"visible": {"bindTo": "isSingleSelectVisible"},
					"wrapClass": ["selectAll-button-container-wrapClass"],
					"items": []
				}
			},
			{
				"operation": "insert",
				"parentName": "SelectAllTiledContainer",
				"propertyName": "items",
				"name": "SelectAllTiled",
				"values": {
					"caption": {
					...
					"bindTo": "SelectAllCheckBox",
					"controlConfig": {
						"className": "BPMSoft.CheckBoxEdit",
						"checkedchanged": {
							"bindTo": "onChangeSelectAllCheckBox"
						},
						"checked": {
							"bindTo": "SelectAllCheckBox"
						}
			...
			},
			{
				...
				"propertyName": "items",
				"values": {
					"id": "SectionSortMenuContainer",
					"selectors": {"wrapEl": "#SectionSortMenuContainer"},
					"itemType": BPMSoft.ViewItemType.CONTAINER,
					"visible": {"bindTo": "IsSortMenuVisible"},
					"wrapClass": ["sort-menu-container-wrapClass"],
					"items": []
				}
			},
			{
				"operation": "insert",
				"name": "SortMenuButton",
				"parentName": "SortMenuContainer",
				"propertyName": "items",
				"values": {
					"itemType": BPMSoft.ViewItemType.BUTTON,
					"caption": {"bindTo": "getSortCaption"},
					"menu": {
			...
			},
			{
				...
				"parentName": "DataGridContainer",
				"propertyName": "items",
				"name": "SelectAllListed",
				"values": {
					"labelConfig": {"visible": false},
					"bindTo": "SelectAllCheckBox",
					"wrapClass": ["selectAll-listed-container"],
					"visible": {"bindTo": "SelectAllListedItemVisible"},
					"controlConfig": {
						"className": "BPMSoft.CheckBoxEdit",
						"checkedchanged": {
							"bindTo": "onChangeSelectAllCheckBox"
			...
			{
				"operation": "insert",
				"name": "DataGridActiveRowOpenActionNew",
				"parentName": "DataGrid",
