/**
 * @class BPMSoft.Configuration.controls.SocialMessageContentEdit
 *
 * Component that allows to input text with tags.
 */
Ext.define("BPMSoft.Configuration.controls.SocialMessageContentEdit", {
...
/**
 * @class BPMSoft.Configuration.controls.SocialMessageHtmlField
 *
 * Html field with tags.
 */
Ext.define("BPMSoft.configuration.controls.SocialMessageHtmlField", {
	extend: "Ext.field.Text",
	alternateClassName: "BPMSoft.SocialMessageHtmlField",
	...
	config: {
		...
		/**
		 * @cfg {BPMSoft.Configuration.controls.SocialMessageContentEdit} component Inner component.
		 */
		component: {
			xtype: "cfsocialmessagecontentedit"
		},
	...
	getPickerQueryConfig: function() {
		var modelName = this.getCurrentModelName();
		var queryConfig = BPMSoft.sdk.LookupGridPage.getQueryConfig(modelName);
	...
	getFormattedInnerValue: function(innerValue) {
		innerValue = BPMSoft.util.String.replaceAll(innerValue, "</div>", "");
		innerValue = BPMSoft.util.String.replaceAll(innerValue, "<div>", "\n");
		return innerValue;
	},
	...
	getValueWithTags: function(value) {
		if (this.currentTags && this.currentTags.length > 0) {
			...
			for (var i = 0, ln = this.currentTags.length; i < ln; i++) {
				...
				var href = BPMSoft.util.encodeServiceUrl(url);
				var link = Ext.String.format(tagTpl, recordId, modelName, href, name);
				value = BPMSoft.util.String.replaceAll(value, this.encodeValueForReplace(name),
	...
	getValueWithoutTags: function(value) {
		...
		for (var i = 0, ln = this.currentTags.length; i < ln; i++) {
			...
			value = BPMSoft.util.String.replaceAll(value, link, this.getTagElement(tag.id, tag.name));
		}
		return value;
	},
	 ...
	 */
	onFocus: function() {
		BPMSoft.nativeApi.Keyboard.ActiveElement = this.element.dom;
		BPMSoft.nativeApi.Keyboard.showIfNeeded();
		this.callParent(arguments);
	 ...
	 */
	onBlur: function() {
		BPMSoft.nativeApi.Keyboard.ActiveElement = null;
		BPMSoft.nativeApi.Keyboard.hideIfNeeded();
		this.callParent(arguments);
	 ...
	 * @param {BPMSoft.model.BaseModel} record Selected record.
	 * @protected
	 * @virtual
	 */
	...
	setInternalValue: function(value) {
		...
		this.element.setHtml(BPMSoft.util.String.replaceAll(value, "\n", "</div><div>"));
	},
	...
	constructor: function() {
		...
		this.currentTags = [];
		this.mentions = [];
		this.parseTask = Ext.create("BPMSoft.util.DelayedTask", this.processFieldValue, this, null, {isCancelable: true});
	},
	...
	/**
	 * Returns picker.
	 * @returns {Ext.BPMSoft.LookupPicker} Picker.
	 */
	getPicker: function() {
		var modelName = this.getCurrentModelName();
		...
		var gridModelConfig = BPMSoft.sdk.LookupGridPage.getConfig(modelName);
		var primaryDisplayColumnName = this.getCurrentModelPrimaryDisplayColumnName();
		var defaultImageUrl = BPMSoft.Configuration.getDefaultOwnerImage();
		var moduleConfig = BPMSoft.ApplicationConfig.getModuleConfig(modelName);
		...
		if (!picker || this.needRecreatePicker()) {
			...
			picker = this.picker = Ext.create("Ext.BPMSoft.LookupPicker", {
				component: {
					store: store,
		...
		filterPanel.addModule({
			...
			component: {
				listeners: {
					painted: function() {
						if (!BPMSoft.util.isWindowsPlatform()) {
