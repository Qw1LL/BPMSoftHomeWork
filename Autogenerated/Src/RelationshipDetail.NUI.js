define('RelationshipDetail', ['ext-base', 'BPMSoft', 'sandbox', 'VwAccountRelationship', 'VwContactRelationship',
		'Contact', 'Account', 'ConfigurationConstants', 'ConfigurationEnums', 'RelationshipDetailStructure',
		'RelationshipDetailResources'], function(Ext, BPMSoft, sandbox, VwAccountRelationship, VwContactRelationship,
...
//				var selectedRows = this.GetSelectedItems();
		...
		this.methods.applyFilter = function(select, args) {
			...
			if (filterPath === 'Account') {
				var filterAccountA = BPMSoft.createColumnFilterWithParameter(
					BPMSoft.ComparisonType.EQUAL, 'Account', filterValue);
				select.filters.add('filterAccountA', filterAccountA);
				return false;
			} else if (filterPath === 'Contact') {
				var filterContactA = BPMSoft.createColumnFilterWithParameter(
					BPMSoft.ComparisonType.EQUAL, 'Contact', filterValue);
