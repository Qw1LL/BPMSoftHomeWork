define("TimelineFilterMenu", ["TimelineFilterMenuResources", "css!TimelineFilterMenu"], function(resources) {
	Ext.define("BPMSoft.controls.TimelineFilterMenu", {
		extend: "BPMSoft.Menu",
		alternateClassName: "BPMSoft.TimelineFilterMenu",
		...
		/**
		 * @inheritdoc BPMSoft.Menu#defaultUlClass
		 * @override
		 */
		...
		/**
		 * @inheritdoc BPMSoft.Menu#getItemsHtml
		 * @override
		 */
		getItemsHtml: function() {
			...
			items.each(function(item) {
				...
				if (currentColumnItems.length >= columnLength) {
					var columnItemHtml = Ext.String.format(this.itemsListTemplate,
						BPMSoft.generateGUID(), currentColumnItems.join(""));
			...
			var columnsContainerHtml = Ext.String.format(this.itemsColumnContainerTemplate,
				BPMSoft.generateGUID(), itemsHtml.join(""));
			return [columnsContainerHtml];
		},

		/**
		 * @inheritdoc BPMSoft.Menu#getTplData
		 * @override
		 */
		getTplData: function() {
		...
		/**
		 * @inheritdoc BPMSoft.Menu#hide
		 * @override
		 */
		hide: function() {
			this.callParent(arguments);
			this.items.each(function(item) {
				if (BPMSoft.isInstanceOfClass(item, "BPMSoft.TimelineFilterMenuItem")) {
		...
		/**
		 * @inheritdoc BPMSoft.Menu#getBindConfig
		 * @override
		 */
		...
		/**
		 * @inheritdoc BPMSoft.Menu#renderContent
		 * @override
		 */
		...
		getMenuItemConfig: function(itemModel) {
			...
			if (Ext.isEmpty(itemId)) {
				this.log(BPMSoft.Resources.Controls.Menu.MenuPrimaryColumnIsEmpty, BPMSoft.LogMessageType.WARNING);
			}
			...
			if (menuItemConfig.className === "BPMSoft.TimelineFilterMenuItem") {
		...
		/**
		 * @inheritdoc BPMSoft.Menu#addItem
		 * @override
		 */
		addItem: function() {
		...
		createApplyButton: function() {
			...
			this._applyButtonControl = Ext.create("BPMSoft.Button", config);
		...
		createAllMenuItem: function() {
			this._allMenuItemControl = Ext.create("BPMSoft.TimelineFilterMenuItem", {
		...
		isAllItemSelected: function() {
			...
			this.items.each(function(item) {
				if (BPMSoft.isInstanceOfClass(item, "BPMSoft.TimelineFilterMenuItem") && !item.checked) {
		 ...
		 * @protected
		 * @param {BPMSoft.TimelineFilterMenuItem} item Menu item.
		 */
		...
		/**
		 * @inheritdoc BPMSoft.Menu#afterRenderOrRerender
		 * @override
		 */
