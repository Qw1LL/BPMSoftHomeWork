	...
	return {
					...
					"conditions": [
						{
							...
							"rightExpression": {
								...
								"value": BPMSoft.SysSettingsRightsOperationType.ALLOW_BY_OPERATION
							}
						}
					]
				}
		...
		methods: {
			...
			_saveRightsOperationInitialValues: function() {
				this.$ReadRightsOperationBackup = BPMSoft.deepClone(this.$ReadRightsOperation);
				this.$ReadWriteRightsOperationBackup = BPMSoft.deepClone(this.$ReadWriteRightsOperation);
				this.$ReadSysAdminOperationBackup = BPMSoft.deepClone(this.$ReadSysAdminOperation);
				this.$ReadWriteSysAdminOperationBackup = BPMSoft.deepClone(this.$ReadWriteSysAdminOperation);
			},
			...
			_restoreRightsOperationInitialValues: function() {
				this.$ReadRightsOperation = BPMSoft.deepClone(this.$ReadRightsOperationBackup);
				this.$ReadWriteRightsOperation = BPMSoft.deepClone(this.$ReadWriteRightsOperationBackup);
				this.$ReadSysAdminOperation = BPMSoft.deepClone(this.$ReadSysAdminOperationBackup);
				this.$ReadWriteSysAdminOperation = BPMSoft.deepClone(this.$ReadWriteSysAdminOperationBackup);
			},

			/**
			 * Create save request based on passed class name - either update or insert.
			 * @private
			 * @return {BPMSoft.SysSettingSaveRequest} Create request.
			 */
			_getSaveRequest: function() {
				const saveRequestClassName = this.isNew
					? "BPMSoft.InsertSysSettingRequest"
					: "BPMSoft.UpdateSysSettingRequest";
				const saveRequest = Ext.create(saveRequestClassName, {
					id: this.get(this.primaryColumnName)
				});
				BPMSoft.each(saveRequest.propertyColumnName, function(columnName, propertyName) {
			...
			_onReferenceSchemaUIdChange: function() {
				...
				const item = BPMSoft.EntitySchemaManager.findItem(referenceSchemaUId);
			 ...
			 * @return {BPMSoft.EntitySchemaQuery} Query to get sys admin operations.
			 * @private
			 */
			_getSysAdminOperationPropertiesQuery: function(operationNames) {
				const query = Ext.create("BPMSoft.EntitySchemaQuery", {
					rootSchemaName: "SysAdminOperation"
				});
				query.addColumn("Id");
				query.addColumn("Name");
				query.addColumn("Code");
				const codeFilter = BPMSoft.createColumnInFilterWithParameters(
					"Code", operationNames, BPMSoft.DataValueType.TEXT);
			...
			_saveRights: function(callback, scope) {
				...
				if (this.$ReadRightsOperation === BPMSoft.SysSettingsRightsOperationType.ALLOW_BY_OPERATION) {
					readOperationCode = this.$ReadSysAdminOperation && this.$ReadSysAdminOperation.Code;
				} else if (this.$ReadRightsOperation === BPMSoft.SysSettingsRightsOperationType.RESTRICT_ALL) {
					readOperationCode = "CanManageSysSettings";
				} else {
					readOperationCode = null;
				}
				if (this.$ReadWriteRightsOperation === BPMSoft.SysSettingsRightsOperationType.ALLOW_BY_OPERATION) {
					readWriteOperationCode = this.$ReadWriteSysAdminOperation && this.$ReadWriteSysAdminOperation.Code;
				} else if (this.$ReadWriteRightsOperation === BPMSoft.SysSettingsRightsOperationType.RESTRICT_ALL) {
					readWriteOperationCode = "CanManageSysSettings";
				} else {
					readWriteOperationCode = null;
				}
				BPMSoft.SysSettings.setSysSettingsRightsAsync({
			...
			_setRightsSysAdminOperation: function(operationRightsAttributeName, operationAttributeName, operationValue) {
				if (operationValue) {
					if (operationValue.$Code === "CanManageSysSettings") {
						this.set(operationRightsAttributeName, BPMSoft.SysSettingsRightsOperationType.RESTRICT_ALL);
						this.set(operationAttributeName, null);
					} else {
						this.set(operationRightsAttributeName,
							BPMSoft.SysSettingsRightsOperationType.ALLOW_BY_OPERATION);
					...
					this.set(operationRightsAttributeName, BPMSoft.SysSettingsRightsOperationType.ALLOW_ALL);
					this.set(operationAttributeName, null);
				}
			},
			...
			_loadSysAdminOperations: function(callback, scope) {
				...
				if (!code || !this.isUseSysSettingsRights()) {
					...
					BPMSoft.SysSettings.getSysSettingsRightsAsync({sysSettingsCode: this.$Code})
			...
			_getSecureValue: function(rawValue) {
				if (BPMSoft.useSecureSettingsOnClient !== true) {
					return "123456789";
				}
				return rawValue;
			},
			...
			onEntityInitialized: function() {
				...
				if (this.Ext.isEmpty(primaryColumnValue) || this.isNew) {
					this.set(this.primaryColumnName, this.BPMSoft.generateGUID());
				}
				this.callParent(arguments);
				const typeDefaultConfig = this.BPMSoft.SysSettings.getTypes();
			...
			asyncValidate: function(callback, scope) {
				this.callParent([
					function(result) {
						...
						if(this.isLookup() && this.Ext.isEmpty(this.$ReferenceSchema)) {
							result.success = false,
							result = {
								message: Ext.String.format(
									BPMSoft.Resources.BaseViewModel.fieldValidationError,
									this.getColumnCaptionByName("ReferenceSchema"),
			...
			loadLookupData: function(filter, list, columnName) {
				...
				if (column.referenceSchemaName) {
					...
					if(column.referenceSchemaName == "SysLanguage") 
					filter = "русский (Россия)"

					if (column.referenceSchemaName == 'SysCulture') 
					filter = 'ru-RU'
			...
			loadValueFromSysSettings: function(code, callback, scope) {
				BPMSoft.SysSettings.querySysSetting(code, function(sysSettingObject) {
			...
			clearValues: function() {
				const valueAttributeNames = this.getValueAttributeNames();
				BPMSoft.each(valueAttributeNames, function(valueAttributeName) {
			...
			saveValue: function(callback, scope) {
				...
				BPMSoft.SysSettings.postSysSettingsValues(config, callback, scope);
			},
			 ...
			 * @param {BPMSoft.core.collections.Collection} list List to be fulfilled.
			 */
			prepareTypeList: function(filter, list) {
				...
				list.clear();
				list.loadAll(this.BPMSoft.SysSettings.getTypes());
			},
			 ...
			 * @virtual
			 * @param {String} filter Filter string.
			 * @param {BPMSoft.core.collections.Collection} list List to fulfill.
			 */
			...
			onSaveToFileClick: function() {
				const link = BPMSoft.FileDownloader.getFileLink(this.entitySchema.uId, this.$Id);
			 ...
			 * @protected
			 * @param {Function} callback Callback function.
			 * @param {BPMSoft.BaseSchemaViewModel} scope Callback function scope.
			 */
			validateCodeDuplicate: function(callback, scope) {
				if (!this.changedValues || this.Ext.isEmpty(this.changedValues.Code)) {
					...
					const idFilter = select.createColumnFilterWithParameter(this.BPMSoft.ComparisonType.NOT_EQUAL,
							"Id", id);
					select.filters.addItem(idFilter);
					const codeFilter = select.createColumnFilterWithParameter(this.BPMSoft.ComparisonType.EQUAL,
							"Code", code);
			 ...
			 * @param {BPMSoft.BaseSchemaViewModel} scope Finite callback function scope.
			 */
			 ...
			 * @param {BPMSoft.SysSettingsRightsOperationType} value System setting rights operation type.
			 * @return {Boolean} Visibility of the lookup.
			 */
			isAllowByOperation: function(value) {
				return value === BPMSoft.SysSettingsRightsOperationType.ALLOW_BY_OPERATION;
			},
			...
			isUseSysSettingsRights: function() {
				return BPMSoft.Features.getIsEnabled("UseSysSettingsRights");
			},
			...
			isEnableReadSysSettingsRights: function() {
				...
				if (this.isSecureTextSysSettingType()) {
					enableReadSysSettingsRights = enableReadSysSettingsRights &&
						BPMSoft.useSecureSettingsOnClient === true;
				}
			...
			/**
			 * @inheritdoc BPMSoft.BaseViewModel#saveEntity
			 * @override
			 */
		...
		diff: /**SCHEMA_DIFF*/[
			...
			},
			{
				...
				"values": {
					...
					"value": BPMSoft.SysSettingsRightsOperationType.ALLOW_ALL
				},
				"index": 0
			},
			{
				...
				"values": {
					...
					"value": BPMSoft.SysSettingsRightsOperationType.RESTRICT_ALL
				},
				"index": 1
			},
			{
