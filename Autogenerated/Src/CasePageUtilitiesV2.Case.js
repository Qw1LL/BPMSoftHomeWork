define("CasePageUtilitiesV2", ["StorageUtilities"],
	function(StorageUtilities) {
		/**
		 * @class BPMSoft.configuration.mixins.CasePageUtilitiesV2
		 * Mixin, implemented quickly saving case card with the selected case status.
		 */
		Ext.define("BPMSoft.configuration.mixins.CasePageUtilitiesV2", {
			alternateClassName: "BPMSoft.CasePageUtilitiesV2",
			...
			generateResolvedButtonMenuItems: function() {
				this.log(this.Ext.String.format(this.BPMSoft.Resources.ObsoleteMessages.ObsoleteMethodMessage,
					"generateResolvedButtonMenuItems", "initResolvedButtonCollection"), BPMSoft.LogMessageType.WARNING);
			},
			...
			getResolvedButtonMenu: function() {
				this.log(this.Ext.String.format(this.BPMSoft.Resources.ObsoleteMessages.ObsoleteMethodMessage,
					"getResolvedButtonMenu", "initResolvedButtonCollection"), BPMSoft.LogMessageType.WARNING);
			},
			...
			initCollection: function() {
				if (!this.get("ResolvedButtonMenuItems")) {
					this.set("ResolvedButtonMenuItems", this.Ext.create("BPMSoft.BaseViewModelCollection"));
				}
			 ...
			 * @protected
			 * @param {BPMSoft.Collection} caseStatuses Response collection from query.
			 */
			initializeCaseStatuses: function(caseStatuses) {
				this.caseStatuses = {};
				var caseStatusCollection = caseStatuses.getItems();
				BPMSoft.each(caseStatusCollection, function(currentStatus) {
					var element = {
			 ...
			 * @protected
			 * @param {BPMSoft.Collection} availableStages Available stages from Dcm.
			 * @param {BPMSoft.Collection} caseStatusCollection Response from query to CaseStatus.
			 * @return {Object} CaseNextStatusElementCollection.
			 */
			createCaseNextStatus: function(availableStages, caseStatusCollection) {
				var result = {};
				BPMSoft.each(availableStages, function(currentStage) {
			 ...
			 * @private
			 * @return {BPMSoft.EntitySchemaQuery} Esq to case statuses.
			 */
			...
			getCaseNextStatuses: function() {
				...
				sortById.orderDirection = this.BPMSoft.OrderDirection.ASC;
				var sortByCreatedOn = esq.addColumn("CreatedOn");
				sortByCreatedOn.orderPosition = 1;
				sortByCreatedOn.orderDirection = this.BPMSoft.OrderDirection.ASC;
			...
			getCaseStatus: function() {
				...
				esq.filters.add("caseFilter", this.BPMSoft.createColumnFilterWithParameter(
					BPMSoft.ComparisonType.EQUAL, "Id", this.caseRecordId));
			...
			getFlatCaseStatusCache: function() {
				...
				this.BPMSoft.each(this.caseStatusCache, function(outerObj) {
					this.BPMSoft.each(outerObj, function(innerObj, index) {
