define("GridLayoutUtils", ["ext-base", "BPMSoft"], function(Ext, BPMSoft) {
	Ext.define("BPMSoft.configuration.GridLayoutUtils", {
		alternateClassName: "BPMSoft.GridLayoutUtils",
		singleton: true,
		...
		compareItemsPositions: function(item1, item2) {
			var item1Layout = item1.layout || BPMSoft.deepClone(this.defaultItemLayout);
			var item2Layout = item2.layout || BPMSoft.deepClone(this.defaultItemLayout);
		...
		fixItemsIntersections: function(items, columnsCount) {
			...
			BPMSoft.each(sortedItems, function(item) {
				var itemBitmap = this.getItemBitmap(item.layout, maxColumnsCount);
				BPMSoft.each(solvedItems, function(currentSolvedItem) {
	...
	return BPMSoft.GridLayoutUtils;
});
