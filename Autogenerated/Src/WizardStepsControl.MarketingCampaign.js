/**
 * @class BPMSoft.controls.WizardStepsControl
 * ##### ######## ########## ##### #######.
 */
Ext.define("BPMSoft.controls.WizardStepsControl", {
	extend: "BPMSoft.AbstractContainer",
	alternateClassName: "BPMSoft.WizardStepsControl",
	...
	init: function() {
		...
		this.addEvents(
			 ...
			 * @param {BPMSoft.WizardStepsControl} this
			 */
			"afterStepChange",
			/**
			 * @event
	...
	constructor: function(config) {
		...
		BPMSoft.each(this.wizardSteps, function(item, index) {
			item.tplData = {
				clienIndex: index + 1,
	...
	/**
	 * @inheritdoc BPMSoft.AbstractContainer#initItems
	 * @overriden
	 */
	initItems: function() {
		this.callParent(arguments);
		BPMSoft.each(this.wizardSteps, function(step, index) {
			var parentSelector = this.id;
			var wrapElId = parentSelector + "-" + index;
			var stepEl = Ext.create("BPMSoft.Button", {
				tpl: [
					// jscs:disable validateQuoteMarks
					/*jshint quotmark: false */
	...
	generateStepStyles: function(buffer, data) {
		...
		if (clientIndex <= controlScope.passedStep) {
			...
			var imgUrl = BPMSoft.ImageUrlBuilder.getUrl(imgConf);
	 ...
	 * ########## ####### ##### ## ###### ####.
	 * @protected
	 * @param {BPMSoft.Button} stepControl ####### ########## ###### ####.
	 */
