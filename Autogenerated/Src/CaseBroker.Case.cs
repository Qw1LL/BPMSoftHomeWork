namespace BPMSoft.Configuration.CaseService
{
	using System;
	...
	public class CaseBroker
	{
		...
		public virtual bool CloseOnCondition(Entity entity, Predicate<Entity> predicate, Guid statusId, 
			...
			if (predicate(entity)) {
				_log.InfoFormat($"[CaseId: {entity.PrimaryColumnValue}] - Start closing case");
				var userConnection = entity.UserConnection;
				var localEntity = GetEntity(entity, needFetch);
				...
				localEntity.Save(false);
				_log.InfoFormat($"[CaseId: {entity.PrimaryColumnValue}] - Case is closed");
				return true;
			}
			return false;
		}
