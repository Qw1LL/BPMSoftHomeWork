define('ColumnPageViewModel', ['ext-base', 'BPMSoft', 'sandbox', 'ColumnPageViewModelResources', 'LookupUtilities',
	'ColumnHelper'], function(Ext, BPMSoft, sandbox, resources, LookupUtilities, ColumnHelper) {
		var ajaxProvider;
		var columnConfig;
		var entitySchema;
		var entityColumn;
		...
		function generate(parentSandbox, stateColumnConfig) {
			...
			var viewModelConfig = {
				...
				values: {
					EnumCollection: new BPMSoft.Collection(),
					EnumList: new BPMSoft.Collection(),
		...
		function getIsLookup(dataValueType) {
			return (dataValueType === BPMSoft.DataValueType.LOOKUP || dataValueType === BPMSoft.DataValueType.ENUM);
		}

		function getIsEnum(dataValueType) {
			return (dataValueType === BPMSoft.DataValueType.ENUM);
		}
		...
		function duplicateChecked() {
			...
			var columnName = BPMSoft.deepClone(this.get('Name'));
			...
			var isExists = false;
			BPMSoft.each(columnConfig.changeColumns, function(item) {
		...
		function init(onInit) {
			...
			if (itemConfig) {
				...
				if (!Ext.isEmpty(baseColumn)) {
					entityColumn = BPMSoft.deepClone(baseColumn);
				}
				if (!Ext.isEmpty(changeColumn)) {
					entityColumn = BPMSoft.deepClone(changeColumn);
				}
		...
		function getChangedColumnByName(name) {
			...
			BPMSoft.each(changeColumns, function(item) {
				if (item.name === name) {
					column = item;
				}
			});
		...
		function getLookupFilters() {
			...
			filterCollection.add("SysWorkspace", BPMSoft.createColumnFilterWithParameter(
		...
		function generateItemConfig() {
			...
			if (entityColumn.isLookup) {
				if (this.get('IsEnum')) {
					itemConfig.dataValueType = entityColumn.dataValueType = BPMSoft.DataValueType.ENUM;
				}
		...
		function updateItemConfig() {
			...
			if (entityColumn.isLookup) {
				itemConfig.dataValueType = entityColumn.dataValueType = this.get('IsEnum') ?
					BPMSoft.DataValueType.ENUM : BPMSoft.DataValueType.LOOKUP;
			}
		...
		function loadEnumItems(filter, list) {
			...
			var enumCollection = this.get('EnumCollection');
			BPMSoft.each(enumCollection.getItems(), function(item) {
