define("SortableGridViewModelMixin", ["SortableGridViewModelMixinResources"], function(resources) {
	Ext.define("BPMSoft.core.mixins.SortableGridViewModelMixin", {
		alternateClassName: "BPMSoft.SortableGridViewModelMixin",
		...
		_getDefaultSortProfile: function() {
			return [{}, {orderDirection: BPMSoft.OrderDirection.ASC}];
		},
		...
		_saveSortingSettings: function(index, direction) {
			...
			BPMSoft.utils.saveUserProfile(this._getProfileKey(), columns, false);
		},
		...
		_getColumnSortDirection: function(orderDirection, dataValueType) {
			if (orderDirection) {
				orderDirection = (orderDirection === BPMSoft.OrderDirection.ASC) ?
						BPMSoft.OrderDirection.DESC : BPMSoft.OrderDirection.ASC;
			} else {
		 ...
		 * Sorts grid by column index.
		 * @protected
		 * @deprecated 7.13.0 Use {@link BPMSoft.SortableGridViewModelMixin#sortByColumn} instead.
		 * @param {Object} index Column index.
		 */
		...
		initSortingSettings: function(config, callback, scope) {
			...
			BPMSoft.ProfileUtilities.getProfile({profileKey: this._getProfileKey()}, function(profile) {
				if (!profile) {
