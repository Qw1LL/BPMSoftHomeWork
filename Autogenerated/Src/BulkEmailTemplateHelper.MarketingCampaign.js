define("BulkEmailTemplateHelper", ["BulkEmailTemplateHelperResources", "MarketingEnums", "ServiceHelper",
	...
	function(resources, MarketingEnums, ServiceHelper) {
		Ext.define("BPMSoft.configuration.BulkEmailTemplateHelper", {

			alternateClassName: "BPMSoft.BulkEmailTemplateHelper",
			...
			_isOperatingWithMjml: function(config) {
				var items;
				if (!BPMSoft.isEmptyObject(config)) {
					items = config.Items;
				}
			 ...
			 * @private
			 * @return {BPMSoft.EntitySchemaQuery} Query for preview blocks.
			 */
			_getBlocksEntitySchemaQuery: function(config) {
				...
				var esqFilter = entitySchemaQuery.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL, "Id",
			...
			_getUnsubscribeBlockConfig: function(config, callback, scope) {
				...
				esq.getEntityCollection(function(response) {
					...
					if (response.success && !response.collection.isEmpty()) {
						...
						blockConfig = (serializedBlockConfig && BPMSoft.decode(serializedBlockConfig)) || {};
					}
			...
			_isExistUnsubscribeLinkInBlock: function(block, macros, emailContentExporter) {
				...
				if (block.ItemType === "mjblock") {
					var blockCopy = BPMSoft.deepClone(block);
					config = {
						ItemType: "sheet",
						Items: [
							blockCopy
			 ...
			 * @param {BPMSoft.BaseContentExporter} emailContentExporter Instance of content exporter.
			...
			_checkUnsubscribeLinkForStaticBlocks: function(config, macros, emailContentExporter) {
				...
				BPMSoft.each(staticBlocks, function(block) {
			...
			hasUnsubscribeLink: function(html, macrosCollection) {
				...
				BPMSoft.each(macrosCollection, function(macros) {
			...
			/**
			 * Returns instance of the BPMSoft.BaseContentExporter.
			 * @param {Object} config Content builder template config.
			 * @returns {BPMSoft.BaseContentExporter}
			 */
		...
		return Ext.create("BPMSoft.configuration.BulkEmailTemplateHelper");
	}
);
