define("CardUtilities", ["BusinessRuleModule", "ConfigurationEnums"],
	function(BusinessRuleModule, ConfigurationEnums) {
		...
		function generateItemInfo(schemaItem) {
			...
			switch (itemType) {
				case BPMSoft.ViewModelSchemaItem.ATTRIBUTE:
					BPMSoft.each(schemaItem.columns, function(column) {
						addColumnToCollection(itemColumnPath, column);
					}, this);
					BPMSoft.each(schemaItem.orders, function(order) {
					...
					if (schemaItem.rules !== undefined) {
						...
						BPMSoft.each(schemaItem.rules, function(itemRule) {
							if (Ext.isEmpty(itemRule.logical)) {
								itemRule.logical = BPMSoft.LogicalOperatorType.AND;
							}
							if (!Ext.isEmpty(itemRule.conditions) && !Ext.isEmpty(itemRule.property)) {
								BPMSoft.each(itemRule.conditions, function(condition) {
							...
							if (itemRule.ruleType === BusinessRuleModule.enums.RuleType.FILTRATION ||
								...
								if (Ext.isEmpty(itemRule.type)) {
									itemRule.type = BPMSoft.FilterType.COMPARE;
								}
				...
				case BPMSoft.ViewModelSchemaItem.GROUP:
					...
					if (schemaItem.items !== undefined) {
						BPMSoft.each(schemaItem.items, generateItemInfo, this);
					}
					break;
		...
		function prepareRadioGroupItem(radioGroup, radioItem) {
			...
			BPMSoft.each(radioItem.items, generateItemInfo, this);
		}
		...
		function isDetail(schemaItem) {
			return schemaItem.type === BPMSoft.ViewModelSchemaItem.DETAIL;
		}

		function addGroup(schemaItem) {
			var groups = this.info.groups || [];
			if (Ext.isEmpty(schemaItem.id)) {
				schemaItem.id = BPMSoft.generateGUID() + "-group";
			}
		...
		function prepareSysSettingsDefaultValue(column, columnName) {
			...
			if (defaultValue && defaultValue.source === BPMSoft.EntitySchemaColumnDefSource.SETTINGS) {
		...
		function generateContainerInfo(container) {
			...
			BPMSoft.each(container, generateItemInfo, this);
			return info;
		}
		...
		function getInfo(schema) {
			...
			if (schema.schema.customPanel) {
				BPMSoft.each(schema.schema.customPanel, generateItemInfo, this);
			}
			BPMSoft.each(schema.schema.leftPanel, generateItemInfo, this);
			BPMSoft.each(schema.schema.rightPanel, generateItemInfo, this);
			if (schema.schema.customBottomPanel) {
				BPMSoft.each(schema.schema.customBottomPanel, generateItemInfo, this);
			}
			BPMSoft.each(schema.entitySchema.columns, prepareSysSettingsDefaultValue, this);
			BPMSoft.each(schema.sysSettings, function(sysSettingName) {
