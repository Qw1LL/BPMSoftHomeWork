	...
	return {
		...
		methods: {
			...
			_isExpressionTypeValid: function() {
				...
				return expressionType && expressionType.value !== BPMSoft.TimerExpressionTypes.Empty;
			},
			...
			_getExpressionTypes: function() {
				var result = {};
				BPMSoft.each(BPMSoft.TimerExpressionTypes, function(value, name) {
					if (value !== BPMSoft.TimerExpressionTypes.Empty) {
						result[value] = {
							value: value,
			...
			_getCronValidationInfo: function(cronString) {
				var info = BPMSoft.CronExpression.validate(cronString);
				if (info.isValid) {
					var cronObject = info.cron;
					var secondsParameter = cronObject.getParameter(BPMSoft.cron.Parameters.Seconds);
			...
			_initExpressionType: function(element) {
				...
				if (value === BPMSoft.TimerExpressionTypes.Empty) {
					newValue = {value:  BPMSoft.TimerExpressionTypes.Empty};
				}
			...
			_initMisfireInstruction: function(element) {
				...
				this.set("MisfireInstruction", misfireInstruction === BPMSoft.TimerMisfireInstructionTypes.FireNow);
			},
			...
			_initCurrentUserTimeZone: function(element, callback, scope) {
				var esq = this.Ext.create("BPMSoft.EntitySchemaQuery", {rootSchema: SysAdminUnit});
				esq.addColumn("TimeZoneId", "TimeZoneCode");
				this.$TimeZoneList = this.Ext.create("BPMSoft.Collection");
			...
			_initCronExpression: function(element) {
				...
				if (!BPMSoft.CronExpression.validate(value)) {
					value = null;
				}
			...
			_timerPeriodValidator: function() {
				...
				if (!Ext.isEmpty(startDateTime) && !Ext.isEmpty(endDateTime)) {
					var clearStart = BPMSoft.clearSeconds(startDateTime);
					var clearEnd = BPMSoft.clearSeconds(endDateTime);
			...
			_saveMisfireInstruction: function(element) {
				...
				element.setPropertyValue("misfireInstruction", misfireInstruction
					? BPMSoft.TimerMisfireInstructionTypes.FireNow
					: BPMSoft.TimerMisfireInstructionTypes.IgnoreMisfires
				);
			},
			...
			_saveStartEndDateTimes: function(element) {
				...
				if (doSaveStartDate) {
					startDateValue = BPMSoft.toLocalISOString(startDate);
				}
				var doSaveEndDate = isEndDateTimeInSaveState && this.getIsDateTimeValid(endDateTime);
				if (doSaveEndDate) {
					endDateValue = BPMSoft.toLocalISOString(endDateTime);
				}
			...
			clearDefaultSectionsValues: function(model, expressionType) {
				...
				var customCronExpressionType = BPMSoft.TimerExpressionTypes.CustomCronExpression;
			...
			isElementValid: function(validationResult) {
				...
				var expressionType = this.get("ExpressionType");
				if (expressionType && expressionType.value === BPMSoft.TimerExpressionTypes.Empty) {
					result = false;
				}
				return result;
			},
			...
			getIsCustomCronExpressionSectionVisible: function() {
				return this._isSectionActive(BPMSoft.TimerExpressionTypes.CustomCronExpression);
			},
			...
			getIsSingleRunSectionVisible: function() {
				return this._isSectionActive(BPMSoft.TimerExpressionTypes.SingleRun);
			},
			...
			getIsDaySectionVisible: function() {
				return this._isSectionActive(BPMSoft.TimerExpressionTypes.Day);
			},
			...
			getIsWeekSectionVisible: function() {
				return this._isSectionActive(BPMSoft.TimerExpressionTypes.Week);
			},
			...
			getIsMonthSectionVisible: function() {
				return this._isSectionActive(BPMSoft.TimerExpressionTypes.Month);
			},
			...
			getIsMinuteHourSectionVisible: function() {
				return this._isSectionActive(BPMSoft.TimerExpressionTypes.MinuteHour);
			},
			...
			getIsStartEndTimeGroupActive: function() {
				var sections = [
					BPMSoft.TimerExpressionTypes.CustomCronExpression,
					BPMSoft.TimerExpressionTypes.Day,
					BPMSoft.TimerExpressionTypes.Week,
					BPMSoft.TimerExpressionTypes.Month,
					BPMSoft.TimerExpressionTypes.Year,
					BPMSoft.TimerExpressionTypes.MinuteHour
				];
			...
			getIsYearSectionVisible: function() {
				return this._isSectionActive(BPMSoft.TimerExpressionTypes.Year);
			},
