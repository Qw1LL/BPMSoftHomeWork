Ext.define("BPMSoft.configuration.store.SocialMessageGridPage", {
	...
	loadModifiedRecords: function(callback) {
		...
		BPMSoft.DataUtils.loadRecords({
			...
			success: function(records) {
				for (var i = 0, ln = records.length; i < ln; i++) {
					var record = records[i];
					BPMSoft.Array.pushUnique(this.socialMessageModifiedRecordIds, record.get("RecordId"));
				}
	...
	constructor: function() {
		...
		this.socialMessageModifiedRecordIds = [];
		this.socialMessageEntityLoader = Ext.create("BPMSoft.Configuration.SocialMessageEntityLoader");
		this.socialMessageLikeManager = Ext.create("BPMSoft.Configuration.SocialMessageLikeManager");
	},
			...
			this.socialMessageLikeManager.loadForCurrentUser({
				...
				callback: function(loadedRecords) {
					...
					this.loadModifiedRecords(function() {
						BPMSoft.configuration.store.SocialMessageGridPage.superclass.onProxyLoad.apply(this, arg);
					}.bind(this));
				},
				scope: this
			});
