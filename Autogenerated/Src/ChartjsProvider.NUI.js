define("ChartjsProvider",
	...
	Ext.define("BPMSoft.configuration.ChartjsProvider", {
		extend: "BPMSoft.AbstractChartProvider",
		alternateClassName: "BPMSoft.ChartjsProvider",
		...
		statics: {
			...
			_isSupportedChartType: function(chartType) {
				const chartJsConfigMapping = BPMSoft.ChartjsConfigAdapter.chartConfigMapping;
				return chartJsConfigMapping.hasOwnProperty(chartType);
			},

			_isParsableChartConfig: function(chartConfig) {
				try {
					BPMSoft.ChartjsConfigAdapter.getConfig(chartConfig);
					return true;
				}
				catch(e) {
					if (BPMSoft.isDebug) {
		...
		_createChart: function() {
			const config = BPMSoft.ChartjsConfigAdapter.getConfig(this.config);
