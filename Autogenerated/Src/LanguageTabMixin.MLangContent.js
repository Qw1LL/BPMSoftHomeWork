define("LanguageTabMixin", [], function() {
	/**
	 * @class BPMSoft.configuration.mixins.LanguageTabMixin
	 * Language tabs mixin.
	 */
	Ext.define("BPMSoft.configuration.mixins.LanguageTabMixin", {
		alternateClassName: "BPMSoft.LanguageTabMixin",
		 ...
		 * @returns {BPMSoft.EntitySchemaQuery} Esq for getting the languages.
		 * @private
		 */
		_getLanguagesListEsq: function() {
			...
			esq.filters.add("IsUsedFilter", BPMSoft.createColumnFilterWithParameter(
				BPMSoft.ComparisonType.EQUAL, "Language.IsUsed", 1));
			esq.filters.add("IsNotDefaultLanguageFilter", BPMSoft.createColumnFilterWithParameter(
				BPMSoft.ComparisonType.NOT_EQUAL, "Language.Id", this.sysLanguage.Id));
			esq.filters.add("ParentIsCurrentEntity", BPMSoft.createColumnFilterWithParameter(
				BPMSoft.ComparisonType.EQUAL, this.multilingualConnectionColumnName,
				this.recordId));
			return esq;
		},
		...
		_addMultiLanguageColumns: function(esq) {
			BPMSoft.each(this.multilingualColumnList, function(column) {
		...
		addDeleteMenuItem: function() {
			...
			menuItems.addItem(Ext.create("BPMSoft.BaseViewModel", {
		...
		addMultiLanguageEntity: function(languageId, copyRecordId) {
			...
			var languageItem = this.generateLangItem(BPMSoft.generateGUID(), language);
			languageItem.set("IsUsed", true);
			if (copyRecordId) {
		 ...
		 * @param {BPMSoft.BatchQuery} batchQuery Query for all operations.
		 * @protected
		 */
		updateExistMultiLanguageEntities: function(batchQuery) {
			...
			BPMSoft.each(languageToUpdate, function(language) {
		 ...
		 * @param {BPMSoft.BaseViewModel} language Multilingual entity.
		 * @returns {BPMSoft.UpdateQuery} Update query.
		 * @protected
		 */
		getContentsUpdateEsq: function(language) {
			...
			updateQuery.filters.add("languageIdFilter", updateQuery.createColumnFilterWithParameter(
				BPMSoft.ComparisonType.EQUAL, "Language", language.get("Id")));
		 ...
		 * @param {BPMSoft.BatchQuery} batchQuery Multilingual entity.
		 * @protected
		 */
		insertNewMultiLanguageEntities: function(batchQuery) {
			...
			BPMSoft.each(languagesToInsert, function(language) {
				if (language.get("Id") !== this.sysLanguage.Id) {
		 ...
		 * @param {BPMSoft.BaseViewModel} language which will be added for current record.
		 * @returns {BPMSoft.InsertQuery} Insert Query for record.
		 * @protected
		 */
		getContentsInsertEsq: function(language) {
			...
			insert.setParameterValue("Language", language.get("Id"), BPMSoft.DataValueType.GUID);
			insert.setParameterValue(this.multilingualConnectionColumnName,
				this.recordId, BPMSoft.DataValueType.GUID);
			insert.setParameterValue("Id",
				language.recordId, BPMSoft.DataValueType.GUID);
			return insert;
		},
		...
		updateNotUsedMultilanguages: function(batchQuery) {
			...
			BPMSoft.each(languageToActivate, function(language) {
		...
		getSysLanguageUpdateQuery: function(language) {
			...
			updateQuery.filters.add("IdFilter", updateQuery.createColumnFilterWithParameter(
				BPMSoft.ComparisonType.EQUAL, "Id", language.get("Id")));
			updateQuery.setParameterValue("IsUsed", true, BPMSoft.DataValueType.BOOLEAN);
			return updateQuery;
		},
		...
		addAddLanguageMenuItem: function() {
			var menuItems = this.menuLanguageCollection;
			menuItems.addItem(Ext.create("BPMSoft.BaseViewModel", {values:
		...
		addIsUsedFilter: function(filters, state) {
			filters.add("IsUsedFilter", BPMSoft.createColumnFilterWithParameter(
				BPMSoft.ComparisonType.EQUAL, "IsUsed", state)
			);
		},
		...
		addLanguageToAvailableLanguagesCollection: function(id, caption, name, code) {
			...
			var languageViewModel = Ext.create("BPMSoft.BaseViewModel", {
				"values": {
					"Id": id,
					"Caption": caption,
					"Name": name,
		 ...
		 * @param {BPMSoft.BaseViewModel} item Language collection item.
		 * @protected
		 * @virtual
		 */
		 ...
		 * @protected
		 * @virtual
		 * @returns {BPMSoft.FilterGroup} Returns lookup filter.
		 */
		getLanguageLookupFilterConfig: function() {
			...
			var languagesExceptExistingFilter =
				BPMSoft.createColumnInFilterWithParameters("Id", languageIds);
			languagesExceptExistingFilter.comparisonType = BPMSoft.ComparisonType.NOT_EQUAL;
		...
		addMenuItem: function(languageName, tag) {
			var menuItems = this.menuLanguageCollection;
			menuItems.addItem(Ext.create("BPMSoft.BaseViewModel", {values: Ext.apply({},
		 ...
		 * @param {BPMSoft.BaseViewModel} item Multilingual data.
		...
		/**
		 * Add language to language collection with custom parameters.
		 * @param {BPMSoft.BaseViewModel} languageItem language values.
		 * @protected
		 */
		 ...
		 * @protected
		 * @returns {BPMSoft.BaseViewModel} Returns copied MultiLanguageContent.
		 */
		copyMultiLanguageContentFromLanguage: function(languageId) {
			var languageData = this.languages.find(languageId);
			return BPMSoft.deepClone(languageData.get("MultiLanguageContent"));
		},
		 ...
		 */
		init: function() {
			this.menuLanguageCollection = Ext.create("BPMSoft.BaseViewModelCollection");
			this.languages = Ext.create("BPMSoft.BaseViewModelCollection");
			this.availableLanguages = Ext.create("BPMSoft.BaseViewModelCollection");
		},

		/**
		 * Returns menu collection.
		 * @return {BPMSoft.BaseViewModelCollection} Menu collection.
		 * @public
		 */
		 ...
		 * @return {BPMSoft.BaseViewModelCollection} Languages collection.
		 * @public
		 */
		...
		addMultiLanguageContentEsq: function(language, esq) {
			BPMSoft.each(this.multilingualColumnList, function(item) {
				esq.setParameterValue(item.name,
					...
					}, this).value, BPMSoft.DataValueType.TEXT);
			}, this);
		},
		...
		deleteLanguage: function(language, callback, scope) {
			...
			var contentIdFilter = BPMSoft.createColumnFilterWithParameter(
				BPMSoft.ComparisonType.EQUAL, this.multilingualConnectionColumnName,
				this.recordId);
			var languageFilter = BPMSoft.createColumnFilterWithParameter(
				BPMSoft.ComparisonType.EQUAL, "Language", language.id);
		 ...
		 * @param {BPMSoft.BaseViewModel} item Language data.
		 * @public
		 */
	...
	return BPMSoft.LanguageTabMixin;
});
