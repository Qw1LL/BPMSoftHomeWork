define("EntityConnectionLinksPanelItemUtilities", ["NetworkUtilities",
	...
	function(NetworkUtilities, resources, EntityConnectionLinksResourceUtilities) {
		...
		/**
		 * @class BPMSoft.configuration.mixins.EntityConnectionLinksPanelItemUtilities
		 * Entity connection panel item mixin.
		 * @type {BPMSoft.BaseObject}
		 */
		Ext.define("BPMSoft.configuration.mixins.EntityConnectionLinksPanelItemUtilities", {
			extend: "BPMSoft.core.BaseObject",
			alternateClassName: "BPMSoft.EntityConnectionLinksPanelItemUtilities",
			...
			setIsAddNewRelationVisible: function() {
				var result = false;
				var connectionColumnList = this.get("EntityConnectionColumnList");
				this.BPMSoft.each(connectionColumnList, function(columnName) {
					if (!this.isColumnFilled(columnName)) {
						result = true;
						return false;
					}
			...
			isAnyRelationColumnFilled: function() {
				var result = false;
				this.BPMSoft.each(this.$EntityConnectionColumnList, function(columnName) {
					if (this.isColumnFilled(columnName)) {
						result = true;
						return false;
					}
			...
			getHref: function(columnName) {
				...
				var entitySchemaConfig = this.BPMSoft.configuration.ModuleStructure[referenceSchemaName];
