		...
		return {
			...
			methods: {
				...
				_getEmptyCampaignLogSectionCustomQuickFilter: function() {
					var storage = this.BPMSoft.configuration.Storage.Filters =
							this.BPMSoft.configuration.Storage.Filters || {};
					var campaignLogStr = this.BPMSoft.ModuleUtils.getModuleStructureByName("CampaignLog");
				...
				_getCampaignLogSectionSerializedFilter: function() {
					...
					var campaignColumnFilter = this.BPMSoft.createColumnFilterWithParameter(
							this.BPMSoft.ComparisonType.EQUAL, "Campaign", this.$PrimaryColumnValue);
				...
				*/
				...
				_clearCampaignLogSectionV2GridDataViewFilters: function(callback, scope) {
					...
					this.BPMSoft.require(["profile!" + profileKey], function(profile) {
						if (profile && !this.Ext.isEmpty(profile.CustomFilters)) {
							...
							this.BPMSoft.utils.saveUserProfile(profileKey, profile, false, function() {
				...
				_openCampaignLogSection: function() {
					var campaignLogStr = this.BPMSoft.ModuleUtils.getModuleStructureByName("CampaignLog");
					var token = this.BPMSoft.combinePath(campaignLogStr.sectionModule, campaignLogStr.sectionSchema);
				...
				*/
				 ...
				 * @protected
				 * @overridden
				 * @return {BPMSoft.BaseViewModelCollection} Returns a collection of section actions in the
				...
				_applyAttributesValueConfig: function(config) {
					this.BPMSoft.each(config, function(item) {
				...
				_updateCampaignSchema: function() {
					if (!this.$CampaignSchemaUId) {
						this.refreshColumns(["CampaignSchemaUId"], this.BPMSoft.emptyFn, this, {silent: false});
				...
				onEntityInitialized: function() {
					...
					this.BPMSoft.ServerChannel.on(BPMSoft.EventName.ON_MESSAGE, this.onChannelMessage, this);
				},
				...
				destroy: function() {
					this.callParent(arguments);
					this.BPMSoft.ServerChannel.un(BPMSoft.EventName.ON_MESSAGE, this.onChannelMessage, this);
				},
				...
				openCampaignDesignerModule: function() {
					...
					var url = Ext.String.format(this.get("CampaignDesignerUrlPattern"),
							BPMSoft.workspaceBaseUrl, schemaUId, this.get("Id"), this.entitySchema.uId);
					window.open(url);
				},
				...
				onLaunchCampaign: function() {
					...
					var confirmationCallbackFn = function(returnCode) {
						if (returnCode === BPMSoft.MessageBoxButtons.YES.returnCode) {
							BPMSoft.chain(this._saveCampaignCard, launchFn, this._reloadCampaignCard, this);
						}
					};
				...
				showValidationResultDialog: function(validationResult, next) {
					...
					BPMSoft.CampaignMessageBoxUtilities.showMessageBox(messageBoxConfig);
				},
				...
				getValidationMessageBoxButtons: function(hasErrors) {
					...
					if (!hasErrors) {
						var returnCode = BPMSoft.CampaignMessageBoxUtilities.buttonReturnCodes.CONTINUE;
						var btnConfig = {
							returnCode: returnCode,
							caption: this.get("Resources.Strings.StartCampaignButtonCaption"),
							style: BPMSoft.controls.ButtonEnums.style.ORANGE
						};
						var continueBtn = BPMSoft.CampaignMessageBoxUtilities.getButton(btnConfig);
				...
				onCompleteCampaign: function() {
					...
					var confirmationCallbackFn = function(returnCode) {
						if (returnCode === BPMSoft.MessageBoxButtons.YES.returnCode) {
							BPMSoft.chain(completeFn, this._reloadCampaignCard, this);
						}
					};
