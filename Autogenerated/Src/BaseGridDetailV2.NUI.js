...
define("BaseGridDetailV2", ["BaseGridDetailV2Resources", "ConfigurationEnums", "ChangeLogUtilities", "RightUtilities",
	...
	return {
		...
		properties: {
			 ...
			 */
			queryOperationType: BPMSoft.QueryOperationType.FILTEREDSELECT,
		...
		methods: {
			...
			onActiveRowChange: function() {
				if (BPMSoft.Features.getIsEnabled("CheckCanChangeRightsInDetail") && this.$ActiveRow) {
					const activeRow = this.$ActiveRow;
					const recordId = BPMSoft.isGUID(activeRow) ? activeRow : activeRow.value;
					if (BPMSoft.isGUID(recordId)) {
			...
			loadGridData: function() {
				if (this.get("IsFromPage") && !this.get("MasterRecordId")) {
					const emptyCollection = Ext.create("BPMSoft.Collection");
					this.initIsGridEmpty(emptyCollection);
					const messagePattern = this.get("Resources.Strings.MasterRecordIdErrorMessage");
					const message = BPMSoft.getFormattedString(messagePattern, this.name, location.hash);
					BPMSoft.Logger.log(message);
					return;
				}
			...
			loadTempCollection: function() {
				...
				if (gridData && !gridData.isEmpty()) {
					const tempCollection = this.Ext.create("BPMSoft.BaseViewModelCollection");
			 ...
			 * @override
			 * @return {BPMSoft.FilterGroup} Detail filter group.
			 */
			getFilters: function() {
				...
				const deserializedMasterColumnFilters = BPMSoft.deserialize(masterColumnFilters
					.serialize(serializationMasterColumnInfo));
				const deserializedDetailFilters = BPMSoft.deserialize(detailFilters.serialize(serializationDetailInfo));
				if (this.get("IsRelationshipButtonPressed")) {
					const mainFilterGroup = this.getRelationshipFilters();
					mainFilterGroup.logicalOperation = BPMSoft.LogicalOperatorType.OR;
			...
			isFilterAdded: function() {
				...
				if (filters.find("CustomFilters")) {
					...
					const isFilterGroup = (customFilter instanceof BPMSoft.FilterGroup);
			...
			_getCopyPageUid: function() {
				...
				return typeColumnValue === BPMSoft.GUID_EMPTY ? this.getFirstEditPageUId() : typeColumnValue;
			},
			...
			initRelationshipButton: function(callback) {
				if (this.get("UseRelationship") && this.get("MasterRecordId") && this.get("DetailColumnName")) {
					...
					esq.addAggregationSchemaColumn(this.get("DetailColumnName"),
						BPMSoft.AggregationType.COUNT, "Count");
			...
			setQuickFilterVisible: function() {
				...
				this.updateQuickFilterClasses(true);
			},
			...
			hideQuickFilter: function() {
				this.set("IsDetailFilterVisible", false);
				this.updateQuickFilterClasses(false);
			},
			...
			getOpenCardConfig: function(operation, typeColumnValue, recordId) {
				...
				return {
					...
					moduleName: BPMSoft.configuration.ModuleStructure[this.entitySchemaName]?.cardModule
				};
			},
			/**
			...
			onCardSaved: function() {
				...
				if (cardState === BPMSoft.ConfigurationEnums.CardOperation.ADD
						|| cardState === BPMSoft.ConfigurationEnums.CardOperation.COPY) {
			 ...
			 * @protected
			 */
			onSortClick: BPMSoft.emptyFn,

			/**
			 *
			 * @protected
			 */
			onSetupTotalClick: BPMSoft.emptyFn,
			...
			initProfile: function() {
				...
				if (this._getColumnsConfigured(gridProfile)) {
					...
					profile = BPMSoft.deepClone(this.generateEntityProfile());
				} else {
					profile[dataGridName] = {};
					profile = BPMSoft.deepClone(profile);
				}
			...
			_getColumnsConfigured: function(gridProfile) {
				if (gridProfile && gridProfile.type) {
					const profileConfig = gridProfile.type === BPMSoft.GridType.LISTED
			...
			linkClicked: function(recordId, columnName) {
				...
				const entityStructure = BPMSoft.configuration.EntityStructure[columnSchemaName];
			...
			runProcess: function(config) {
				...
				this.warning(Ext.String.format(obsoleteMessage, "runProcess",
					"BPMSoft.ProcessEntryPointUtilities.runProcessForSelectedRecords"));
				const selectedItems = this.getSelectedItems();
				BPMSoft.each(selectedItems, function(item) {
					const clonedConfig = BPMSoft.deepClone(config);
					clonedConfig.parameter.parameterValue = item;
					BPMSoft.ProcessModuleUtilities.runProcessWithParameters(clonedConfig);
				}, this);
			},
			 ...
			 * @protected
			 * @virtual
			 * @param {BPMSoft.BaseViewModelCollection} toolsButtonMenu Tools button menu items collection.
			 */
			...
			isSelectAllModeVisible: function() {
				const isMultiSelectVisible = this.get("SelectAllMode");
				const isSingleSelectVisible = this.isSingleSelectVisible();
				return this.isListedGridMode() && (isMultiSelectVisible || isSingleSelectVisible);
			},
			
			/**
			 * Returns is ListedGridMode enabled.
			 * @return {Boolean}
			 */
			isListedGridMode: function() {
			...
			createSelectAllButton: function() {
				return this.getButtonMenuItem({
					...
					"Visible": {"bindTo": "isListedGridMode"},
				});
			},
			 ...
			 * @protected
			 * @virtual
			 * @param {BPMSoft.BaseViewModelCollection} toolsButtonMenu Grid detail menu items collection.
			 */
			addGridOperationsMenuItems: function(toolsButtonMenu) {
				...
				BPMSoft.each([
					...
					this.createUnSelectAllButton()
				], function(buttonConfig) {
					if (buttonConfig) {
						if(!buttonConfig?.values?.Caption?.bindTo?.includes('UnSelectAllButtonCaption')) {
							toolsButtonMenu.addItem(this.getButtonMenuSeparator());
						}
			 ...
			 * @protected
			 * @returns {BPMSoft.BaseViewModel} Export to excel menu item.
			 */
			...
			getRecordRightsSetupMenuItemVisible: function() {
				...
				const isRecordRightsSetupMenuItemVisible = isDetailEnabled === true &&
					...
					return BPMSoft.Features.getIsEnabled("CheckCanChangeRightsInDetail") &&
			...
			updateQuickFilterClasses: function (isOpen) {
				const containerWithFilter = this.Ext.getCmp(this.renderTo);
				const containerWithFilterElement= containerWithFilter.getWrapEl();
				isOpen 
					? containerWithFilterElement.addCls("quick-filter-open")
					: containerWithFilterElement.removeCls("quick-filter-open");
			},
			...
			initDetailFilterCollection: function() {
				this.set("DetailFilters", this.Ext.create("BPMSoft.FilterGroup"));
			},
			...
			getRelationshipFilters: function() {
				const mainFilterGroup = this.Ext.create("BPMSoft.FilterGroup");
				const relationshipFilterGroup = this.Ext.create("BPMSoft.FilterGroup");
				...
				if (relationTypePath && relationshipPath && relationType) {
					relationshipFilterGroup.add("relationshipFilter", BPMSoft.createColumnFilterWithParameter(
						BPMSoft.ComparisonType.EQUAL,
						relationTypePath,
						relationType,
						BPMSoft.DataValueType.GUID));
					relationshipFilterGroup.add("relationshipTypeFilter", BPMSoft.createColumnFilterWithParameter(
						BPMSoft.ComparisonType.EQUAL,
						relationshipPath,
						masterRecordId,
						BPMSoft.DataValueType.GUID));
				...
				mainFilterGroup.add("subRelationshipFilterGroup", BPMSoft.createExistsFilter(
					detailColumnName,
			...
			/**
			 * Get Add Record Button Caption.
			 * @protected
			 * @return {String} Text of Caption.
			 */
		...
		diff: /**SCHEMA_DIFF*/[
			...
			},
			{
				...
				"values": {
					"itemType": BPMSoft.ViewItemType.BUTTON,
					"caption": {"bindTo": "getAddRecordButtonCaption"},
