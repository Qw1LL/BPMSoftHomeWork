	...
	return Ext.define("BPMSoft.BaseFolderManagerViewModel", {
		 ...
		 * @protected
		 * @type {BPMSoft.FolderFilterItemViewModel}
		 */
		...
		_getFolderTypeFilter: function() {
			var groupFilters = Ext.create("BPMSoft.FilterGroup");
			groupFilters.logicalOperation = BPMSoft.LogicalOperatorType.OR;
			var folderTypes = this.getRootFolderTypes();
			folderTypes.forEach(function(rootTypeId) {
				groupFilters.addItem(BPMSoft.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL, "Id",
						rootTypeId));
			});
			return groupFilters;
		},
		...
		_findFolder: function(folder, searchId) {
			...
			if (folder.id === searchId) {
				...
				BPMSoft.each(children, function(child) {
		...
		_addCustomFolder: function(items, parentId, child) {
			var childId = child.id || BPMSoft.generateGUID();
		...
		_modifyFolderFunc: BPMSoft.emptyFn,

		handleLoadBatchResult: BPMSoft.emptyFn,

		showCustomFolderInfo: BPMSoft.emptyFn,
		...
		initAllFoldersSelect: function() {
			...
			if (!this.get("useStaticFolders")) {
				select.filters.addItem(select.createColumnFilterWithParameter(BPMSoft.ComparisonType.NOT_EQUAL,
		...
		initFavoriteFoldersSelect: function() {
			...
			filters.addItem(select.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL, "SysAdminUnit",
		...
		initializeRootFolders: function(rows) {
			BPMSoft.each(rows, function(folderTypeItem) {
				var itemValues = {
		...
		initializeCustomFolders: function(foldersCollection, rowConfig) {
			BPMSoft.each(this.getCustomFoldersConfig(), function(item) {
				var rootFolder = {
					Id: item.id || BPMSoft.generateGUID(),
					Name: item.caption,
					Parent: "",
		...
		prepareAllFolders: function(rows, rowConfig, resultItems) {
			BPMSoft.each(rows, function(rowItem) {
		...
		findCustomFolderConfig: function(id) {
			...
			BPMSoft.each(customFolders, function(customFolder) {
		...
		fillExpandedLevels: function(selectedRow, items, expandedLevels) {
			if (items.contains(selectedRow)) {
				...
				if (parent && !BPMSoft.contains(expandedLevels, parent.value)) {
		...
		expandToSelectedItems: function(selectedRows, items) {
			...
			if (selectedRows && selectedRows.length > 0) {
				var itemsCollection = new BPMSoft.Collection();
				itemsCollection.loadAll(items);
				BPMSoft.each(selectedRows, function(selectedRow) {
		...
		onActiveRowAction: function(tag) {
			switch (tag) {
				...
				case "convertFilter":
					...
					break;
				default:
					this.error(Ext.create("BPMSoft.NotImplementedException"));
					break;
			}
		},
		...
		getFolderViewModelConfig: function() {
			var viewModelConfig = {
				...
				columns: BPMSoft.deepClone(this.entitySchema.columns),
				values: {
					editMode: false
				},
				mainViewModel: this
			};
			viewModelConfig.columns = BPMSoft.deepClone(this.entitySchema.columns);
			BPMSoft.each(viewModelConfig.columns, function(column) {
		...
		getFolderEditViewModel: function(folderType) {
			...
			var folderViewModel = Ext.create("BPMSoft.FolderFilterItemViewModel", viewModelConfig);
			var useStaticFolders = this.get("useStaticFolders");
		...
		_createFavoriteFolderFilters: function(select, activeRow) {
			var filters = Ext.create("BPMSoft.FilterGroup");
			filters.add("SysAdminUnit", select.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL,
					"SysAdminUnit", BPMSoft.SysValue.CURRENT_USER.value));
			filters.add("FolderEntitySchemaUId", select.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL,
					"FolderEntitySchemaUId", this.entitySchema.uId));
			filters.add("FolderId", select.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL,
					"FolderId", activeRow));
			return filters;
		},
		...
		_addToFavoriteFolder: function(activeRow) {
			...
			insert.setParameterValue("SysAdminUnit", BPMSoft.SysValue.CURRENT_USER.value,
					BPMSoft.DataValueType.GUID);
			insert.setParameterValue("FolderId", activeRow, BPMSoft.DataValueType.GUID);
			insert.setParameterValue("FolderEntitySchemaUId", this.entitySchema.uId, BPMSoft.DataValueType.GUID);
		...
		editFolderFilters: function() {
			...
			this.sandbox.publish("CustomFilterExtendedMode", {
				folder: folder,
				filter: BPMSoft.deserialize(folder.get("SearchData"))
			}, [this.sandbox.id]);
		},
		...
		processMoveFolders: function(folders, callback) {
			...
			var moveFilter = BPMSoft.createColumnInFilterWithParameters(this.entitySchema.primaryColumnName,
					folders);
			moveFilter.comparisonType = BPMSoft.ComparisonType.NOT_EQUAL;
			filters.addItem(moveFilter);

			var rootEmailFilter = BPMSoft.createColumnFilterWithParameter(BPMSoft.ComparisonType.NOT_EQUAL,
					"FolderType", ConfigurationConstants.Folder.Type.RootEmail);
			filters.addItem(rootEmailFilter);

			if (!this.get("useStaticFolders")) {
				var typeFilter = BPMSoft.createColumnFilterWithParameter(BPMSoft.ComparisonType.NOT_EQUAL,
						"FolderType", ConfigurationConstants.Folder.Type.General);
			...
			var captionsConfig = [
				{
					...
					sortColumnDirection: BPMSoft.core.enums.OrderDirection.ASC
				}
			];
			var config = {
				entitySchemaName: this.entitySchema.name,
		...
		changeFolderParent: function(folders, parentId, callback) {
			...
			var folder = Ext.create(BPMSoft.FolderFilterItemViewModel, folderConfig);
		...
		onDeleteAccept: function onDeleteAccept() {
			...
			if (selectedRows.length || activeRow) {
				...
				BPMSoft.each(selectedValues, function(currentRecordId) {
					var actualFolderId = this.getActualFolderId(currentRecordId);
					actualSelectedValues.push(actualFolderId);
				}, this);
				if (BPMSoft.Features.getIsEnabled("UseServiceForDeleteFolder")) {
					this.deleteFoldersInternal(actualSelectedValues);
				} else {
					BPMSoft.each(actualSelectedValues, function(actualFolderId) {
		...
		getDeleteFromFavoritesESQ: function(selectedId) {
			var deleteESQ = Ext.create("BPMSoft.DeleteQuery", {
				rootSchemaName: "FolderFavorite"
			});
			var filters = Ext.create("BPMSoft.FilterGroup");
			filters.add("FolderId", deleteESQ.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL, "FolderId",
					selectedId));
			filters.add("FolderEntitySchemaUId", deleteESQ.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL,
					"FolderEntitySchemaUId", this.entitySchema.uId));
		...
		getGridRecordByItemValues: function(rowConfig, itemValues) {
			var gridRecord = Ext.create("BPMSoft.FolderGridRowViewModel", {
				entitySchema: this.entitySchema,
				rowConfig: rowConfig,
				values: itemValues,
				isNew: false,
		...
		_renameFolder: function(handler, scope) {
			...
			BPMSoft.utils.inputBox(
					caption,
					handler.bind(scope),
					[BPMSoft.MessageBoxButtons.SAVE.returnCode, BPMSoft.MessageBoxButtons.CANCEL.returnCode],
					this,
					controls
			);
		},
		...
		getRenameDialogControls: function() {
			...
			if (Ext.isFunction(modifyFolderFunc) && modifyFolderFunc !== BPMSoft.emptyFn) {
		...
		_isChosenOkButton: function(returnCode, controlData) {
			return returnCode === BPMSoft.MessageBoxButtons.SAVE.returnCode && controlData.name.value;
		},
		 ...
		 * @return {BPMSoft.FilterGroup} Filters group.
		 */
		getFolderFilters: function(rowId) {
			var filtersGroup = BPMSoft.createFilterGroup();
			...
			if (this._isCurrentEditElementTypeIsSearch()) {
				...
				if (!Ext.isEmpty(searchData)) {
					filtersGroup = BPMSoft.deserialize(searchData);
				}
			} else {
				var entityColumnName = this._entityColumnNameInFolderEntity;
				var inFolderSchemaName = this._inFolderEntitySchemaName;
				filtersGroup.add("filterStaticFolder", BPMSoft.createColumnInFilterWithParameters(
		...
		_getFolderMenuItemConfig: function() {
			return {
				...
				filter: BPMSoft.deserialize(this.currentEditElement.get("SearchData")),
