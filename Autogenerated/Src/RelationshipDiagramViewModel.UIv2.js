...
define("RelationshipDiagramViewModel", ["RelationshipDiagramViewModelResources", "ServiceHelper",
	...
	function(resources, ServiceHelper, NetworkUtilities, LookupUtilities, RightUtilities) {
		Ext.define("BPMSoft.configuration.RelationshipDiagramViewModel", {
			extend: "BPMSoft.BaseViewModel",
			alternateClassName: "BPMSoft.RelationshipDiagramViewModel",
			mixins: {
			...
			PortPosition: {
				Top: this.BPMSoft.diagram.PortPosition.Top,
				Bottom: this.BPMSoft.diagram.PortPosition.Bottom,
				Left: this.BPMSoft.diagram.PortPosition.Left,
				BottomLeft: {
					name: "{X=-1,Y=-1}",
					offset: {
						x: 0.09,
			 ...
			 */
			init: function() {
				this.set("Nodes", this.Ext.create("BPMSoft.Collection"));
				this.set("Accounts", null);
				this.set("NewAccountList", this.Ext.create("BPMSoft.Collection"));
			...
			getNotInAccountOnDiagramFilter: function() {
				...
				var notInFilter = this.BPMSoft.createColumnInFilterWithParameters("Id", accountIds);
			...
			relationshipDiagramServiceCallback: function(response) {
				...
				if (errorInfo) {
					this.BPMSoft.showInformation(errorInfo.message || errorInfo.errorCode);
				}
			},
			...
			placeOneLevelAccounts: function(accounts) {
				...
				if (!rootAccount.isNew) {
					rootAccount.toolsConstraint = BPMSoft.diagram.ToolsConstraint.NodeSelectionTool |
						BPMSoft.diagram.ToolsConstraint.NodeAddTool;
				}
			...
			placeAccountsRecursive: function(parentAccount, offsetX) {
				...
				for (var i = 0; i < accounts.length; i++) {
					...
					if (!account.isNew) {
						account.toolsConstraint = BPMSoft.diagram.ToolsConstraint.NodeSelectionTool |
							BPMSoft.diagram.ToolsConstraint.NodeAddTool |
							BPMSoft.diagram.ToolsConstraint.NodeRemoveTool;
					}
					if (level === (minLevel + 1)) {
						account.inPort = this.PortPosition.Top;
						account.outPort = this.PortPosition.BottomLeft;
			...
			createNodeConnections: function(accounts) {
				var nodes = this.get("Nodes");
				accounts.forEach(function(account) {
					if (account.parentId !== null && account.parentId !== this.BPMSoft.GUID_EMPTY) {
			...
			getNodeConfig: function(config) {
				...
				var node = {
					...
					"nodeType": this.BPMSoft.diagram.UserHandlesConstraint.Node,
					"isCurrent": isCurrent,
					"level": config.level,
					...
					"toolsConstraint": BPMSoft.diagram.ToolsConstraint.NodeSelectionTool |
						BPMSoft.diagram.ToolsConstraint.NodeAddTool |
						BPMSoft.diagram.ToolsConstraint.NodeRemoveTool
				};
				...
				if (!this.Ext.isEmpty(type)) {
					node.labels.push(this.getNodeLabelConfig({
						"text": type,
						"name": this.BPMSoft.generateGUID(),
			...
			onAddParentAccountButtonClick: function(node) {
				...
				newParentAccount = {
					id: this.BPMSoft.generateGUID(),
					parentId: this.BPMSoft.GUID_EMPTY,
					level: node.level - 1,
					children: [],
					isNew: true,
					isRoot: true,
					toolsConstraint: BPMSoft.diagram.ToolsConstraint.NodeRemoveTool,
					columnName: "NewParentAccount"
				};
			...
			onRemoveParentRelationshipButtonClick: function(node) {
				...
				if (node.isNew) {
					...
					this.BPMSoft.showConfirmation(confirmationMessage, function(returnCode) {
				...
				for (var i = accounts.length - 1; i >= 0; i--) {
					...
					if (account.isNew) {
						if (account.isRoot) {
							...
							rootAccount.isRoot = true;
							rootAccount.parentId = this.BPMSoft.GUID_EMPTY;
						}
			...
			updateRelationship: function(accountId, parentAccountId, callback) {
				...
				var updateQuery = function(result) {
					if (this.Ext.isEmpty(result)) {
						var updateAccount = this.Ext.create("BPMSoft.UpdateQuery", {
							rootSchemaName: "Account"
						});
						updateAccount.filters.add(updateAccount.createColumnFilterWithParameter(
							this.BPMSoft.ComparisonType.EQUAL, "Id",
							accountId));
						updateAccount.setParameterValue("Parent", parentAccountId, this.BPMSoft.DataValueType.GUID);
						...
						this.hideBodyMask();
						this.BPMSoft.showInformation(result);
					}
				};
			...
			onLookupChange: function(newValue, columnName) {
				if (newValue && newValue.value !== this.BPMSoft.GUID_EMPTY) {
			...
			getAccountsTreeData: function(accounts) {
				...
				var rootAccount = BPMSoft.find(accounts, function(account) {
			...
			fillChildrenData: function(accounts, parentItem) {
				...
				BPMSoft.each(childrenAccounts, function(account) {
			...
			generateTitleUrl: function(account) {
				...
				if (account.id !== this.$MasterRecordId) {
					...
					nodeTitleUrl = this.BPMSoft.combinePath(this.BPMSoft.workspaceBaseUrl, "Nui",
		...
		return BPMSoft.RelationshipDiagramViewModel;
	});
