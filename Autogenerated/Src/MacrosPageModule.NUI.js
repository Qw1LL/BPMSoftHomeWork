define("MacrosPageModule", ["ext-base", "BPMSoft", "CardViewGenerator", "CardViewModelGenerator", "LookupUtilities",
		"Macros", "sandbox", "MacrosPageModuleResources", "StorageUtilities", "ConfigurationConstants"],
	function(Ext, BPMSoft, CardViewGenerator, CardViewModelGenerator, LookupUtilities, Macros, sandbox, resources,
		...
		function createViewModelConfig() {
			var viewModel = {
				...
				values: {
					...
					Name: null,
					Command: null,
					CommandList: new BPMSoft.Collection(),
					CommandsLoaded: false,
					MainParam: null,
					MainParamList: new BPMSoft.Collection(),
					AdditionalParam: null,
					AdditionalParamList: new BPMSoft.Collection(),
					AdditionalParamValue: null,
					AdditionalParamValueList: new BPMSoft.Collection(),
					MainParamType: null,
					MainParamTypeList: new BPMSoft.Collection(),
					mainParamCaption: "",
				...
				methods: {
					...
					save: function() {
						if (this.validate()) {
							var scope = this;
							BPMSoft.EntitySchemaQuery.clearCache("Macros_AdditionalMacrosParamValues");
					...
					saveMacros: function(callback, scope) {
						...
						var commandValue = command ? command.value : BPMSoft.GUID_EMPTY;
						var mainParamValue = mainParam ? mainParam.value : BPMSoft.GUID_EMPTY;
						var additionalParamValue = additionalParam ? additionalParam.value : BPMSoft.GUID_EMPTY;
					...
					getQueryClassName: function(action) {
						...
						if (action === "add" || action === "copy") {
							queryClassName = "BPMSoft.InsertQuery";
						} else if (action === "edit") {
							queryClassName = "BPMSoft.UpdateQuery";
						}
					...
					getSaveMacroQuery: function(macroConfig) {
						...
						query.setParameterValue("Name", macroConfig.name, BPMSoft.DataValueType.TEXT);
						query.setParameterValue("Command", macroConfig.commandValue, BPMSoft.DataValueType.GUID);
						query.setParameterValue("MainParam", macroConfig.mainParamValue, BPMSoft.DataValueType.GUID);
						if (macroConfig.additionalParamValue !== BPMSoft.GUID_EMPTY) {
						...
						if (action === "edit") {
							...
							var idFilter = query.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL,
									"Id", recordId);
						...
						if (item) {
							...
							switch (commandCode) {
								...
								case "startbp":
									...
									this.loadAdditionalParamList("", this.get("AdditionalParamList"));
									this.set("additionalParamValueCaption", this.get("AdditionalParam") ? this.get("AdditionalParam").displayValue : item.displayValue);
									break;
								default :
									break;
							}
							if (mainParamList.contains(item.value) && commandCode === "create") {
								...
								if (BPMSoft.isGUID(columnType) &&
									columnType !== BPMSoft.GUID_EMPTY) {
					...
					getLookupQueryFilters: function(tag) {
						...
						switch (tag) {
							case "MainParam":
								...
								filter = BPMSoft.createColumnInFilterWithParameters("Id", idArray);
							...
							case "VwSysProcess":
								...
								var businessProcessTagFilter = BPMSoft.createColumnFilterWithParameter(
					 ...
					 * @param {BPMSoft.Collection} list Drop-down List.
                     */
					loadAdditionalParamList: function(filter, list) {
						...
						var tempList = new BPMSoft.Collection();
					...
					loadAdditionalParamValueList: function(searchQuery, list) {
						...
						var customListFilterGroup = BPMSoft.createFilterGroup();
						customListFilterGroup.logicalOperation = BPMSoft.LogicalOperatorType.AND;
						list.clear();
						switch (commandCode) {
							case "search":
					...
					getPrimaryDisplayColumnFilter: function(primaryDisplayColumnName, searchQuery) {
						return BPMSoft.createColumnFilterWithParameter(
							BPMSoft.ComparisonType.START_WITH, primaryDisplayColumnName, searchQuery);
					},
					...
					getEntitySchemaQuery: function() {
						...
						BPMSoft.each(info, function(column) {
						...
						BPMSoft.each(entity.columns, function(column) {
							if (column.columnPath === "AdditionalParamValue") {
								...
								if (values.length === 2) {
									...
									value = BPMSoft.deserialize(values[2]);
								}
		...
		function init() {
			...
			mainParamList = new BPMSoft.Collection();
			additionalParamList = new BPMSoft.Collection();
			macrosList = new BPMSoft.Collection();
			allList = new BPMSoft.Collection();
			...
			callServiceMethod("GetFixedCommandList", function(response) {
				...
				if (responseArray) {
					for (var i = 0; i < responseArray.length; i++) {
						...
						additionalParamId =
							BPMSoft.isEmpty(additionalParamId) || additionalParamId === BPMSoft.GUID_EMPTY
								? null
								: additionalParamId;
		...
		function render(renderTo) {
			...
			if (viewModel) {
				generatedViewConfig = BPMSoft.deepClone(viewConfig);
				view = Ext.create("BPMSoft.Container", generatedViewConfig);
				view.bind(viewModel);
			...
			generatedViewConfig = BPMSoft.deepClone(viewConfig);
			view = Ext.create("BPMSoft.Container", generatedViewConfig);
			viewModel = Ext.create("BPMSoft.BaseViewModel", createViewModelConfig());
			view.bind(viewModel);
			view.render(renderTo);
			initMainHeader();
		}
