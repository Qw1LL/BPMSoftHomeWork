define("LinkPreview", ["css!LinkPreview"],
	function() {
		/**
		 * @class BPMSoft.controls.LinkPreview
		 * Link preview control.
		 */
		Ext.define("BPMSoft.controls.LinkPreview", {
			alternateClassName: "BPMSoft.LinkPreview",
			...
			/**
			 * @inheritdoc BPMSoft.BaseProgressBarIndicator#getBindConfig
			 * @override
			 */
			...
			loadWebPageLinkPreview: function(url, callback, scope) {
				if (!BPMSoft.Features.getIsEnabled("LinkPreview") || Ext.isEmpty(url) || !BPMSoft.isUrl(url)) {
			...
			setLinkPreviewInfo: function(linkPreviewInfo) {
				linkPreviewInfo = BPMSoft.isJsonObject(linkPreviewInfo) ? Ext.decode(linkPreviewInfo) : linkPreviewInfo;
				linkPreviewInfo = !Ext.isEmpty(linkPreviewInfo) ? linkPreviewInfo : null;
				if (BPMSoft.isEqual(this.linkPreviewInfo, linkPreviewInfo)) {
					return;
				}
