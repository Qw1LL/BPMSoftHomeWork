define("ContactEnrichmentSchema", ["LookupUtilities", "NetworkUtilities", "RightUtilities", "BaseEnrichmentViewModel",
	...
	return {
		...
		methods: {
			 ...
			 * @param {BPMSoft.Collection} list Account list.
			 */
			...
			getAccountsQuery: function(partialName) {
				...
				esq.addColumn("Id");
				esq.addColumn("Name");
				esq.filters.add("startFilter", BPMSoft.createColumnFilterWithParameter(
					BPMSoft.ComparisonType.START_WITH, "Name", partialName));
				return esq;
			},
			...
			onContactLookupColumnChanged: function(model, contact) {
				if (contact) {
					...
					this.checkContactRights(BPMSoft.emptyFn, this);
				} else {
					this.set("ContactId", "");
				}
			},
			 ...
			 * @private
			 * @param {BPMSoft.BaseViewModel} item ContactCommunication instance.
			 ...
			 * @private
			 * @param {BPMSoft.BaseViewModel} item EnrchProcessedData instance.
			 * @param {String} contactRecordId Contact record unique identifier.
			...
			createEnrichEmailDataValues: function() {
				return [{
					...
					"Key": "Status",
					"Value": this.BPMSoft.EmailMiningEnums.EnrichEmailDataStatus.ENRICHED
				}];
			},
			 ...
			 * @private
			 * @return {BPMSoft.BaseViewModelCollection} Selected contact communication items collection.
			 */
			 ...
			 * @private
			 * @return {BPMSoft.BaseViewModelCollection} Processed contact communication items collection.
			 */
			 ...
			 * @private
			 */
			stopHidingEnrichmentContainer: BPMSoft.emptyFn,
			...
			getContactId: function() {
				...
				var isNewContact = this.isEmpty(contactId) || BPMSoft.isEmptyGUID(contactId);
				return isNewContact ? this.BPMSoft.generateGUID() : contactId;
			},
			 ...
			 * @protected
			 * @return {BPMSoft.InsertQuery} Sign that contact does not exist in database.
			 */
			getIsNewContact: function() {
				var contactId = this.get("ContactId");
				return this.isEmpty(contactId) || this.BPMSoft.isEmptyGUID(contactId);
			},

			/**
			 * @inheritdoc BPMSoft.BaseEnrichmentSchema#init
			 * @overridden
			 */
			init: function(callback, scope) {
				this.set("AccountList", Ext.create("BPMSoft.Collection"));
				this.on("change:ContactLookupColumn", this.onContactLookupColumnChanged, this);
				this.callParent([function() {
					this.BPMSoft.chain(
						this.checkContactRights,
						function(next) {
							this.BPMSoft.SysSettings.querySysSettingsItem("DefaultContactCommunicationType",
									function(value) {
			...
			/**
			 * @inheritdoc BPMSoft.BaseEnrichmentSchema#hideModule
			 * @overridden
			 */
			hideModule: function() {
			 ...
			 * @overridden
			 */
			getEntitySchemaQuery: BPMSoft.emptyFn,
			...
			/**
			 * @inheritdoc BPMSoft.BaseEnrichmentSchema#selectExistingEnrichedData
			 * @overridden
			 */
			selectExistingEnrichedData: BPMSoft.emptyFn,
			...
			getEnrichmentDataList: function(data) {
				...
				this.BPMSoft.each(data, function(dataElement) {
					dataCollection.add({
			...
			setAccountByOrganizationEntity: function(communication, accounts) {
				...
				var foundAccount = BPMSoft.findWhere(accounts, {
			...
			setDefaultAccountAttributes: function(accounts) {
				if (Ext.isEmpty(this.get("AccountName"))) {
					if (!Ext.isEmpty(accounts)) {
						var additionalAccount = BPMSoft.first(accounts);
			...
			/**
			 * @inheritdoc BPMSoft.BaseEnrichmentSchema#onLoadEnrichmentData
			 * @overridden
			 */
			onLoadEnrichmentData: function(response) {
				...
				var responseCollection = this.Ext.create("BPMSoft.BaseViewModelCollection");
			...
			setAccountAttributes: function(id, name) {
				...
				var url = this.BPMSoft.combinePath(viewModuleUrl, relativeUrl);
				var account = {
					value: id,
					displayValue: name
				};
			 ...
			 * @param {BPMSoft.BaseViewModelCollection} enrchTypeMappingCollection Enrch type mapping collection.
			 */
			getEnrichmentDataItem: function(communication, enrchTypeMappingCollection) {
				if (communication.DuplicationStatus ===
						this.BPMSoft.EmailMiningEnums.EnrchDuplicateStatus.EXISTS_IN_ENTITY) {
					return null;
				}
				...
				var item = this.Ext.create("BPMSoft.BaseEnrichmentViewModel", {
					values: {
						Value: value,
			...
			/**
			 * @inheritdoc BPMSoft.BaseEnrichmentSchema#getHeaderCaption
			 * @overridden
			 */
			getHeaderCaption: BPMSoft.emptyFn,
			...
			/**
			 * @inheritdoc BPMSoft.BaseEnrichmentSchema#saveData
			 * @overridden
			 */
			saveData: function() {
			...
			/**
			 * @inheritdoc BPMSoft.BaseEnrichmentSchema#onLastActionChanged
			 * @overridden
			 */
