...
function(AccountOrganizationChart, structure, resources) {
	structure.userCode = function() {
		...
		this.methods.onDeleteAccept = function() {
			...
			BPMSoft.each(selectedRows, function(recordId) {
		...
		this.methods.deleteItem = function(recordId, batch, scope) {
			...
			var toDelete = new BPMSoft.Collection();
			...
			BPMSoft.each(toDelete.getItems(), function(item) {
			...
			if (grid.find(recordId)) {
				...
				var filter = BPMSoft.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL, "Id", recordId);
