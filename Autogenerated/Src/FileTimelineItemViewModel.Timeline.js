define("FileTimelineItemViewModel", ["FileTimelineItemViewModelResources",
	...
	function() {
		/**
		 * @class BPMSoft.configuration.FileTimelineItemViewModel
		 * File timeline item view model class.
		 */
		Ext.define("BPMSoft.configuration.FileTimelineItemViewModel", {
			alternateClassName: "BPMSoft.FileTimelineItemViewModel",
			...
			_findSchemaByName: function(name) {
				var filteredCollection = BPMSoft.EntitySchemaManager.items.filterByFn(function(item) {
			...
			/**
			 * @inheritdoc BPMSoft.BaseTimelineItemViewModel#init
			 * @override
			 */
			init: function() {
				this.set("Extensions", Ext.create("BPMSoft.Collection"));
				this.callParent();
			 ...
			 * @protected
			 * @param {BPMSoft.core.collections.Collection} collection Query result from file extension table.
			 * @param {Function} callback Callback function.
			 * @param {Object} scope Execution context.
			 */
			fillExtensionsIcons: function(collection, callback, scope) {
				var extensions = this.get("Extensions");
				var defaultIconUrl = BPMSoft.ImageUrlBuilder.getUrl(this.get("Resources.Images.DefaultFileIcon"));
				var defIconId = BPMSoft.generateGUID();
			...
			setFileTypeIconSrc: function() {
				...
				if (!this._isFileImage(itemExtension)) {
					...
					this.set("FileTypeSrc", BPMSoft.ImageUrlBuilder.getUrl(this.get("Resources.Images.ImageFileIcon")));
				}
			},

			/**
			 * @inheritdoc BPMSoft.BaseTimelineItemViewModel#getRecordUrl
			 * @override
			 */
			getRecordUrl: function() {
				var foundSchema = this._findSchemaByName(this.get("EntitySchemaName"));
				if (foundSchema) {
					return BPMSoft.FileDownloader.getFileLink(foundSchema.uId, this.get("Id"));
				} else {
