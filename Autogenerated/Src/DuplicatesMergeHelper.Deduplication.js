define("DuplicatesMergeHelper", function() {
	...
	var duplicatesMergeHelper = Ext.define("BPMSoft.configuration.mixins.DuplicatesMergeHelper", {

		alternateClassName: "BPMSoft.DuplicatesMergeHelper",
		...
		getColumnsConfig: function(conflictColumns) {
			var duplicatesSchema = BPMSoft[this.entitySchemaName];
			var primaryColumnName = duplicatesSchema.primaryColumnName;
			var columns = {};
			BPMSoft.each(conflictColumns, function(conflictColumn, itemIndex) {
		...
		getMergeModuleConfig: function(conflicts) {
			...
			BPMSoft.each(conflicts, function(row) {
				var resultRow = {};
				BPMSoft.each(columns, function(column, columnName) {
					var cell = row[column.index];

					var cellValue = cell.Value;
					if (column.dataValueType === BPMSoft.DataValueType.DATE) {
						cellValue = this.normalizeDateDataTypeColumn(cellValue);
					}
					if (column.dataValueType === BPMSoft.DataValueType.IMAGELOOKUP) {
						cellValue = cellValue.value || BPMSoft.emptyString;
					}
		...
		dictionaryToObject: function(dictionary) {
			var resultObject = {};
			BPMSoft.each(dictionary, function(keyValuePair) {
