			...
			methods: {
				...
				addColumns: function(select) {
					...
					if (this.getIsFeatureEnabled("NotificationV2")) {
						...
						anniversaryDateColumn.orderDirection = BPMSoft.OrderDirection.ASC;
					}
				},

				/**
				 * @inheritdoc BPMSoft.BaseNotificationsSchema#addFilters
				 * @overridden
				 */
				addFilters: function(select) {
					...
					filterGroup.add("Contact", this.BPMSoft.createColumnIsNotNullFilter("[Contact:Id:SubjectId].Id"));
					filterGroup.add("Account", this.BPMSoft.createColumnIsNotNullFilter("[Account:Id:SubjectId].Id"));
					select.filters.add("IsWithSubjectId", filterGroup);
					if (this.getIsFeatureEnabled("NotificationV2")) {
						var selectionStartDate = this.BPMSoft.addDays(new Date(), -this.$daysToSubtract);
						select.filters.add("AnniversaryDate", this.BPMSoft.createColumnFilterWithParameter(
							this.BPMSoft.ComparisonType.GREATER_OR_EQUAL, "AnniversaryDate", selectionStartDate));
					}
				},

				/**
				 * @inheritdoc BPMSoft.BaseNotificationsSchema#applySelect
				 * @overridden
				 */
				 ...
				 * @return {BPMSoft.EntitySchemaQuery} Query of notifications to the merger.
				 */
				getNotificationMergerSelect: function() {
					var mergeSelect = this.Ext.create("BPMSoft.EntitySchemaQuery", {
						rootSchemaName: this.entitySchemaName
					});
					mergeSelect.addColumn("SubjectId", "Id");
					mergeSelect.addAggregationSchemaColumn("RemindTime",
							this.BPMSoft.AggregationType.MAX, "RemindTime");
					return mergeSelect;
				},
				...
				getFormattedDate: function(date) {
					date = date && this.BPMSoft.parseDate(date);
					return this.BPMSoft.getTypedStringValue(date, this.BPMSoft.DataValueType.DATE);
				},
