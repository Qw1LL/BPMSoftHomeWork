define("ContentBuilderConfigToMjmlConverter", ["BPMSoft", "ContentBuilderConstants", "CustomerFontMixin"],
	function (BPMSoft) {
		Ext.define("BPMSoft.configuration.ContentBuilderConfigToMjmlConverter", {
			extend: "BPMSoft.BaseObject",
			alternateClassName: "BPMSoft.ContentBuilderConfigToMjmlConverter",
			mixins: {
			...
			_getTagName: function(config) {
				var bodyItemType = BPMSoft.ContentBuilderBodyItemType[config.ItemType];
			...
			_getHeadTagName: function(itemType) {
				var headItemType = BPMSoft.ContentBuilderHeadItemType[itemType];
				if (headItemType) {
			...
			_recalculateGroupColumnsWidth: function(config, groupWidth) {
				BPMSoft.each(config.Items, function(column) {
			...
			_fillTextAdditionalConfigAttributes: function(mjmlConfig, config) {
				mjmlConfig.attributes.align = mjmlConfig.attributes["text-align"] || BPMSoft.Align.CENTER;
				delete mjmlConfig.attributes["text-align"];
			...
			_borderStylesToCssStyle: function(mjmlConfig) {
				var allEquals = true;
				var borderStyle = BPMSoft.emptyString;
				var resultBorderString = BPMSoft.emptyString;
				BPMSoft.each(this.borderProps, function(prop) {
			...
			_fillChildren: function(mjmlConfig, config) {
				if (mjmlConfig && mjmlConfig.tagName !== undefined) {
					BPMSoft.each(config.Items, function (itemConfig) {
