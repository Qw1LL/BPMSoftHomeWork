	...
	public class EmailTemplateExecutor
	{
		...
		public Guid CreateActivity() {
			...
			if (SendIndividualEmail) {
				var activity = new BPMSoft.Configuration.Activity(UserConnection);
				activity.SetDefColumnValues();
				SetDefValues(activity);
				activity.TypeId = BPMSoft.Configuration.ActivityConsts.EmailTypeUId;
				...
				activity.StatusId = BPMSoft.Configuration.ActivityConsts.NewStatusUId;
		...
		public bool TryCreateSender() {
			...
			_sendFromAddress = BPMSoft.UI.WebControls.Utilities.ControlUtilities.HtmlDecode(string.Format("{0} <{1}>", user, email));
#endif
			return true;
		}
