define("UriJsonConverter", ["ServiceJsonConverter"], function() {
	Ext.define("BPMSoft.services.UriJsonConverter", {
		alternateClassName: "BPMSoft.UriJsonConverter",
		...
		_normalizeUri: function(uri) {
			uri = uri.trim();
			const hash = BPMSoft.getUrlHash(uri);
			if (hash) {
	...
	return BPMSoft.UriJsonConverter;
});
