			...
			return {
				...
				methods: {
					...
					getContactFolderInsert: function(campaignId, contactFolderId) {
						...
						insertQuery.setParameterValue("Campaign", campaignId, BPMSoft.DataValueType.GUID);
					...
					isAudienceStepRoteWithDay: function(callback, scope) {
						...
						select.filters.addItem(select.createColumnFilterWithParameter(
								BPMSoft.ComparisonType.EQUAL, "[CampaignStep:Id:SourceStep].Type.Id",
								CampaignEnums.StepType.CAMPAIGN_AUDIENCE));
						select.filters.addItem(select.createColumnFilterWithParameter(
								BPMSoft.ComparisonType.EQUAL, "[CampaignStep:Id:SourceStep].Campaign.Id",
								campaignId));
