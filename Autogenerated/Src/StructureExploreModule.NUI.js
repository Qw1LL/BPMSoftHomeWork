define("StructureExploreModule", ["StructureExploreModuleResources", "StructureExplorerView",
	"StructureExplorerViewModel", "EntityStructureHelper", "StructureExplorerUtilities", "MaskHelper",
			"ConfigurationEnums", "css!StructureExploreModule"],
		...
		function createConstructor(context) {
			...
			function expandBase() {
				...
				itemViewModel.ComboBoxListChange = function(comboBoxValue) {
					...
					itemViewModel.set("EntitySchemaColumn", BPMSoft.deepClone(comboBoxValue));
					if (!comboBoxValue) {
						viewModel.set("ComboBoxListEnabled", false);
					} else {
				...
				var itemView = Ext.create(viewConfig.className || "BPMSoft.Container", viewConfig);
			...
			function _getAggregationColumnName(config) {
				...
				if (aggregationType && aggregationType === BPMSoft.AggregationType.COUNT) {
					aggregationColumnName = ConfigurationEnums.AggregationFunction.COUNT;
				} else if (filterType && filterType === BPMSoft.FilterType.EXISTS) {
					aggregationColumnName = ConfigurationEnums.AggregationFunction.EXISTS;
				} else if (filterType && filterType === BPMSoft.FilterType.INNER_JOIN) {
					aggregationColumnName = ConfigurationEnums.AggregationFunction.INNER_JOIN;
				}
			...
			function _tryUpdateCustomColumns(columns, schema) {
				...
				for (var i = 0; i < columns.length; i++) {
					...
					columnToUpdate.usageType = BPMSoft.EntitySchemaColumnUsageType.General;
				}
			}
			function render(renderTo) {
				viewCollection = new BPMSoft.Collection();
				viewModelCollection = new BPMSoft.Collection();
				...
				EntityStructureHelper.getItemCaption(schemaName, function(name) {
					...
					if (paramSchemaName && BPMSoft[paramSchemaName]) {
						var schema = BPMSoft[paramSchemaName];
			...
			function initViewModel() {
				...
				viewModel.select = function() {
					...
					let loadPrimaryDisplayColumn = BPMSoft.Features.getIsEnabled("LoadPrimaryDisplayColumnForAggregateColumn");
					...
					if (!response.referenceSchemaName && backwardReferenceSchemaName) {
						...
						if(response.referenceSchemaName!==response.lastReferenceSchemaName) {
							response.referenceSchemaName=response.lastReferenceSchemaName;
						}
					}
					if (columnValue.isAggregative && loadPrimaryDisplayColumn) {
						BPMSoft.require([response.lastReferenceSchemaName], function(loadedSchema) {
