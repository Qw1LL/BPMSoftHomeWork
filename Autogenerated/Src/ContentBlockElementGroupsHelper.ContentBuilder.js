define("ContentBlockElementGroupsHelper", [], function() {
	...
	Ext.define("BPMSoft.ContentBlockElementGroupsHelper", {
		...
		_removeGroup: function(groupName) {
			if(this._groups[groupName]) {
				BPMSoft.removeStyleSheet(groupName);
		...
		_createGroup: function(selectedElements, elements) {
			...
			const selectedItems = elements.filterByFn(function(item) {
				return BPMSoft.contains(selectedElements, item.$Id);
			}, this);
		 ...
		 * @param {BPMSoft.Collection} elements Elements collection.
		 */
		...
		ungroupElements: function(selectedElements, elements) {
			...
			BPMSoft.each(selectedElementsGroups, function(groupName) {
				this._removeGroup(groupName);
			}, this);
			elements.filterByFn(function(item) {
				return BPMSoft.contains(selectedElementsGroups, item.$GroupName);
			}, this).each(function(item) {
				item.$GroupName = null;
			}, this);
		},
		 ...
		 * @param {BPMSoft.Collection} elements Elements collection.
		 */
		validateGroupIntersection: BPMSoft.throttle(function(id, elements, callback, scope) {
			if (!BPMSoft.isEmptyObject(this._groups)) {
