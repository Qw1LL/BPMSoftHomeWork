define("FilterConverter", ["ServiceHelper"], function(ServiceHelper) {
	 ...
	 */
	Ext.define("BPMSoft.configuration.FilterConverter", {
		extend: "BPMSoft.core.BaseObject",
		alternateClassName: "BPMSoft.FilterConverter",
		singleton: true,
		 ...
		 * @return {BPMSoft.core.enums.ComparisonType}
		 */
		getComparisonType: function(serverComparisonType) {
			var ComparisonType = BPMSoft.core.enums.ComparisonType;
			switch (serverComparisonType) {
				...
				case "InnerJoin":
					return ComparisonType.INNER_JOIN;
				case "LeftExclusive":
					return ComparisonType.LEFT_EXCLUSIVE;
				default:
					return ComparisonType.EQUAL;
			}
		},
		 ...
		 * @return {BPMSoft.core.enums.AggregationType}
		 */
		getAggregationType: function(serverAggregationType) {
			var AggregationType = BPMSoft.core.enums.AggregationType;
			switch (serverAggregationType) {
				...
				default:
					throw new BPMSoft.exceptions.NotImplementedException({
		 ...
		 * @return {BPMSoft.core.enums.QueryMacrosType}
		 */
		getMacrosType: function(serverMacrosType) {
			var MacrosType = BPMSoft.core.enums.QueryMacrosType;
		 ...
		 * @return {BPMSoft.core.enums.DatePartType}
		 */
		getDatePartType: function(serverDatePartType) {
			switch (serverDatePartType) {
				case "DayOfMonth":
					return BPMSoft.core.enums.DatePartType.DAY;
				case "DayOfWeek":
					return BPMSoft.core.enums.DatePartType.WEEK_DAY;
				case "Hour":
					return BPMSoft.core.enums.DatePartType.HOUR;
				case "HourMinute":
					return BPMSoft.core.enums.DatePartType.HOUR_MINUTE;
				case "Month":
					return BPMSoft.core.enums.DatePartType.MONTH;
				case "Year":
					return BPMSoft.core.enums.DatePartType.YEAR;
				default:
					return null;
			}
		},
		...
		getFilterDataValueType: function(serverFilterConfig) {
			...
			return BPMSoft.core.enums.ServerDataValueType[serverDataValueType.id];
		},
		...
		getIsInFilter: function(serverFilterConfig) {
			...
			return (dataValueType === BPMSoft.core.enums.DataValueType.LOOKUP &&
		...
		getDateTimeParameterExpressionValue: function(serverParameterValue) {
			...
			if (parameterValue && !dateValidateRegexp.test(parameterValue)) {
				throw new BPMSoft.exceptions.InvalidFormatException({
		...
		getMappingParameterExpressionValue: function(serverParameterValue) {
			...
			return {
				value: value,
				displayValue: displayValue,
				Id: BPMSoft.utils.guid.generateGUID()
			};
		},
		...
		getParameterExpressionValue: function(serverParameterValue, dataValueType) {
			...
			var DataValueType = BPMSoft.core.enums.DataValueType;
		 ...
		 * @param {Object} serverExpressionConfig Server expression config.
		 * @param {Object} serverParameterValue Server value config.
		 * @return {BPMSoft.ParameterExpression}
		 */
		createParameterExpression: function(serverExpressionConfig, serverParameterValue) {
			var parameterDataType = BPMSoft.core.enums.DataValueType.TEXT;
			var serverDataValueType = serverExpressionConfig.dataValueType;
			if (serverDataValueType) {
				parameterDataType = BPMSoft.core.enums.ServerDataValueType[serverDataValueType.id];
			}
		 ...
		 * @param {String} parentItemKey Parent item key.
		 * @param {Object} serverFilterConfig Server filter config.
		 * @return {BPMSoft.AggregationQueryExpression}
		 */
		createAggregationExpression: function(serverExpressionConfig, metaPathMapping, parentItemKey,
			...
			var expression = Ext.create("BPMSoft.AggregationQueryExpression", {
				functionType: BPMSoft.core.enums.FunctionType.AGGREGATION,
		 ...
		 * @private
		 * @param {Object} serverExpressionConfig Server expression config.
		 * @return {BPMSoft.FunctionExpression}
		 */
		createMacrosExpression: function(serverExpressionConfig) {
			...
			var expression = Ext.create("BPMSoft.FunctionExpression", {
				functionType: BPMSoft.core.enums.FunctionType.MACROS,
		 ...
		 * @private
		 * @param {Object} serverExpressionConfig Server expression config.
		 * @param {BPMSoft.core.enums.DatePartType} datePartType Date part type.
		 * @param {Object} metaPathMapping MetaPath mapping object.
		 * @param {String} parentItemKey Parent item key.
		 * @param {Object} serverFilterConfig Server filter config.
		 * @return {BPMSoft.FunctionExpression}
		...
		createDatePartExpression: function(serverExpressionConfig, datePartType, metaPathMapping, parentItemKey,
		                                   serverFilterConfig) {
			var functionArgument = Ext.create("BPMSoft.ColumnExpression");

			var expression = Ext.create("BPMSoft.FunctionExpression", {
				functionType: BPMSoft.core.enums.FunctionType.DATE_PART,
		...
		createMappingParameterExpression: function(serverExpressionConfig) {
			...
			var parameterDataType = BPMSoft.core.enums.DataValueType.MAPPING;
			var parameterValue = this.getParameterExpressionValue(serverParameterValue, parameterDataType);
			var expression = Ext.create("BPMSoft.ParameterExpression", {
				parameterValue: parameterValue,
				parameterDataType: parameterDataType
		 ...
		 * @return {BPMSoft.data.expressions.BaseExpression}
		 */
		 ...
		 * @private
		 * @param {Object} serverExpressionConfig Server expression config.
		 * @return {BPMSoft.data.expressions.ParameterExpression[]}
		 */
		...
		createCompareFilter: function(serverFilterConfig, metaPathMapping, parentItemKey) {
			...
			return Ext.create("BPMSoft.CompareFilter", {
		...
		createInFilter: function(serverFilterConfig) {
			...
			return Ext.create("BPMSoft.InFilter", {
		 ...
		 * @return {BPMSoft.IsNullFilter}
		 */
		createIsNullFilter: function(isNull) {
			return Ext.create("BPMSoft.IsNullFilter", {
				isNull: isNull
			});
		},
		 ...
		 * @private
		 * @param {BPMSoft.core.enums.ComparisonType} comparisonType Comparison type.
		 * @param {Object} serverFilterConfig Server filter config.
		 * @param {Object} metaPathMapping MetaPath mapping object.
		 * @param {String} parentItemKey Parent item key.
		 * @return {BPMSoft.data.filters.ExistsFilter}
		 */
		createExistsFilter: function(comparisonType, serverFilterConfig, metaPathMapping, parentItemKey) {
			...
			var subFilters = this.createFilterGroup(serverFilterConfig.subFilters, metaPathMapping);
			var filter = Ext.create("BPMSoft.data.filters.ExistsFilter", {
		...
		/**
		 * Creates InnerJoin filter.
		 * @private
		 * @param {BPMSoft.core.enums.ComparisonType} comparisonType Comparison type.
		 * @param {Object} serverFilterConfig Server filter config.
		 * @param {Object} metaPathMapping MetaPath mapping object.
		 * @param {String} parentItemKey Parent item key.
		 * @return {BPMSoft.data.filters.InnerJoinFilter}
		 */
		createInnerJoinFilter: function(comparisonType, serverFilterConfig, metaPathMapping, parentItemKey) {
			var leftExpression;
			if (serverFilterConfig.rightExpression) {
				leftExpression = this.createColumnExpression(serverFilterConfig.rightExpression, metaPathMapping,
					parentItemKey);
			...
			var subFilters = this.createFilterGroup(serverFilterConfig.subFilters, metaPathMapping);
			var filter = Ext.create("BPMSoft.data.filters.InnerJoinFilter", {
				leftExpression: leftExpression,
				leftExpressionCaption: serverFilterConfig.rightExpression.caption,
				comparisonType: comparisonType,
				subFilters: subFilters
		 ...
		 * @param {Object} metaPathMapping MetaPath mapping object.
		 * @param {String} parentItemKey Parent item key.
		 * @return {BPMSoft.data.filters.CompareFilter} If server item is datePart type filter, returns new datePart
		...
		createDatePartFunctionFilter: function(comparisonType, serverFilterConfig, metaPathMapping, parentItemKey) {
			...
			var filter = Ext.create("BPMSoft.data.filters.CompareFilter", {
				leftExpression: leftExpression,
				comparisonType: comparisonType,
				rightExpression: rightExpression
			});
		 ...
		 * @param {Object} metaPathMapping MetaPath mapping object.
		 * @param {String} parentItemKey Parent item key.
		 * @return {BPMSoft.data.filters.BaseFilter}
		 */
			...
			switch (comparisonType) {
				...
				case ComparisonType.NOT_EXISTS:
					filter = this.createExistsFilter(comparisonType, serverFilterConfig, metaPathMapping,
						parentItemKey);
					break;
				case ComparisonType.INNER_JOIN:
				case ComparisonType.LEFT_EXCLUSIVE:
					filter = this.createInnerJoinFilter(comparisonType, serverFilterConfig, metaPathMapping,
						parentItemKey);
					break;
				default:
					throw new BPMSoft.exceptions.NotImplementedException({
		...
		createFilterGroup: function(serverItemConfig, metaPathMapping) {
			var key = serverItemConfig.uId || BPMSoft.utils.guid.generateGUID();
			...
			if (serverItemConfig.logicalOperation) {
				filterGroup.logicalOperation = (serverItemConfig.logicalOperation === "Or")
					? BPMSoft.core.enums.LogicalOperatorType.OR
					: BPMSoft.core.enums.LogicalOperatorType.AND;
			}
			return filterGroup;
		},
		...
		resolveEntitySchemaMetaPaths: function(metaPathMapping, callback) {
			...
			for (var groupKey in metaPathMapping) {
				...
				var existingItem = BPMSoft.utils.array.findWhere(columnMetaPathsKeyValues, {Key: key});
				if (existingItem) {
		 ...
		 * @param {Function} callback Callback function.
		 * @param {BPMSoft.FilterGroup} callback.filterEditData Filter edit data.
		...
		deserializeFilterEditData: function(dataSourceFilterCollectionData, callback) {
			if (!dataSourceFilterCollectionData) {
				callback({filterEditData: Ext.create("BPMSoft.FilterGroup")});
				return;
			}
	...
	return BPMSoft.FilterConverter;
});
