/**
 * @class BPMSoft.configuration.ConfigurationFileApi
 * ##### ConfigurationFileApi ############ ### ######## ###### ## ###### # ######## #### ######### ##### upload.
 * ### ######## ####### ###### ## ###### #### ########### ## ##### ###### ##### ####### ############### ###########
 * ## ######
*/
define("ConfigurationFileApi", ["ext-base", "BPMSoft", "ConfigurationFileApiResources", "MaskHelper"],
	function(Ext, BPMSoft, resources) {
		Ext.define("BPMSoft.configuration.ConfigurationFileApi", {
			extend: "BPMSoft.BaseObject",
			alternateClassName: "BPMSoft.ConfigurationFileApi",
			singleton: true,
			...
			onPrepare: function(file, options) {
				...
				Ext.apply(data, {
					...
					fileId: data.fileId || BPMSoft.generateGUID(),
					mimeType: file.type,
			...
			_getIsOldUploadMethodEnabled: function() {
				return BPMSoft.Features.getIsEnabled("UseOldFileUploadMethod");
			},
			...
			upload: function(config, callback) {
				if (config.file) {
					this.log(Ext.String.format(BPMSoft.Resources.ObsoleteMessages.ObsoletePropertyMessage, "file",
						"files"));
				...
				this.validateFilesSize(config.files, function(files) {
					...
					const baseUrl = BPMSoft.getConfigurationWebServiceBaseUrl();
					const url = BPMSoft.combinePath(baseUrl, "rest", servicePath);
			...
			getImageFile: function(url, callback, scope) {
				...
				xhr.onload = function() {
					...
					Ext.apply(blob, {
						...
						name: BPMSoft.generateGUID().toString(),
						webkitRelativePath: ""
					});
