define("ESNFeedUtils", ["ext-base", "BPMSoft", "ESNFeedModuleResources", "NetworkUtilities", "ModalBox",
	"AutosizeModule", "ESNConstants", "FormatUtils", "ServiceHelper"],
		function(Ext, BPMSoft, resources, NetworkUtilities, ModalBox, AutosizeModule, ESNConstants, FormatUtils,
				ServiceHelper) {
	 ...
	 * @private
	 */
	var schemasCollection = new BPMSoft.Collection();
	...
	function loadSocialMessages(config, callback, scope) {
		...
		var columnNames = BPMSoft.deepClone(socialMessageColumnNames);
		if (!Ext.isEmpty(sortColumnName) && columnNames.indexOf(sortColumnName) === -1) {
			columnNames.push(sortColumnName);
		}
		var sortDirection = config.sortDirection || BPMSoft.OrderDirection.DESC;
		...
		var id = config.id || BPMSoft.GUID_EMPTY;
		if (BPMSoft.isEmptyGUID(id)) {
			var parentId =  config.parentId;
			var filters = esq.filters;
			if (parentId) {
				filters.addItem(BPMSoft.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL, "Parent",
					parentId));
			} else {
			...
			if (esqConfig && esqConfig.filter) { // ###### ## ######## ###### (#### ##### ########### ## ######## ######)
				...
				filterGroup.addItem(BPMSoft.createColumnFilterWithParameter(
					BPMSoft.ComparisonType.EQUAL, "EntityId", BPMSoft.SysValue.CURRENT_USER_CONTACT.value));
			...
			esq.getEntityCollection(function(result) {
				if (result.success) {
					...
					if (collection.getCount() > 0) {
						...
						var likesCollection = new BPMSoft.Collection();
						var socialLikeEsq = Ext.create("BPMSoft.EntitySchemaQuery", {
							rootSchemaName: "SocialLike"
						});
						socialLikeEsq.addColumn("SocialMessage");
						socialLikeEsq.filters.add("socialMessagesFilter",
							BPMSoft.createColumnInFilterWithParameters("SocialMessage", collection.getKeys()));
						socialLikeEsq.filters.add("currentUserFilter",
							BPMSoft.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL, "User",
	...
	function getCurrentUserPostsFilter() {
		var filterGroup = Ext.create("BPMSoft.FilterGroup");
		var currentUserId = BPMSoft.SysValue.CURRENT_USER.value;
		...
		var socialSubscription = BPMSoft.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL,
		...
		var subFilter = BPMSoft.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL,
			socialUnsubscriptionPath, currentUserId);
		subFilterGroup.addItem(subFilter);
		var existsFilter = BPMSoft.createNotExistsFilter(socialUnsubscriptionPath, subFilterGroup);
	...
	function decorateItem(sandbox) {
		...
		this.set("Comments", Ext.create("BPMSoft.BaseViewModelCollection"));
		this.set("LoadedComments", Ext.create("BPMSoft.BaseViewModelCollection"));
		...
		var entitySchemaUId = this.get("EntitySchemaUId") || BPMSoft.GUID_EMPTY;
		var entityId = this.get("EntityId") || BPMSoft.GUID_EMPTY;
		if (!BPMSoft.isEmptyGUID(entitySchemaUId) && !BPMSoft.isEmptyGUID(entityId)) {
			if (schemasCollection.contains(entitySchemaUId)) {
				...
				esq.filters.add("UId", BPMSoft.createColumnFilterWithParameter(
					BPMSoft.ComparisonType.EQUAL, "UId", entitySchemaUId));
	...
	function getPrimaryImageValue(entity, entitySchemaName, callback, scope) {
		var moduleStructure = BPMSoft.configuration.ModuleStructure[entitySchemaName];
	...
	function getImageValue(entity) {
		...
		if (entity) {
			...
			if (Ext.isObject(image) || BPMSoft.isGUID(image)) {
				imageValue = image.value || image;
			} else {
				return BPMSoft.ImageUrlBuilder.getUrl(resources.localizableImages.DefaultCreatedBy);
			}
		}
	...
	function getEntityImage() {
		...
		return imageValue ? getImageValue(entity) :
			BPMSoft.ImageUrlBuilder.getUrl(resources.localizableImages.NoChannel);
	}
	...
	function onCommentPublishClick() {
		...
		insertSocialMessage(post, function(item) {
			var loadedComments = this.get("LoadedComments");
			BPMSoft.each(loadedComments.getItems(), function(comment) {
	...
	function getLikeImage() {
		...
		return {
			...
			url: BPMSoft.ImageUrlBuilder.getUrl(isLikedMe
	...
	function deleteLike(callback, scope) {
		...
		var userIdFilter = BPMSoft.createColumnFilterWithParameter(
		...
		var entityIdFilter = BPMSoft.createColumnFilterWithParameter(
			BPMSoft.ComparisonType.EQUAL, "SocialMessage", scope.get("Id"));
	...
	function getLikedUsersViewModel() {
		var config = {
			...
			methods: {
				...
				getLikeUsersItems: function(likedUsersCollection) {
					...
					var likedUsersConfig = Ext.create("BPMSoft.Container", {
		...
		return Ext.create("BPMSoft.BaseViewModel", config);
	}
