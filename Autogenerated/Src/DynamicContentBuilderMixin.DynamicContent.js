define("DynamicContentBuilderMixin", ["DynamicContentBuilderMixinResources",
	...
	function(resources, DynamicContentEnums, ReplicaBuilder) {
		Ext.define("BPMSoft.configuration.mixins.DynamicContentBuilderMixin", {
			alternateClassName: "BPMSoft.DynamicContentBuilderMixin",
			...
			_unRegisterDCBuilderMessages: function() {
				var messages = this.BPMSoft.keys(this.dynamicContentBuilderMessages);
			 ...
			 * @protected
			 * @param {BPMSoft.BaseViewModelCollection} items Content items collection.
			 */
			...
			onDCAttributeChange: function(attribute) {
				BPMSoft.each(this.$Items, function(group) {
			...
			validateAttributeUsings: function(attrIndex) {
				...
				BPMSoft.each(this.$Items, function(item) {
					if (item.$ItemType === "blockgroup") {
						BPMSoft.each(item.$Items, function(segment) {
			...
			onDynamicBlockDelete: function(blockId) {
				...
				if (selectedGroup.$Items.isEmpty()) {
					...
					if (this.$ContentBuilderState === BPMSoft.ContentBuilderState.HTML) {
			 ...
			 * @protected
			 * @param {BPMSoft.DynamicContentBlockGroupViewModel} group Group instance for delete.
			 ...
			 * @protected
			 * @param {BPMSoft.DynamicContentBlockViewModel} block Content block view model.
			 * @return {BPMSoft.DynamicContentBlockGroupViewModel} New block group view model.
			 */
			 ...
			 * @protected
			 * @param {BPMSoft.ContentBlockViewModel} block Block config.
			 */
			 ...
			 * @protected
			 * @param {BPMSoft.DynamicContentBlockViewModel} selectedItem Source block view model.
			 * @param {Object} config Config for dynamic block instance.
			 */
			addBlockToGroup: function(selectedItem, config) {
				...
				if (activeBlock) {
					if (this.isMjmlConfig()) {
						...
						newBlock.$Id = BPMSoft.generateGUID();
					}
			 ...
			 * @protected
			 * @param {Object} config Config for dynamic block instance.
			 * @return {BPMSoft.DynamicContentBlockGroupViewModel} New content block view model.
			 */
			...
			reIndexBlocks: function() {
				...
				BPMSoft.each(this.$Items, function(item) {
					if (item.$ItemType === "blockgroup") {
						BPMSoft.each(item.$Items, function(block) {
			...
			getActiveReplicaMask: function() {
				return this.BPMSoft.reduce(this.$Items.getItems(), function(mask, block) {
			...
			/**
			 * Creates instance of BPMSoft.DynamicContentBuilderHelper.
			 * @public
			 * @virtual
			 * @returns {BPMSoft.DynamicContentBuilderHelper} Instance of BPMSoft.DynamicContentBuilderHelper.
			 */
			createContentBuilderHelper: function() {
				return Ext.create("BPMSoft.DynamicContentBuilderHelper", {
			...
			extendChildrenItemTypes: function() {
				Ext.apply(this.childItemTypes, {
					block: "BPMSoft.DynamicContentBlockViewModel",
					blockgroup: "BPMSoft.DynamicContentBlockGroupViewModel",
					mjblock: "BPMSoft.DynamicContentMjBlockViewModel",
					htmlblock: "BPMSoft.DynamicContentHtmlBlockViewModel"
				});
			}
