define('ProcessHelper', ['ext-base', 'BPMSoft', 'ProcessHelperResources'],
	...
	function cardOnLoadEntryPoint(response) {
		...
		if (response.length > 1) {
			var buttonItems = [];
			BPMSoft.each(response, function(item) {
				buttonItems.push({
		...
		var entryPointsButton = Ext.create('BPMSoft.Button', buttonConfig);
	...
	function getIsDateTimeDataValueType(dataValueType) {
		return (dataValueType && (dataValueType === BPMSoft.DataValueType.DATE
	...
	function getServerValueByDataValueType(value, dataValueType) {
		try {
			if (getIsDateTimeDataValueType(dataValueType)) {
				return BPMSoft.encodeDate(value);
			}
		} catch (e) {
		}
		return value;
