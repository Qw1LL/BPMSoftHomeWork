define("TimelineDataStorage", ["BaseTimelineItemViewModel", "EsqTimelineDataProvider", "EsnTimelineDataProvider"],
	 ...
	 */
	Ext.define("BPMSoft.configuration.TimelineDataStorage", {
		extend: "BPMSoft.BaseViewModelCollection",
		alternateClassName: "BPMSoft.TimelineDataStorage",
		 ...
		 * @type {String}
		 */
		_defaultViewModelClassName: "BPMSoft.BaseTimelineItemViewModel",

		/**
		 * Current order direction of the collection.
		 * @private
		 * @type {BPMSoft.OrderDirection}
		 */
		 ...
		 */
		_internalCollection: Ext.create("BPMSoft.Collection"),
		 ...
		 * @type {String}
		 */
		_defaultDataProviderClassName: "BPMSoft.EsqTimelineDataProvider",
		 ...
		 * @type {Boolean}
		 */
		_useEsnRights: BPMSoft.Features.getIsEnabled("UseEsnRights"),
		...
		_abortRequests: function() {
			BPMSoft.each(this._providers, function(provider) {
				provider.abort();
			});
		},
		...
		_loadEntityItems: function(queryResult) {
			BPMSoft.each(queryResult.rows, function(row) {
				var id = row.Id;
				// check for e-mail
				if (!row?.Author?.value || !row?.Recipient?.value 
						|| row?.Author?.value != row?.Recipient?.value) {
		...
		_loadItems: function(responses) {
			BPMSoft.each(responses, function(response) {
		...
		_getViewModelByQueryResult: function(row, rowConfig) {
			BPMSoft.each(rowConfig, function(column, columnName) {
				...
				if (rowDataType === BPMSoft.DataValueType.DATE || rowDataType === BPMSoft.DataValueType.DATE_TIME ||
					...
					row[columnName] = Ext.isEmpty(rawDate) ? null : BPMSoft.parseDate(rawDate);
				}
			}, this);
		...
		_initPagingOptions: function() {
			var pagingOptions = {};
			var queryConfig = this._queryConfig;
			BPMSoft.each(queryConfig.entities, function(entity) {
		...
		_clearPagingOptionsRowCounts: function() {
			BPMSoft.each(this._pagingOptions, function(config) {
				config.rowCount = 0;
			}, this);
		},
		 ...
		 * @private
		 * @param {String} entityConfigKey Entity config key.
		 * @return {BPMSoft.BaseTimelineViewModel}
		 */
		 ...
		 * @param {BPMSoft.Collection} collection Collection.
		 ...
		 * @private
		 * @param {BPMSoft.BaseViewModel} itemA Comparable item.
		 * @param {BPMSoft.BaseViewModel} itemB Comparable item.
		...
		_sortItemsByOrderColumn: function(itemA, itemB) {
			...
			if (this._orderDirection === BPMSoft.OrderDirection.DESC) {
		 ...
		 * @private
		 * @return {BPMSoft.Collection} Added items.
		 */
		_addDataPortion: function() {
			var nextItems = Ext.create("BPMSoft.Collection");
		...
		_initDataProviders: function() {
			BPMSoft.each(this._getDataProviderClassNames(), function(providerClassName) {
				if (this._isIgnoredDataProvider(providerClassName) ||
						BPMSoft.findWhere(this._providers, {alternateClassName: providerClassName})) {
					return;
				}
		...
		_getDataProviderClassNames: function() {
			var classNames = [];
			var queryConfig = this._queryConfig;
			BPMSoft.each(queryConfig.entities, function(entityConfig) {
		...
		_isIgnoredDataProvider: function(providerClassName) {
			var esnDataProviderClassName = "BPMSoft.EsnTimelineDataProvider";
		...
		_buildProvidersConfig: function(entitiesToSelect, queryConfig) {
			var providersConfig = [];
			BPMSoft.each(entitiesToSelect, function(entityConfig) {
		...
		_getProviderConfig: function(providersConfig, entityDataProviderClassName) {
			var providerConfig = BPMSoft.findWhere(providersConfig, {className: entityDataProviderClassName});
			if (!providerConfig) {
				providerConfig = {
		...
		_loadDataByProvidersConfig: function(providersConfig, queryConfig, callback, scope) {
			BPMSoft.eachAsyncAll(providersConfig, function(providerConfig, nextFn) {
				var provider = BPMSoft.findWhere(this._providers, {alternateClassName: providerConfig.className});
				provider.getData({
		...
		loadData: function(callback, scope) {
			this._abortRequests();
			var queryConfig = BPMSoft.deepClone(this._queryConfig);
		...
		/**
		 * @inheritdoc BPMSoft.BaseViewModelCollection#clear
		 * @override
		 */
		clear: function() {
