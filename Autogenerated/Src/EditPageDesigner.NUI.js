define('EditPageDesigner', ['ext-base', 'core', 'BPMSoft', 'sandbox', 'StructureExplorerUtilities',
	...
	function(Ext, core, BPMSoft, sandbox, StructureExplorerUtilities, EditPageDesignerGroupEdit, viewGenerator,
		...
		function loadCardView(renderTo) {
			...
			if (viewModel) {
				...
				var config = BPMSoft.deepClone(generatedViewConfig);
				...
				cardView = Ext.create(generatedCardViewConfig.className || 'BPMSoft.Container',
					...
					createSchemaParents(schema, function(schemaConfig, parentSchemaConfig) {
						...
						EditPageDesignerHelper.getModifications(cardSchema.name, function(list) {
							...
							viewModel = Ext.create(viewModelConfig.name, {
								values: {
									NewColumns: new BPMSoft.Collection(),
							...
							var generatedViewConfig = viewGenerator.generate();
							var view = Ext.create(generatedViewConfig.className || 'BPMSoft.Container',
								generatedViewConfig);
		...
		function generateContainerId(schema) {
			if (Ext.isEmpty(schema.EditStructureContainerId)) {
				...
				schema.EditStructureContainerId = BPMSoft.generateGUID();
			}
			if (schema.items) {
		...
		function createViewModelClassBySchema(schema) {
			...
			viewModelConfig.insertModification = function(modification, customConfig, suctomItemConfig) {
				...
				var schemaModification = BPMSoft.deepClone(modification);
