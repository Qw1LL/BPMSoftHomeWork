define('DocumentModule', ['ext-base', 'BPMSoft', 'DocumentModuleResources', 'GeneralDetails', 'ConfigurationEnums'],
	function(Ext, BPMSoft, resources, GeneralDetails, ConfigurationEnums) {
		...
		function pageValidate() {
			...
			if (isValid && (!this.changedValues || !Ext.isEmpty(this.changedValues.Number))) {
				...
				var buttonsConfig = {
					buttons: [BPMSoft.MessageBoxButtons.YES.returnCode, BPMSoft.MessageBoxButtons.NO.returnCode],
					defaultButton: 0
				};
				if (!Ext.isEmpty(number)) {
					...
					esq.addColumn('Id');
					var filterId = BPMSoft.createColumnFilterWithParameter(BPMSoft.ComparisonType.NOT_EQUAL,
						'Id', id);
					var filterNum = BPMSoft.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL,
						'Number', number);
		...
		function RemindToOwnerDateOnChange() {
			...
			if (remindToOwner) {
				var dateTime = this.getSysDefaultValue(BPMSoft.SystemValueType.CURRENT_DATE_TIME);
