define("ExpressionEdit", ["css!ExpressionEdit", "ExpressionEditViewGenerator", "ExpressionEnums"], function() {
	Ext.define("BPMSoft.control.ExpressionEdit", {
		extend: "BPMSoft.Container",
		alternateClassName: "BPMSoft.ExpressionEdit",
		 ...
		 * View generator.
		 * @private
		 * @type {BPMSoft.ExpressionEditViewGenerator}
		 */
		viewGenerator: null,
		...
		_getExpressionMenuTypeList: function(typeList) {
			const menuList = [];
			BPMSoft.each(typeList, function(item) {
				if (!item) {
					return;
				}
				const tag = item.tag;
		...
		_isDataSourceExpressionType: function(expressionType) {
			return expressionType === BPMSoft.ExpressionEnums.ExpressionType.COLUMN && this.expressionTypeList &&
				Ext.Array.contains(this.expressionTypeList, BPMSoft.ExpressionEnums.ExpressionType.DATASOURCE);
		},
		...
		_getDataSourcesExpressionTypeMenuItem: function() {
			return {
					tag: {
						expressionType: BPMSoft.ExpressionEnums.ExpressionType.DATASOURCE
					},
					items: this._getDataSourcesMenuConfig(),
					markerValue: BPMSoft.ExpressionEnums.ExpressionType.DATASOURCE
				};
		},
		...
		_getColumnExpressionTypeMenuItem: function() {
			...
			return {
				"tag": {
					"expressionType": BPMSoft.ExpressionEnums.ExpressionType.COLUMN,
		...
		_getDataSourcesMenuConfig: function() {
			const list = [];
			const scope = this;
			const dataSources = Ext.create("BPMSoft.Collection");
		 ...
		 * @protected
		 * @return {BPMSoft.ExpressionEditViewGenerator} Control view generator
		 */
		getViewGenerator: function() {
			if (!this.viewGenerator) {
				this.viewGenerator = Ext.create("BPMSoft.ExpressionEditViewGenerator", {
					"schemaName": this.id
				});
			}
		...
		getDefaultValueByDataValueType: function() {
			var isBooleanType = this.dataValueType === BPMSoft.DataValueType.BOOLEAN;
		...
		getDefaultPlaceholder: function(expressionType) {
			var expressionTypeConfig = BPMSoft.BusinessRuleElementHelper.getElementByType(expressionType);
		...
		isConstantExpressionType: function(expressionType) {
			return expressionType === BPMSoft.ExpressionEnums.ExpressionType.CONSTANT;
		},
		...
		getValueControlVisible: function() {
			...
			if (this.isConstantExpressionType(expressionType) &&
				BPMSoft.isLookupDataValueType(this.getDataValueType()) &&
		...
		_getConstantMenuConfig: function() {
			var list = [];
			var scope = this;
			var constants = Ext.create("BPMSoft.Collection");
		...
		_getSysValueExpressionMenuItem: function() {
			...
			const sysValueItem = {
				tag: {
					expressionType: BPMSoft.ExpressionEnums.ExpressionType.SYSVALUE
				},
				items: sysValuesMenuConfig,
				markerValue: BPMSoft.ExpressionEnums.ExpressionType.SYSVALUE
			};
			return sysValueItem;
		},
		...
		_getSysValuesMenuConfig: function() {
			var list = [];
			var scope = this;
			var sysValues = Ext.create("BPMSoft.Collection");
		...
		updateTypeMenu: function() {
			...
			if (control && control.menu) {
				...
				control.menu = Ext.create("BPMSoft.Menu", {
		...
		getExpressionTypeCaption: function(expressionType) {
			...
			if (!Ext.isEmpty(expressionType)) {
				var typeConfig = BPMSoft.BusinessRuleElementHelper.getElementByType(expressionType);
		...
		getDataValueTypeCaption: function(dataValueType) {
			...
			BPMSoft.each(BPMSoft.DataValueType, function(value, key) {
				if (value === dataValueType) {
					response = BPMSoft.Resources.DataValueType[key];
				}
			}, this);
			return response;
		},
		 ...
		 * @protected
		 * @return {BPMSoft.Component} Type button control.
		 */
		 ...
		 * @protected
		 * @return {BPMSoft.Component} Reference schema control.
		 */
		 ...
		 * @protected
		 * @return {BPMSoft.Collection} Reference schema list.
		 */
		getReferenceSchemaList: function() {
			if (!this.referenceSchemaList) {
				this.referenceSchemaList = Ext.create("BPMSoft.Collection");
		 ...
		 * @param {BPMSoft.Collection} list Reference schema list.
		 */
		 ...
		 * @protected
		 * @param {BPMSoft.Collection} list Data list.
		 */
		...
		getCanUseReferenceSchema: function() {
			var dataValueType = this.getDataValueType();
			var expressionType = this.getExpressionType();
			return this.isConstantExpressionType(expressionType) && BPMSoft.isLookupDataValueType(dataValueType);
		},
		...
		subscribeToolControlEvents: function() {
			this.processToolControl(function(control) {
				let propertyName;
				if (control.className === "BPMSoft.CheckBoxEdit") {
		...
		subscribeValueControlEvents: function() {
			...
			var isLookup = BPMSoft.isLookupDataValueType(dataValueType) && this.isConstantExpressionType(expressionType);
			this.processValueControl(function(control) {
				...
				if (expressionType === BPMSoft.ExpressionEnums.ExpressionType.COLUMN ||
					(expressionType === BPMSoft.ExpressionEnums.ExpressionType.PARAMETER &&
						this.contentType === BPMSoft.ContentType.LOOKUP)) {
				...
				if (expressionType === BPMSoft.ExpressionEnums.ExpressionType.SYSVALUE) {
					control.on("prepareList", this.onPrepareSysValuesList, this);
				}
				if (expressionType === BPMSoft.ExpressionEnums.ExpressionType.ATTRIBUTE) {
		 ...
		 * @protected
		 * @param {String} filter Control filter.
		 * @param {BPMSoft.Collection} list System values list.
		 */
		...
		getValueControlValuePropertyName: function(dataValueType) {
			return this.isConstantExpressionType(this.expressionType) && BPMSoft.isBooleanDataValueType(dataValueType)
				? "checked"
				: "value";
		},
		 ...
		 * @protected
		 * @param {BPMSoft.Component} control Control.
		...
		enrichValueControlConfigBeforeGenerate: function(config) {
			...
			if (BPMSoft.isTextDataValueType(dataValueType)) {
				config.dataValueType = BPMSoft.DataValueType.TEXT;
				config.contentType = config.contentType || BPMSoft.ContentType.SHORT_TEXT;
			}
			if (BPMSoft.isMoneyCompatibleDataValueType(dataValueType)) {
				config.dataValueType = BPMSoft.DataValueType.FLOAT;
				config.decimalPrecision = 8;
			}
			if (BPMSoft.isLookupDataValueType(dataValueType)) {
				Ext.apply(config, {"dataValueType": BPMSoft.DataValueType.ENUM});
			}
			if (!BPMSoft.isBooleanDataValueType(dataValueType)) {
		...
		enrichValueControlConfigAfterGenerate: function(config, dataValueType) {
			if (this.isConstantExpressionType(this.expressionType) && BPMSoft.isLookupDataValueType(dataValueType)) {
				config.list = this.getValueList();
			}
			if (this.expressionType === BPMSoft.ExpressionEnums.ExpressionType.COLUMN) {
				config.list = this.getEntitySchemaColumnList();
			}
			if (this.expressionType === BPMSoft.ExpressionEnums.ExpressionType.SYSVALUE) {
				config.list = this._getSysValuesList();
			}
			if (this.expressionType === BPMSoft.ExpressionEnums.ExpressionType.PARAMETER) {
				config.list = this._getPageSchemaParametersList();
			}
			if (this.expressionType === BPMSoft.ExpressionEnums.ExpressionType.ATTRIBUTE) {
		 ...
		 * Returns value list.
		 * @protected
		 * @return {BPMSoft.Collection} Value list.
		 */
		getValueList: function() {
			if (!this.valueList) {
				this.valueList = Ext.create("BPMSoft.Collection");
		 ...
		 * @protected
		 * @return {BPMSoft.Collection} Entity schema column list.
		 */
		getEntitySchemaColumnList: function() {
			if (!this.entitySchemaColumnList) {
				this.entitySchemaColumnList = Ext.create("BPMSoft.Collection");
		 ...
		 * @private
		 * @return {BPMSoft.Collection} System values list.
		 */
		_getSysValuesList: function() {
			if (!this.sysValuesList) {
				this.sysValuesList = Ext.create("BPMSoft.Collection");
		...
		_getAttributesList: function() {
			if (!this.attributesList) {
				this.attributesList = Ext.create("BPMSoft.Collection");
		...
		_getPageSchemaParametersList: function() {
			if (!this.pageSchemaParameterList) {
				this.pageSchemaParameterList = Ext.create("BPMSoft.Collection");
		 ...
		 * @protected
		 * @return {BPMSoft.DataValueType} Value control data value type.
		 */
		getValueControlDataValueType: function() {
			var dataValueType;
			switch (this.expressionType) {
				case BPMSoft.ExpressionEnums.ExpressionType.SYSSETTING:
				...
				case BPMSoft.ExpressionEnums.ExpressionType.CONSTANT:
					dataValueType = this.dataValueType || BPMSoft.DataValueType.TEXT;
					break;
				default:
					dataValueType = null;
			}
