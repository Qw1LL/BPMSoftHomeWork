define("SectionViewModelGenerator", ["ext-base", "BPMSoft", "SectionViewModelGeneratorResources",
	...
	function(Ext, BPMSoft, resources, ConfigurationEnums, ConfigurationConstants, SysModuleAnalyticsReport,
		...
		function getFullViewModelSchema(sourceViewModelSchema) {
			...
			baseSchemaMethods.addSelectColumns = function(select) {
				...
				var moduleStructure = BPMSoft.configuration.ModuleStructure[getSectionCode(this)];
			...
			baseSchemaMethods.getExtendedFilterDisplayValue = function(extendedFilter) {
				var comparisonTypeFlipped = BPMSoft.invert(BPMSoft.ComparisonType);
				var logicalOperatorTypeFlipped = BPMSoft.invert(BPMSoft.LogicalOperatorType);
			...
			baseSchemaMethods.pushSectionTypeFilter = function(select) {
				var moduleConfig = BPMSoft.configuration.ModuleStructure[getSectionCode(this)];
			...
			baseSchemaMethods.pushDemoUserFilter = function(select) {
				var currentUser = this.getSysDefaultValue(BPMSoft.SystemValueType.CURRENT_USER);
				if (!Ext.isEmpty(currentUser) && !Ext.isEmpty(currentUser.displayValue) &&
					...
					select.filters.add("demoFilter", BPMSoft.createColumnFilterWithParameter(
						BPMSoft.ComparisonType.EQUAL, "CreatedBy", currentUserContact.value));
				}
			};
			...
			baseSchemaMethods.modifyLinksQuery = function(esq) {
				...
				BPMSoft.each(linksConfig, function(columnName) {
			...
			baseSchemaMethods.addChart = function() {
				...
				BPMSoft.Router.pushState(null, null, token);
			};
			...
			baseSchemaMethods.addToStaticFolder = function() {
				...
				if (records && records.length) {
							...
							records.forEach(function(record) {
								selectedRows.forEach(function(folder) {
									...
									if (!context.isObjectInArray(existingRecords, {
										...
										insert.setParameterValue("Folder", folder.value, BPMSoft.DataValueType.GUID);
					...
					var lookUpConfig = {
						...
						filters: BPMSoft.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL, "FolderType",
			...
			baseSchemaMethods.deleteFromStaticFolder = function() {
				...
				if (records && records.length) {
					var entitySchemaName = this.entitySchema.name;
					var query = Ext.create("BPMSoft.DeleteQuery", { rootSchemaName: entitySchemaName + "InFolder" });
					query.filters.addItem(BPMSoft.createColumnInFilterWithParameters(entitySchemaName, records));
					query.filters.add(BPMSoft.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL,
			...
			baseSchemaMethods.startSectionDesigner = function() {
				...
				for (var name in BPMSoft.configuration.ModuleStructure) {
					if (!BPMSoft.configuration.ModuleStructure.hasOwnProperty(name)) {
						continue;
					}
					var sectionInfo = BPMSoft.configuration.ModuleStructure[name];
					...
					BPMSoft.each(linksConfig, function(columnName) {
						...
						if (columnName === primaryDisplayColumnName) {
							item["get" + columnName + "LinkPrimary"] = function() {
								...
								var entityStructure = BPMSoft.configuration.EntityStructure[getSectionCode(scope)];
								var patch = [];
								var cardSchema;
								if (entityStructure && Ext.isArray(entityStructure.pages) &&
									entityStructure.pages.length > 0) {
									var moduleStructure =
										BPMSoft.configuration.ModuleStructure[getSectionCode(scope)];
							...
							if (column && column.isLookup && column.referenceSchemaName) {
								...
								var entityStructure = BPMSoft.configuration.EntityStructure[schemaName];
								if (entityStructure && Ext.isArray(entityStructure.pages) &&
									entityStructure.pages.length > 0) {
									item["get" + columnName + "Link"] = function() {
										...
										if (!Ext.isEmpty(columnTypeId) && entityStructure.pages.length > 1) {
											BPMSoft.each(entityStructure.pages, function(pageItem) {
			...
			baseSchemaMethods.setCustomProfile = function(key, value) {
				...
				BPMSoft.utils.saveUserProfile(customSectionProfileKey,
			...
			baseSchemaMethods.getEditPages = function() {
				var structureConfig = BPMSoft.configuration.ModuleStructure[getSectionCode(this)];
...
//						}
		...
		function getCurrentItemPageConfig(scope) {
			var moduleStructure = BPMSoft.configuration.ModuleStructure[getSectionCode(scope)];
			if (moduleStructure) {
		...
		function openCardModule(action, tag, recordId) {
			var config = BPMSoft.configuration.ModuleStructure[getSectionCode(this)];
			...
			var newState = BPMSoft.deepClone(params.state);
		...
		function checkColumnsNames(columns, name) {
			if (columns[name] !== undefined) {
				...
				throw new BPMSoft.ItemAlreadyExistsException({message: errorMessage});
			}
		}

		function getViewModelColumns(columns, columnsContainer) {
			BPMSoft.each(columnsContainer, function(schemaItem) {
				var itemType = schemaItem.type;
				switch (itemType) {
					case BPMSoft.ViewModelSchemaItem.ATTRIBUTE:
						var columnConfig = BPMSoft.utils.common.deepClone(schemaItem);
		...
		return {
			...
			generate: function(viewModelSchema) {
				...
				var config = {
					...
					alternateClassName: "BPMSoft." + fullViewModelSchema.name,
					entitySchema: fullViewModelSchema.entitySchema,
					loadAll: false,
