define("DropdownLookupMixin", [], function() {
	Ext.define("BPMSoft.configuration.mixins.DropdownLookupMixin", {
		alternateClassName: "BPMSoft.DropdownLookupMixin",
		...
		/**
		 * Loads the BPMSoft.Collection created from enum into the list.
		 * @public
		...
		/**
		 * Returns BPMSoft.Collection created from the enum definition.
		 * @private
		...
		getListFromEnum: function(enumName, scope) {
			...
			collection = scope.Ext.create("BPMSoft.Collection");
			scope.BPMSoft.each(enums, function(item) {
		 ...
		 * @return {BPMSoft.EntitySchemaQuery} EntitySchemaQuery for current lookup.
		 */
		 ...
		 * @public
		 * @param {BPMSoft.FilterGroup} filterGroup Filters to apply.
		...
		prepareLookupList: function(filterGroup, filterParameter, columnName, listAttrName, scope) {
			...
			filterGroup.addItem(BPMSoft.createColumnFilterWithParameter(
				BPMSoft.ComparisonType.CONTAIN, "Name", filterParameter));
			esq.filters.add(filterGroup);
			esq.getEntityCollection(function(result) {
				if (!result.success) {
					return;
				}
				var viewModelCollection = Ext.create("BPMSoft.Collection");
	...
	return BPMSoft.DropdownLookupMixin;
});
