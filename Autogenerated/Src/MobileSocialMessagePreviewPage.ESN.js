...
Ext.define("BPMSoft.configuration.view.SocialMessagePreviewPage", {
	...
	config: {
		...
		pageType: BPMSoft.PageTypes.Preview,
	 ...
	 * @virtual
	 * @cfg-applier
	 * @returns {BPMSoft.Configuration.FeedCommentsList} The list.
	 */
	applyFeedCommentsList: function(config) {
		...
		return Ext.factory(config, "BPMSoft.Configuration.FeedCommentsList");
	},
		...
		var config = {
			...
			items: [
				...
				},
				{
					...
					placeHolder: BPMSoft.LocalizableStrings.CommentInputPlaceholder,
					clearIcon: false,
...
Ext.define("BPMSoft.configuration.controller.SocialMessagePreviewPage", {
	 ...
	 * Returns the store.
	 * @private
	 * @returns {BPMSoft.store.BaseStore} Store.
	 */
	getStore: function() {
		var view = this.getView();
	...
	initializeCommentInput: function() {
		...
		if (rawConfig.focusCommentInput && !BPMSoft.util.isWindowsPlatform()) {
			setTimeout(function() {
	...
	completeCommentDataSaving: function(commentRecord) {
		...
		if (!BPMSoft.Configuration.isFeedInHybridMode() || !parentRecordIsDelivered) {
			BPMSoft.Array.pushUnique(store.socialMessageModifiedRecordIds, commentRecord.get("Id"));
		}
	...
	addComment: function(config) {
		...
		var parentRecord = this.getRecord();
		var queryConfigForSave = Ext.create("BPMSoft.QueryConfig", {
	...
	updateModifiedRecordInList: function(record) {
		...
		var view = this.getView();
		var list = view.getFeedCommentsList();
		BPMSoft.Array.pushUnique(store.socialMessageModifiedRecordIds, recordId);
	...
	refreshPreviousPages: function(operationConfig) {
		...
		var pageHistoryItem = this.getPageHistoryItem();
		BPMSoft.PageNavigator.refreshPreviousPages(config, pageHistoryItem);
	},
	...
	initializeView: function() {
		...
		if (!Ext.os.is.Phone) {
			var mainPageView = BPMSoft.util.getMainController().getView();
	...
	initBackButton: function(viewMode) {
		...
		if (viewMode === BPMSoft.ViewModes.StaticGrid && parentPageHistoryItem &&
	...
	initMenuButton: function(viewMode) {
		var view = this.getView();
		var navigationPanel = view.getNavigationPanel();
		if (viewMode === BPMSoft.ViewModes.StaticGrid) {
	 ...
	 * @protected
	 * @virtual
	 * @param {BPMSoft.Configuration.FeedCommentsList} list List.
	 */
	onListBeforeInitialize: function(list) {
		var model = this.self.Model;
		var gridConfig = BPMSoft.sdk.GridPage.getConfig(model);
	...
	initializeQueryConfig: function() {
		var modelConfig = BPMSoft.ApplicationConfig.getModelConfig(this.self.Model);
	...
	loadData: function() {
		...
		return BPMSoft.configuration.controller.SocialMessagePreviewPage.superclass.loadData.call(this).then(function() {
			this.loadStore();
		}.bind(this));
	},
	...
	onActionSelected: function(actionConfig) {
		...
		if (record) {
			...
			if (actionClassName === "BPMSoft.ActionEdit" || actionConfig.name === "add") {
				var view = this.getView();
				view.stopActiveAnimation();
			} else if (actionClassName === "BPMSoft.ActionDelete" && BPMSoft.Configuration.isFeedInHybridMode()) {
				var parentRecordIsDelivered = this.isParentRecordDelivered(this.getRecord());
	...
	onBeforeLoadStore: function(store, operation) {
		var idFilter = Ext.create("BPMSoft.Filter", {
			property: "Id",
			value: this.recordId
		});
	...
	onLikesContactPickerSelect: function(list, record) {
		...
		BPMSoft.util.openPreviewPage("Contact", {
	...
	onFeedCommentsListLinkTap: function(list, config) {
		BPMSoft.Configuration.openLink(config);
	},
	 ...
	 * @protected
	 * @virtual
	 * @returns {BPMSoft.ComboBoxPicker} Component.
	 */
	getLikesContactPicker: function() {
		if (this.likesContactPicker) {
			return this.likesContactPicker;
		}
		var likesContactStore = Ext.create("BPMSoft.store.BaseStore", {
			model: "SocialLike"
		});
		var base64str = BPMSoft.IconsConfiguration.get("MobileImageListDefaultContactPhoto");
		var defaultImageUrl = BPMSoft.util.getBase64ImageUrl(base64str);
		var likesContactPicker = this.likesContactPicker = Ext.create("BPMSoft.ComboBoxPicker", {
			component: {
			...
			hideOnItemTap: false,
			title: BPMSoft.LocalizableStrings.SocialMessagePreviewPageLikesContactPickerTitle,
			popup: true
		});
	...
	getDefaultTitle: function() {
		return BPMSoft.LocalizableStrings.SocialMessagePreviewPageTitle;
	},
