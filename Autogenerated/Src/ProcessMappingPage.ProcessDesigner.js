...
define("ProcessMappingPage", ["BPMSoft", "ProcessMappingPageResources", "ModalBox", "AcademyUtilities",
		"ckeditor-base", "EntitySchemaSelectMixin", "MappingPageTabUtilities", "css!LookupPageCSS"],
		function(BPMSoft, resources, ModalBox, AcademyUtilities) {
			return {
				mixins: {
					entitySchemaSelectMixin: "BPMSoft.EntitySchemaSelectMixin",
					mappingPageTabUtilities: "BPMSoft.MappingPageTabUtilities"
				},
				attributes: {
					/**
					 * Tabs collection.
				...
				methods: {
					...
					initLookupValue: function(initialLookupSchemaUId, callback, scope) {
						if (initialLookupSchemaUId) {
							BPMSoft.EntitySchemaManager.getInstanceByUId(initialLookupSchemaUId, function(schema) {
					...
					addQuote: function() {
						...
						var isConst = (source === BPMSoft.ProcessSchemaParameterValueSource.ConstValue);
						var notEmpty = !Ext.isEmpty(formula);
						var isText = BPMSoft.isTextDataValueType(dataValueType);
					...
					initFormulaText: function(callback, scope) {
						...
						var source = this.get("Source");
						if (source === BPMSoft.ProcessSchemaParameterValueSource.ConstValue) {
						...
						BPMSoft.FormulaParserUtils.getFormulaDisplayValue(value, schema,
					 ...
					 * @param {BPMSoft.core.collections.Collection} lookupValueResult.selectedRows Selcted rows.
					 */
					onResultSelectedRows: function(lookupValueResult) {
						...
						if (selectedRows && selectedRows.length) {
							...
							if (activeTab === "TabFunctionsMapping") {
								...
								displayValue = BPMSoft.ProcessSchemaDesignerUtilities.addParameterMask(displayValue);
								text = displayValue;
							}
					 ...
					 * @param {BPMSoft.core.collections.Collection} lookupValueResult.selectedRows Active row.
					 */
					 ...
					 * @protected
					 * @virtual
					 * @param {BPMSoft.BaseViewModel} viewModel View model for loading data.
					 */
					...
					initMappingPageUISetting: function() {
						var mappingType = this.get("MappingType");
						if (mappingType > BPMSoft.MappingEnums.MappingType.ALL) {
					...
					isSingleTab: function() {
						var mappingType = this.get("MappingType");
						return mappingType !== BPMSoft.MappingEnums.MappingType.ALL && this.isPowerOfTwo(mappingType);
					},
					...
					updateStyleSheet: function(cssSelector, cssProperty, cssValue) {
						...
						BPMSoft.updateStyleSheet(cssSelector, cssProperty, cssValue);
					},

					/**
					 * Sets ModalBox caption.
					 * @private
					 * @param {BPMSoft.MappingEnums.MappingType} mappingType Mapping type.
					 */
					setModalBoxCaption: function(mappingType) {
						var caption;
						if (mappingType === BPMSoft.MappingEnums.MappingType.ALL) {
							caption = resources.localizableStrings.HeaderCaption;
						} else if (mappingType === BPMSoft.MappingEnums.MappingType.LOOKUP) {
							...
							caption = `${resources.localizableStrings.SelectCaption}: ${resources.localizableStrings.HeaderSysSettingsMappingCaption}`;
						}  else {
					 ...
					 * Unloads tab module.
					 * @private
					 * @param {BPMSoft.BaseViewModel} viewModel Tab view model.
					 */
					...
					onSaveClick: function() {
						var sandbox = this.sandbox;
						var bodyMaskId = BPMSoft.Mask.show();
						var modalBoxMaskId = BPMSoft.Mask.show({
							selector: "#" + this.renderTo,
							timeout: 0
						});
						this.getFormulaValue(function(result) {
							BPMSoft.Mask.hide(modalBoxMaskId);
					...
					showValidationInfo: function(unrecognizedMacros) {
						var message = "";
						BPMSoft.each(unrecognizedMacros, function(item) {
					...
					getParameterValues: function(selectedRow) {
						...
						if (activeTab === "TabSysSettingsMapping") {
							var sysSettingDisplayValueMacros = BPMSoft.FormulaMacros.prepareSysSettingDisplayValue(
							...
							var lookupValueMacros = BPMSoft.FormulaMacros.prepareLookupValue(schema.value,
								selectedRow.value);
							var lookupDisplayValueMacros = BPMSoft.FormulaMacros.prepareLookupDisplayValue(
					...
					getFunctionWithSelectedText: function(value, displayValue) {
						...
						if (selectedText) {
							BPMSoft.each(BPMSoft.process.constants.FUNCTIONS, function(func) {
								if (func.value === value) {
					...
					getSysSettingsDataValueTypeFilter: function() {
						var isInvokedByFormula = this.get("MappingType") === BPMSoft.MappingEnums.MappingType.ALL;
						var dataValueType = isInvokedByFormula ? null : this.get("DataValueType");
						var valueTypeGroup = BPMSoft.getSysSettingsValueTypeGroup(dataValueType);
					 ...
					 * @private
					 * @return {BPMSoft.FilterGroup} System setting reference schema filter.
					 */
					getSysSettingReferenceSchemaFilter: function() {
						...
						return BPMSoft.createColumnInFilterWithParameters("ReferenceSchemaUId", referenceSchemaArray);
					},
					...
					onFormulaHelpButtonClick: function() {
						BPMSoft.BaseSchemaDesignerUtilities.openHelp(AcademyUtilities, this.name);
					},
					...
					_getLookupFilter: function() {
						...
						return lookupSerializableFilter && BPMSoft.deserialize(lookupSerializableFilter) || null;
					}
				...
				diff: /**SCHEMA_DIFF*/[
					...
					},
					{
						...
						"values": {
							...
							"click": {"bindTo": "close"},
							"classes": {"textClass": ["main-buttons","t-btn-style-default"]}
						}
					},
					{
						"operation": "insert",
