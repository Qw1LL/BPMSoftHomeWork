define("MultiDeleteDetailModuleV2", ["SchemaBuilderV2", "ViewGeneratorV2", "DefaultProfileHelper", "BaseNestedModule",
	...
	function(SchemaBuilder, ViewGenerator, LookupProfileHelper) {
		Ext.define("BPMSoft.configuration.MultiDeleteDetailModule", {
			extend: "BPMSoft.BaseNestedModule",
			alternateClassName: "BPMSoft.MultiDeleteDetailModule",
			 ...
			 * @type {String}
			 */
			viewModelClassName: "BPMSoft.MultiDeleteDetailViewModel",

			/**
			 * The class name of the generator of view configuration.
			 * @type {String}
			 */
			viewConfigClassName: "BPMSoft.MultiDeleteDetailViewConfig",
			...
			getPreparedModuleConfig: function() {
				var moduleConfig = this.BPMSoft.deepClone(this.moduleConfig) || {};
			...
			/**
			 * Returns instance of the BPMSoft.ViewGenerator class.
			 * @return {BPMSoft.BaseGenerator} Instance of the BPMSoft.ViewGenerator.
			 * @private
			 */
			...
			getPrepareProfile: function(profile) {
				...
				var listedConfig = this.BPMSoft.decode(profile.listedConfig);
			...
			initializeEntitySchema: function(callback, scope) {
				...
				if (entitySchema) {
					...
					this.sandbox.requireModuleDescriptors([entitySchemaName], function() {
						this.BPMSoft.require([entitySchemaName], callback, scope);
					}, scope);
				}
			},
			...
			getColumnsFromListedConfig: function(profile) {
				var listedConfig = this.BPMSoft.decode(profile.listedConfig || profile.DataGrid.listedConfig);
				var listedConfigColumns = listedConfig.items;
				var columns = [];
				this.BPMSoft.each(listedConfigColumns, function(item) {
			...
			checkEntityForColumnsExists: function(columns) {
				var entityColumns = this.BPMSoft[this.entitySchemaName].columns;
		...
		return BPMSoft.MultiDeleteDetailModule;
	});
