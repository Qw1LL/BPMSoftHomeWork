define("GridSettingsViewModelGeneratorV2", ["ext-base", "BPMSoft", "GridSettingsViewModelGeneratorV2Resources",
		"ConfigurationConstants", "ColumnSettingsUtilities", "MaskHelper"],
	function(Ext, BPMSoft, resources, ConfigurationConstants, ColumnSettingsUtilities, MaskHelper) {
		/**
		 * ########## ############ ###### #############
		 * @param {Object} config
		 * @return {Object}
				...
				methods: {
					...
					saveSettings: function() {
						...
						if (!saveConfig || Ext.isEmpty(saveConfig.saveProfile) || saveConfig.saveProfile) {
							...
							BPMSoft.utils.saveUserProfile(profileKey, profile, false, function() {
					...
					saveSettingsForAllUsers: function() {
						...
						this._checkUserSettings({
							...
							BPMSoft.utils.saveUserProfile(profileKey, profile, true, function() {
								this.sandbox.publish("GridSettingsChanged", {
									newProfileData: profile
								}, [this.sandbox.id]);
					...
					getTiledProfileConfigV2: function() {
						...
						for (var i = 0; i < collection.length; i++) {
							...
							for (var j = 0; j < row.cells.length; j++) {
								...
								if (BPMSoft.contains(addedColumns, metaPath) && cell.aggregationType) {
					...
					getListedProfileConfigV2: function() {
						...
						if (collection.length > 0) {
							...
							for (var k = 0; k < firstRow.cells.length; k++) {
								...
								var metaPath = columnKeySplittedArray[0];
								var columnPath = metaPath;
								if (BPMSoft.contains(addedColumns, metaPath) && listedCell.aggregationType) {
					...
					generateTiledCollectionV2: function(config) {
						...
						for (var rowIndex = initialIndex; rowIndex < rowsCount + initialIndex; rowIndex++) {
							...
							for (var j = 0; j < columns.length; j++) {
								var cell = columns[j];
								var isTitleText = cell.type === BPMSoft.GridCellType.TITLE;
								var isURLType = cell.type === BPMSoft.GridCellType.LINK;
								var column = j;
								if (j > 0) {
						...
						var handler = function(args) {
							...
							for (var i = 0; i < cells.length; i++) {
								...
								if (column + width > currentColumnIndex) {
									...
									var isURLType = args.columnType === BPMSoft.GridCellType.LINK;
