define("ActivityDatesMixin", [],
	function() {
		/**
		 * @class BPMSoft.configuration.mixins.ActivityDatesMixin
		 * Activity dates mixin.
		 */
		Ext.define("BPMSoft.configuration.mixins.ActivityDatesMixin", {
			alternateClassName: "BPMSoft.ActivityDatesMixin",
			 ...
			 * @protected
			 * @return {BPMSoft.Collection} DueDate default values list.
			 */
			 ...
			 * @protected
			 * @param {BPMSoft.Collection} data DueDate default values collection.
			 * @return {BPMSoft.Collection} DueDate default values list copy.
			 */
			setDueDateDefaultList: function(data) {
				var defaultList = this.BPMSoft.deepClone(data.getItems());
			 ...
			 * @param {BPMSoft.Collection} dataList DueDate default values collection.
			 */
			loadDueDateList: function(filterText, dataList) {
				...
				var startDate = this.BPMSoft.deepClone(this.get(startDateColumnName));
				var dueDate = this.BPMSoft.deepClone(this.get(dueDateColumnName));
			 ...
			 * @param {BPMSoft.Collection} dataList DueDate current values collection.
			 * @protected
			 */
			...
			onRemindToAuthorChanged: function() {
				...
				if (remindToOwner) {
					let dueDate = this.BPMSoft.deepClone(this.get("DueDate"));
					if (!this.Ext.isDate(dueDate)) {
						return;
					}
					dueDate = this.BPMSoft.clearSeconds(dueDate);
			...
			onRemindToOwnerChanged: function() {
				...
				if (remindToOwner) {
					let startDate = this.BPMSoft.deepClone(this.get("StartDate"));
					if (!Ext.isDate(startDate)) {
						return;
					}
					startDate = this.BPMSoft.clearSeconds(startDate);
