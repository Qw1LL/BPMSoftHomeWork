	...
	return {
		...
		methods: {
			...
			_getPlaybookEsq: function() {
				...
				const currentCultureId = BPMSoft.SysValue.CURRENT_USER_CULTURE.value;
				esq.addColumn("=[Playbook:Case:Id].KnowledgeBase", "KnowledgeBase");
				esq.filters.addItem(this.BPMSoft.createExistsFilter("=[Playbook:Case:Id].KnowledgeBase"));
				esq.filters.addItem(this.BPMSoft.createColumnFilterWithParameter(
					this.BPMSoft.ComparisonType.EQUAL, "UId", caseUid));
				esq.filters.addItem(this.BPMSoft.createColumnFilterWithParameter(
					this.BPMSoft.ComparisonType.EQUAL, "=[Playbook:Case:Id].StageId", this.$ActiveActionId));
				esq.filters.addItem(this.BPMSoft.createColumnFilterWithParameter(
					this.BPMSoft.ComparisonType.EQUAL, "=[Playbook:Case:Id].[SysCulture:Language:Culture].Id", currentCultureId));
				return esq;
			},
