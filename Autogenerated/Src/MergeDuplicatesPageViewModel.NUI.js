define("MergeDuplicatesPageViewModel", ["ext-base", "BPMSoft", "sandbox", "MergeDuplicatesPageViewModelResources",
	"Account", "Contact", "LookupUtilities", "BaseFiltersGenerateModule"],
		function(Ext, BPMSoft, sandbox, resources, Account, Contact, LookupUtilities, BaseFiltersGenerateModule) {
			function generate(parentSandbox, entitySchemaName) {
				...
				BPMSoft.each(viewModelConfig.entitySchema.columns, function(column, name) {
			...
			function init() {
				this.ajaxProvider = BPMSoft.AjaxProvider;
			...
			function onAcceptClick() {
				...
				BPMSoft.each(entityColumns, function(columnName) {
					...
					if (value) {
						if (column.isLookup) {
							update.setParameterValue(columnName, value.value, BPMSoft.DataValueType.GUID);
						} else {
				...
				if (communicationDetailItem.InstanceId) {
					...
					if (detail && detail.collection && detail.collection.length > 0) {
						...
						BPMSoft.each(detail.collection, function(item) {
				...
				if (addressDetailItem.InstanceId) {
					...
					if (_detail && _detail.collection && _detail.collection.length > 0) {
						...
						BPMSoft.each(_detail.collection, function(item) {
							var toDelete = item.get("toDelete") || false;
							if (!toDelete) {
								var emptyGuid = BPMSoft.GUID_EMPTY;
			...
			function load(list) {
				...
				var filter = BPMSoft.createColumnInFilterWithParameters(entitySchema.primaryColumnName, list);
			...
			function loadDetails() {
				if (this.detailsInfo) {
					BPMSoft.each(this.detailsInfo, function(item) {
				...
				var communicationDetailLoad = function onCommunicationDetailLoad(responce) {
					...
					BPMSoft.each(responce.collection.collection.items, function(item) {
						item.set("Id", BPMSoft.generateGUID());
				...
				var commEsq = Ext.create("BPMSoft.EntitySchemaQuery", {
				...
				commEsq.filters.addItem(BPMSoft.createColumnInFilterWithParameters(entitySchemaName, this.list));

				commEsq.getEntityCollection(communicationDetailLoad, this);

				var addressEsq = Ext.create("BPMSoft.EntitySchemaQuery", {
				...
				addressEsq.filters.addItem(BPMSoft.createColumnInFilterWithParameters(entitySchemaName, this.list));
			...
			function indexObjectOf(collection, obj) {
				...
				var existIndex = -1;
				BPMSoft.each(collection, function(item, index) {
			...
			function renderValueVariants() {
				...
				BPMSoft.each(containerViewCollection, function(args) {
					var cloneConfig = BPMSoft.deepClone(args.config);
					var renderTo = Ext.get(args.renderToName);
					var containerView = Ext.create("BPMSoft.Container", cloneConfig);
