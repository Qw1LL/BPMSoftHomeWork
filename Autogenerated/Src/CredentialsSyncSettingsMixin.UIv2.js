define("CredentialsSyncSettingsMixin", ["ModalBox", "CredentialsSyncSettingsMixinResources"],
	function(ModalBox, resources) {
		...
		/**
		 * @class BPMSoft.configuration.mixins.CredentialsSyncSettingsMixin
		 * The mixin to work with synchronization credentials settings page.
		 */
		Ext.define("BPMSoft.configuration.mixins.CredentialsSyncSettingsMixin", {
			alternateClassName: "BPMSoft.CredentialsSyncSettingsMixin",
			...
			_getOAuthCookie: function() {
				...
				if (cookie) {
					return this.BPMSoft.decode(cookie);
				}
				return null;
			},
			...
			getOAuthErrorMessage: function(values) {
				...
				if (this.BPMSoft.includes(searchString, "forbid") ||
						this.BPMSoft.includes(searchString, "bad request")) {
					return this.getLocalizableStringValue("ForbiddenError");
				}
				if (this.BPMSoft.includes(searchString, "already exists")) {
			...
			onOAuthFlowError: function(values) {
				...
				this.log(values.exception, this.BPMSoft.LogMessageType.ERROR);
			},
		...
		return BPMSoft.CredentialsSyncSettingsMixin;
	}
);
