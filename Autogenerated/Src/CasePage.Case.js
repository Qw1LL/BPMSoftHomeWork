	...
	function(ModalBox, FormatUtils, NetworkUtilities, BusinessRuleModule, Enums, CasesEstimateLabel,
		...
		return {
			...
			mixins: {
				 ...
				 */
				CaseTermUtilities: "BPMSoft.CaseTermUtilities",
				/**
			...
			diff: /**SCHEMA_DIFF*/[
				...
				// Todo Sokolov services поле пустое
				// {
				// 	"operation": "insert",
				...
				{
					"operation": "insert",
					"name": "CaseCreatedOnValueContainer",
					"values" : {
						"itemType": this.BPMSoft.ViewItemType.CONTAINER,
						"classes": {
							"wrapClassName": ["caseCreatedOnValueContainer"]
						},
						"items": []
					},
				...
				{
					"operation": "insert",
					"name": "CaseCreatedOnValueCaption",
					"values": {
						"itemType": this.BPMSoft.ViewItemType.LABEL,
						"caption": {"bindTo": "getCreatedOnDateCaption"},
					...
					"parentName": "CaseCreatedOnValueContainer",
					"propertyName": "items"
				},
				{
					"operation": "insert",
				...
				},
				{
					...
					"values": {
						...
						"tip": {
							...
							"displayMode": this.BPMSoft.controls.TipEnums.displayMode.WIDE
						},
						"enabled": {"bindTo": "CanChangeCaseSatisfactionLevel"},
				...
				"SatisfactionLevel": {
					lookupListConfig: {
						...
						orders: [
							{
								columnPath: "Point",
								direction: this.BPMSoft.OrderDirection.DESC
							}
						]
					}
				},
					...
					lookupListConfig: {
						...
						filter: function() {
							...
							if (!status || status.IsFinal) {
								filterGroup.add("emptyFilter", this.BPMSoft.createColumnIsNullFilter("Id"));
								return filterGroup;
							}
							...
							filterGroup.add("StatusFilterCurrent", this.BPMSoft.createColumnFilterWithParameter(
								this.BPMSoft.ComparisonType.EQUAL,
			...
			methods: {
				...
				setSolutionsTime: function(response) {
					var time = response && response.GetContactTimezoneResult ?
							this.BPMSoft.utils.date.parseDate(response.GetContactTimezoneResult) :
							null;
				...
				autochangeOwner: function() {
					...
					if ((status.value === ServiceDeskConstants.CaseState.Reopened) &&
						...
						var contact = this.BPMSoft.SysValue.CURRENT_USER_CONTACT;
						if (!owner) {
				...
				handleResolvedStatus: function(status) {
					if (status.IsResolved) {
						this.getContactTimeAsync(this.BPMSoft.SysValue.CURRENT_USER_CONTACT.value,
						...
						this.getContactTimeAsync(this.BPMSoft.SysValue.CURRENT_USER_CONTACT.value,
							function(response) {
								if (response && response.GetContactTimezoneResult) {
									this.set("ClosureDate", this.BPMSoft.utils.date.parseDate(response.GetContactTimezoneResult));
								}
							});
				 ...
				 * @param {Object} status Case status
				 */
				handlePausedStatus: this.BPMSoft.emptyFn,
				...
				onStatusChanged: function() {
					...
					if (this.isStatusDef() === false && !this.get("RespondedOn")) {
						this.getContactTimeAsync(this.BPMSoft.SysValue.CURRENT_USER_CONTACT.value,
							function(response) {
								if (response && response.GetContactTimezoneResult) {
									this.set("RespondedOn", this.BPMSoft.utils.date.parseDate(response.GetContactTimezoneResult));
								}
							});
					}
				...
				needToSetActualStatus: function(status) {
					status = status || this.get("Status");
					return this.isNew || !this.BPMSoft.isEqual(this.get("PreviousStatus"), status);
				},
				...
				/**
				 * @inheritdoc BPMSoft.BasePageV2#onProcessCardSaved
				 * @overridden
				 */
				...
				onSaved: function() {
					...
					if (this.get("IsCloseOnSave") && !this.get("NextPrcElReady")) {
						...
						if (!this.isNewMode()) {
							...
							//TODO Remove all "if" block after implementation of the task.
				...
				setInitialValues: function() {
					this.BPMSoft.SysSettings.querySysSettingsItem("CaseClosureCodeDef", function(value) {
				...
				setPriorityImage: function() {
					...
					esq.addColumn("Image");
					esq.filters.add("defaultPriorityFilter", this.BPMSoft.createColumnFilterWithParameter(
						this.BPMSoft.ComparisonType.EQUAL, "Id", priority.value));
				...
				getCaseColumnNamesToCopy: function() {
					...
					this.BPMSoft.each(caseColumns, function(column) {
				...
				applyCaseHistoryColumns: function(newCaseColumnValues, historyDataColumnValues) {
					...
					for (var i = 0; i < newCaseColumnValues.length; i++) {
						...
						var isCopiedColumnNotExist = this.Ext.isEmpty(
								BPMSoft.findWhere(copiedCaseColumns, {name: newCaseColumnName}));
				...
				/**
				 * @inheritdoc BPMSoft.LookupQuickAddMixin#checkRightsCallback
				 * @overridden
				 */
				extractValuePairsFromFilters: function(entitySchema, columnName, lookupFilters) {
					if (columnName === "Contact") {
						lookupFilters = this.Ext.create("BPMSoft.Collection");
				...
				onPrepareOwner: function(filter) {
					...
					let filtersCollection = this.BPMSoft.createFilterGroup();
					if (this.isAssigneeGroupFilled()) {
						...
						filtersCollection.add("existsFilter",
							this.BPMSoft.createColumnIsNotNullFilter("[SysAdminUnit:Contact].Id"));
					}
				...
				isAssigneeGroupFilled: function() {
					const group = this.get("Group");
					return group && group.value && group.value !== this.BPMSoft.GUID_EMPTY;
				},
				 ...
				 * @virtual
				 */
				getOwnerDependenceFilters: this.BPMSoft.emptyFn,
				 ...
				 * @param {BPMSoft.BaseSchemaViewModel} scope Context of callback-function
				 */
				getUsersInGroups: function(callback, scope) {
					scope.getGroupListCollection(function(groupCollection) {
						...
						if (group && group.value !== scope.BPMSoft.GUID_EMPTY) {
							groupCollection = [group.value];
						}
						if (groupCollection.length > 0) {
							esq.filters.add("GroupCollectionFilter", scope.BPMSoft.createColumnInFilterWithParameters(
								"[SysUserInRole:SysUser].SysRole", groupCollection));
						} else {
							esq.filters.add("emptyFilter", scope.BPMSoft.createColumnIsNullFilter("Id"));
						}
						esq.getEntityCollection(function(result) {
				...
				onPrepareGroup: function(filter) {
					...
					this.getGroupEsq(function(esq, scope) {
						esq.getEntityCollection(function(result) {
							...
							var filtersCollection = scope.BPMSoft.createFilterGroup();
							if (existsCollection.length > 0) {
								filtersCollection.add("existsFilter", scope.BPMSoft.createColumnInFilterWithParameters(
									"Id", existsCollection));
							} else {
								filtersCollection.add("emptyFilter", scope.BPMSoft.createColumnIsNullFilter("Id"));
							}
				 ...
				 * @private
				 * @param {BPMSoft.EntitySchemaQuery} esq entitySchemaQuery
				 */
				addGroupEsqFilters: function(esq) {
					esq.filters.add("ContactNotExistsFilter", this.BPMSoft.createColumnIsNullFilter("Contact"));
				},
				...
				/**
				 * Forms the case date of creation caption.
				 * @protected
				 * @virtual
				 * @return {String} Case date of creation caption.
				 */
				getCreatedOnDateCaption: function() {
					var createdOn = this.get("CreatedOn");
					var caption = this.get("Resources.Strings.CreatedOnCaption") + " ";
					return createdOn ? caption : "";
				},
				...
				getCreatedOnDate: function() {
					var createdOn = this.get("CreatedOn");
					return createdOn ? " " + FormatUtils.smartFormatDate(createdOn): "";
				},
				...
				initSysSettings: function() {
					this.BPMSoft.SysSettings.querySysSettingsItem("ClearAssigneeOnCaseReopening",
						function(value) {
				...
				initCopyContext: function() {
					...
					if (defValues && defValues.mode === "CreateFromHistory") {
						this.set("Operation", BPMSoft.ConfigurationEnums.CardOperation.ADD);
