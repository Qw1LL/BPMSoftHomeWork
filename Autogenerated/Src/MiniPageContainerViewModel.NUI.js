define("MiniPageContainerViewModel", ["ConfigurationEnumsV2"], function() {
	Ext.define("BPMSoft.configuration.MiniPageContainerViewModel", {
		extend: "BPMSoft.BaseModel",
		alternateClassName: "BPMSoft.MiniPageContainerViewModel",
		...
		/**
		 * Allow event propagation on MiniPage container click
		 * @type {Boolean}
		 */
		...
		getSchemaName: function(entityName, typeId) {
			var entitySchemaConfig = BPMSoft.ModuleUtils.getModuleStructureByName(entityName);
		...
		prepareContainerParameters: function(config) {
			...
			this.miniPageMode = config.operation || BPMSoft.ConfigurationEnums.CardOperation.VIEW;
		...
		_setBodyMiniPageCls: function(isOpened) {
			...
			var operation = this.get("parameters").operation;
			(isOpened && operation) ? htmlEl.classList.add("ts-minipage-opened") : htmlEl.classList.remove("ts-minipage-opened");
		},
		...
		subscribeOnMessages: function(moduleId) {
			...
			this.sandbox.subscribe("HistoryStateChanged", function() {
				const isViewMode = this._isMiniPageMode(BPMSoft.ConfigurationEnums.CardOperation.VIEW);
				if (isViewMode) {
					this.closeMiniPage();
				}
			}, this);
		...
		/**
		 * @private
		 * @param {BPMSoft.ConfigurationEnums.CardOperation} mode
		 * @return {Boolean}
		 */
		...
		processMiniPageModeChanging: function(mode) {
			...
			switch (mode) {
				...
				case BPMSoft.ConfigurationEnums.CardOperation.ADD:
					this.isFixed = true;
					BPMSoft.MiniPageListener.unSubscribeFixedGlobalEvents(this);
					break;
				default:
					break;
			}
		...
		unRegisterMessages: function() {
			if (this.messages) {
				var messages = BPMSoft.keys(this.messages);
		...
		mouseDown: function(event) {
			if (this.isFixed && !this.isClickedOnMiniPageContainer(event) && !this.isClickedOnAlignElement(event)) {
				...
				if (this.canHide(event)) {
					this.hide();
					BPMSoft.MiniPageListener.unSubscribeGlobalEvents(this);
				}
			}
		},
		...
		canHide: function(event) {
			...
			this.needLoadModule = false;
			if (isMenuEl) {
				BPMSoft.MiniPageListener.subscribeMenuContainerEvents(this.isFixed);
			}
		...
		/**
		 * Event handler for MiniPageContainer click event
		 * Used in:
		 * @class {MiniPageListener.subscribeContainerEvents, MiniPageListener.unsubscribeContainerEvents}
		 * @param {Event} event Event object.
		 */
		onContainerClick: function (event) {
			if (this.allowClickPropagation === false) event.stopPropagation();
		},
		...
		clearTimer: function(timers) {
			...
			BPMSoft.each(timers, function(timerId) {
				if (timerId) {
