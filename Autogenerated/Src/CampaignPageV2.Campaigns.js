define("CampaignPageV2", ["SimpleDiagram", "CampaignPageV2Resources", "CampaignDiagramPropertyEnums",
		...
		function(SimpleDiagram, resources, PropertyEnums, CampaignLocalizableHelper, GeneralDetails,
			...
			return {
				...
				mixins: {
					Analytics: "BPMSoft.CampaignAnalytics",
					...
					},
{
				...
				methods: {
					 ...
					 */
					campaignDiagramValidator: Ext.create("BPMSoft.CampaignDiagramValidator"),
					...
					_applyDiagramConnections: function(diagram) {
						...
						BPMSoft.each(diagram.connectors, function(connector) {
					...
					initializeDiagramItems: function() {
						...
						var items = this.get("DiagramItems");
						items.clear();
						this.BPMSoft.each(flowElements.nodes, this.addNodeDiagramItem, this);
						this.BPMSoft.each(flowElements.connectors, this.addEdgeDiagramItem, this);
					...
					onCampaignDiagramChanged: function() {
						...
						BPMSoft.each(diagram.nodes || [], this.updateDiagramItem, this);
						BPMSoft.each(diagram.connectors || [], this.updateDiagramItem, this);
					},
					...
					onCampaignDiagramConnectionChange: function(event) {
						var element = BPMSoft.deepClone(event.element);
						if (element) {
					 ...
					 * @param {Object} config Configuration object.
					 * @returns {BPMSoft.CampaignDiagramItem}
					 */
					createDiagramItem: function(config, additionalConfig) {
						...
						if (Ext.isString(config.portsSet)) {
							config.portsSet = BPMSoft.diagram.PortsSet[config.portsSet];
						}
					...
					getUpdatedFlowElementsConfig: function(flowElements) {
						...
						BPMSoft.each(flowElements.nodes, function(item) {
					...
					copySchemaData: function(schemaData) {
						var modifiedSchemaData = BPMSoft.deepClone(schemaData);
						BPMSoft.each(modifiedSchemaData.nodes, function(item) {
							var oldNodeName = item.name;
							var newNodeName = BPMSoft.generateGUID();
							item.name = newNodeName;
							if (item.hasOwnProperty("addInfo") && item.addInfo.hasOwnProperty("RecordId")) {
								delete item.addInfo.RecordId;
							}
							BPMSoft.each(modifiedSchemaData.connectors, function(item) {
								item.name = BPMSoft.generateGUID();
					 ...
					 * ignoring element with type BPMSoft.CampaignEnums#StepType.CREATE_LEAD.
