/**
 * @class BPMSoft.configuration.FeedCommentsList
 */
Ext.define("BPMSoft.configuration.FeedCommentsList", {
	extend: "BPMSoft.configuration.FeedList",
	...
	getApplyItemTplLikesInfoFn: function() {
		return function(values) {
			...
			if (isLiked) {
				likeCount--;
				if (likeCount === 0) {
					text = BPMSoft.LocalizableStrings.MobileFeedCommentsListLikeYou;
				} else {
					text = Ext.String.format(BPMSoft.LocalizableStrings.MobileFeedCommentsListLikeYouAndOthers,
						likeCount);
				}
			} else if (likeCount === 1) {
				text = Ext.String.format(BPMSoft.LocalizableStrings.MobileFeedCommentsListLikeOther, likeCount);
			} else {
				text = Ext.String.format(BPMSoft.LocalizableStrings.MobileFeedCommentsListLikeOthers, likeCount);
			}
