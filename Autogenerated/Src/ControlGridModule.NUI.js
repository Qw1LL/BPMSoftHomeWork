define("ControlGridModule", ["BPMSoft", "ext-base"], function(BPMSoft, Ext) {
	/**
	 * @class BPMSoft.configuration.ControlGrid
	 * ##### ######### ###### ########### ######## # #######.
	 */
	Ext.define("BPMSoft.configuration.ControlGrid", {

		extend: "BPMSoft.Grid",

		alternateClassName: "BPMSoft.ControlGrid",
		...
		renderCell: function(result, options) {
			...
			BPMSoft.each(cell.key, function(column) {
		...
		formatControlCellContent: function(cell, data, column) {
			...
			if (type === "caption") {
				...
				cell.html += '<span class="grid-label">' + BPMSoft.utils.string.encodeHtml(name) + '</span>';
				...
				var isControlExist = BPMSoft.some(this.controlsCollection, function(item) {
		...
		/**
		 * @inheritDoc BPMSoft.Grid#getBindConfig
		 * @protected
		 * @override
		 */
		...
		renderControlsCollection: function() {
			BPMSoft.each(this.controlsCollection, function(item) {
				var control = {};
		...
		updateControlItem: function(item) {
			...
			BPMSoft.each(this.controlsCollection, function(collectionItem) {
		...
		onDeleteItem: function(item) {
			...
			var controlDeleting = BPMSoft.where(this.controlsCollection, whereCondition);
			if (controlDeleting.length > 0) {
				this.controlsCollection = BPMSoft.without(this.controlsCollection, controlDeleting[0]);
			}
		},
