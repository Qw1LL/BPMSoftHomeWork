	...
	CommentsWithFilesModulesHelper) {
		structure.userCode = function() {
				...
				if (tabName === 'favorites') {
					select.filters.add('FavoritesFilter',
						select.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL,
							'[Favorites:KnowledgeBase].Contact', BPMSoft.SysValue.CURRENT_USER_CONTACT.value));
				}
			};
			...
			this.methods.modifyGridConfig = function(gridConfig) {
				...
				BPMSoft.each(gridConfig.activeRowActions, function(action) {
			...
			this.methods.loadGridCollectionItems = function(items) {
				...
				var tempCollection = new BPMSoft.Collection();
			...
			this.methods.getExternalViewRowAction = function(id) {
				...
				var model;
				BPMSoft.each(collectionModels, function(item) {
					if (item.id === id) {
						model = item.model;
					}
				}, this);
			...
			this.methods.modifyItems = function(responseItems) {
				...
				BPMSoft.each(responseItems.collection.items, function(item) {
					item.scope = this;
				}, this);
			};
		};
		...
		function getTagsValue(response) {
			...
			if (response.rowsAffected > 0) {
				var tags = [];
				BPMSoft.each(response.rows, function(item) {
		...
		function generateView(id) {
			...
			return Ext.create('BPMSoft.Container', elementView);
		}
					...
					setLikeIt: function() {
						...
						if (!likeSet) {
							...
							deleteQuery.execute(function() {
								...
								insert.setParameterValue('KnowledgeBase', recordId, BPMSoft.DataValueType.GUID);
							...
							this.set('likeSet', false);
							this.set('LikeItIconConfig', resources.localizableImages.NoLikeItIcon);
							var likeDeleteQuery = Ext.create('BPMSoft.DeleteQuery', {
								rootSchemaName: 'Like'
							});
							likeDeleteQuery.filters.add('IdFilter',
								likeDeleteQuery.createColumnFilterWithParameter(BPMSoft.ComparisonType.EQUAL,
									'KnowledgeBase', recordId));
