define("MessageHistoryUtilities", ["SchemaBuilderV2"],
	function() {
		/**
		 * @class BPMSoft.configuration.MessageHistoryUtilities
		 * Utilities for message history.
		 */
		Ext.define("BPMSoft.configuration.mixins.MessageHistoryUtilities", {
			alternateClassName: "BPMSoft.MessageHistoryUtilities",
			...
			/**
			 * {BPMSoft.SchemaBuilder} instance to generate the presentation and view model of pages.
			 * @private
			...
			initNotifiers: function(sectionId, callback, scope) {
				...
				select.getEntityCollection(function(result) {
					if (result.success) {
						this.schemaBuilder = this.schemaBuilder || this.Ext.create("BPMSoft.SchemaBuilder");
						...
						this.BPMSoft.chain.apply(this, chainArguments);
					}
				}, this);
			},
			 ...
			 * @protected
			 * @param {BPMSoft.EntitySchemaQuery} esq EntitySchemaQuery object for CaseMesageHistory entity.
			...
			loadHistoryMessages: function(config, callback, scope) {
				...
				if (BPMSoft.Features.getIsEnabled("UseMessageHistoryPagination") && !messageId) {
					esq.isPageable = true;
				...
				if (messageId) {
					if (BPMSoft.Features.getIsEnabled("CanUpdateHistoryMessage")) {
				...
				} else if (!messageId && !BPMSoft.Features.getIsEnabled("UseMessageHistoryPagination")) {
					var sortDirection = config.sortDirection || this.BPMSoft.OrderDirection.DESC;
					...
					if (sortColumnLastValue) {
						filters.addItem(this.BPMSoft.createColumnFilterWithParameter(
							sortDirection === this.BPMSoft.OrderDirection.DESC
								? this.BPMSoft.ComparisonType.LESS
			 ...
			 * @protected
			 * @param {BPMSoft.EntitySchemaQuery} esq Message history query.
			 */
			addEsqParametersFromNotifiers: function(esq) {
				for (var notifier in this.Notifiers) {
					if (this.Notifiers.hasOwnProperty(notifier)) {
						...
						if (viewModel.historyMessageEsqApply !== this.BPMSoft.emptyFn) {
							viewModel.historyMessageEsqApply(esq);
						}
						if (viewModel.addAdditionalFilters !== this.BPMSoft.emptyFn) {
			 ...
			 * @return {BPMSoft.EntitySchemaQuery} Message history query.
			 */
			getHistoryMessageEsq: function(config) {
				...
				var esq = this.Ext.create("BPMSoft.EntitySchemaQuery", options);
				...
				var columnNames = this.BPMSoft.deepClone(messageHistoryColumnNames);
