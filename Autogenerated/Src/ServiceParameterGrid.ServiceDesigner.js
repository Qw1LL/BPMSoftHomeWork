define("ServiceParameterGrid", ["ServiceParameterGridResources", "css!ServiceDesignerStyles"],
	...
	return {
		...
		methods: {
			 ...
			 * @param {BPMSoft.ServiceParameterViewModel} value Element from GridData correspond to ActiveRow.
			 * @private
			 */
			...
			_generateParameterUniqName: function(propertyName, prefix) {
				return BPMSoft.ServiceDesignerUtilities.generateSequencesName(propertyName, prefix,
			 ...
			 * @param {BPMSoft.ObjectCollection} collection Collection in which find element.
			 * @private
			 */
			_findParameterByUId: function(targetUId, collection) {
				...
				if (targetParemeter) {
					if (!targetParemeter.itemProperties) {
						targetParemeter.itemProperties = this.Ext.create("BPMSoft.ObjectCollection");
					}
				} else {
			...
			_removeSelectedRows: function() {
				...
				BPMSoft.each(this.$SelectedRows, function(selectedRow) {
			...
			_getDefaultParameterConfig: function() {
				const namePrefix = BPMSoft.ServiceSchemaManager.schemaNamePrefix + "Parameter";
				...
				return {
					...
					defValue: {
						value: null,
						source: BPMSoft.services.enums.ServiceParameterValueSource.CONST
					},
					dataValueTypeName: BPMSoft.services.enums.DataValueTypeName.Text,
			...
			initParameters: function(callback, scope) {
				BPMSoft.chain(
					...
					function(next) {
						BPMSoft.ServiceSchemaManager.getInstanceByUId(this.$ServiceSchemaUId, next, this);
					},
			...
			constructor: function() {
				this.callParent(arguments);
				this._onActiveRowChangeThrottled = BPMSoft.throttle(this._onActiveRowChange, 300);
			...
			onCopyButtonClick: function() {
				if (this.$CanEditSchema) {
					...
					if (this.$ActiveRow) {
						...
						this._refreshGrid();
						this._updateDuplicatedCode();
						const newArray = BPMSoft.deepClone(this.$ExpandHierarchyLevels);
			...
			/**
			 * Generates default instance of BPMSoft.ServiceParameter.
			 * @type {Object} config Additional values of properties to set in new instance.
			 * @returns {BPMSoft.ServiceParameter}
			 */
			createParameter: function(config) {
				...
				return Ext.create("BPMSoft.ServiceParameter", constructorConfig);
			},

			/**
			 * Returns is selected parameter can has nested items.
			 * @param {BPMSoft.ServiceParameterViewModel} parameter Parameter to detects ability to has nested items.
			 * @public
			 * @returns {Boolean}
			 */
			canAddNestedItem: function(parameter) {
				var allowedType = BPMSoft.services.enums.DataValueTypeName.Object;
