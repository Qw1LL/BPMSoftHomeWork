define("ExtendedFilterEditModuleV2", ["ext-base", "BPMSoft", "sandbox", "ExtendedFilterEditModuleV2Resources",
	"ExtendedFilterEditModelV2", "ExtendedFilterEditViewV2"],
	function(Ext, BPMSoft, sandbox, resources, ExtendedFilterEditModel, ExtendedFilterEditView) {
		...
		function _setFiltersExpressions(filter, sectionFilterValue) {
			if (sectionFilterValue && BPMSoft.isGUID(sectionFilterValue.value)) {
				filterManager.setRightExpressionsValues(filter, [{
					value: BPMSoft.getRightExpressionValue(filter),
		...
		function initializeProvider(renderTo) {
			...
			require([this.entitySchemaFilterProviderModule], function(EntitySchemaFilterProviderModule) {
						...
						filterManager.setFilters(filter || Ext.create("BPMSoft.FilterGroup"));
		...
		function createFilterEditContainer(renderTo) {
			...
			var filterEditContainer = Ext.create("BPMSoft.Container", view);
		...
		function render(renderTo) {
			...
			require([extendedFilterViewId, extendedFilterViewModelId],
				function(filterView, filterViewModel) {
					...
					viewModel = Ext.create("BPMSoft.BaseViewModel", viewModel);
