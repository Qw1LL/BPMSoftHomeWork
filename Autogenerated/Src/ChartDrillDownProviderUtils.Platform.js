define("ChartDrillDownProviderUtils", [], function() {
	Ext.define("BPMSoft.configuration.ChartDrillDownProviderUtils", {
		alternateClassName: "BPMSoft.ChartDrillDownProviderUtils",
		...
		getDatePartsFromDate: function(timeStamp, dateTimeFormat) {
			var date = new Date(timeStamp);
			var result = {};
			if (BPMSoft.contains(dateTimeFormat, "Year")) {
				result.Year = date.getFullYear();
			}
			if (BPMSoft.contains(dateTimeFormat, "Month")) {
				result.Month = date.getMonth() + 1;
			}
			if (BPMSoft.contains(dateTimeFormat, "Week")) {
				result.Week = Ext.Date.getWeekOfYear(date);
			}
			if (BPMSoft.contains(dateTimeFormat, "Day")) {
				result.Day = date.getDate();
			}
			if (BPMSoft.contains(dateTimeFormat, "Hour")) {
