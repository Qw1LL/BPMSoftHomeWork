		...
		function createConstructor(context) {
			...
			function adjustFolderFilterViewModel(folderFilterViewModel) {
				...
				folderFilterViewModel.folderSelecting = function(currentFilter) {
					...
					var selectedValues = [];
					if (!currentFilter) {
						BPMSoft.each(selectedFiltersState, function(currentValue) {
			...
			function adjustFixedFilterViewModel(fixedFilterViewModel) {
				fixedFilterViewModel.showSimpleFilterEdit = function(filterName) {
					...
					var addConditionViewConfig = QuickFilterView.generateSimpleEditFilterConfig(filterName,
						filtersViewItemsNames.simpleFilterEditContainerName);
					var addConditionView = Ext.create("BPMSoft.Container", addConditionViewConfig);
			...
			function render(renderTo) {
				...
				if (fixedFilterConfig && fixedFilterConfig.filters) {
					BPMSoft.each(fixedFilterConfig.filters, function(filter) {
				...
				sandbox.requireModuleDescriptors(referenceSchemas, function() {
					...
					require(requiredModules,
						function(folderEntitySchema, inFolderEntitySchema) {
							...
							var view = Ext.create("BPMSoft.Container", viewConfig.quickFilterViewConfig);
							...
							BPMSoft.each(filterModulesMap, function(item) {
								...
								var filterViewModel = item.viewModel = Ext.create('BPMSoft.BaseViewModel',
								...
								var filterView = Ext.create("BPMSoft.Container", preparedFilterViewConfig);
								filterView.bind(filterViewModel);
							...
							if (viewModelConfig.fixedFilterViewModelConfig && viewConfig.fixedFilterViewConfig) {
								...
								viewModelConfig.fixedFilterViewModelConfig.methods.getAddedFixedFilter =
									getAddedFixedFilter;
								var fixedFilterViewModel = Ext.create('BPMSoft.BaseViewModel',
								...
								var fixedFilterView = Ext.create("BPMSoft.Container",
							...
							if (viewModelConfig.folderFilterViewModelConfig && viewConfig.folderFilterViewConfig) {
								...
								var folderFilterViewModel = Ext.create('BPMSoft.BaseViewModel',
								...
								var folderFilterView = Ext.create("BPMSoft.Container",
							...
							if (viewModelConfig.customFilterViewModelConfig && viewConfig.customFilterViewConfig) {
								...
								var customFilterViewModel = Ext.create('BPMSoft.BaseViewModel',
								...
								var customFilterView = Ext.create("BPMSoft.Container",
							...
							if (config && config.fixedFilterConfig) {
								...
								BPMSoft.each(filters, function(filtersItem) {
			...
			function saveFilterState() {
				...
				BPMSoft.each(filterModulesMap, function(item, index) {
				...
				if (filterState.folderFilterState) {
					BPMSoft.utils.saveUserProfile(filterFolderProfileKey, filterState.folderFilterState, false);
				}
					...
					switch (filterType) {
						...
						case 'Folder':
							if (!filterObj.ignoreFolderFilters) {
								...
								BPMSoft.utils.saveUserProfile(filterFolderProfileKey, {}, false);
							}
							break;
						case 'Custom':
			...
			function getReferenceSchemas(entitySchema) {
				...
				BPMSoft.each(entitySchema.columns, function(column) {
					if ((column.dataValueType === BPMSoft.DataValueType.LOOKUP ||
						column.dataValueType === BPMSoft.DataValueType.ENUM) &&
