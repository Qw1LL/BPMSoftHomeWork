define("QuickFilterModuleV2", ["LabelDateEdit", "performancecountermanager", "ConfigurationConstants",
	...
	function(LabelDateEdit, performanceManager, ConfigurationConstants) {
		/**
		 * @class BPMSoft.configuration.QuickFilterModule
		 * Quick filter module class.
		 */
		Ext.define("BPMSoft.configuration.QuickFilterModule", {
			alternateClassName: "BPMSoft.QuickFilterModule",
			 ...
			 * @type {String}
			 */
			viewModelClassName: "BPMSoft.FiltersContainerViewModel",
			 ...
			 */
			stringColumnSearchComparisonType: BPMSoft.ComparisonType.START_WITH,
			...
			initSysSettingsValues: function(callback, scope) {
				this.BPMSoft.SysSettings.querySysSettings(["StringColumnSearchComparisonType"],
			...
			initFiltersModules: function(callback, scope) {
				var modulesNames = [];
				BPMSoft.each(this.moduleConfig, function(typeConfig) {
					modulesNames.push(typeConfig.viewConfigModuleName, typeConfig.viewModelConfigModuleName);
				}, this);
				BPMSoft.require(modulesNames, callback, scope);
			},
			...
			generateViewConfig: function(config) {
				var viewConfig = {};
				BPMSoft.each(this.moduleConfig, function(typeConfig, typeName) {
			...
			requireFoldersSchemas: function(config) {
				this.sandbox.requireModuleDescriptors(config.descriptorNames, function() {
					this.BPMSoft.require(config.schemaNames,
			...
			subscribeSandboxEvents: function() {
				...
				this.internalSubscribe("UpdateExtendedFilterValues", function(values) {
					...
					var customFilterViewModel = this.viewModel.getFilterViewModel("CustomFilters");
					this.BPMSoft.each(values, function(value, property) {
			...
			initFiltersViewItemsNames: function() {
				...
				BPMSoft.each(filtersViewItemsNames, function(parameterValue, parameterName) {
			...
			_isEmptySectionFilters: function() {
				...
				var emptyFilters = sectionFilters.filterByFn(function(item) {
					...
					return isObjectOrArray && BPMSoft.isEmptyObject(item);
				});
			...
			render: function(renderTo) {
				...
				var quickFilterViewConfig = BPMSoft.deepClone(this.viewConfig);
				var view = this.view = this.Ext.create("BPMSoft.Container", quickFilterViewConfig);
				...
				filtersViewModels.eachKey(function(filterName, filterViewModel) {
					...
					if (filterViewConfig) {
						var filterView = this.Ext.create("BPMSoft.Container",
							BPMSoft.deepClone(filterViewConfig));
		...
		return BPMSoft.QuickFilterModule;
	});
