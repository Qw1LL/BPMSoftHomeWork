define('CardProcessViewGenerator', ['ext-base', 'BPMSoft', 'BusinessRuleModule', 'CardProcessViewGeneratorResources',
	'ActionUtilitiesModule', 'ConfigurationEnums', 'GridUtilities'],
	function(Ext, BPMSoft, BusinessRuleModule, resources, actionUtilities, ConfigurationEnums, GridUtilities) {
	...
	function isEditCheckBox(action, dataValueType) {
		return dataValueType === BPMSoft.DataValueType.BOOLEAN && isEdit(action);
	}
	function isEditImage(action, dataValueType) {
		return dataValueType === BPMSoft.DataValueType.IMAGE && isEdit(action);
	}
	...
	function generateLabelBindings(action, schemaItem, schemaItemBindings) {
		...
		if (action === ConfigurationEnums.CardState.View) {
			if (schemaItem.type === BPMSoft.ViewModelSchemaItem.ATTRIBUTE) {
				...
				if (action === "view" && schemaItem.columnType === BPMSoft.ViewModelColumnType.VIRTUAL_COLUMN) {
	...
	function getValueLabelConfig(schemaItem, bindings, labelType, action) {
		...
		var converter = BPMSoft.getTypedStringValue;
		if (action === "view" && schemaItem.columnType === BPMSoft.ViewModelColumnType.VIRTUAL_COLUMN) {
			converter = function(item) {
				return BPMSoft.getTypedStringValue(item, converterTypeForBinding(schemaItem.dataValueType));
			};
		}
		...
		if (entitySchema && entitySchema.name && BPMSoft[entitySchema.name]) {
			var columns = BPMSoft[entitySchema.name].columns;
	...
	function getControlConfig(schemaItem, bindings, action) {
		...
		var customConfig = BPMSoft.utils.common.deepClone(schemaItem.customConfig);
		if (action === ConfigurationEnums.CardState.View) {
		...
		//customConfig = customConfig && BPMSoft.utils.common.deepClone(schemaItem.customConfig);
	...
	function updateItemsForEditStructure(items) {
		BPMSoft.each(items, function(item) {
			if (item.className === 'BPMSoft.ControlGroup') {
				item.collapsed = false;
			}
			if (item.className === 'BPMSoft.controls.HtmlEdit') {
				item.className = 'BPMSoft.TextEdit';
				if (item.margin) {
					delete item.margin;
				}
			}
	...
	function generateView(config) {
		...
		BPMSoft.each(config.columnsObject, function(schemaItem) {
			...
			if (action === ConfigurationEnums.CardState.EditStructure) {
				...
				var containerId = schemaItem.EditStructureContainerId;//|| BPMSoft.generateGUID();
				var focusConfig = {
					tag: {
	...
	function getMoveToConfig(info, action, buttonsConfig) {
		var buttonConfig = { className: 'BPMSoft.Button' };
