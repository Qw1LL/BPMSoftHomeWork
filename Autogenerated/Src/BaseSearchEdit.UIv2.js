define("BaseSearchEdit", ["BaseSearchEditResources"], function(resources) {
	...
	/**
	 * @class BPMSoft.controls.BaseSearchEdit
	 * Single-line search control class based on text edit control.
	 */
	Ext.define("BPMSoft.controls.BaseSearchEdit", {
		extend: "BPMSoft.TextEdit",
		alternateClassName: "BPMSoft.BaseSearchEdit",
		...
		rightIconConfig: {
			...
			url: BPMSoft.ImageUrlBuilder.getUrl(resources.localizableImages.LookupImage)
		},
		 ...
		 * @return {Boolean}
		 */
		beforeFiringEvents: BPMSoft.emptyFn,
		...
		/**
		 * Key up handler. Generates searchValueDeleted event {@link BPMSoft.BaseSearchEdit#searchValueDeleted}.
		...
		combineRightIconStyles: function() {
			...
			if (!Ext.isEmpty(this.rightIconConfig)) {
				var rightIconUrl = BPMSoft.ImageUrlBuilder.getUrl(this.rightIconConfig);
