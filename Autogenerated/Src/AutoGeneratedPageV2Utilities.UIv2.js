define("AutoGeneratedPageV2Utilities", ["BPMSoft", "ext-base", "AutoGeneratedPageV2UtilitiesResources"],
	function(BPMSoft, Ext) {
		...
		function generateButtonCustomDiff(control, columnIdx) {
			...
			return {
				...
				"values": {
					...
					"style": color === "green" ? "orange" : color,
					"layout": {
						"column": columnIdx,
						"row": 0,
						"colSpan": 2
		...
		function generateControlAttribute(control) {
			...
			if (control.dataValueType === BPMSoft.DataValueType.FLOAT) {
		...
		function generateLookupControlValues(control) {
			customValues[getLookupListName(control.name)] = new BPMSoft.Collection();
		}
		...
		function generateTextEditCustomDiff(control) {
			...
			if (control.isMultiLine === true) {
				diff.values.contentType = BPMSoft.ContentType.LONG_TEXT;
			}
			return diff;
		}
		...
		function generateLookupCustomDiff(config) {
			...
			if (config.dataValueType === BPMSoft.DataValueType.ENUM) {
				lookupCustomDiff.values.contentType = "BPMSoft.ContentType.ENUM";
			} else {
		...
		return {
			...
			getCustomDiff: function(executionData) {
				...
				let rowIdx = 0;
				BPMSoft.each(leftContainer, function(element) {
					switch (element.type) {
