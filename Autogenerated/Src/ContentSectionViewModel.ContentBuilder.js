define("ContentSectionViewModel", ["ContentSectionViewModelResources", "ContentBuilderConstants",
	 ...
	 */
	Ext.define("BPMSoft.controls.ContentSectionViewModel", {
		extend: "BPMSoft.controls.BaseContentBlockViewModel",
		alternateClassName: "BPMSoft.ContentSectionViewModel",
		 ...
		 * @type {String}
		 */
		className: "BPMSoft.ContentSection",
		...
		childItemTypes: {
			...
			mjgroup: "BPMSoft.ContentMjGroupViewModel"
		},
		...
		getItemEditConfig: function(item) {
			...
			if (item.$ItemType === BPMSoft.ContentBuilderBodyItemType.mjgroup.value) {
				itemConfig.Items = [];
				BPMSoft.each(item.$Items, function(subItem) {
		...
		updateStructureItems: function(itemsToUpdate) {
			BPMSoft.each(itemsToUpdate, function(item) {
		 ...
		 * Changes items order.
		 * @protected
		 * @param {BPMSoft.BaseViewModelCollection} items Items to reverse order.
		 */
		...
		reverseItemsOrder: function(styles) {
			...
			var itemStyles = BPMSoft.deepClone(this.$Styles);
		 ...
		 * @param {BPMSoft.ContentSectionViewModel} selectedItem Selected section.
		 * @param {String} columnId Unique identifier of column.
		 * @returns {BPMSoft.ContentMjGroupViewModel} Group view model.
		 */
		 ...
		 * @protected
		 * @param {BPMSoft.ContentMjGroupViewModel} group Group to add item.
		 * @param {BPMSoft.ContentColumnViewModel} column Column to add.
		 ...
		 * @returns {BPMSoft.ContentMjGroupViewModel} New group.
		 */
		 ...
		 * @protected
		 * @param {BPMSoft.ContentMjGroupViewModel} group Group to split.
		 * @param {BPMSoft.ContentColumnViewModel} rightColumn Column which provides split action.
		 ...
		 * @protected
		 * @param {BPMSoft.ContentMjGroupViewModel} group Column's parent group.
		 * @param {BPMSoft.ContentColumnViewModel} rightColumn Column to replace.
		 * @param {Number} index Collection index to insert action.
		 */
		 ...
		 * @protected
		 * @param {BPMSoft.ContentMjGroupViewModel} group Group to remove.
		...
		replaceGroupWithColumns: function(group, index, isReverse) {
			BPMSoft.each(group.$Items, function(column) {
				var id = column.$Id;
				var itemConfig = column.serializeViewModel();
				delete itemConfig.WrapperStyles.width;
		 ...
		 * @protected
		 * @param {BPMSoft.ContentMjGroupViewModel | BPMSoft.ContentColumnViewModel} elementToGroup
		 * Left element to group.
		 * @param {BPMSoft.ContentMjGroupViewModel | BPMSoft.ContentColumnViewModel} nextElementToGroup
		...
		applyGroupingAction: function(elementToGroup, nextElementToGroup, index, isReverse) {
			...
			switch (groupingCode) {
				case BPMSoft.ContentBuilderGroupingCode.COLUMN_COLUMN:
					...
					break;
				case BPMSoft.ContentBuilderGroupingCode.COLUMN_GROUP:
					this.addColumnToGroup(nextElementToGroup, elementToGroup, !isReverse);
					break;
				case BPMSoft.ContentBuilderGroupingCode.GROUP_COLUMN:
					this.addColumnToGroup(elementToGroup, nextElementToGroup, isReverse);
					break;
				case BPMSoft.ContentBuilderGroupingCode.GROUP_GROUP:
		 ...
		 * @protected
		 * @param {BPMSoft.ContentMjGroupViewModel} group Current group.
		 * @param {BPMSoft.ContentColumnViewModel} leftColumn Left column to ungroup.
		 * @param {BPMSoft.ContentColumnViewModel} rightColumn Right column to ungroup.
		 * @param {Number} index Collection index for insert action.
		 * @param {Boolean} isReverse Flag to indicate reverse items order for parent section viewModel.
		 */
	...
	return BPMSoft.ContentSectionViewModel;
});
