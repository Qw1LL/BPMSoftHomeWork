		...
		function(resources, modalbox, ConfigurationEnums, MaskHelper) {
			...
			window.initializeGoogleMaps = function() {
				...
				if (!mapsConfig.isAddressesExists()) {
					...
					BPMSoft.utils.showInformation(resources.localizableStrings.DataWithoutAddresses, null, null,
							{buttons: ["ok"]});
				} else {
					BPMSoft.each(mapsConfig.mapsData, function(item) {
			...
			function geocodingFinished(mapsConfig) {
				...
				if (validation.status) {
					...
					BPMSoft.utils.showInformation(validation.message, null, null,
							{buttons: ["ok"]});
				}
			}
			...
			function init() {
				if (!isApiLoad) {
					var apiKey = BPMSoft.SysSettings.cachedSettings.GoogleMapsApiKey;
			...
			function getView(renderTo) {
				...
				var mapsConfig = Ext.create("BPMSoft.HtmlControl", {
					id: "googlemap",
			...
			function getMapsConfig() {
				...
				mapsConfig.isGeocodingFinished = function() {
					...
					var count = 0;
					var isFinished = true;
					BPMSoft.each(this.mapsData, function(item) {
					...
					if (isFinished) {
						...
						BPMSoft.each(this.mapsData, function(item) {
							if (item.location) {
								...
								BPMSoft.each(dublicates, function(item, index) {
				...
				mapsConfig.isAddressesExists = function() {
					...
					BPMSoft.each(mapsConfig.mapsData, function(value) {
						if (value.address) {
			...
			function addMarkersByAddresses(mapsData) {
				...
				BPMSoft.each(mapsData, function(item) {
					if (item.location) {
