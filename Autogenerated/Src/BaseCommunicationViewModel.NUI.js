define("BaseCommunicationViewModel", ["ConfigurationConstants", "BaseCommunicationViewModelResources",
	...
	function(ConfigurationConstants, resources, CommunicationUtils, EmailHelper) {
		Ext.define("BPMSoft.configuration.BaseCommunicationViewModel", {
			extend: "BPMSoft.BaseViewModel",
			alternateClassName: "BPMSoft.BaseCommunicationViewModel",
			mixins: {
			...
			isChanged: function() {
				var result = false;
				BPMSoft.each(this.changedValues, function() {
			...
			getSaveQuery: function() {
				...
				if (entitySchema == null) {
					throw new BPMSoft.exceptions.ArgumentNullOrEmptyException({argumentName: "entitySchema"});
				}
				if (this.isDeleted) {
					throw BPMSoft.InvalidOperationException();
				}
				var query = null;
				if (this.isNew) {
			 ...
			 * ######### ###### ### ####### ######.
			 * @private
			 * @return {BPMSoft.EntitySchemaQuery} ######### EntitySchemaQuery.
			 */
			...
			/**
			 * ########## ########## ######### ### ############# # ########## ########## #####.
			 * @throws BPMSoft.QueryExecutionException #### ### ####### ######## ######, ############ ##############
			 * ########.
			...
			getSocialNetworkAccountsCount: function(socialNetworkType, callback, scope) {
				...
				entitySchemaQuery.addAggregationSchemaColumn("Id", BPMSoft.AggregationType.COUNT, resultColumnName);
				...
				filterGroup.addItem(BPMSoft.createColumnFilterWithParameter(
					BPMSoft.ComparisonType.EQUAL, "Public", true));
				var publicAccountFilterGroup = Ext.create("BPMSoft.FilterGroup");
				publicAccountFilterGroup.logicalOperation = BPMSoft.LogicalOperatorType.AND;
				publicAccountFilterGroup.addItem(BPMSoft.createColumnFilterWithParameter(
					BPMSoft.ComparisonType.EQUAL, "Public", false));
				publicAccountFilterGroup.addItem(BPMSoft.createColumnFilterWithParameter(
				...
				filters.add("socialNetworkTypeFilter", BPMSoft.createColumnFilterWithParameter(
					BPMSoft.ComparisonType.EQUAL, "Type", socialNetworkType.value));
			...
			handleMissingSocialNetworkAccount: function(socialNetworkName) {
				var message = BPMSoft.getFormattedString(resources.localizableStrings.NoSynchronizationAccont,
					socialNetworkName);
