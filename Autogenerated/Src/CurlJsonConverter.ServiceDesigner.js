define("CurlJsonConverter", ["UriJsonConverter"], function() {
	Ext.define("BPMSoft.services.CurlJsonConverter", {
		alternateClassName: "BPMSoft.CurlJsonConverter",
		extend: "BPMSoft.ServiceJsonConverter",
		...
		_consoleCommandToArgs: function(consoleArguments) {
			...
			BPMSoft.each(args, function(item, key) {
				args[key] = item.replace(/{QUOTES}/g, "\"");
			});
			args = this._stripSlashes(args);
			return BPMSoft.deleteEmptyItems(args);
		},
		...
		_parseArgs: function(args) {
			...
			jsonData[this.sectionAuth] = {authType: BPMSoft.services.enums.AuthType.None};
		...
		_findAliasByLongOpt: function(longOption) {
			let shortName;
			BPMSoft.each(this._aliases, function(item, key) {
		...
		/**
		 * @inheritdoc BPMSoft.ServiceJsonConverter#constructor
		 * @override
		 */
	...
	return BPMSoft.CurlJsonConverter;
