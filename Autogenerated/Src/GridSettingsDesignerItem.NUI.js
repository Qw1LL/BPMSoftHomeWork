define("GridSettingsDesignerItem", [], function() {
	Ext.define("BPMSoft.controls.GridSettingsDesignerItem", {
		alternateClassName: "BPMSoft.GridSettingsDesignerItem",
		...
		/**
		 * @inheritdoc BPMSoft.Draggable#showDropZoneHint
		 * @override
		 */
		...
		_getInitialResizeParams: function(event) {
			var isRtl = BPMSoft.getIsRtlMode();
		...
		_setBodyScrollXVisible: function(value) {
			...
			if (value) {
				BPMSoft.utils.dom.removeClassName(bodyElDom, "hidden-scroll-x");
			} else {
				BPMSoft.utils.dom.addClassName(bodyElDom, "hidden-scroll-x");
			}
		},
		...
		/**
		 * @inheritdoc BPMSoft.GridLayoutEditItem#onMoveDragDrop
		 * @overridden
		 */
		...
		/**
		 * @inheritdoc BPMSoft.GridLayoutEditItem#getIntersectionPositionByEvent
		 * @overridden
		 */
		...
		/**
		 * @inheritdoc BPMSoft.GridLayoutEditItem#onMoveDragOver
		 * @overridden
		 */
		...
		getTop: function() {
			...
			if (this.row) {
				return Ext.String.format("calc({0}em - {1}px)", topIndent, this.row * this.cellBorderLineSize);
			}
			return Ext.String.format("calc({0}em)", topIndent);
		},

		/**
		 * @inheritdoc BPMSoft.GridLayoutEditItem#getLeft
		 * @overridden
		 */
		getLeft: function() {
			var gridLayoutEdit = this.getGridLayoutEditInstance();
			var leftIndent = this.column * gridLayoutEdit.cellWidth;
			if (this.column) {
				return Ext.String.format("calc({0}% - {1}px)", leftIndent, this.cellBorderLineSize);
			}
			return Ext.String.format("calc({0}%)", leftIndent);
		},

		/**
		 * @inheritdoc BPMSoft.GridLayoutEditItem#getHeight
		 * @overridden
		 */
		getHeight: function() {
		...
		/**
		 * @inheritdoc BPMSoft.GridLayoutEditItem#getWidth
		 * @overridden
		 */
		getWidth: function() {
			var gridLayoutEdit = this.getGridLayoutEditInstance();
			var width = this.colSpan * gridLayoutEdit.cellWidth;
			if (this.column) {
				return Ext.String.format("calc({0}% + {1}px)", width, this.cellBorderLineSize);
			}
			return Ext.String.format("calc({0}%)", width);
		},

		/**
		 * @inheritdoc BPMSoft.GridLayoutEditItem#getDraggableConfig
		 * @overridden
		 */
		getDraggableConfig: function() {
			var config = this.callParent(arguments) || {};
			BPMSoft.each(config, function(item) {
		 ...
		 * @protected
		 * @param {String} state Item state. Value of BPMSoft.GridSettingsDesignerItem#itemStates.
		 */
		...
		/**
		 * @inheritdoc BPMSoft.Draggable#onResizeLeftElDrag
		 * @overridden
		 */
		...
		/**
		 * @inheritdoc BPMSoft.Draggable#onResizeRightElDrag
		 * @overridden
		 */
		...
		setDraggableConstraints: function(currentDraggable) {
			this.callParent(arguments);
			var gridLayoutEdit = this.getGridLayoutEditInstance();
			if (gridLayoutEdit.gridType === BPMSoft.GridType.LISTED) {
		...
		/**
		 * @inheritdoc
		 * @override BPMSoft.Draggable#createDraggableClone
		 */
		...
		/**
		 * @inheritdoc BPMSoft.GridLayoutEditItem#onResizeRightDragOver
		 * @overridden
		 */
		...
		/**
		 * @inheritdoc
		 * @override BPMSoft.GridLayoutEditItem#getMarginWidth
		 */
		...
		/**
		 * @inheritdoc BPMSoft.GridLayoutEditItem#calculateColSpan
		 * @override
		 */
