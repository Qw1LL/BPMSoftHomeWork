	...
	return {
		...
		methods: {
			...
			initDetailsConfig: function() {
				BPMSoft.SysSettings.querySysSettingsItem("DetailsConfig", function (detailsConfig) {
					try {
			...
			getDetailIds: function() {
				var eventTags = [];
				this.BPMSoft.each(this.details, function(detailConfig, detailName) {
			...
			processDetails: function(workFn, callback, scope) {
				...
				this.BPMSoft.chain.apply(chainContext, chain);
			},
			...
			getDetailId: function(detailName) {
				...
				if (this.BPMSoft.isEmptyObject(detail)) {
					this.log("Detail not found: " + detailName, BPMSoft.LogMessageType.ERROR);
					return;
				}
			...
			subscribeDetailsEvents: function() {
				this.BPMSoft.each(this.details, this.subscribeDetailEvents, this);
			},
			...
			subscribeDetailEvents: function(detailConfig, detailName) {
				...
				var detail = this.BPMSoft.deepClone(detailConfig);
				Ext.applyIf(detail, {
			...
			/**
			 * @inheritdoc BPMSoft.BaseSchemaViewModel#isModelItemEnabled
			 * @override
			 */
			...
			_isColumnTagInExclusionList: function(tag) {
				...
				return this.BPMSoft.contains(exclusionsList, tag);
			},
			...
			getDetailInfo: function(detail) {
				...
				var isFromPage = BPMSoft.Features.getIsDisabled("DisableCheckMasterRecordId");
				return Ext.apply({}, {
			...
			_isDetailNameInExclusionList: function(detailSchemaName) {
				...
				return this.BPMSoft.contains(exclusionsList, detailSchemaName);
			},
			...
			getDetailFilter: function(detail) {
				...
				if (Ext.isEmpty(masterRecordId) && masterFieldColumn &&
					masterFieldColumn.dataValueType !== BPMSoft.DataValueType.TEXT) {
					return BPMSoft.createColumnIsNullFilter(detailColumn);
				}
				var dataValueType;
			...
			getDetailDefaultValues: function(detail) {
				...
				this.BPMSoft.each(detail.defaultValues, function(detailDefaultValue, name) {
					var value;
			...
			_safeLoadDetail: function(config) {
				...
				if (containerId || config.forceLoad) {
					...
					detailConfig = Ext.applyIf(this.BPMSoft.deepClone(detail), this.get(detailName));
			...
			getLookupDefaultValueQueryColumn: function(value) {
				...
				if (entityColumn && this.BPMSoft.isLookupDataValueType(entityColumn.dataValueType)) {
			 ...
			 * @return {BPMSoft.BatchQuery} Entity lookup default values query.
			 */
			getEntityLookupDefaultValuesQuery: function(queryColumns) {
				var query = this.Ext.create("BPMSoft.BatchQuery");
				this.BPMSoft.each(queryColumns, function(queryColumn) {
			...
			applyColumnsOrderToLookupQuery: function(esq, columnName) {
				...
				this.BPMSoft.each(lookupListConfig.orders, function(order) {
			...
			setDefaultValuesToColumns: function(queryResults, queryColumns) {
				this.BPMSoft.each(queryResults, function(queryResult, index) {
			 ...
			 * @virtual
			 */
			getDefaultValues: this.BPMSoft.emptyFn,
			 ...
			 */
			getUpdateDetailConfig: this.BPMSoft.emptyFn,
			 ...
			 * @return {Object} Config for message.
			 */
			getCardModuleResponseConfig: this.BPMSoft.emptyFn,
			...
			/**
			 * @inheritdoc BPMSoft.EntityResponseValidationMixin#validateResponse
			 * @protected
			 */
			 ...
			 * @virtual
			 * @param {Function} callback Callback-function.
			 * @param {BPMSoft.BaseSchemaViewModel} scope Callback execution context.
			 */
			...
			setEntityDefaultValues: function(callback, scope) {
				...
				this.BPMSoft.each(defaultValues, function(defaultValue) {
					...
					if (schemaColumn && this.BPMSoft.isLookupDataValueType(schemaColumn.dataValueType)) {
						return;
					}
			 ...
			 * @return {BPMSoft.EntitySchemaQuery} Lookup query.
			 */
