	...
	return {
		...
		methods: {
			...
			_baseImageHandler: function(config, saveCallback, next) {
				const name = config.name || BPMSoft.generateGUID() + ".png";
				...
				if (imagePath) {
					...
					const file = BPMSoft.getImageFile(url, name);
			...
			_isElementToIgnore: function(element) {
				var result = false;
				BPMSoft.each(this.classesToIgnoreOnPreviewImage, function(className) {
			...
			_findSmartBlock: function(vm, blocks) {
				...
				BPMSoft.each(vm.$Items, function(item) {
					if (item.$ItemType === BPMSoft.ContentBuilderBodyItemType.mjraw.value) {
						blocks[item.$Id] = item;
						return;
					}
			...
			_replaceAllIframes: function() {
				...
				BPMSoft.each(smartBlocks, function(item, id) {
					var el = Ext.get(id + "-content-mjraw-element");
					var parent = el.parent();
					var clonedEl = el.dom.cloneNode(true);
					clonedEl.setAttribute("id", BPMSoft.generateGUID());
			 ...
			 * @protected
			 * @return {BPMSoft.EntitySchemaQuery} Entity schema query for content sheet.
			 */
			...
			/**
			 * @inheritdoc BPMSoft.ContentBuilder#getEditConfig
			 * @override
			 */
			...
			restoreTemplateSheet: function(dataToRestore) {
				if (dataToRestore) {
					BPMSoft.each(dataToRestore, function(clone, id) {
						var el = Ext.get(id + "-content-mjraw-element");
						clone.remove();
						el.show();
					}, this);
			...
			savePreviewImage: function(domElement, callback, scope) {
				...
				BPMSoft.convertElementToCanvas(domElement.id, {
					scale: 0.75,
					allowTaint: true,
					useCORS: true,
			 ...
			 * @param {BPMSoft.EntitySchemaQuery} query Update query to save email template data.
			 */
			addImageIdParameter: function(imageId, query) {
				...
				if (imageId && imageColumnName) {
					query.setParameterValue(imageColumnName, imageId, BPMSoft.DataValueType.GUID);
				}
			},
			 ...
			 * @param {Function} next Callback to call next function in BPMSoft.chain.
			 * @deprecated
			 */
			...
			saveImage: function(file, callback) {
				...
				BPMSoft.FileHelper.uploadFile(BPMSoft.QueryOperationType.INSERT, data, callback, this);
			},
			 ...
			 * @return {BPMSoft.UpdateQuery} UpdateQuery for updating config and html of target email
			 * template.
			 */
			...
			initParameters: function() {
				...
				var recordId = BPMSoft.isGUID(parameters[2]) ? parameters[2] : null;
			...
			prepareConfig: function(configText, html) {
				if (!Ext.isEmpty(configText)) {
					return BPMSoft.decode(configText);
				}
				var config = {
					"ItemType": "sheet",
					"Items": []
