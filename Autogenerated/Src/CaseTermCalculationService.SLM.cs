namespace BPMSoft.Configuration.Calendars
{
	...
	public class CaseTermCalculationService : BaseService
	{
		...
		private ServiceTermResponse InternalCalculateTerms(Dictionary<string, object> arguments, string registrationDate, 
			TermCalculationLogStore logStore = null) {
			...
			var entryPoint = new CaseTermCalculateEntryPoint(UserConnection, logStore);
		...
		public string CalculateTerms(Dictionary<string, object> arguments, string registrationDate) {
			...
			try {
				...
				{
					stringResponse.ReactionTime =
						response.ReactionTime == default(DateTime) && response.SolutionTime.HasValue
							? response.SolutionTime.Value.ToString(_dateFormat)
							: response.ReactionTime.Value.ToString(_dateFormat);
				}
		...
		public TermCalculationLogStore LogCalculateTermsReactionTime(CaseCalculationLoggerRequest request) {
			...
			ServiceTermResponse calculationResult = InternalCalculateTerms(calculationParameters, request.RegistrationDate, logStore);
