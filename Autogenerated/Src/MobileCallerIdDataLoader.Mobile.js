/**
 * @class BPMSoft.configuration.CallerIdDataLoader
 */
Ext.define("BPMSoft.configuration.CallerIdDataLoader", {
	extend: "BPMSoft.nativeApi.BaseCallerIdDataLoader",
	alternateClassName: "BPMSoft.CallerIdDataLoader",
			...
			success: function(otherPhones) {
				...
				config.success = function(data) {
					...
					if (hasOtherPhones && phonesCount < maxRecordCount) {
						...
						BPMSoft.Object.splice(data, maxRecordCount);
	...
	getCommunicationTypesFilter: function() {
		return Ext.create("BPMSoft.Filter", {
			...
			funcArgs: [BPMSoft.GUID.MobilePhone, BPMSoft.GUID.HomePhone, BPMSoft.GUID.WorkPhone,
				BPMSoft.GUID.OtherPhone]
		});
	},
	...
	getEmployeeContactTypeFilter: function() {
		return Ext.create("BPMSoft.Filter", {
			...
			value: BPMSoft.ContactTypes.Employee
		});
	},
	...
	getPhonesData: function(records) {
		...
		for (var i = 0, ln = records.length; i < ln; i++) {
			...
			if (number) {
				...
				if (Ext.isEmpty(countryId) || BPMSoft.util.isEmptyGuid(countryId)) {
	...
	formatLocalPhoneNumber: function(config) {
		...
		if (!Ext.isEmpty(trunkPrefix) && BPMSoft.String.startsWith(phoneNumber, trunkPrefix)) {
...
BPMSoft.CallerId.setDataLoader("BPMSoft.CallerIdDataLoader");
