	...
	return {
		...
		methods: {
			...
			_onCredentialsLocationChange: function(model, value) {
				const locationCaptionEnum = BPMSoft.services.enums.CredentialsLocationCaption;
			...
			_onAccessTypeChange: function(model, value) {
				const accessTypeCaptionEnum = BPMSoft.services.enums.AccessTypeCaption;
			...
			_createExistingUsersFilter: function() {
				const existsfilterGroup = this.Ext.create("BPMSoft.FilterGroup");
				...
				const filter = BPMSoft.createExistsFilter("[VwOAuthAppUser:SysUser].Id", existsfilterGroup);
				return filter;
			},
			...
			_initSecretKey: function(callback, scope) {
				BPMSoft.ServiceOAuthAuthenticatorEndpointHelper.getOAuthServiceClientSecretKey(
			...
			onEntityInitialized: function(callback, scope) {
				...
				if (!this.$CredentialsLocationInRequest) {
					this.$CredentialsLocationInRequest = BPMSoft.services.enums.CredentialsLocation.BASIC_HEADER;
				}
				if (!this.$AccessType) {
					this.$AccessType = BPMSoft.services.enums.AccessType.OFFLINE;
				}
			...
			/**
			 * @inheritDoc BPMSoft.BaseSchemaViewModel#openLookup
			 * @overridden
			 * @protected
			 */
			...
			assignSharedUser: function() {
				const sharedUser = {
					value: BPMSoft.SysValue.CURRENT_USER.value,
					displayValue: BPMSoft.SysValue.CURRENT_USER.displayValue
				};
			...
			init: function(callback, scope) {
				this.callParent([function() {
					BPMSoft.chain(
						...
						function() {
							BPMSoft.ServiceOAuthAuthenticatorEndpointHelper.onLoggedIn({
			...
			getAppDefaultImage: function() {
				return BPMSoft.ImageUrlBuilder.getUrl(this.get("Resources.Images.DefaultAppIcon"));
			},
			...
			getRedirectUrlHelpHtml: function() {
				const infoImgUrl = BPMSoft.ImageUrlBuilder.getUrl(resources.localizableImages.InfoImage);
				const redirectHelpText = this.getRedirectUrlHelpText();
				const url = BPMSoft.ServiceOAuthAuthenticatorEndpointHelper.getOAuthServiceAuthorizationCodeRedirectUrl()
				const line2HelpText = this.get("Resources.Strings.RedirectUrlHelpLine2");
				return "<div class=\"info-help\"><img src = \"" + infoImgUrl + "\"><p>" + redirectHelpText + "<span class=\"info-help__link\">" +
				url +
						"</span><br/>" +
						line2HelpText +
						"</p></div>";
			...
			onLoginSharedUser: function(callback, scope) {
				this.save({
					...
					callBaseSilentSavedActions: true,
					callback: function() {
						BPMSoft.OAuth20Utilities.canLoginOAuthUser(BPMSoft.SysValue.CURRENT_USER.value, this.$Id,
							function(result) {
								if (result) {
			...
			prepareCredentialsLocationList: function(filter, list) {
				...
				const locationValues = [
					BPMSoft.services.enums.CredentialsLocation.BASIC_HEADER,
					BPMSoft.services.enums.CredentialsLocation.REQUEST_BODY,
					BPMSoft.services.enums.CredentialsLocation.QUERY_STRING
				];
				const locationCaptionEnum = BPMSoft.services.enums.CredentialsLocationCaption;
				BPMSoft.each(locationValues, function(key) {
					result.add(key, {
						value: key,
			...
			prepareAccessTypeList: function(filter, list) {
				var result = this.Ext.create("BPMSoft.Collection");
				const accessType = [
					BPMSoft.services.enums.AccessType.ONLINE,
					BPMSoft.services.enums.AccessType.OFFLINE,
					BPMSoft.services.enums.AccessType.NOT_USE,
				];
				const accessTypeCaptionEnum = BPMSoft.services.enums.AccessTypeCaption;
				BPMSoft.each(accessType, function(key) {
					result.add(key, {
						value: key,
